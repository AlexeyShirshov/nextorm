...
| M8 | ✅ сделано | 🟡 Moderate | CPU, strings | `string.Format` бокс убран (кэш имён); `IndexOf` амортизирован `ParamMap` (первый вызов) | `DbPreparedQueryCommand.cs:51-99`, `DbContext.cs` |
...
| M11 | ✅ сделано | 🟡 Moderate | allocations | LINQ → циклы в plan/map-build (4 места); `Select().ToArray()` −72 B/вызов (путь холодный) | `DbContext.cs` |
| **M12** | 🔴 открыто | 🟠 High | allocations, CPU | **implicit-кэш проигрывает явному `Prepare()`**: 17.56 vs 11.94 µs/вызов (1.47×) и 3.76 vs 0.76 KB (4.94×); но кэш **в 2.4× лучше полного отказа от кэша** (42.06 µs, +Gen1) — рекомендация «убрать кэш» снята | `DbContext.cs` (`GetPreparedQueryCommand`), `QueryCommand.PrepareCommand`, `QueryPlanCacheKey` |
| I1 | ✅ закрыто | ℹ️ Info | strings | `string.Format` убран из всех 3 «живых» сайтов (последний — `MakeTop`) | `BaseExpressionVisitor.cs`, `IParamProvider.cs`, `SqlServerDbContext.cs` |
| I2 | ✅ сделано | ℹ️ Info | allocations | LINQ в map-build → циклы (2 сайта в in-memory + 2 в `DbContext` из M11) | `InMemoryDataContext.cs:650,658` |
...
(`ReadOnlySpan<object?>` вместо `object[]?`), а в `DbContext` — приватный span-`GetDbCommand`,
...
`DbContext` переопределяет это без аллокации; `InMemoryContext` может оставить fallback
...
| Context impl | `src/nextorm.core/DataContext/DbContext.cs` | span-`GetDbCommand`; переопределение `ExecuteScalar` |
...
- Внутренний span-путь: `DbPreparedQueryCommand.GetDbCommand(ReadOnlySpan<object?>, …)`; span-версии
  `DbContext.GetDbCommand/ExecuteScalar/ToList/First/FirstOrDefault/Single/SingleOrDefault` и
  соответствующие методы `IDataContext`; `InMemoryContext` конвертирует span→array (`ToParams`) —
...
# 3) тесты (test/nextorm.integration.tests, test/nextorm.core.tests)
```
...
1. **Общий мутируемый `Visitor`.** `ExpressionPlanEqualityComparer.GetHashCode` накапливал `HashCode`
   ...
   `HashCode` и получают значение, не согласованное с `Equals`: ключ попадает в `Dictionary`/
   ...
   **Evidence:** добавлен регресс-тест `GetHashCode_ShouldBeThreadSafe`
   (`test/nextorm.core.tests/ExpressionPlanEqualityComparerTests.cs`) — 8 выражений × 50 000 вызовов
...
> конкурентном использовании одного `DbContext` разные предикаты могут увидеть чужой план.

## I1–I6 (ревизия)

| # | Итог |
|---|---|
| I1 | `string.Format` убран из всех «живых» сайтов: `BaseExpressionVisitor.cs:158` → `DbContext.GetParamName` (кэш имён из M8), `DefaultParamProvider.GetParamName` → растущий кэш `pN`, `SqlServerDbContext.MakeTop` → `$"top({limit})"` (интерполяция `int` не боксит) |
| I2 | LINQ в map-build заменён циклами: 2 сайта в `InMemoryDataContext` + 2 в `DbContext` (закрыты в M11) |
...
| I5 | `LogCommand` (debug-only): `StringBuilder` из `DbContext._sbPool`, `object[]` вместо `ArrayList`, `Append` литералов вместо интерполяции; текст лога не изменился |
...
| # | Что изменено | Файл |
|---|---|---|
| M5 | `ConvertScalar<TResult>`: fast-path `value is TResult`, `long→bool`, `int→long`, `float→double` вместо `IConvertible`-диспетчеризации | `DataContext/DbContext.cs` |
...
| M8 | `string.Format("norm_p{0}", i)` → растущий кэш `DbContext.GetParamName(i)` | `DataContext/DbContext.cs`, `DataContext/Cache/DbPreparedQueryCommand.cs` |
...
**M11 — LINQ в plan/map-build** заменён циклами в `DbContext`:
...
При написании `docs/prepared-vs-cached.md` дефект был **проверен и подтверждён тестом** (не только
чтением кода): cache-hit-ветка `GetPreparedQueryCommand` игнорировала `createEnumerator`
(`DbContext.cs:449-461` были закомментированы), поэтому форма, впервые выполненная буферизованным
...
(`compiledQuery.Enumerator!`). Тот же дефект был и в sync-стриминге (`DbContext.CreateEnumerator`).
...
**Регресс-тесты** (`test/nextorm.sqlite.tests/PlanCacheTests.cs`): `BufferedThenAsyncStreaming_OnTheSameShape_ShouldNotThrow`,
...
`NullReferenceException`. Добавлен `DbContext.RequireEnumerator` — вместо разыменования `null` бросается
...
Написана `docs/prepared-vs-cached.md` (EN, как readme/docs): таблица сравнения, что именно кэшируется и
