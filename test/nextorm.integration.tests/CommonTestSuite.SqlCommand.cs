...
public abstract partial class CommonTestSuite
{
    ...
    {
        // Given
        var cmd = _sut.SimpleEntity.Select(it => new SimpleEntity { Id = it.Id }).PrepareFromSql("select id from simple_entity where id = @id", new { id = 1 }, true, TestContext.Current.CancellationToken);
        // When
    ...
    {
        // Given
        var cmd = _sut.SimpleEntity.Select(it => new SimpleEntity { Id = it.Id }).PrepareFromSql("select id from simple_entity where id = @id+@norm_p0", new { id = 1 }, true, TestContext.Current.CancellationToken);
        // When
