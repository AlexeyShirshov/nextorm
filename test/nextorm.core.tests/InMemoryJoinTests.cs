...
public class InMemoryJoinTests
{
    ...
    public async Task TestCrossJoin()
    {
        var query = _sut.SimpleEntity
            ...
            .Select(p => new { FirstId = p.t1.Id, SecondId = p.t2.Id });
    ...
    public async Task TestCrossJoin8Tables_ShouldCloneAndMaterializeAtEveryArity()
    {
        ...
        var count = 0;
        await foreach (var row in p8c.Select(p => p.t1.Id).ToAsyncEnumerable())
            count++;
    ...
    public async Task TestLeftJoin()
    {
        var query = _sut.SimpleEntity
            ...
            .Select(p => new { FirstId = p.t1.Id, Right = p.t2 });
    ...
    public async Task TestJoin4()
    {
        var query = _sut.SimpleEntity
            .Join(_sut.SimpleEntity, (t1, t2) => t1.Id == t2.Id + 1)
            .Join(_sut.SimpleEntity, (p, t3) => p.t2.Id == t3.Id - 1)
            .Join(_sut.SimpleEntity, (p, t4) => p.t3.Id == t4.Id)
            .Select(p => new { FirstId = p.t1.Id, SecondId = p.t2.Id, ThirdId = p.t3.Id, FourthId = p.t4.Id });
    ...
    public async Task TestRightJoinChained()
    {
        var query = _sut.SimpleEntity
            .Join(_sut.SimpleEntity, (t1, t2) => t1.Id == t2.Id + 1)
            .RightJoin(_sut.SimpleEntity, (p, t3) => p.t2.Id == t3.Id - 1)
            .Select(p => new { RightId = p.t3.Id, Left = p.t1, Mid = p.t2 });
    ...
    public async Task TestFullJoinChained()
    {
        var query = _sut.SimpleEntity
            .Join(_sut.SimpleEntity, (t1, t2) => t1.Id == t2.Id + 1)
            .FullJoin(_sut.SimpleEntity, (p, t3) => p.t2.Id == t3.Id - 1)
            .Select(p => new { RightId = p.t3.Id, Left = p.t1, Mid = p.t2 });
