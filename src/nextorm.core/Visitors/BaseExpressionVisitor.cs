...
public class BaseExpressionVisitor : ExpressionVisitor, ICloneable, IDisposable
{
    ...
    protected override Expression VisitMethodCall(MethodCallExpression node)
    {
            ...
            && TryGetNumericConversion(convertArg.Type, node.Method.ReturnType, out var convertTarget))
        {
            ...
            Visit(convertArg);
            _builder!.Append(" as ").Append(_dataProvider.MakeTypeName(convertTarget)).Append(')');
        ...
        else if (node.Method.DeclaringType == typeof(NORM.NORM_SQL) /*&& _tableProvider is IParamProvider paramProvider*/)
        {
                ...
                && node.Arguments is [Expression exp] && exp.Type.IsAssignableTo(typeof(QueryCommand)))
            {
                ...
                if (!_paramMode)
                {
                    _builder!.Append(_dataProvider.MakeSubqueryPredicate(predicateKeyword, sql!, AsPredicate));
                }
                ...
                if (innerQuery is not null)
                {
                    ...
                    if (!_paramMode)
                    {
                        ...
                        if (col.NeedAliasForColumn)
                            _builder!.Append(_dataProvider.MakeColumnReference(innerCol.PropertyName!));
                        else
                        {
    ...
    protected override Expression VisitUnary(UnaryExpression node)
    {
            ...
            && TryGetNumericConversion(node.Operand.Type, node.Type, out var target))
        {
            ...
            _builder!.Append("cast(");
            Visit(node.Operand);
            _builder!.Append(" as ").Append(_dataProvider.MakeTypeName(target)).Append(')');
            ...
            case ExpressionType.Coalesce:
                if (!_paramMode)
                {
                    ...
                    _builder!.Append(node.Type == typeof(bool)
                        ? _dataProvider.MakeBoolCoalesce(coalesceLeft, coalesceRight)
                        : _dataProvider.MakeCoalesce(coalesceLeft, coalesceRight));
                    return node;
                }
                break;
