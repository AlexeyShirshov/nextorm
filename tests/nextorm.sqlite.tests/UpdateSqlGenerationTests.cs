...
/// <summary>
/// SQL generation of the update builder on SQLite (no database connection): the <c>SET</c> list and the
/// <c>WHERE</c> filter.
/// </summary>
public class UpdateSqlGenerationTests
{
    ...
    [Fact]
    public void Update_ReturningProjection_ShouldRenderReturning()
    {
        using var ctx = SqliteTestContext.Create();
