...
| Регулярные выражения (`Regex.IsMatch` / `Regex.Replace`, константный шаблон) | `s regexp ...` / `regexp_replace(...)` (регистрируемые) | бросает `NotSupportedException` | `s ~ ...` / `regexp_replace(...)` | `regexp_like(...)` / `regexp_replace(...)` | `s regexp ...` / `regexp_replace(...)` | `match(...)` / `replaceRegexpAll(...)` | нативно `System.Text.RegularExpressions` |
