...
| Материализация запроса (`ToTempTable`/`ToTable`) | `CREATE TEMPORARY TABLE ... AS SELECT` | бросает `NotSupportedException` | `CREATE [TEMPORARY] TABLE ... AS SELECT` | `CREATE TEMPORARY TABLE ... AS SELECT` | `CREATE TEMPORARY TABLE ... AS SELECT` | бросает `NotSupportedException` | бросает `NotSupportedException` |
