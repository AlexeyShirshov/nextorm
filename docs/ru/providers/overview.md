...
| Массовая вставка (`BulkInsertInto`) | портируемый `INSERT ... VALUES` | нативный `SqlBulkCopy` | нативный бинарный `COPY` | портируемый `INSERT ... VALUES` | портируемый `INSERT ... VALUES` | портируемый `INSERT ... VALUES` | бросает `NotSupportedException` |
...
| Метки запроса (`WithTag`) | **Унифицировано** | Портируемый блочный комментарий `/* tag */` сразу после `SELECT` на всех SQL-провайдерах; нативные SQL Server `OPTION (LABEL)` и ClickHouse `log_comment` не используются, in-memory принимает вызов как no-op (см. [Метки запроса](../guide/17-query-hints.md)). |
