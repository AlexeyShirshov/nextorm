...
* `nextorm.sqlite`, `nextorm.sqlserver` и `nextorm.postgres` добавляют каждый свой конкретный контекст
  (`SqliteDbContext`, `SqlServerDbContext`, `PostgresDbContext`) и расширение регистрации `Use…`.
