...
- предикаты полнотекстового поиска (`NORM.SQL.contains`/`freetext`) в SQL Server, PostgreSQL и MySQL/MariaDB, текстовые JSON-функции SQL Server (`json_value`/`json_query`/`json_modify`) и табличные функции SQL Server `openjson`/`string_split`;
