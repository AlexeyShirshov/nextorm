...
| **Функции приведения/частей даты ClickHouse вне ClickHouse** | Поверхность `toDate`/`toDateTime`/`toDate32`, `toYear`/…, `toStartOf*`, `toMonday`, `toYYYYMM`/`toYYYYMMDD` и `toUnixTimestamp` ([`SupportsDateConversionFunctions`](xref:NextORM.Core.ISqlDialect.SupportsDateConversionFunctions)) доступна только в ClickHouse; остальные провайдеры отклоняют её через `NotSupportedException`. Портируемые аналоги — `date_trunc`/`date_add`/`end_of_month` и части `DateTime` (см. [Скалярные функции](../guide/11-scalar-functions.md#приведение-и-части-даты-clickhouse)). |
