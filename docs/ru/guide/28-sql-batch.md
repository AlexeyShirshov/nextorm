...
Материализация и немедленное чтение — частый, но не единственный случай. Имя необязательно: `ToTempTable()` сгенерирует его и вернёт, поэтому таблицу можно прочитать обратно через `From(name)`:

```csharp
var name = ctx.From<IOrder>()
    .Where(x => x.Total > minTotal)
    .Select(x => new { x.Id, x.Total })
    .ToTempTable();   // например, "__nextorm_temp_1a2b3c4d"
...
* `CreateTempTableAs(name, source, options?)` и `CreateTableAs(name, source, options?)` добавляют материализации, по порядку, сколько угодно.
...
| SQL Server | Одна `;`-склеенная команда. `SqlBatch` намеренно не используется: он выполняет каждую команду в своей области видимости, поэтому созданная одной командой `#temp` не была бы видна следующей. Таблицу уровня сессии задавайте именем с префиксом `#` и `CreateTableAs` (на SQL Server нет `CREATE TEMPORARY TABLE ... AS SELECT`). |
