...
тело CTE обязано быть `SELECT`. Общие (read) CTE — в
[Общих табличных выражениях](09-cte.md); тела `UPDATE`/`DELETE` запланированы и будут описаны в своих
гайдах.
...
* **Key upsert, но не полный `MERGE`** — key upsert через `ON CONFLICT`/`ON DUPLICATE KEY`/`MERGE`
  **реализован** (см. [Upsert (key merge)](#upsert-key-merge)); полный `MERGE` с произвольными ветками
  `WHEN MATCHED`/`WHEN NOT MATCHED` и `DELETE` — нет. `INSERT ... SELECT` **реализован** (см. «Батч из
  запроса» выше). (Строка «только дефолты» **поддерживается**: см. [Запись значений](#запись-значений).)
* **Нет `Prepare()`** на мутациях в этой версии: каждое выполнение билдера рендерит и выполняет одну
  команду.
...
* In-memory-провайдер только для чтения: `Insert()`, `ReturningIdentity` и `ReturningKey` бросают `NotSupportedException`.
