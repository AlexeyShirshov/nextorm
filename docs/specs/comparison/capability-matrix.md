...
| Navigation properties (implicit joins) | **no** | yes | yes | explicit joins only; no relationship metadata. Eager loading (`LoadWith`/`Include`) is likewise out of scope — no navigation property to select on; recorded in [`todo_eager_loading.md`](../roadmap/todo_eager_loading.md) and gap-analysis §5 п.49 |
