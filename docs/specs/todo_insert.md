...
> [WIP_update.md](WIP_update.md), [WIP_delete.md](WIP_delete.md), [WIP_merge.md](WIP_merge.md).
...
1. **Пробел в матрице.** `linq2db-comparison.md:50-51` и `sql-capabilities-gap-analysis.md:132`
...
2. **Реальные сценарии.** Без записи nextorm нельзя использовать как единственный слой доступа к
   ...
   именно тот сценарий, который закрывает [WIP_transactions.md](WIP_transactions.md).
...
- `MutationCommand` привязывает `DbCommand.Transaction` так же, как SELECT после
  [WIP_transactions.md](WIP_transactions.md) (`Func<DbTransaction?>` в исполнитель).
...
- **Нет `INSERT ... SELECT`, upsert и default-values** в фазе 1; `ON CONFLICT`/`ON DUPLICATE KEY` —
  предмет [WIP_merge.md](WIP_merge.md).
...
- Документация: `docs/guide/19-data-modification.md` (+RU), `docs/providers/*` (+RU),
  ...
  `docs/specs/roadmap/linq2db-comparison.md` (+RU),
