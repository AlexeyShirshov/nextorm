# WIP: Слияние данных (`MERGE` / upsert)
...
> `IMutationExecutor`, план-кэш, транзакции) описан в [WIP_insert.md](WIP_insert.md);
...
3. **Пробел в матрице.** `linq2db-comparison.md:50-51` отмечает DML и «bulk copy / merge» как
...
Общий разрыв — в [WIP_insert.md](WIP_insert.md#текущее-состояние-и-разрыв). Специфично для merge:
...
- **Источник-батч.** Большие батчи параметризуются и разбиваются на чанки (как в
  [WIP_insert.md](WIP_insert.md)); `BulkCopy`/бинарная загрузка — вне области.
