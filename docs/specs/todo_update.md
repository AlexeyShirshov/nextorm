...
1. **Симметрия с `INSERT`.** После #3 запись остаётся неполной без update; `linq2db-comparison.md:50`
...
[WIP_insert.md](WIP_insert.md#текущее-состояние-и-разрыв). Дополнительно к общему каркасу
требуется:
