...
1. **Замыкает DML.** Вместе с `INSERT` (#3) и `UPDATE` (#4) закрывает issue-трекер
   `linq2db-comparison.md:50` / `sql-capabilities-gap-analysis.md:132`.
...
[WIP_insert.md](WIP_insert.md#текущее-состояние-и-разрыв). Дополнительно:
...
3. Общая абстракция `Where`-предиката для update/delete (см. [WIP_update.md](WIP_update.md),
