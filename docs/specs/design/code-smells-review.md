...
**Обновление 23.09.2026 (uncommitted worktree — CTAS `CREATE [TEMPORARY] TABLE ... AS SELECT`, `ToTempTable`/`ToTable`).** Build Release `0/0`; новых подавлений/слопа — **0** (соотношение **11/11**). Открыта 🔴 **Находка 126** (CTE-источник рендерится как `WITH … CREATE TABLE … AS SELECT` — невалидный порядок на PG/SQLite/MySQL; публично достижимо через `ctx.With(...)`, тестами не покрыто), 🟡 **127** (`PostgresDialect.MakeCreateTableAsSelect` — полная копия базовой реализации ~30 строк) и 🟡 **128** (`record`-равенство `TempTableOptions`/`CreateTableAsClause` ломается о reference-член `IReadOnlyList<string> Columns`); прочее — ℹ️-наблюдения (в т. ч. по CTE-порядку, `OnCommit`-валидации и 12-перегрузочной поверхности). XML-doc новых публичных членов полон, доки EN+RU (guide 22, toc, api-reference, providers/overview, limitations) обновлены. Публичная сторона — `API-NAMING-REVIEW.md`, CTAS1–CTAS4. **Применено (23.09.2026):** 🔴 126 исправлена (`WITH` внутри тела CTAS), 🟡 127 — DRY-швы `MakeCreateTableAsHead`/`MakeCreateTableAsTail`; тип `TempTableOptions` переименован в `CreateTableAsOptions` (Находка 128 относится к нему же, имя обновлено).
...
`ArgumentOutOfRangeException`; `Infinity` ловится (`> 1`). Фикс — `double.IsFinite` в проверке (FM6). ✅ **Закрыто 24.09.2026:** guard перенесён в `FromOptions.Sample` (`FromOptions.cs:56,59`), `EntityBuilder.Sample` удалён — см. Находку 151 этого журнала и `API-NAMING-REVIEW.md`, FM6/FO5.
...
- **Обёртка применяется до `filter (where ...)`.** `NormSqlTranslator.cs:257-268`: сначала закрывается `count(...)` и применяется `WrapCount`, и только потом `AggregateFilter.Append` дописывает ` filter (where ...)`. Для диалекта, который одновременно `WrapsCountResult => true` и поддерживает фильтр, вышло бы `toInt32(count(*)) filter (where ...)` — невалидно (FILTER обязан следовать за агрегатом, а не за скалярной `toInt32`). ✅ **Закрыто (24.09.2026):** `SupportsFilter`/`SupportsIfAggregates` заменены одним `AggregateFilterStyle` (`AnsiFilter`/`IfCombinator`/`None`); ClickHouse теперь `IfCombinator`, count-ветка `NormSqlTranslator` при фильтре строит `countIf(...)` и только затем применяет `WrapCount`, а ANSI-ветка применяет wrap после `AggregateFilter.Append` — обе дают `toInt32(...)` снаружи агрегата.
...
## 🔎 Точечный аудит 23.09.2026 — DML `MERGE`, фаза 2: полный `MERGE` с ветками, `RETURNING`/`OUTPUT` и источник-запрос, uncommitted working tree; 🔴 — 0, 🟡 — 3, ℹ️ — 8

**Область.** Продолжение точечного аудита key-upsert (Находки 100–102; публичная сторона — MRG1–MRG5). Дельта фазы 2 (uncommitted worktree):

- `MergeBuilder<TEntity>` (`src/nextorm.core/Builders/MergeBuilder.cs`): `Using(EntityBuilder<TEntity>)` `:102`, `Using(QueryCommand<TEntity>)` `:112`, `WhenMatched()` `:175`, `WhenNotMatched()` `:182`, `WhenNotMatchedBySource()` `:190`, `Returning()` `:198`, `Returning<TResult>(...)` `:215`, in-memory `MergeInMemory` `:373-425`, парсеры `AddBranch` `:237-268` / `ParseColumns` `:270-307` / `AddSelectedMember` `:309-318` / `FindWritable` `:320-334` / `FindMapped` `:486-495`, `BuildCommand` `:438-478` (теперь с `returningColumns`);
- новые публичные `MergeMatchedBuilder<TEntity>` / `MergeNotMatchedBuilder<TEntity>` / `MergeNotMatchedBySourceBuilder<TEntity>` (`Builders/MergeBranchBuilders.cs:11,38,61`) и `MergeReturningBuilder<TEntity,TResult>` (`Builders/MergeReturningBuilder.cs:13`);
- internal `MergeMatchKind` / `MergeActionKind` / `MergeBranch` (`Query/Mutations/MergeCommand.cs:4,15,29`), `MergeCommand.Branches` / `ReturningColumns` (override) / `Source` (`:99,102,109`);
- рендер `SqlMutationBuilder.RenderFullMerge` `:447-564` / `AppendMergeAssignments` `:568-587`, `MakeMerge` + `sourceSql`/`sourceParameters` `:161-174`; диспетчер `DataContext.BuildMergeSql` `:395-403` + ветка `BuildReturningSql` `:343`;
- диалект: 6 DIM `ISqlDialect` (`:1170,1176,1182,1189,1195,1201`) + `SqlDialectBase`-двойники (`:734,737,740,743,746,752`) + override'ы PG (`PostgresDialect.cs:31,34,37,40`) и SQL Server (`SqlServerDialect.cs:73,76,79,82`).

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено). Подавления `src/`: **6** `SuppressMessage` (все с `Justification`) + **5** `#pragma warning disable` (все с парным `restore`) = **11/11** оправданных, **0** неоправданных; в файлах фазы 2 — **0**. Слоп: `Skip=` — **0**; `Task.Delay` — **2** baseline (`Task.Delay(0)`-yield, `tests/nextorm.core.tests/InMemoryTests.cs:125,414`, вне диффа); `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — **0**; `slopwatch` локально не установлен (`.config/dotnet-tools.json` — только coverage/reportgenerator/docfx), скан паттернов выполнен вручную. `.editorconfig` — те же **7** inert `dotnet_diagnostic.*.severity = silent` (6 `S*` без `SonarAnalyzer`-пакета + `CA2254`; пре-существующее, не расширялось). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт). CS1591 не подавлен (`NoWarn` по `.csproj` — **0**; `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`), build `0/0` ⇒ XML-doc новых публичных членов полон.

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | ✅ Новых disposable-полей/локальных нет. `MergeBuilder`/`MergeReturningBuilder`/три branch-builder держат `IDataContext`/`MergeBuilder<TEntity>` без владения (как `InsertBuilder`/`DeleteBuilder`) — `IDisposable` не требуется. `RenderFullMerge` берёт pooled `StringBuilder` в `try/finally` (`SqlMutationBuilder.cs:487,560-563`); вложенный `AppendValuesRows` — тот же контракт. `CA2000`/`CA2213`/`CA1816` не затронуты. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — **0**; соотношение проекта **11/11** (0 неоправданных). |
| 3. LINQ на горячем пути | ✅ Per-row LINQ нет: `AddBranch`/`ParseColumns`/`FindWritable`/`FindMapped`/`KeysEqual`/`MergeInMemory` обходят `List`/`IReadOnlyList` циклами; `RenderFullMerge` считает `hasDelete`/`hasBySource` циклом. Мутации минуют план-кэш. |
| 4. God-классы/методы | ⚠️ `MergeBuilder.cs` **196 → 511** строк — перешёл порог >500 (**Находка 122**); `SqlMutationBuilder.cs` 556 → **713** (>500, **Находка 110**, не новая); `RenderFullMerge` **118** строк (>30), `MergeInMemory` **53**; `MergeBranchBuilders.cs` 70, `MergeReturningBuilder.cs` 90, `MergeCommand.cs` 128 — норма. |
| 5. Хэш-ключи / план-кэш | ✅ `MergeBuilder`/`MergeCommand` в план-кэш не пишут; `RenderFullMerge` — чистый рендер. |
| 6. События / исключения | ✅ Новых подписок/`catch` нет; `InvalidOperationException`/`NotSupportedException`/`ArgumentNullException`/`BuildSqlCommandException` с понятными сообщениями. ℹ️ D (write-only `SqlStatementType.Merge` держится). |
| 7. NRT / возвратный тип | ✅ `Branches`/`ReturningColumns`/`Source` — nullable-аннотированы; `command.Branches!` обоснован диспетчеризацией по `Branches is not null`; сборка `0/0`. |

### 🟡 Находка 120 (P2, контракт/валидация) — явный селектор ветки обходит guard'ы key/identity, которые применяет безселекторная ветка

**Место:** `src/nextorm.core/Builders/MergeBuilder.cs:237-268` (`AddBranch`), `:270-307` (`ParseColumns`), `:320-334` (`FindWritable`); ср. безселекторная ветка `:243-258`.

**Что не так.** Безселекторная ветка (`columns is null`) исключает `IsIdentity`/`IsComputed` для вставки и дополнительно `IsKey` для update (`:246-255`). Явный селектор идёт через `ParseColumns` → `FindWritable`, который отсеивает **только** `IsComputed` (`:326-327`), но пропускает identity и ключевые колонки. Поэтому `ThenUpdate(d => new { d.Id, d.Name })` рендерит `target.id = source.id` — обновление колонки ON-условия (провайдеры это либо запрещают, либо семантика спорна), а `ThenInsert(d => new { d.<identity> })` по identity-колонке выпишет явное значение identity (PG `GENERATED ALWAYS`/SQL Server `IDENTITY` без `IDENTITY_INSERT` — ошибка). Ни guard'а, ни `<remarks>`, ни теста: SQL-gen-тесты используют `ThenUpdate(d => new { d.Name })`/`ThenInsert(d => new { d.Id, d.Name })`, где `Id` — объявленный (не генерируемый) ключ.

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** в `AddBranch` после `ParseColumns` для `Update` отклонить/отфильтровать `IsKey`, для `Update` и `Insert` — `IsIdentity` (по аналогии с `:246-255`), с понятным `NotSupportedException`; либо явно задокументировать «явный селектор — что сказал, то и записалось» в `<remarks>` `WhenMatched`/`WhenNotMatched` и гайде. Добавить негативный SQL-gen-тест. Публичную форму не менять.

**Проверка:** `FindWritable` (`:320-334`) проверяет только `IsComputed`; явная ветка `AddBranch:260-262` фильтров не имеет; `rg 'IsKey|IsIdentity' MergeBuilder.cs` — только `:76,137,251,395,470`.

### 🟡 Находка 121 (P2, поведение in-memory vs БД) — `MergeInMemory` не применяет гейт `WhenMatchedUpdate()`/`WhenNotMatchedInsert()`

**Место:** `src/nextorm.core/Builders/MergeBuilder.cs:373-425` (`MergeInMemory`) ↔ `:438-478` (`BuildCommand`).

**Что не так.** DB-путь `BuildCommand` для key-формы требует `_whenMatchedUpdate && _whenNotMatchedInsert` (`:464-465`). In-memory-путь проверяет только `_source`/`_branches`/`_sourceEntities`/`_keys` (`:375-382`) и всегда делает upsert, полностью игнорируя оба флага: `ctx.MergeInto<T>().Using(e).OnKeys().Merge()` на in-memory молча вставит/обновит строку, а на БД бросит `InvalidOperationException`. Расхождение семантики контекстов нигде не задокументировано и не покрыто (`MergeBuilderTests` всегда вызывает оба флага; кейс «без веток» есть только для БД-пути в виде `Returning_WithoutBranches_ShouldThrow`).

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** добавить в начало `MergeInMemory` тот же гейт (либо валидировать флаги в `Merge`/`MergeAsync` до ветвления на контекст); добавить негативный тест `InMemory_Merge_WithoutBranches_ShouldThrow`. Публичную форму не менять.

**Проверка:** `MergeInMemory:373-425` не читает `_whenMatchedUpdate`/`_whenNotMatchedInsert` (0 вхождений); `BuildCommand:464-465` читает оба.

### 🟡 Находка 122 (P2, god class / длинные методы) — `MergeBuilder.cs` перешёл 500 строк; `RenderFullMerge` — 118 строк

**Место:** `src/nextorm.core/Builders/MergeBuilder.cs` — **511** строк (было **196** в фазе 1); `src/nextorm.core/DataContext/SqlMutationBuilder.cs:447-564` (`RenderFullMerge`, **118** строк), `:373-425` (`MergeInMemory`, **53**); `SqlMutationBuilder.cs` 556 → **713** (>500, **Находка 110**, не новая).

**Что не так.** Фаза 2 добавила в билдер четыре публичные точки входа, парсер селектора (~70 строк) и in-memory-upsert; билдер совмещает теперь key-upsert + full-merge + returning + разбор выражений. `RenderFullMerge` — один метод на три сценария (VALUES-источник / query-источник / новые ветки + `[output|returning]` + терминатор) с вложенным `switch`. Пороги §5 навыка (>500 строк, >30 строк) превышены, как и у пре-существующих `SqlMutationBuilder`/`DataContextExtensions` (Находки 110/119).

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** вынести разбор веток (`AddBranch`/`ParseColumns`/`AddSelectedMember`/`FindWritable`) в internal `MergeBranchParser`/селектор-помощник; распилить `RenderFullMerge` на `RenderMergeUsing(...)` / `RenderMergeBranches(...)` / `RenderMergeReturning(...)` — в общем русле Находки 110 (разделение `SqlMutationBuilder` по инструкциям). Публичную форму не менять.

**Проверка:** `wc -l MergeBuilder.cs` — 511; `RenderFullMerge` — строки 447–564 (118), `MergeInMemory` — 373–425 (53); `SqlMutationBuilder.cs` — 713.

### ℹ️ Наблюдения (фикс не требуется)

- **A. Per-row рефлексия в `MergeInMemory`.** `KeysEqual` (`:427-436`) читает `key.PropertyInfo.GetValue` на каждый ключ × каждого кандидата, а цикл обновления (`:419-420`) — `GetValue`/`SetValue` на колонку × строку. In-memory-путь, не ADO; масштабируется на больших коллекциях. Измерение/оптимизация — `nextorm-inmemory-perf-analyst`; по умолчанию не требуется (продолжение ℹ️ A key-upsert про `Using`).
- **B. `RenderFullMerge` считает `columns` впустую для query-источника.** `:483` (`RenderColumns(... ColumnProperties(command.Columns))`) используется только в VALUES-ветке `:507`; при `command.Source is not null` (батч-колонок нет) это пустой массив. Холодный путь, дефекта нет.
- **C. Порядок параметров `MakeMerge` ≠ `MakeInsert`.** `MakeMerge(dialect, quote, naming, command, keywordCase, parameterProvider, sourceSql, sourceParameters)` (`:161-174`) против `MakeInsert(..., keywordCase, sourceSql, sourceParameters, parameterProvider)` (`:28-36`). Внутренний контракт `SqlMutationBuilder`; при рефакторинге Находки 110 — унифицировать.
- **D. `SqlStatementType.Merge` по-прежнему write-only.** `MutationCommand.StatementType` не читается (0 потребителей; продолжение ℹ️ C key-upsert / ℹ️ C DELETE). Пре-существующий каркас, не новая проблема.
- **E. Форматирование: склеенный `if` + `throw`.** `MergeBuilder.cs:264` — `if (...)            throw new NotSupportedException(...)` на одной строке. Компилируется, EOL CRLF; артефакт редактирования (тот же класс, что закрытая Находка 118). Косметика.
- **F. Пробелы покрытия.** (1) In-memory без флагов (Находка 121) не покрыт. (2) Явный селектор с key/identity (Находка 120) не покрыт. (3) `MergeReturningBuilder.Single`/`SingleAsync`/`ToListAsync` и `ToSql` in-memory-ветка (`:64-70`) тестами не покрыты (интеграция бьёт только `ToList`). (4) Источник-запрос покрыт только SQL-gen (`FullMerge_QuerySource_*` SQL Server/PG), интеграционно `Using(EntityBuilder)` не исполняется. (5) `ThenUpdate(selector)`/`ThenInsert(selector)` — только SQL-gen. (6) Интеграция фаз 2: PG `FullMerge_Returning`/`FullMerge_MatchedDelete`, SQL Server `FullMerge_MatchedDelete`/`FullMerge_NotMatchedBySource`; query-source и `ThenDelete`-на-PG без `RETURNING` не исполнялись против контейнера. Не регрессия — расширение покрытия.
- **G. EOL чист.** Все 10 изменённых/новых файлов фазы 2 — CRLF (`file`: «with CRLF line terminators»; CR=LF для каждого: `MergeBuilder.cs` 511/511, `MergeBranchBuilders.cs` 70/70, `MergeReturningBuilder.cs` 90/90, `MergeCommand.cs` 128/128, `SqlMutationBuilder.cs` 713/713, `DataContext.cs` 661/661, `ISqlDialect.cs` 1384/1384, `SqlDialectBase.cs` 859/859, `PostgresDialect.cs` 470/470, `SqlServerDialect.cs` 667/667). Рекуррентный LF-дефект (Находки 20/22/29/112) не воспроизвёлся.
- **H. Нет per-branch `AND condition` и `DO NOTHING`.** SQL-`MERGE` допускает `WHEN MATCHED AND <cond>` и `DO NOTHING` (PG примеры в офиц. доке); поверхность фазы 2 их не моделирует. Вне scope текущего среза (RFC явно ограничен ветками без условий) — кандидат на будущую фазу, не дефект.

**Итог.** 🔴 — нет; 3 находки P2 (120–122) и наблюдения — маршрут `nextorm-design-engineer` (публичную форму не менять; 120/121 — контракт/поведение, 122 — размер/сопровождаемость; E — косметика). Публичная сторона — `API-NAMING-REVIEW.md`, MRG6–MRG9.

**Проверка (23.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); подавления `src/` — **6+5=11** (0 неоправданных), новых в файлах фазы 2 — **0**; `Skip=` — 0, `Task.Delay` — 2 baseline `(0)`; `find PublicAPI*.txt` — **0** (Шаг 5); `slopwatch` не установлен, скан паттернов вручную; `roslyn types Merge` — 4 новых публичных типа (`MergeMatchedBuilder`, `MergeNotMatchedBuilder`, `MergeNotMatchedBySourceBuilder`, `MergeReturningBuilder<,>`) + `MergeBuilder<TEntity>`; `roslyn members NextORM.Core.MergeBuilder` — **15** публичных методов (8 key-upsert + 7 новых), ctor `internal`; `roslyn members MergeMatchedBuilder` — 3 / `MergeNotMatchedBuilder` — 2 / `MergeNotMatchedBySourceBuilder` — 1 / `MergeReturningBuilder` — 5 публичных методов, ctor `internal` у всех; `wc -l` — `MergeBuilder.cs` 511, `MergeBranchBuilders.cs` 70, `MergeReturningBuilder.cs` 90, `MergeCommand.cs` 128, `SqlMutationBuilder.cs` 713; EOL — CRLF (10/10). Сфокусированные тесты (Release, `--no-build`): core `MergeBuilderTests` **9/9**; `MergeSqlGenerationTests` **postgres 15/15, sqlserver 10/10, sqlite 4/4, mysql 4/4, mariadb 2/2, clickhouse 2/2** — итого **46/46, 0 failed, 0 skipped**. Контейнерная интеграция аудитом не перезапускалась (по отчёту автора — 1222 total, 0 failed, 48 skipped). Публичная сторона — `API-NAMING-REVIEW.md`, MRG6–MRG9.

**Применено (23.09.2026, после аудита).** **Находка 120 — закрыта:** `MergeBuilder.AddBranch` валидирует явный селектор (`ValidateExplicitColumns`) — `ThenUpdate(selector)`/`ThenInsert(selector)` отклоняют identity-колонку, а matched-ветка — ещё и key-колонку (симметрично фильтрации безселекторной ветки); негативные тесты `FullMerge_UpdateSelectorOnKey_ShouldThrow`, identity — в SQL-gen/core. **Находка 121 — закрыта:** `MergeInMemory` требует `WhenMatchedUpdate()`+`WhenNotMatchedInsert()` (как DB-путь `BuildCommand`); негативный тест `InMemory_Merge_WithoutBranches_ShouldThrow`. Наблюдение **E** — закрыто (форматирование `if`/`throw` в `MergeBuilder.AddBranch`). **Находка 122** (гod-class `MergeBuilder.cs` 511 строк; `RenderFullMerge` 118 строк) — **отложена** до рефакторинга вместе с Находкой 110 (`SqlMutationBuilder`), маршрут `nextorm-design-engineer`; в этом изменении — только применённые 120/121 без смены публичной формы. Тесты после правок: core `MergeBuilderTests` **11/11**, sqlserver `MergeSqlGenerationTests` **10/10**, postgres **15/15**, build `0/0`.

**Рефакторинг применён (23.09.2026, uncommitted working tree).** **Находка 122 — закрыта** (размер/сопровождаемость, поведение не менялось). `MergeBuilder<TEntity>` стал `partial`: публичная fluent-поверхность и сборка команды остались в `Builders/MergeBuilder.cs` (**333** строки, было 531), in-memory-upsert (`MergeInMemory`/`KeysEqual`) вынесен в `Builders/MergeBuilder.InMemory.cs` (**74**), разбор селектора веток (`AddBranch`/`ParseColumns`/`AddSelectedMember`/`FindWritable`/`ValidateExplicitColumns`/`FindMapped`) — в `Builders/MergeBuilder.Columns.cs` (**134**); остаток — по образцу `InsertBuilder`/`InsertBuilder.Values.cs`. `SqlMutationBuilder.RenderFullMerge` (**118 → 40** строк) распилен на `ValidateMergeBranches` (`:488`), `AppendMergeUsingSource` (`:510`), `AppendMergeOnKeys` (`:544`), `AppendMergeBranch` (`:556`), `AppendMergeReturning` (`:593`); `SqlMutationBuilder.cs` — **758** строк (было 713; рост — сигнатуры хелперов, `>500` остаётся за Находкой 110, но oversized-метода больше нет). Публичная/внутренняя поверхность не изменена (`roslyn members NextORM.Core.MergeBuilder` — те же **15** публичных методов + `internal` ctor/`AddMatchedUpdate`/`AddDelete`/`AddNotMatchedInsert`/`BuildReturningCommand`/`DataContext`). `dotnet build nextorm.sln -c Release` — **0/0**; EOL CRLF (4/4 файла: `MergeBuilder.cs`, `MergeBuilder.InMemory.cs`, `MergeBuilder.Columns.cs`, `SqlMutationBuilder.cs`). Тесты (Release, `--no-build`): core `MergeBuilderTests` **11/11**, `MergeSqlGenerationTests` — postgres **15/15**, sqlserver **10/10**, sqlite **4/4**, mysql **4/4**, mariadb **2/2**, clickhouse **2/2** — итого **48/48, 0 failed, 0 skipped**; SQL byte-identical подтверждён exact-string ассершенами SQL-gen-тестов (полный `MERGE` рендерят только Postgres/SQL Server — они зелёные).

## 🔎 Точечный аудит 23.09.2026 — DML `MERGE`, фаза 3b: условные ветки (`On`/`WHEN ... AND`), uncommitted working tree; 🔴 — 1, 🟡 — 2, ℹ️ — 7

**Область.** Продолжение разделов key-upsert (Находки 100–102) и фазы 2 (Находки 120–122; публичная сторона — MRG6–MRG9, фаза 3a — `DO NOTHING`). Дельта фазы 3b (uncommitted worktree):

- `MergeBuilder<TEntity>` (`src/nextorm.core/Builders/MergeBuilder.cs`): `On(Expression<Func<TEntity,TEntity,bool>>)` `:161`, перегрузки `WhenMatched(...)` `:202`, `WhenNotMatched(...)` `:221`, `WhenNotMatchedBySource(...)` `:242`; поля `_matchCondition` `:38`, `_registry` `:39`; создание carrier'а в `BuildCommand` `:354-371` (`_registry ??= new EntityBuilder<TEntity>(_dataContext).ToCommand()` `:369`);
- branch-билдеры несут опциональное условие: `Builders/MergeBranchBuilders.cs:14,50,82` (ctor), `MergeBuilder.AddMatchedUpdate/AddDelete/AddDoNothing/AddNotMatchedInsert` `:280,284,291,298`;
- `MergeCommand.MatchCondition` `Query/Mutations/MergeCommand.cs:133`, `Registry` `:139`, ctor-параметры `:96-97`; `MergeBranch.Condition` `:61`;
- рендер: `QueryPlanner.RenderMergeCondition` `DataContext/QueryPlanner.cs:158-195` (два `ColumnsProvider.Add(entityType,false)` `:183-184`, `PushScope(condition.Parameters)` `:185`, `MakeWhere(..., dim: 1)` `:190`); `Query/MergeAliasProvider.cs:8-36`; `DataContext.BuildMergeSql` `DataContext/DataContext.cs:396-440` (условия — в общий аккумулятор `:410-429`, источник `:437-438`); `SqlMutationBuilder.RenderFullMerge` `:512`, `ValidateMergeBranches` `:558-589` (гейт `SupportsMergeConditionalBranches` `:587`), `AppendMergeUsingSource` `:591-623` (`AddRange` `:613-614`), `AppendMergeOnKeys` `:625-642` (предпочитает `matchConditionSql` `:629-633`), `AppendMergeWhen` `:645`;
- диалект: DIM `SupportsMergeConditionalBranches` (`Dialect/ISqlDialect.cs:1196`, `Dialect/SqlDialectBase.cs:746`) + override PostgreSQL `src/nextorm.postgres/PostgresDialect.cs:40` и SQL Server `src/nextorm.sqlserver/SqlServerDialect.cs:82`;
- тесты: SQL-gen PG `tests/nextorm.postgres.tests/MergeSqlGenerationTests.cs:298-338`, SS `tests/nextorm.sqlserver.tests/MergeSqlGenerationTests.cs:210-250`; интеграция `tests/nextorm.integration.tests/CommonTestSuite.Merge.cs:94-171`.

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено). Подавления `src/`: **6** `SuppressMessage` (все с `Justification`) + **5** `#pragma warning disable` (все с парным `restore`) = **11/11** оправданных, **0** неоправданных; в файлах фазы 3b — **0**. Слоп: `Skip=` — **0**; `Task.Delay` — **2** baseline (`tests/nextorm.core.tests/InMemoryTests.cs:125,414`, вне диффа); `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — **0**; `slopwatch` локально не установлен (`.config/dotnet-tools.json` — только coverage/reportgenerator/docfx), скан паттернов выполнен вручную. `.editorconfig` — те же **7** inert `dotnet_diagnostic.*.severity = silent` (6 `S*` без `SonarAnalyzer`-пакета + `CA2254`; пре-существующее, не расширялось). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт). CS1591 не подавлен (`NoWarn` по `.csproj` — **0**; `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`), build `0/0` ⇒ XML-doc новых публичных членов полон.

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | ✅ Новых disposable-полей/локальных нет: `MergeAliasProvider.Instance` — статический синглтон без состояния; `RenderMergeCondition` создаёт `SqlBuildContext`/`StringBuilder`, builder локальный (не pooled); `RenderFullMerge` берёт pooled `StringBuilder` в `try/finally` (`SqlMutationBuilder.cs:536,552-555`). `CA2000`/`CA2213`/`CA1816` не затронуты. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — **0**; соотношение проекта **11/11** (0 неоправданных). |
| 3. LINQ на горячем пути | ✅ `MergeAliasProvider.FindAlias` — `switch` без LINQ; `BuildMergeSql`/`ValidateMergeBranches` — циклы `for`/`foreach` по `IReadOnlyList`; `RenderMergeCondition` — один проход визитора. Новых аллокаций LINQ на per-row пути нет. |
| 4. God-классы/методы | ⚠️ `MergeBuilder.cs` 333 → **415**, `SqlMutationBuilder.cs` 758 → **866** (`>500` — Находка 110, не новая); новые методы `BuildMergeSql` **45** строк, `RenderMergeCondition` **38**, `AppendMergeUsingSource` **33**, `ValidateMergeBranches` **32** — все превышают порог §5 (>30), но относятся к отложенным 110/122 (не переоткрываются, см. ℹ️ C). |
| 5. Хэш-ключи / план-кэш | ✅ `RenderMergeCondition`/`BuildMergeSql` — чистый рендер, в план-кэш не пишут; `Registry` — per-builder carrier, не ключ. |
| 6. События / исключения | ✅ Новых подписок/`catch` нет; гейт `SupportsMergeConditionalBranches` бросает `NotSupportedException` с понятным текстом (`:587-588`); `ArgumentNullException` на `condition` во всех четырёх входах. |
| 7. NRT / возвратный тип | ✅ `MatchCondition`/`Condition`/`Registry` nullable-аннотированы; `command.Branches!` обоснован диспетчеризацией `Branches is not null`; сборка `0/0`. Единственная неточность — `command.Source!` (`DataContext.cs:415`, ℹ️ A). |

### 🔴 Находка 123 (корректность) — full `MERGE` с параметризованным источник-запросом дублирует параметры (`AddRange` списка в самого себя)

**Место:** `src/nextorm.core/DataContext/DataContext.cs:437-438` (`BuildMergeSql`, full-merge-ветка) ↔ `src/nextorm.core/DataContext/SqlMutationBuilder.cs:613-614` (`AppendMergeUsingSource`); `src/nextorm.core/DataContext/QueryPlanner.cs:77,113` (`RenderSource` возвращает переданный список как есть); точка привязки — `src/nextorm.core/DataContext/QueryExecutor.cs:97-98` (`CreateMutationCommand`).

**Что не так.** `RenderSource(command.Source, provider, accumulator)` (`DataContext.cs:437`) в overload'е `RenderSource(source, parameterProvider, parameters)` (`QueryPlanner.cs:72-113`) делает `var @params = parameters ?? new List<Parameter>();` (`:77`) и возвращает **тот же самый** `@params` (`:113`). Поэтому `sourceParameters`, полученный в `DataContext.cs:437`, — это тот же объект, что `accumulator`, переданный в `MakeMerge` отдельным аргументом `parameters` (`:438`). В `AppendMergeUsingSource` (`SqlMutationBuilder.cs:613-614`) выполняется `parameters.AddRange(sourceParameters)` — добавление списка в самого себя (fast-path `ICollection<T>`: `EnsureCapacity`, затем `CopyTo` в хвост, без version-check — исключения нет, но каждый параметр оказывается в итоговом `List<Parameter>` **дважды**). Далее `QueryExecutor.CreateMutationCommand` (`:97-98`) перебирает список и на каждый элемент делает `cmd.Parameters.Add(_createParam(name, value))`: на Npgsql/MySqlConnector два `DbParameter` с одним именем (`@p0`) — `ArgumentException` («already defined») при исполнении; на провайдерах, терпимых к дублям, — молча неверная/избыточная привязка. Триггер: любой параметризованный источник, например `ctx.MergeInto<T>().Using(ctx.From<T>().Where(x => x.Age > min)).OnKeys().WhenMatched().ThenUpdate().WhenNotMatched().ThenInsert().Merge()`. Сам дизайн «условия + источник + VALUES в одном аккумуляторе» верен (непрерывная нумерация `@pN`, что и требовалось), но `sourceParameters` не должен быть тем же объектом, что `parameters`.

**Покрытие не ловит:** SQL-gen `FullMerge_QuerySource_ShouldRenderUsingSelect` (PG `:257-268`, SS `:167-178`) использует источник **без** параметров (`sourceParameters.Count == 0` ⇒ ветка `AddRange` не выполняется), а тесты условий — VALUES-источник; интеграционный `Using(EntityBuilder)` с параметром не исполняется. Комбинация «query-источник + параметр» не покрыта ни одним тестом.

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** не передавать аккумулятор как `sourceParameters` (передать `null` — параметры источника уже лежат в `accumulator`), либо в `AppendMergeUsingSource` защититься `if (!ReferenceEquals(sourceParameters, parameters) && sourceParameters is { Count: > 0 })`; либо возвращать из `RenderSource` копию, когда аккумулятор задан. Добавить SQL-gen тест с параметризованным источник-запросом (проверка числа/имён `Parameter`) и, при возможности, интеграционный. Публичную форму не менять.

**Проверка:** трассировка по коду (`QueryPlanner.cs:77,113`; `DataContext.cs:437-438`; `SqlMutationBuilder.cs:184` (передача `parameters` в `RenderFullMerge`), `:613-614`; `QueryExecutor.cs:97-98`); исполнение на контейнере в этом проходе не воспроизводилось (нет БД у аудита) — дефект выведен из кода; `rg 'RenderSource\(command\.Source, provider, accumulator\)'` — 1; `rg 'parameters\.AddRange\(sourceParameters\)'` — 2 (второй, INSERT-путь `:92`, безопасен: там `sourceParameters` — свежий список, а `parameters` — новый в `MakeInsert`).

### 🟡 Находка 124 (P2, контракт/валидация) — асимметричные guard'ы `On`/`OnKeys`: `OnKeys()` после `On()` молча игнорируется

**Место:** `src/nextorm.core/Builders/MergeBuilder.cs:161-169` (`On`) ↔ `:130-151` (`OnKeys`); `:339-340,369-370` (`BuildCommand`); `src/nextorm.core/DataContext/SqlMutationBuilder.cs:625-642` (`AppendMergeOnKeys`, `:629-633`).

**Что не так.** `On()` проверяет `_keys` и бросает `InvalidOperationException` («On can only be specified once; OnKeys was already called», `:164-165`), но `OnKeys()` **не** проверяет `_matchCondition`; повторный `On()` тоже не отклоняется (guard смотрит только на `_keys`). Поэтому цепочка `...On(...).OnKeys()` создаёт команду с **обоими** (`Keys` непустой и `MatchCondition != null`), а `AppendMergeOnKeys` (`:629-633`) при наличии `matchConditionSql` возвращается до рендера ключей — `OnKeys()` тихо не даёт ничего, без ошибки и предупреждения (обратный порядок, `On()` после `OnKeys()`, корректно падает). Это то же семейство, что MRG3/MRG9 (две DSL-линии), но новый случай — guard односторонний.

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** сделать guard симметричным — в `OnKeys()` при `_matchCondition is not null` бросать `InvalidOperationException` («OnKeys cannot be combined with On»), а в `On()` дополнительно отклонять повторный вызов; либо явно разрешить комбинацию «ключ + доп. условие» и рендерить `ON target.k = source.k AND <condition>`. Добавить негативный core-тест. Публичную форму не менять.

**Проверка:** `OnKeys` `:130-151` не читает `_matchCondition` (0 вхождений); `On` `:164` читает только `_keys`; `AppendMergeOnKeys:629-633` — early return; тестов на `On(...)+OnKeys()` нет (`rg 'OnKeys' tests/nextorm.core.tests tests/nextorm.postgres.tests tests/nextorm.sqlserver.tests` — только валидные цепочки).

### 🟡 Находка 125 (P2, латентный дефект/хрупкость) — `MergeAliasProvider` раздаёт `target`/`source` безусловно и не регистрирует источники

**Место:** `src/nextorm.core/Query/MergeAliasProvider.cs:20-35` (`FindAlias`/`GetNextAlias`); `src/nextorm.core/DataContext/QueryPlanner.cs:158-195` (`RenderMergeCondition`); ср. `DefaultAliasProvider`.

**Что не так.** `FindAlias(idx)` — `target`/`source` для 0/1 и `null` для idx ≥ 2; `GetNextAlias(FromExpression) => "target"` (`:30`), `GetNextAlias(QueryCommand) => "source"` (`:35`) — константы без учёта того, сколько источников уже выделено. Пока условие плоское (`(t, s) => t.x == s.y` с константами/параметрами), всё верно: провайдер в `RenderMergeCondition` свежий (`ColumnsProvider` создан заново `:83`), два `Add(entityType)` дают индексы 0/1 (`:183-184`). Но если тело условия содержит вложенный источник (подзапрос/derived в `MakeWhere`), рендер вложенного `FROM` вызовет `GetNextAlias(...)` (`SqlSourceRenderer.cs:410,442,509,593,628,712`) и получит **повторный** `target`/`source` — коллизия алиасов (невалидный/неоднозначный SQL); третий источник получил бы `null` через `FindAlias(idx ≥ 2)`. Тестов на подзапрос в условии нет, достижимость в текущей поверхности не подтверждена, поэтому это хрупкость, а не воспроизведённый дефект; константный провайдер при этом не следует контракту `IAliasProvider` «вернуть следующий свободный псевдоним».

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** использовать `MergeAliasProvider` только для двух внешних источников MERGE, а вложенные источники рендерить дефолтным провайдером, либо сделать провайдер stateful (`target`/`source` для первых двух, далее `DefaultAliasProvider`-нумерация); либо явно задокументировать/отклонить вложенные источники в условии `NotSupportedException`. Публичную форму не менять.

**Проверка:** `rg 'GetNextAlias' MergeAliasProvider.cs` — две константные реализации; `rg 'MergeAliasProvider'` — единственная точка использования `QueryPlanner.cs:178` (плюс определение).

### ℹ️ Наблюдения (фикс не требуется)

- **A. `command.Source!` (`DataContext.cs:415`) — null-forgiving на значении, которое законно `null` в VALUES-ветке.** `var registry = command.Registry ?? command.Source!;` безопасен: `registry` читается только при наличии условий, а тогда `BuildCommand` уже создал `Registry` (`MergeBuilder.cs:369`); в VALUES-ветке с условиями `Source` действительно `null`, и `Source!` не разыменовывается. Честнее было бы `command.Registry!` (гарантирован). Дефекта нет.
- **B. Мёртвая ветка `BuildMergeSql` (`DataContext.cs:398-405`).** `command.Branches is null && command.Source is not null` недостижима: key-upsert-форма `BuildCommand` отклоняет query-источник (`MergeBuilder.cs:373-374`), а полный `MERGE` всегда даёт `Branches` непустым (или `null` только у key-upsert). Вдобавок при достижении `RenderMerge` не использует `sourceParameters`. Защитный, но dead-code; кандидат на удаление при рефакторинге.
- **C. Новые длинные методы.** `BuildMergeSql` (45), `RenderMergeCondition` (38), `AppendMergeUsingSource` (33), `ValidateMergeBranches` (32) превышают 30-строчный порог §5; относятся к уже отложенным Находкам 110/122 и **не** переоткрываются как отдельная находка. `MergeBuilder.cs` 415, `SqlMutationBuilder.cs` 866 (>500).
- **D. `Registry`-carrier.** `new EntityBuilder<TEntity>(_dataContext).ToCommand()` (`MergeBuilder.cs:369`) создаёт целую (одноразовую) SELECT-команду только ради `IQueryRegistry` для рендера условий; кэшируется в `_registry`, корректности не вредит, но косвенно — при рефакторинге уместен лёгкий internal-носитель реестра. Публичной формы не касается.
- **E. Пробелы покрытия.** (1) `On(...)+OnKeys()` (Находка 124) — 0 тестов; (2) условие с захваченной переменной — именно триггер Находки 123 — не тестируется; (3) вложенный источник в условии (125) — 0; (4) `PostgresDialectTests.MergeHooks_ShouldMatchPostgres` (`tests/nextorm.postgres.tests/PostgresDialectTests.cs:18-26`) обновлён под флаги фазы 2/3a, но **не** ассертит `SupportsMergeConditionalBranches`; аналогичного SQL Server-хук-теста нет; (5) в core `MergeBuilderTests` нет ни одного кейса с условием (только in-memory/валидация фазы 2). Не регрессия — расширение покрытия.
- **F. EOL чист.** Все 3b-файлы и правленые — CRLF (`file`: «with CRLF line terminators»; CR=LF для каждого): `MergeBuilder.cs` 415/415, `MergeBranchBuilders.cs` 97/97, `MergeAliasProvider.cs` 36/36, `QueryPlanner.cs` 528/528, `SqlMutationBuilder.cs` 866/866, `DataContext.cs` 703/703, `ISqlDialect.cs` 1441/1441, `SqlDialectBase.cs` 902/902, `PostgresDialect.cs` 523/523, `SqlServerDialect.cs` 670/670, `MergeSqlGenerationTests.cs` PG 339/339 / SS 251/251, `CommonTestSuite.Merge.cs` 172/172. Рекуррентный LF-дефект (Находки 20/22/29/112) не воспроизвёлся.
- **G. XML-doc `<param name="condition">`.** У `On`/`WhenMatched(condition)`/`WhenNotMatched(condition)` дан пример (`(t, s) => ...`), но словами не сказано, что **первый** параметр — target (существующая строка), а **второй** — source (входящая); это разъясняет гайд (`docs/guide/19-insert-statement.md:450`, +RU `:453`). CS1591 не нарушен; при заморозке дополнить `<param>` одной фразой.

**Итог.** 🔴 — **1** (123, дублирование параметров при query-источнике); 2 находки P2 (124–125) и наблюдения — маршрут `nextorm-design-engineer` (публичную форму не менять; 123 — корректность, 124 — контракт/валидация, 125 — хрупкость). Публичная сторона — `API-NAMING-REVIEW.md`, MRG10–MRG11.

**Проверка (23.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); подавления `src/` — **6+5=11** (0 неоправданных), новых в файлах фазы 3b — **0**; `Skip=` — 0, `Task.Delay` — 2 baseline `(0)`; `find PublicAPI*.txt` — **0** (Шаг 5); `slopwatch` не установлен, скан паттернов вручную; `wc -l` — `MergeBuilder.cs` 415, `MergeBranchBuilders.cs` 97, `MergeAliasProvider.cs` 36, `MergeCommand.cs` 158, `QueryPlanner.cs` 528, `SqlMutationBuilder.cs` 866, `DataContext.cs` 703, `ISqlDialect.cs` 1441, `SqlDialectBase.cs` 902; `roslyn members NextORM.Core.MergeBuilder` — **19** публичных методов (15 → +4: `On` + три `When*(condition)`), ctor `internal`; новых публичных типов — **0**; `rg SupportsMergeConditionalBranches` — `ISqlDialect.cs:1196`, `SqlDialectBase.cs:746`, `PostgresDialect.cs:40`, `SqlServerDialect.cs:82`; `rg 'MergeAliasProvider'` — определение + `QueryPlanner.cs:178`; EOL — CRLF (13/13 файлов). Сфокусированные тесты (Release, `--no-build`, фильтр `FullyQualifiedName~Merge`): core `MergeBuilderTests` **11/11**; `MergeSqlGenerationTests` — postgres **21/21** (вкл. `PostgresDialectTests.MergeHooks_ShouldMatchPostgres`), sqlserver **16/16**, sqlite **4/4**, mysql **4/4**, mariadb **2/2**, clickhouse **2/2**; итого **60/60, 0 failed, 0 skipped**. Контейнерная интеграция аудитом не перезапускалась (по отчёту автора — 3b-кейсы PG/SS исполнены, SQLite/MySQL получили `NotSupportedException`). Публичная сторона — `API-NAMING-REVIEW.md`, MRG10–MRG11.

**Применено (23.09.2026, после аудита 3b).** **Находка 123 (🔴) — закрыта:** в `DataContext.BuildMergeSql` query-source ветка больше не передаёт общий аккумулятор как `sourceParameters` (передаётся `null`), поэтому `AddRange` списка в самого себя устранён (дублирующиеся `@pN`). Регрессионные тесты: SQL-gen PG `FullMerge_QuerySourceWithParameter_ShouldRenderSourceParameterOnce`; интеграционный `Merge_QuerySourceWithParameter_ShouldExecuteOnce` — исполнен на **PostgreSQL и SQL Server** (проверено контейнерами), на остальных бросает. **Находка 124 — закрыта:** `OnKeys()` бросает, если уже вызван `On(...)`, а `On(...)` — при повторном вызове (симметричные guard'ы). **Наблюдение** `command.Source!` → `command.Registry!`. **Находка 125** (`MergeAliasProvider` небезопасен для вложенного источника в условии) — **оставлена как латентная** (вложенные источники в условии merge не поддержаны и не покрыты; отдельный гейт — при необходимости до заморозки). Покрытие расширено: new tests + прогон `~Merge` по всем провайдерам и контейнерной интеграции (PG 6, SQL Server 6, SQLite 6, MySQL 6 — 0 failed).

## 🔎 Точечный аудит 23.09.2026 — CTAS (`CREATE [TEMPORARY] TABLE ... AS SELECT`): `ToTempTable`/`ToTable`, uncommitted working tree; 🔴 — 1, 🟡 — 2, ℹ️ — 9

**Область.** Новый публичный `TempTableExtensions` — **12** терминалов (`ToTempTable`/`ToTempTableAsync`/`ToTempTableSql`/`ToTable`/`ToTableAsync`/`ToTableSql`, каждый на `EntityBuilder<TResult>` и на `QueryCommand<TResult>`; `src/nextorm.core/Builders/TempTableExtensions.cs:28,40,51,65,75,88,97,109,120,134,144,157`), публичные `TempTableOptions` (`Builders/TempTableOptions.cs:22`) и `TempTableOnCommit` (`:7`), публичный `CreateTableAsClause` (`Query/CreateTableAsClause.cs:15`), internal `CreateTableAsCommand` (`Query/Mutations/CreateTableAsCommand.cs:9`) + значение `SqlStatementType.CreateTableAsSelect` (`Query/Mutations/MutationCommand.cs:25`); рендер `SqlMutationBuilder.MakeCreateTableAsSelect` (`DataContext/SqlMutationBuilder.cs:401-443`), диспетчер `DataContext.BuildCreateTableAsSql` (`DataContext/DataContext.cs:392,453-457`), **4** DIM `ISqlDialect` (`DataContext/Dialect/ISqlDialect.cs:1395,1402,1408,1415,1428`) + `SqlDialectBase`-двойники (`:867-901`); override'ы PostgreSQL (`src/nextorm.postgres/PostgresDialect.cs:397-441`), SQLite (`src/nextorm.sqlite/SqliteDialect.cs:243`), MySQL (`src/nextorm.mysql/MySqlDialect.cs:401,404`; MariaDB наследует). Тесты: core `CreateTableAsTests` (4), SQL-gen (postgres 6, sqlite 6, mysql 6, mariadb 5, sqlserver 3, clickhouse 3), integration `CommonTestSuite.CreateTableAs.cs` (3) + `ITestProvider.SupportsCreateTableAsSelect`.

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено). Подавления `src/`: **6** `SuppressMessage` (все с `Justification`) + **5** `#pragma warning disable` (все с парным `restore`) = **11/11** оправданных, **0** неоправданных; в CTAS-файлах — **0**. Слоп: `Skip=` — **0**; `Task.Delay` — **2** baseline (`tests/nextorm.core.tests/InMemoryTests.cs:125,414`, вне диффа); `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — **0**; `slopwatch` локально не установлен (`.config/dotnet-tools.json` — только coverage/reportgenerator/docfx), скан паттернов выполнен вручную. `.editorconfig` — те же **7** inert `dotnet_diagnostic.*.severity = silent` (6 `S*` без `SonarAnalyzer`-пакета + `CA2254`; пре-существующее, не расширялось). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт). CS1591 не подавлен (`NoWarn` по `.csproj` — **0**; `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`), build `0/0` ⇒ XML-doc новых публичных членов полон.

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | ✅ Новых disposable-полей/локальных нет: `TempTableExtensions` — статический класс без состояния; исполнение идёт через существующий `IMutationExecutor.Execute` → `QueryExecutor.CreateMutationCommand` (`using var cmd`, `QueryExecutor.cs:112,124`). `CA2000`/`CA2213`/`CA1816` не затронуты. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — **0**; соотношение проекта **11/11** (0 неоправданных). |
| 3. LINQ на горячем пути | ✅ LINQ нет вовсе: `MakeCreateTableAsSelect` обходит `options.Columns` циклом `for` (`:433-435`) и копирует параметры `new List<Parameter>(sourceParameters)` (`:442`); `TempTableExtensions` — один вызов на терминал. DDL-путь, не hot path. |
| 4. God-классы/методы | ⚠️ `TempTableExtensions` **202** строки, `TempTableOptions` **45**, `CreateTableAsClause` **21**, `CreateTableAsCommand` **37** — норма. `SqlMutationBuilder.MakeCreateTableAsSelect` — **43** строки (>30 §5) в уже раздутом `SqlMutationBuilder.cs` **866** (>500, **Находка 110**, не новая, не переоткрывается). |
| 5. Хэш-ключи / равенство | 🟡 `TempTableOptions` (`sealed record`) и `CreateTableAsClause` (`readonly record struct`) несут `IReadOnlyList<string>? Columns`, по которому неявное record-равенство/`GetHashCode` сравнивает **ссылку** (`IReadOnlyList` без value-equality) — **Находка 128**. В план-кэш CTAS не пишется (мутации кэш не используют), поэтому дефект латентный. |
| 6. События / исключения | ✅ Новых подписок/`catch` нет; отказы — `NotSupportedException`/`ArgumentException` с понятными текстами (`SqlMutationBuilder.cs:411,416,419,422,425`; `TempTableExtensions.cs:180,189,199`). |
| 7. NRT / возвратный тип | ✅ `Columns`/`DataContext`/`withSql` nullable-аннотированы; сборка `0/0`. `TempTableExtensions.ToTempTable`/`ToTable` возвращают `void`, `*Async` — `Task` (не `async void`), `*Sql` — `string`; XML-doc полный. |

### 🔴 Находка 126 (корректность) — CTAS с CTE-источником рендерит `WITH … CREATE TABLE … AS SELECT` (невалидный порядок на всех поддерживающих провайдерах)

**Место:** `src/nextorm.core/DataContext/SqlMutationBuilder.cs:441` (`var sql = (withSql ?? string.Empty) + dialect.MakeCreateTableAsSelect(clause, sourceSql, keywordCase);`); вход — `src/nextorm.core/DataContext/DataContext.cs:453-456` (`BuildCreateTableAsSql`); вынос `WITH` из тела — `src/nextorm.core/DataContext/QueryPlanner.cs:93-104`; базовый рендер — `src/nextorm.core/DataContext/Dialect/SqlDialectBase.cs:899` (`as ` + selectSql); PG — `src/nextorm.postgres/PostgresDialect.cs:435`.

**Что не так.** `RenderSource` (`QueryPlanner.cs:95-102`) вынимает объявленные CTE в `withSql`, а тело оставляет без них; `MakeCreateTableAsSelect` склеивает `withSql` **перед** `create [temporary] table … as <select>`. Итог — `with c as (select …) create temporary table t as select … from c`. Форма `WITH … INSERT` (тот же приём для INSERT, `DataContext.cs:356`; тест `InsertSqlGenerationTests.cs:494`) для INSERT корректна, но для CTAS — нет: `WITH` принадлежит SELECT-выражению, поэтому в PostgreSQL и SQLite он обязан стоять **после** `AS` (`create table t as with c as (…) select …`), а в MySQL — сразу перед `SELECT` (`CREATE TABLE … [AS] WITH … SELECT`, docs MySQL 8.0 «WITH … CREATE TABLE … SELECT»). `WITH … CREATE TABLE` — синтаксическая ошибка на всех трёх CTAS-провайдерах. Достижимо публично: `ctx.With("recent", q).From("recent")…ToTempTable("t")` (`DataContextExtensions.With` `:884`, `CteQuery.From` `:118`); теми же путями `With(name, insert)` (data-modifying CTE) с `ToTempTable`. CTE-источник не покрыт ни одним SQL-gen/интеграционным CTAS-тестом, поэтому дефект не пойман.

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** встраивать `withSql` в тело до вызова диалекта (например, `sourceSql = (withSql ?? string.Empty) + sourceSql; withSql = null;`) либо расширить сигнатуру `MakeCreateTableAsSelect`, чтобы порядок соответствовал грамматике провайдера (`create … as with … select`; при необходимости — отдельная ветка для MySQL). Добавить SQL-gen тест с CTE-источником на PostgreSQL/SQLite/MySQL и интеграционный (`With(...).From(...).ToTempTable`). Публичную форму не менять.

**Проверка:** трассировка по коду (`QueryPlanner.cs:93-104`; `DataContext.cs:453-456`; `SqlMutationBuilder.cs:441`; `SqlDialectBase.cs:889-901`; `PostgresDialect.cs:411-441`); исполнение на БД не воспроизводилось (у аудита нет контейнера, scratch-тест добавлять запрещено) — дефект выведен из кода; CTE-тест для CTAS — **0**.

### 🟡 Находка 127 (P2, дублирование/сопровождаемость) — `PostgresDialect.MakeCreateTableAsSelect` повторяет базовую реализацию целиком

**Место:** `src/nextorm.postgres/PostgresDialect.cs:409-441` ↔ `src/nextorm.core/DataContext/Dialect/SqlDialectBase.cs:883-901`.

**Что не так.** PG-переопределение копирует базовые `create`/`temporary`/`table`/`if not exists`/`table`/`(columns)`/`as`/`selectSql` (~30 строк) и лишь добавляет `on commit …` перед `as` и `with no data` после тела. Два независимых тела рендера одного и того же синтаксиса разойдутся при любой правке базы (это то же семейство, что **Находка 115** `MakeUpdateJoin` ↔ `MakeDeleteJoin`, и **30/15** emit-циклов). Холодный путь, дефекта поведения нет.

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** выделить в `SqlDialectBase` защищённые точки расширения (`AppendCreateTableAsSelectHead`/`AppendCreateTableAsSelectTail`) либо один виртуальный `AppendOnCommitAndData`, чтобы PG дополнял базу, а не копировал её. Публичную форму не менять.

**Проверка:** построчное сравнение `PostgresDialect.cs:409-441` и `SqlDialectBase.cs:883-901`; общая часть — `:411-423` против `:885-899`.

### 🟡 Находка 128 (P2, равенство/латентный дефект) — `record`-равенство `TempTableOptions` и `CreateTableAsClause` ломается о reference-член `Columns`

**Место:** `src/nextorm.core/Builders/TempTableOptions.cs:22-44` (секция `Columns: IReadOnlyList<string>?` `:44`); `src/nextorm.core/Query/CreateTableAsClause.cs:15-21` (`Columns` в позиционном списке `:19`).

**Что не так.** Оба типа объявлены как `record`/`readonly record struct`, что обещает структурное `Equals`/`GetHashCode`, но `IReadOnlyList<string>` не имеет value-equality: два «одинаковых» набора опций с разными списками-объектами не равны, а с одним и тем же — равны. Сегодня это ни на что не влияет (CTAS не пишется в план-кэш и не используется как ключ), поэтому латентно; но `record` в публичной поверхности создаёт ложное ожидание. `TempTableOptions` при этом — единственный объект, который пользователь может сравнить/закэшировать.

**Стало (рекомендация, маршрут — `nextorm-design-engineer`):** либо объявить типы `sealed class`/обычной `struct` с явным `Equals`/`GetHashCode` (по содержимому `Columns`), либо задокументировать reference-семантику `<remarks>`. Для `CreateTableAsClause` (dialect-вход, не пользовательский) достаточно комментария. Публичную форму не менять.

**Проверка:** `roslyn members NextORM.Core.TempTableOptions` — `Columns.get` типа `IReadOnlyList<string>?`; в план-кэш (`QueryPlanEqualityComparer`) ни один из типов не входит (`rg 'TempTableOptions|CreateTableAsClause' src/nextorm.core/Query/QueryPlanEqualityComparer.cs` — 0).

### ℹ️ Наблюдения (фикс не требуется)

- **A. `OnCommit` допустим только с `ToTempTable` — валидация на этапе сборки SQL.** Признано корректным: проверка `if (options.OnCommit is not PreserveRows && !command.Temporary) throw …` (`SqlMutationBuilder.cs:418-420`) согласована с общим контрактом «опция не поддержана ⇒ `NotSupportedException` при рендере», тем же, что у флагов колонок/`WITH NO DATA` (`:415-426`) и у capability-флагов других DML. Рантайм-ловушка `ToTable(..., OnCommit = …)` ловится до обращения к БД (в т. ч. в `ToTableSql`) с actionable-текстом. Остаётся отметить: `OnCommit = PreserveRows` (значение по умолчанию) на `ToTable` молча принимается как «не задано» — намеренно, не дефект.
- **B. `Temporary` вне `TempTableOptions`, две терминальные ветки (`ToTempTable`/`ToTable`) — принято.** Явная интенция в коде вызова и на уровне терминала перевешивает экономию одной перегрузки; `Temporary` не «забыт», а вынесен, поэтому `TempTableOptions` не имеет молчаливого флага. Цена — имя `TempTableOptions` под оба терминала (публичная сторона, CTAS1).
- **C. 12-перегрузочная поверхность — принято.** 6 имён × 2 приёмника (builder до `Select` + готовый `QueryCommand<TResult>` после `Select`) повторяют ось, уже принятую для других терминалов (`Delete`/`DeleteAsync`/`ToSql` на билдере и на joined-билдере); `*Async` имеют sync-пару (AGENTS.md соблюдён), `CancellationToken` последним (CA1068), `*Sql` — без исполнения. Сокращать не рекомендуется.
- **D. `CreateTableAsClause` публичен — необходимо и согласовано.** `ISqlDialect.MakeCreateTableAsSelect(CreateTableAsClause, …)` — публичный член публичного интерфейса, поэтому тип параметра обязан быть публичным; `internal`-вариант потребовал бы унести хук с `ISqlDialect` (ломает паттерн всех `Make*`-хуков). Публичные `*Clause` в сигнатурах диалекта — уже норма (`TemporalClause`, `ForJsonClause`, `ForXmlClause`, `Paging`).
- **E. `SqlStatementType.CreateTableAsSelect` и `EntityType` write-only.** `StatementType` не читается нигде (как и у MERGE/DELETE, ℹ️ D тех разделов); `CreateTableAsCommand` кладёт в `EntityType` тип результата, который CTAS-рендер не использует (`BuildCreateTableAsSql` его игнорирует). Задокументировано в XML-doc `CreateTableAsCommand`; пре-существующий каркас, не новая проблема.
- **F. Пробелы покрытия.** (1) CTE-источник (триггер Находки 126) — **0** тестов; (2) `ToTable` (persistent) интеграционно не исполняется — integration бьёт только `ToTempTable`; (3) перегрузки на готовом `QueryCommand<TResult>` не покрыты (в тестах только builder-формы); (4) `OnCommit`/`Columns`/`WithData` покрыты только SQL-gen, интеграционно не исполнялись; (5) `UnsupportedProvider_ShouldThrow` бьёт `ToTempTable`, но не `ToTable`; (6) SQLite/MySQL/MariaDB SQL-gen не проверяют `ToTableSql` (persistent) и комбинацию `Columns`+quoted; (7) `OnCommit = PreserveRows` явно — не проверено. Не регрессия — расширение покрытия.
- **G. EOL чист.** Все CTAS-файлы — CRLF: `CreateTableAsClause.cs`, `TempTableOptions.cs`, `TempTableExtensions.cs`, `CreateTableAsCommand.cs`, `SqlMutationBuilder.cs`, `DataContext.cs`, `ISqlDialect.cs`, `SqlDialectBase.cs`, `PostgresDialect.cs`, `SqliteDialect.cs`, `MySqlDialect.cs`, `CreateTableAsTests.cs`, `CreateTableAsSqlGenerationTests.cs` (×6), `CommonTestSuite.CreateTableAs.cs`. Рекуррентный LF-дефект (Находки 20/22/29/112) не воспроизвёлся.
- **H. `AnalysisLevel=latest-all` (прогон этого прохода).** Новых design-rule срабатываний на публичных типах нет (`CA1716`/`CA1724`/`CA1002`/`CA1051`/`CA1815` по `TempTable*`/`CreateTableAsClause` — **0**). Единственное новое — `CA1859` на private `TempTableExtensions.ExecuteAsync:166` (`Task` → `Task<int>`); правило **системное** (110 срабатываний по решению), вне дефолтного набора, сборка `0/0` его не гейтит — фикс не требуется.
- **I. `RequireExecutor`-сообщение перечисляет SQL Server.** `TempTableExtensions.cs:200` предлагает «SQLite, PostgreSQL, SQL Server, MySQL or MariaDB» для «data definition» в целом; для CTAS SQL Server затем отклоняется на уровне диалекта (`SqlMutationBuilder.cs:410`). Хелпер общий с другими мутациями, поэтому текст не CTAS-специфичен; путаницы в потоке нет.

**Итог.** 🔴 — **1** (126, невалидный порядок `WITH` для CTE-источника, публично достижимо, тестами не покрыто); **2** находки P2 (127 — дублирование рендера, 128 — record-равенство о reference-член) и наблюдения — маршрут `nextorm-design-engineer` (публичную форму не менять; 126 — корректность, 127/128 — maintainability/контракт). Публичная сторона — `API-NAMING-REVIEW.md`, CTAS1–CTAS4.

**Проверка (23.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); `-p:AnalysisLevel=latest-all` — новых design-rule по CTAS нет (CA1859 — системный); подавления `src/` — **6+5=11** (0 неоправданных), новых в CTAS-файлах — **0**; `Skip=` — 0, `Task.Delay` — 2 baseline `(0)`, `Thread.Sleep`/пустых `catch`/`NoWarn` — 0; `slopwatch` не установлен, скан паттернов вручную; `find PublicAPI*.txt` — **0** (Шаг 5); `roslyn members NextORM.Core.TempTableExtensions` — **12** публичных методов + 6 private; `roslyn types TempTable` — `TempTableExtensions`/`TempTableOnCommit`/`TempTableOptions`; `roslyn members ISqlDialect` — 4 DIM `SupportsCreateTableAsSelect*` + `MakeCreateTableAsSelect`; `rg SupportsCreateTableAsSelect src --include=*.cs` — `ISqlDialect.cs:1395`, `SqlDialectBase.cs:867`, `PostgresDialect.cs:397,400,403,406`, `SqliteDialect.cs:243`, `MySqlDialect.cs:401,404` (SQL Server/ClickHouse/MariaDB — дефолт/наследование); `wc -l` — `TempTableExtensions.cs` 202, `SqlMutationBuilder.cs` 866, `DataContext.cs` 705, `ISqlDialect.cs` 1441, `SqlDialectBase.cs` 902, `PostgresDialect.cs` 523; EOL — CRLF (19/19 CTAS-файлов: 8 core + 3 диалекта + 8 тестовых). Сфокусированные тесты (Release, `--no-build`, фильтр `FullyQualifiedName~CreateTableAs`): core **4/4**, postgres **6/6**, sqlite **6/6**, mysql **6/6**, mariadb **5/5**, sqlserver **3/3**, clickhouse **3/3** — итого **33/33, 0 failed, 0 skipped**. Контейнерная интеграция аудитом не перезапускалась (по отчёту автора — build `0/0`; provider-гейты выставлены, `SupportsCreateTableAsSelect` — PG/SQLite/MySQL/MariaDB `true`, SQL Server/ClickHouse `false`). Публичная сторона — `API-NAMING-REVIEW.md`, CTAS1–CTAS4.

---

**Закрытие планов (23.09.2026).** Рабочие планы `docs/specs/roadmap/todo_insert.md` и `todo_merge.md` удалены после выпуска всех их фич (DML-каркас и `INSERT`, key upsert, полный `MERGE` с ветками/условиями/`DO NOTHING`). Инлайновые ссылки вида `todo_insert.md:NN`/`todo_merge.md` в записях выше — исторические артефакты проходов аудита; живой трекинг — этот реестр, `API-NAMING-REVIEW.md` и `sql-capabilities-gap-analysis.md` (item 18).


---

## 🔎 Точечный аудит 23.09.2026 — Транзакции (`ITransactionManager`, привязка `DbCommand.Transaction`), uncommitted working tree; 🔴 — 1, 🟡 — 4, ℹ️ — 9

**Область.** Новая кросс-провайдерная роль `NextORM.Core.ITransactionManager` (`src/nextorm.core/DataContext/Roles/ITransactionManager.cs`, **6** членов: `CurrentTransaction` + 2×`BeginTransaction` + 2×`BeginTransactionAsync` + `UseTransaction`) и проводка `DbCommand.Transaction` во всех путях исполнения. `DataContext` реализует роль **явно** (`DataContext.cs:14,169-179`); capability — новый DIM `ISqlDialect.SupportsTransactions` (`ISqlDialect.cs:543`, `SqlDialectBase.cs:188`, `ClickHouseDialect.cs:338` — `false`), протянутый в `DbConnectionManager` как `Func<bool>` (`DataContext.cs:68`). Проводка: `DbConnectionManager` (`_transaction`/`_transactionOwnedByMe`, `BeginTransaction*`, `UseTransaction`, `EnsureTransactionsSupported`, `EnsureNoActiveTransaction`, `DisposeTransaction`, `ConnDisposed`), `DbPreparedQueryCommand.GetDbCommand` (2 новых 4-арг overload'а + поле `DbCommandTransaction` + `ResetConnection`), `QueryExecutor` (`Func<DbTransaction?>`, `GetDbCommand`, `CreateMutationCommand`), `ResultSetEnumerator` (делегат в `InitEnumerator`), `DataContext.CreateCommand`. In-memory роль не реализует (в `IDataContext` не входит). Рядом в рабочем дереве лежит **не относящийся к задаче** bulk-insert WIP — он не рассматривается.

**База (этот проход).** `dotnet build src/nextorm.core/nextorm.core.csproj -c Release` — **0 warnings / 0 errors**; полный `dotnet build nextorm.sln -c Release` — **3 errors**, все в untracked bulk-insert тестах (`tests/nextorm.{mysql,sqlserver,postgres}.tests/BulkInsertSqlGenerationTests.cs` — `InsertEntity.Description` отсутствует), **не** в транзакционных файлах. Подавления в транзакционных файлах — **0** новых; проектный баланс **11/11** (6 `SuppressMessage` с `Justification` + 5 `#pragma` с `restore`). Слоп: `Skip=` — 0, `Task.Delay` — 2 baseline (`tests/nextorm.core.tests/InMemoryTests.cs:125,414`), `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов вручную. `.editorconfig` — те же 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5). CS1591 не подавлен (`NoWarn` по `.csproj` — 0), build `0/0` ⇒ XML-doc новых публичных членов полон.

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | 🔴/🟡 Владение транзакцией разобрано находками 129–130: `DisposeTransaction` (`DbConnectionManager.cs:257-264`) освобождает только `_transactionOwnedByMe`, порядок «транзакция → соединение» в `DisposeConnection` (`:234,244`) верный, но «отвязка» контекстной транзакции `UseTransaction(null)` (`:125-130`) и `ConnDisposed` (`:205-208`) освобождение теряют. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — 0; соотношение проекта 11/11 (0 неоправданных). |
| 3. LINQ на горячем пути | ✅ Транзакционный путь LINQ не использует: `DbCommandTransaction != transaction` — ссылочное сравнение (`DbPreparedQueryCommand.cs:195`); делегаты `Func<DbTransaction?>` захвачены один раз. |
| 4. God-классы/методы | ⚠️ `DataContext.cs` **788** (>500), но декомпозиция класса закрыта ранее (реестр, строка 455) — +~20 транзакционных строк не переоткрывают Находку 4; `ResultSetEnumerator.cs` **343**, `DbConnectionManager.cs` **265**, `ITransactionManager.cs` **44**. |
| 5. Хэш-ключи / план-кэш | ✅ Транзакция не входит в ключ плана (на SQL не влияет); `cmd.Transaction` переустанавливается на **каждом** исполнении (`GetDbCommand`), при `ResetConnection`/`ConnDisposed` снимается — замечание к `ResetConnection` в 132 (латентно). |
| 6. События / исключения | ✅ Новых подписок/`catch` нет; гейт `EnsureTransactionsSupported` бросает `NotSupportedException` с понятным текстом (`:143-147`), повторный вход — `InvalidOperationException` (`:149-153`), чужая транзакция — `InvalidOperationException` (`:136-137`). |
| 7. Аллокации (zero-cost) | ✅ При `CurrentTransaction is null` путь исполнения не добавляет бокса/замыкания: делегат в ctor (`QueryExecutor.cs:24,39`), `() => _connectionManager.CurrentTransaction` один раз (`DataContext.cs:83`), свойство без аллокации (`DbConnectionManager.cs:70`), сравнение ссылок. |

### 🔴 Находка 129 (корректность/владение, P1) — `UseTransaction(null)` у контекстной транзакции теряет владение: транзакция не освобождается и остаётся открытой

**Место:** `src/nextorm.core/DataContext/DbConnectionManager.cs:123-130` (ветка `transaction is null`) ↔ `:257-264` (`DisposeTransaction`), `:139-140` (`UseTransaction` ставит `_transactionOwnedByMe = false`); контракт владения — `src/nextorm.core/DataContext/Roles/ITransactionManager.cs:37-43`; таблица владения — `docs/specs/roadmap/todo_transactions.md:100-107`.

**Что не так.** Ветка `UseTransaction(null)` безусловно обнуляет `_transaction` и `_transactionOwnedByMe` и **не** вызывает `DisposeTransaction`. Если контекст владел транзакцией (`BeginTransaction*`, `_transactionOwnedByMe == true`), вызов `UseTransaction(null)`:
1. отвязывает контекст, но не завершает и не освобождает `DbTransaction` — последующая `DisposeTransaction` при `DisposeConnection` его уже не увидит (`_transaction is null`), т. е. `ROLLBACK` не выполняется, а соединение остаётся в незавершённой локальной транзакции;
2. снимает привязку `cmd.Transaction` (`DataContext.cs:202-203`, `QueryExecutor.cs:100-101`) при **открытой** транзакции соединения — на SQL Server/MySQL следующий же запрос бросит `InvalidOperationException` (тот самый дефект issue #32, который фича и закрывала; ср. `todo_transactions.md:47-48,118-119`), на PostgreSQL/SQLite выполнится неявно/вне ожидаемой транзакции. Контракт владения из RFC («создана `BeginTransaction*` → контекст завершает и/или освобождает при `Dispose`») не выполняется, а API молча перекладывает владение на вызывающего, ничего об этом не сообщая.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** в ветке `UseTransaction(null)` при `_transactionOwnedByMe` либо освобождать (откатывать) контекстную транзакцию перед отвязкой, либо бросать `InvalidOperationException` («commit/rollback the context-owned transaction before detaching»); `null` должен снимать только **чужую** (caller-supplied) привязку. Дополнить XML-doc `UseTransaction`. Добавить core/SQLite-тест: `BeginTransaction(); UseTransaction(null)` → откат/исключение, соединение не в транзакции. Публичную форму не менять.

**Проверка:** трассировка `DbConnectionManager.cs:123-130,139-140,257-264`; ветка `null` не читает `_transactionOwnedByMe` и не вызывает `DisposeTransaction` (`rg '_transactionOwnedByMe'` — `:29,89,113,128,140,208,259,263`); тестов на `UseTransaction(null)` при владении нет (см. 133 и `roslyn refs NextORM.Core.ITransactionManager` — только `DataContext.cs`). Дефект выведен из кода; БД не запускалась.

### 🟡 Находка 130 (корректность/жизненный цикл, P2) — `ConnDisposed` сбрасывает контекстную транзакцию без освобождения

**Место:** `src/nextorm.core/DataContext/DbConnectionManager.cs:198-215` (особенно `:205-208`).

**Что не так.** Обработчик `ConnDisposed` (подписан только на **caller-supplied** соединение, `:162-163`) при внешнем `Dispose()` соединения безусловно обнуляет `_transaction`/`_transactionOwnedByMe`, не освобождая `DbTransaction`. Сценарий «предоставленное соединение + `BeginTransaction()` на нём + вызывающий освободил соединение» даёт потерянную незавершённую транзакцию (без `ROLLBACK`). Комментарий (`:205-206`) обоснованно не диспозит транзакцию поверх уже освобождённого соединения (провайдер может бросить), но потеря состояния — молчаливая, а не задокументированная. Ср. 129: та же семья «владение ↔ отвязка».

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** best-effort освобождение в `try { _transaction?.Dispose(); } catch { /* соединение уже мертво */ }` с логом, **или** явно задокументировать в XML-doc `ITransactionManager`, что внешнее освобождение соединения прерывает контекстную транзакцию без `ROLLBACK`. Публичную форму не менять.

**Проверка:** трассировка `:162-163` (подписка только для `!_connWasCreatedByMe`), `:198-215`; `DisposeTransaction` вызывается только из `DisposeConnection` (`:234`), т. е. `ConnDisposed` — единственный путь очистки без освобождения.

### 🟡 Находка 131 (DRY/сопровождаемость, P2) — привязка транзакции реализована дважды и читается двумя механизмами

**Место:** `src/nextorm.core/DataContext/DataContext.cs:202-203` (`_connectionManager.CurrentTransaction`) ↔ `src/nextorm.core/DataContext/QueryExecutor.cs:100-101` и `:67,80` (`_currentTransaction()`); источник делегата — `DataContext.cs:83`.

**Что не так.** Одно и то же runtime-состояние читается и применяется к команде в трёх местах двумя разными способами: свойство `IConnectionManager.CurrentTransaction` (в `CreateCommand`) и делегат `Func<DbTransaction?>` (в `QueryExecutor`/`ResultSetEnumerator`). Пути разошлись (`CreateCommand` — inline `if`, `CreateMutationCommand` — свой inline `if`), а `DbPreparedQueryCommand` ведёт ещё и собственный bookkeeping `DbCommandTransaction`. Дефекта поведения нет (оба читают `_transaction`), но новое место привязки легко забыть; это тот же класс, что DRY-находки 127/115.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** свести к одному хелперу (`ApplyTransaction(cmd)` либо переиспользовать `DataContext.CreateCommand` в `CreateMutationCommand`; в идеале — единственный делегат, как в `QueryExecutor`). Публичную форму не менять.

**Проверка:** `rg 'CurrentTransaction' src/nextorm.core` — `DbConnectionManager.cs:70`, `DataContext.cs:83,169,202`, `ITransactionManager.cs:15`; `rg 'cmd.Transaction = ' src` — `DbPreparedQueryCommand.cs:198,214`, `DataContext.cs:203`, `QueryExecutor.cs:101`.

### 🟡 Находка 132 (латентный дефект/хрупкость, P2) — `ResetConnection` обнуляет `DbCommandTransaction` вне `if`, оставляя возможную «висящую» привязку

**Место:** `src/nextorm.core/DataContext/Cache/DbPreparedQueryCommand.cs:211-220` ↔ `:195-199`.

**Что не так.** `ResetConnection` снимает `DbCommand.Transaction` только внутри `if (DbCommand?.Connection == conn)` (`:211-215`), а `DbCommandTransaction = null` выполняет **безусловно** (`:220`). Если команда привязана к другому живому соединению (ветка `if` не выполняется), bookkeeping говорит «транзакции нет», тогда как `DbCommand.Transaction` может остаться заданным; следующее исполнение с `transaction == null` не затрёт его (`:195` — `null != null` ложно) и stale-транзакция разойдётся с вновь привязанным соединением. В текущем per-context кэше недостижимо (оба вызывающих — `ConnDisposed`/`DisposeConnection` — передают собственное соединение контекста, и оно же `DbCommand.Connection`), поэтому латентно; тем не менее bookkeeping и состояние рассинхронизируются.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** перенести `DbCommandTransaction = null;` внутрь `if` (рядом со снятием `DbCommand.Transaction`) либо снимать `DbCommand.Transaction` безусловно после обнуления соединения. Публичную форму не менять.

**Проверка:** `:211-220`; `DbCommandTransaction` читается только в `:195`.

### 🟡 Находка 133 (верификация, P2) — у фичи нет ни одного теста; план тестов RFC не реализован

**Место:** план — `docs/specs/roadmap/todo_transactions.md:241-257`; факт — `tests/` (0 файлов), `roslyn refs NextORM.Core.ITransactionManager`.

**Что не так.** `roslyn refs NextORM.Core.ITransactionManager` даёт **7** ссылок, и все — определение (`ITransactionManager.cs:12`) + явные реализации в `DataContext.cs` (`:14,169,171,173,175,177,179`); ни один тестовый проект роль не упоминает. `find tests -iname '*ransaction*'` — **0**. Не реализовано из плана: core-тесты (in-memory не реализует роль; `UseTransaction(null)` снимает привязку; повторный `BeginTransaction` бросает), SQLite `TransactionTests` (begin/commit/rollback, «запрос видит незакоммиченное», `cmd.Transaction`, `UseTransaction` на чужой, dispose контекста откатывает, снятие при disposal соединения), интеграция PG/SS/MySQL и EF-shared-transaction, ClickHouse `NotSupportedException`, регресс-предохранитель «запрос внутри чужой транзакции не падает». Именно эти пробелы скрыли 129–130 (объекты `IDisposable`, компиляцией не ловятся).

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** добавить тесты минимум на владение/очистку (`BeginTransaction`→`Dispose` откатывает; `UseTransaction(null)` при владении; `UseTransaction` на чужое соединение бросает; `ConnDisposed`-сценарий), на привязку `cmd.Transaction` (SQLite-снимок/PRAGMA или мок команды) и на гейт ClickHouse; интеграционные — по плану. Публичную форму не менять.

**Проверка:** `roslyn refs NextORM.Core.ITransactionManager` — 7 (только def + `DataContext.cs`); `find tests -iname '*ransaction*'` — 0; `rg 'BeginTransaction|UseTransaction|CurrentTransaction' tests/` — 0 (вне `obj/`).

### ℹ️ Наблюдения (фикс не требуется)

- **A. Zero-cost путь без транзакции выполнен.** `QueryExecutor` захватывает `Func<DbTransaction?>` в конструкторе (`:24,39`), `DataContext` строит его один раз (`:83`), `DbConnectionManager.CurrentTransaction` — авто-свойство (`:70`), `GetDbCommand` сравнивает ссылку (`DbPreparedQueryCommand.cs:195`), `CreateCommand`/`CreateMutationCommand` — pattern-match. Ни бокса, ни замыкания per-execution; требование `todo_transactions.md:224-226` соблюдено.
- **B. Все пути создания команды покрыты.** `rg 'CreateCommand(' src` — `DataContext.cs:200` (биндит через `:202`), `QueryExecutor.cs:97` (+`:100`); prepared-команды идут через `GetDbCommand` (биндит `:195-198`). Пропущенных `conn.CreateCommand()` без привязки нет.
- **C. Порядок освобождения верный.** `DisposeConnection` вызывает `DisposeTransaction()` (`:234`) до `_conn.Dispose()` (`:244`) — транзакция не освобождается поверх освобождённого соединения (в отличие от `ConnDisposed`, 130); `ResetConnection` для кэша — до освобождения соединения.
- **D. `CreateCommand(sql)` биндит транзакцию в момент создания.** Для планирования/подготовки корректно (prepared-команды перепривязываются на каждом исполнении), но публичный метод, чью команду вызывающий сохранил между `BeginTransaction`/`UseTransaction`, получит устаревшую привязку; XML-doc `CreateCommand` (`:195-197`) про транзакцию не говорит (API-реестр, TR3).
- **E. Вложенность/повторный вход отклоняются по решению проекта (фаза 2).** `EnsureNoActiveTransaction` (`:149-153`) бросает и на повторный `BeginTransaction`, и на `UseTransaction(tx != null)` при активной; сообщение «use the active transaction instead» подкреплено `CurrentTransaction`. Savepoints/ambient — вне фазы 1 (`todo_transactions.md:220-222`).
- **F. Гейт ClickHouse единообразен.** `EnsureTransactionsSupported` вызывается в `BeginTransaction`/`BeginTransactionAsync` (`:81,105`) и в `UseTransaction(tx != null)` (`:132`); `UseTransaction(null)` гейт не трогает (отвязка не требует поддержки). DIM `=> true` сохраняет совместимость внешних реализаций `ISqlDialect`.
- **G. Потокобезопасность не задокументирована.** RFC прямо требует «задокументировать» (`todo_transactions.md:185-186`), но в XML-doc `ITransactionManager` и публичном гайде этого нет; `_transaction`/`_transactionOwnedByMe` — без синхронизации. Публичная сторона — API-реестр, TR3.
- **H. EOL чист.** `ITransactionManager.cs` 44/44, `DbConnectionManager.cs` 265/265, `DataContext.cs` 788/788, `QueryExecutor.cs` 575/575, `ResultSetEnumerator.cs` 343/343, `DbPreparedQueryCommand.cs` 222/222, `ISqlDialect.cs` 1515/1515, `SqlDialectBase.cs` 942/942, `ClickHouseDialect.cs` 812/812 — CRLF (CR == строк). Рекуррентный LF-дефект (Находки 20/22/29/112) не воспроизвёлся.
- **I. Пробелы покрытия сверх 133.** Не проверены: `BeginTransaction(IsolationLevel)`/async-перегрузки; позитивный `UseTransaction` на транзакции того же соединения; `CurrentTransaction` после `Dispose` контекста; диалекты, наследующие `SupportsTransactions => true`; ClickHouse `UseTransaction(tx != null)` (гейт `NotSupportedException`).

**Итог.** 🔴 — **1** (129 — потеря владения/незавершённая транзакция при `UseTransaction(null)`); 🟡 — **4** (130 — очистка без освобождения в `ConnDisposed`; 131 — дублирование привязки; 132 — латентный bookkeeping `ResetConnection`; 133 — нулевое покрытие) — маршрут `nextorm-design-engineer` (публичную форму не менять). Публичная сторона — `API-NAMING-REVIEW.md`, TR1–TR5. Zero-cost, подавления, EOL и порядок освобождения — чисто.

**Проверка (23.09.2026).** `dotnet build src/nextorm.core -c Release` — **0/0**; `dotnet build nextorm.sln -c Release` — **3 errors** (unrelated bulk-insert untracked tests, вне области); подавления `src/` — **6+5=11** (0 неоправданных), новых в транзакционных файлах — 0; `Skip=` — 0, `Task.Delay` — 2 baseline (`InMemoryTests.cs:125,414`), `Thread.Sleep`/пустых `catch`/`NoWarn` — 0; `catch` в `src/` — 2 законных `TargetInvocationException` (`InMemoryAggregates.cs:84`, `InMemoryCorrelatedPlan.cs:217`); `slopwatch` не установлен, скан вручную; `find -name 'PublicAPI*.txt'` — **0** (Шаг 5); `roslyn refs NextORM.Core.ITransactionManager` — 7 (def + 6 явных реализаций; тестов 0); `rg 'SupportsTransactions' src` — `ISqlDialect.cs:543`, `SqlDialectBase.cs:188`, `DataContext.cs:68`, `ClickHouseDialect.cs:338`; `wc -l` — `ITransactionManager.cs` 44, `DbConnectionManager.cs` 265, `DataContext.cs` 788, `QueryExecutor.cs` 575, `ResultSetEnumerator.cs` 343, `DbPreparedQueryCommand.cs` 222, `ISqlDialect.cs` 1515, `SqlDialectBase.cs` 942, `ClickHouseDialect.cs` 812. Тесты фичи не запускались — их нет (133). Публичная сторона — `API-NAMING-REVIEW.md`, TR1–TR5.

**Применено (23.09.2026, тот же worktree).**
- **129 (🔴) — исправлено.** `UseTransaction(null)` теперь бросает `InvalidOperationException`, если активная транзакция контекстная (`_transactionOwnedByMe`), и снимает только чужую (caller-supplied); `CurrentTransaction` самовосстанавливается — завершённая (`Connection == null`) транзакция отбрасывается через `DisposeTransaction`. `DataContext/DbConnectionManager.cs`. Тест: `TransactionTests.UseTransaction_WithNull_WhenContextOwned_Throws`.
- **130 (🟡) — принято (документировано).** `ConnDisposed` не освобождает мёртвую транзакцию: соединение уже disposed и провайдер откатил её; комментарий уточнён.
- **131 (🟡) — отложено (by design).** Привязка идёт через `Func<DbTransaction?>` (DIP) для `QueryExecutor`/`ResultSetEnumerator` и через свойство `_connectionManager.CurrentTransaction` в `DataContext.CreateCommand` — сознательное разделение слоёв, не дублирование; все пути вызываются на каждом исполнении.
- **132 (🟡) — исправлено.** `DbCommandTransaction = null` перенесён внутрь `if (DbCommand?.Connection == conn)`, рядом с обнулением `DbCommand.Transaction`.
- **133 (🟡) — закрыто тестами.** `tests/nextorm.sqlite.tests/TransactionTests.cs` (15), `tests/nextorm.core.tests/TransactionRoleTests.cs` (1), `tests/nextorm.clickhouse.tests/TransactionTests.cs` (4), `tests/nextorm.integration.tests/CommonTestSuite.Transactions.cs` (5 × провайдер), `tests/nextorm.integration.tests/EfCoreSharedTransactionTests.cs` (EF Core shared transaction).

---

## 🔎 Точечный аудит 23.09.2026 — Bulk insert (`BulkInsertBuilder` / нативные пути), uncommitted working tree; 🔴 — 2, 🟡 — 5, ℹ️ — 8

**Область.** Новые файлы: `Builders/BulkInsertBuilder.cs` (**471**), `Builders/BulkInsertReturningBuilder.cs` (**207**), `Query/Mutations/BulkInsertCommand.cs` (**129**), `DataContext/PortableBulkInsertExecutor.cs` (**180**), `DataContext/SyncToAsyncEnumerable.cs` (**30**). Правки bulk-части (транзакционные правки того же worktree — аудит 129–133, не переоткрываются): `DataContext.cs` — явная реализация `IBulkInsertExecutor` (`:356-381`), `ResolveBulkColumnNames` (`:384`), `protected virtual BulkInsertRows`/`BulkInsertRowsAsync` (`:404,415`); `DataContextExtensions.cs` — `BulkInsertInto<TEntity>` (`:50`); `SqlMutationBuilder.cs` — ignore/keep-identity в `MakeInsert`; `ISqlDialect.cs` — **9** DIM (`:1111-1170`); `SqlDialectBase.cs` — **9** virtual (`:714-730`); диалекты (`PostgresDialect` 3, `SqlServerDialect` 2, `SqliteDialect` 2, `MySqlDialect` 3, `MariaDbDialect` 1, `ClickHouseDialect` 3 override); `PostgresDataContext.cs` — `COPY BINARY` (`:78-119`); `SqlServerDataContext.cs` — `SqlBulkCopy` (`:120-186`). Тесты: `tests/nextorm.core.tests/BulkInsertBuilderTests.cs` (7 кейсов в 8 `[Fact]`), `tests/nextorm.{sqlite,postgres,sqlserver,mysql}.tests/BulkInsertSqlGenerationTests.cs` (5/…), `tests/nextorm.integration.tests/CommonTestSuite.BulkInsert.cs` (5).

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (3 ошибки untracked bulk-тестов, зафиксированные аудитом транзакций, устранены). Подавления `src/` — **6 `SuppressMessage` + 5 `#pragma` = 11/11** (0 неоправданных); в bulk-файлах — **0** новых. Слоп: `Skip=` — 0 (в `CommonTestSuite.BulkInsert.cs:51,69` — `Assert.SkipUnless` по capability провайдера, не отключённый тест), `Task.Delay` — 2 baseline (`InMemoryTests.cs:125,414`), `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0. `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов вручную. `.editorconfig` — те же 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5). CS1591 не подавлен (`NoWarn` по `.csproj` — 0), build `0/0` ⇒ XML-doc новых публичных/protected-членов полон.

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | 🟡/ℹ️ Нативные пути освобождают созданное: Postgres `using var writer` (`:83`, `:105`), SqlServer `using var bulk` (`:134`, `:162`). Но `using (table)` стоит **после** цикла наполнения (`:123-132`, `:151-160`): если `table.Rows.Add(row)` бросит на середине, `DataTable` не освобождён (только managed, ℹ️ D). `SyncToAsyncEnumerable.DisposeAsync` освобождает внутренний `IEnumerator` (`:24-28`). Утечек unmanaged нет. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — 0; проектный баланс 11/11. |
| 3. LINQ на горячем пути | ✅ По строкам LINQ нет: `ToRow`/`ProjectSync`/`BuildBatch`/нативные циклы — плоские `for`/`foreach`. Единственный `Select` — `PostgresDataContext.BuildCopyCommand:117` (`columnNames.Select(Dialect.QuoteIdentifier)`) — один раз на bulk-операцию, не на строку. |
| 4. God-классы/методы | ⚠️ `BulkInsertBuilder.cs` **471** (<500, но 14 public-методов/13 полей), `BulkInsertReturningBuilder.cs` 207, `PortableBulkInsertExecutor.cs` 180, `BulkInsertCommand.cs` 129, `SyncToAsyncEnumerable.cs` 30 — новых >500 нет. `DataContext.cs` 788 (>500 — закрытая Находка 4, не переоткрывается). Методы <30 строк (`ToSql` ~23, `BuildCommand` ~23). |
| 5. Хэш-ключи / план-кэш | ✅ Новых `Dictionary`/`HashSet`/`GetHashCode` нет; bulk в план-кэш не пишется; `BulkBatchOptions` — reference-equality, ключом не является. |
| 6. События/исключения | ✅ Новых подписок/`catch` нет; гейты дают понятные `NotSupportedException` (`EnsureTimeoutSupported`, `MakeInsert` для SQL Server + `IgnoreDuplicates`, дефолт `BulkInsertRows`) и `InvalidOperationException` (нет источника/повторный `Values`). Искл. — Находка 136. |
| 7. Аллокации (горячий путь) | 🟡 `ToRow` — `object?[]` на строку + `PropertyInfo.GetValue` на колонку/строку (Находка 138); `BuildBatch` — `InsertValue[]` на колонку на батч (`:156-165`); SqlServer-натив буферизует весь set в `DataTable` (Находка 137). |

### 🔴 Находка 134 (P1, контракт/поведение) — `Timeout(seconds)` молча игнорируется на нативном пути, хотя XML-doc обещает обратное

**Место:** `src/nextorm.core/Builders/BulkInsertBuilder.cs:100-111` (XML-doc и `Timeout`), `:321-343` (`BuildCommand`, `:342` — `_timeoutSeconds`), `src/nextorm.core/Query/Mutations/BulkInsertCommand.cs:108-109` (`TimeoutSeconds`), `src/nextorm.core/DataContext/PortableBulkInsertExecutor.cs:110-115` (`EnsureTimeoutSupported`), `src/nextorm.core/DataContext/DataContext.cs:404,415` (сигнатуры `BulkInsertRows*` без timeout), `src/nextorm.postgres/PostgresDataContext.cs:78,100`, `src/nextorm.sqlserver/SqlServerDataContext.cs:120,148`.

**Что не так.** `TimeoutSeconds` доходит только до `BulkInsertCommand` и читается единственным потребителем — `EnsureTimeoutSupported` в портативном executor. Путь `IBulkInsertExecutor.BulkInsert` (`DataContext.cs:360-381`) передаёт в нативный hook лишь `tableName`/`columnNames`/`columns`/`rows`; `SqlBulkCopy.BulkCopyTimeout` и таймаут `NpgsqlBinaryImporter` не выставляются. Итог: `Timeout(30)` на PostgreSQL/SQL Server (нативные пути) — **тихий no-op**; на портативном пути (SQLite/MySQL/MariaDB/ClickHouse, а также при `KeepIdentity()`/`IgnoreDuplicates()`/`Returning*`) тот же вызов даёт `NotSupportedException`. Публичная опция либо не работает, либо падает — в зависимости от того, какой путь выбран, и по коду этого не видно. XML-doc `Timeout` («Only the native bulk path honours it») утверждает ровно обратное коду.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** либо прокинуть timeout в native-hooks — добавить `int? timeoutSeconds` в `BulkInsertRows`/`BulkInsertRowsAsync` (сигнатуры новые/unreleased) и выставить `SqlBulkCopy.BulkCopyTimeout`/таймаут Npgsql-импортёра, либо запретить `Timeout` на обоих путях и поправить XML-doc. Публичную форму (кроме новых protected-хуков) не менять.

**Проверка:** `rg 'TimeoutSeconds' src/` — только `BulkInsertBuilder.cs:33,56,109,342`, `BulkInsertCommand.cs:75,109`, `PortableBulkInsertExecutor.cs:112`; нативные `BulkInsertRows*` (`PostgresDataContext.cs:78,100`, `SqlServerDataContext.cs:120,148`) timeout-параметра не имеют и `Timeout`/`BulkCopyTimeout` не читают (`rg 'BulkCopyTimeout|Timeout' src/nextorm.{postgres,sqlserver}` — 0 в bulk-путях).

### 🔴 Находка 135 (P1, корректность/интеграция) — SQL Server нативный путь не регистрирует `SqlBulkCopy` в текущей транзакции контекста: `WriteToServer` бросает при активной транзакции

**Место:** `src/nextorm.sqlserver/SqlServerDataContext.cs:169-176` (`CreateBulkCopy`: `new SqlBulkCopy((SqlConnection)GetConnection())` — без `SqlBulkCopy(SqlConnection, SqlBulkCopyOptions, SqlTransaction)`/`ExternalTransaction`), вызовы `:132-137`, `:160-165`; источник активной транзакции — `src/nextorm.core/DataContext/DataContext.cs:83` (`() => _connectionManager.CurrentTransaction`).

**Что не так.** Документация Microsoft («Transaction and Bulk Copy Operations») прямо: если не передать `SqlTransaction` и передать `null`, **а на соединении есть активная транзакция — бросается исключение**. Bulk-фича и транзакционная фича (`ITransactionManager`) лежат в одном worktree и не интегрированы: сценарий `ctx.BeginTransaction(); ctx.BulkInsertInto<E>().Values(...).BulkInsert();` на SQL Server упадёт `InvalidOperationException` из `SqlBulkCopy` (или bulk уйдёт вне ожидаемой транзакции — в зависимости от версии драйвера), а не запишет строки в транзакции. PostgreSQL `BeginBinaryImport` исполняется на текущем соединении и участвует в его транзакции (не затронут), но аудитом против контейнера не проверялся.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** прокинуть текущую транзакцию в native-hook (`DataContext` уже владеет делегатом `CurrentTransaction`, `DataContext.cs:83`) и в `CreateBulkCopy` создавать `new SqlBulkCopy(conn, SqlBulkCopyOptions.Default, (SqlTransaction?)currentTransaction)`; для `null` — текущее поведение. Добавить тест «bulk внутри `BeginTransaction`» (SQLite-эквивалент не ловит SQL Server-специфику — нужен интеграционный SQL Server). Публичную форму не менять.

**Проверка:** MS docs `sql/connect/ado-net/sql/transaction-bulk-copy-operations` (§Using existing transactions: «if you do not specify a SqlTransaction … and the connection has an active transaction, an exception is thrown»); `rg 'ExternalTransaction|SqlBulkCopyOptions' src/nextorm.sqlserver` — 0; `CreateBulkCopy` (`:169-176`) транзакцию не принимает.

### 🟡 Находка 136 (P2, контракт/валидация) — returning-терминалы не проверяют источник: `ToListAsync` даёт `NullReferenceException`, `ToList`/`ToSql` — неверное сообщение

**Место:** `src/nextorm.core/Builders/BulkInsertReturningBuilder.cs:46-49` (`ToSql`), `:75-78` (`ToList`), `:114-119` (`ToListAsync`), `:131-161,163-192` (`RunAsync`); проекция — `src/nextorm.core/Builders/BulkInsertBuilder.cs:310` (`ProjectSyncRows` → `ProjectSync(_syncSource!)`).

**Что не так.** `Values` не обязателен для `Returning()`/`ReturningKey()`, а XML-doc возвращающих терминалов обещает `InvalidOperationException` «No Values source was supplied». Фактически: `ToListAsync()` при `AsyncSource is null` уходит в sync-ветку и вызывает `ProjectSyncRows()`; `ProjectSync` — `yield`-итератор (`BulkInsertBuilder.cs:378-383`), поэтому NRE (`source` = `null`) бросается только при перечислении в `RunAsync` (`:142`) — вместо `InvalidOperationException`. `ToList()`/`ToSql()` без источника бросают `InvalidOperationException` с сообщением «...the source is async», хотя источник не задан вовсе. `BulkInsert()`/`BulkInsertAsync()` того же билдера валидируют корректно (`EnsureSyncSource`/`EnsureSourceSet`), returning-терминалы — нет.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** вызывать общий `EnsureSourceSet`/`EnsureSyncSource` из `ToList`/`ToListAsync`/`ToSql` returning-билдера (надёжнее — вынести проверку в `BulkInsertBuilder` и экспонировать internal-метод). Тест: `ReturningKey<int>().ToList()` и `.ToListAsync()` без `Values` → `InvalidOperationException` с упоминанием `Values`. Публичную форму не менять.

**Проверка:** трассировка `:48,77,116-118,310,378-383`; теста на source-less returning-терминал нет (`BulkInsertBuilderTests` бьёт `ReturningKey` только с `Values`, `:87`).

### 🟡 Находка 137 (P2, контракт/аллокации) — `MaxBatchSize` игнорируется нативным путём, а XML-doc SQL Server велит им ограничивать set; натив буферизует весь набор в `DataTable`

**Место:** `src/nextorm.core/Builders/BulkInsertBuilder.cs:113-121` (doc «the native path ignores it»), `:375-376` (`UseNative` не смотрит на `_maxBatchSize`); `src/nextorm.sqlserver/SqlServerDataContext.cs:110-118` (doc «bound the set with `BulkInsertBuilder.MaxBatchSize` for very large writes»), `:122-131` (наполнение `DataTable` целиком), `:150-159`.

**Что не так.** `BulkInsertBuilder.MaxBatchSize` документирован как «portable path chunks; the native path ignores it», но XML-doc `SqlServerDataContext.BulkInsertRows` прямо предлагает ограничивать им «very large writes». Поскольку `UseNative` (`:375-376`) учитывает только `_ignoreConflicts`/`_keepIdentity`, на SQL Server весь набор накапливается в `DataTable` до `WriteToServer` (O(N) памяти + overhead `DataTable` на строку), и `MaxBatchSize` этому не мешает — две XML-доки и фактическое поведение противоречат друг другу. PostgreSQL `COPY` стримит построчно (`:85-89`), т.е. там проблема только в no-op опции.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** либо учесть `_maxBatchSize` в SQL Server-нативе (чанковать `DataTable`+`WriteToServer`, задействовать `SqlBulkCopy.BatchSize`), либо убрать совет из XML-doc и явно сказать «нативный путь буферизует набор целиком и не ограничивается `MaxBatchSize`». Публичную форму не менять.

**Проверка:** `rg '_maxBatchSize|MaxBatchSize' src/` — `BulkInsertBuilder.cs:30,116,327,329`; `UseNative` (`:375-376`) читает только `_ignoreConflicts`/`_keepIdentity`; `rg 'BatchSize|_maxBatchSize' src/nextorm.sqlserver` — 0.

### 🟡 Находка 138 (P2, аллокации/горячий путь) — проекция строки: `object?[]` на каждую строку + `PropertyInfo.GetValue` на каждую колонку/строку

**Место:** `src/nextorm.core/Builders/BulkInsertBuilder.cs:295-306` (`ToRow`), `:378-390` (`ProjectSync`/`ProjectAsync`); `src/nextorm.core/DataContext/PortableBulkInsertExecutor.cs:154-179` (`BuildBatch` — `InsertValue[]` на колонку на батч, `:158-162`).

**Что не так.** `ToRow` создаёт `new object?[columns.Count]` на каждую строку и вызывает `columns[i].PropertyInfo.GetValue(entity)` — reflection-диспатч + бокс value-type на каждую колонку/строку. Для bulk-сценария (основной кейс — большие N) это доминирующая CPU/alloc-стоимость проекции; тот же паттерн есть в `InsertBuilder.Values` (`InsertBuilder.Values.cs:124`), `UpdateBuilder.cs:90`, `DeleteBuilder.cs:162`, где N мало. `BuildBatch` поверх этого аллоцирует `InsertValue[]` на каждую колонку на батч. Reflection можно убрать, закэшировав compiled-getter (прецедент — `RowMapperFactory.cs:30-56` компилирует reader-getter на колонку); бокс при `object?[]`-контракте остаётся без typed-проекции.

**Стало (рекомендация, маршрут `nextorm-design-engineer`/`nextorm-db-perf-analyst`):** закэшировать per-property compiled accessor (`Func<object, object?>`, построенный из `PropertyInfo` в process-wide метаданных `IPropertyMetadata`) либо строить `Func<TEntity, object?[]>` один раз на тип; измерить allocation/throughput на `SqliteBenchmark*` до и после. Публичную форму не менять.

**Проверка:** исходник `:301-303`; `rg 'PropertyInfo.GetValue' src/` — 9 мест (паттерн пре-существующий); `RowMapperFactory.cs` — прецедент compiled accessor; бенчмарк bulk-пути аудитом не запускался (маршрут perf-аналитику).

### 🟡 Находка 139 (P2, DRY/сопровождаемость) — цикл батчинга продублирован 6 раз (3 в executor + 3 в returning-билдере), нарезка `ToSql` — дважды

**Место:** `src/nextorm.core/DataContext/PortableBulkInsertExecutor.cs:16-45`, `:57-81`, `:83-108` (три почти идентичных цикла «add → flush на `rowsPerBatch` → `Progress` → partial batch»); `src/nextorm.core/Builders/BulkInsertReturningBuilder.cs:75-107` (`ToList`), `:131-161`/`:163-192` (два `RunAsync`); `:121-129` (`ToListAsyncSyncAsync`/`ToListAsyncRowsAsync` — pass-through обёртки без логики); нарезка первого батча — `BulkInsertBuilder.ToSql` `:218-240` и `BulkInsertReturningBuilder.ToSql` `:46-69` (идентична).

**Что не так.** Один контракт (rowsPerBatch/NewBatch/flush/Progress/partial batch, sync и async) реализован независимо **6** раз, а нарезка первого батча для `ToSql` — **2** раза. Любая правка семантики (например, Progress или отмена) требует синхронного изменения всех копий — это то же семейство, что Находки 109/115/117/131. Дефекта поведения сейчас нет.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** один общий batch-runner (`RunBatches(rowsPerBatch, add, flush, progress, ct)` с sync/async overload'ами) + общий `SliceFirstBatch(command)` для `ToSql`; удалить pass-through обёртки. Публичную форму не менять.

**Проверка:** чтение циклов; `rg 'rowsPerBatch' src/nextorm.core` — `PortableBulkInsertExecutor.cs:23,59,85,120,146`, `BulkInsertBuilder.cs:226`, `BulkInsertReturningBuilder.cs:55,84,137,169`.

### 🟡 Находка 140 (P2, EOL) — все 11 новых файлов bulk-insert записаны с LF-концевиками

**Место:** новые (untracked) — `Query/Mutations/BulkInsertCommand.cs` (129 строк, `crlf=0`), `DataContext/PortableBulkInsertExecutor.cs` (180, 0), `DataContext/SyncToAsyncEnumerable.cs` (30, 0), `Builders/BulkInsertBuilder.cs` (471, 0), `Builders/BulkInsertReturningBuilder.cs` (207, 0), `tests/nextorm.core.tests/BulkInsertBuilderTests.cs` (109, 0), `tests/nextorm.integration.tests/CommonTestSuite.BulkInsert.cs` (91, 0), `tests/nextorm.{mysql,postgres,sqlite,sqlserver}.tests/BulkInsertSqlGenerationTests.cs` (53/62/63/52, 0) — **все LF-only**.

**Что не так.** AGENTS.md («Line endings») требует CRLF (`core.autocrlf=true`); все изменённые (отслеживаемые) файлы bulk-инкремента — CRLF (`DataContext.cs` 788/788, `DataContextExtensions.cs` 940/940, `SqlMutationBuilder.cs` 896/896, `ISqlDialect.cs` 1515/1515, `SqlDialectBase.cs` 942/942, `PostgresDataContext.cs` 120/120, `SqlServerDataContext.cs` 186/186, диалекты — `crlf=total`). LF-only — ровно новые файлы (источник — `file`: «Unicode text, UTF-8 text»). Тот же рекуррентный дефект генерации новых файлов, что **Находки 20/22/29/51/112**; смешанных EOL нет.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** нормализовать 11 файлов в CRLF (`perl -pi -e 's/\r?\n/\r\n/g' <файл>`) и, поскольку дефект рекуррентный, добавить `.gitattributes`-гард `* text=auto eol=crlf`.

**Проверка:** `grep -c $'\r'` = 0 для всех 11; `file` новых файлов — без «with CRLF line terminators»; изменённые — `crlf=total`.

### ℹ️ Наблюдения (фикс не требуется)

- **A. Сборка чистая.** `dotnet build nextorm.sln -c Release` — **0/0**; три ошибки untracked bulk-тестов, зафиксированные аудитом транзакций (129–133), устранены.
- **B. XML-doc полный.** Все новые публичные/protected-члены (14 + 3 + 1 + 2), **9** DIM `ISqlDialect` + **9** base-virtual + **14** provider-override задокументированы; `GenerateDocumentationFile=true`, `TreatWarningsAsErrors=true`, `NoWarn` — 0, CS1591 не подавлен. Публичная сторона — `API-NAMING-REVIEW.md`, BK1–BK6.
- **C. Extend-only соблюдён.** Все новые члены `ISqlDialect` — DIM с дефолтами (внешние реализации интерфейса не ломаются); `protected virtual BulkInsertRows*` — новые (не `abstract`) на `DataContext`; `DataContext` реализует internal `IBulkInsertExecutor` **явно** — публичная поверхность конкретного типа не растёт; переименований/удалений нет.
- **D. `DataTable` в SQL Server-нативе освобождается не на всех путях.** `using (table)` стоит после цикла наполнения (`SqlServerDataContext.cs:123-132`, `:151-160`); при исключении в `table.Rows.Add` `DataTable` не освобождается. Только managed-память, поэтому ℹ️; при правке 137 логично поднять `using` над циклом.
- **E. Портативный путь по умолчанию неограничен.** `BulkBatchOptions.IsUnbounded` (`BulkInsertCommand.cs:30`) ⇒ один `INSERT ... VALUES`; при больших N и portable-only опциях (`KeepIdentity`/`IgnoreDuplicates`/`Returning*`) можно упереться в лимит параметров провайдера (SQL Server 2100, SQLite 999/32766, PostgreSQL 65535). Поведение задокументировано (`BulkInsertCommand.cs:3-6`), авто-чанкинга по лимиту нет — кандидат в отложенные, не находка.
- **F. Native-vs-portable выбор согласован с XML-doc.** `UseNative` (`BulkInsertBuilder.cs:375-376`) уводит на portable при `IgnoreDuplicates`/`KeepIdentity`, а `BulkInsertReturningBuilder` всегда portable (нативный bulk не умеет `RETURNING`/`OUTPUT`) — как и обещано.
- **G. Диагностика гейтов чистая.** `EnsureTimeoutSupported` (portable) и `SqlMutationBuilder.MakeInsert` (SQL Server + `IgnoreDuplicates`) дают `NotSupportedException` с конкретным текстом; дефолтные `BulkInsertRows*` — `NotSupportedException` «declares SupportsBulkCopy but does not implement the native bulk path».
- **H. Пробелы покрытия.** Нет тестов на `Timeout` (134), bulk внутри транзакции (135), source-less returning-терминал (136), `NotifyAfter`/`MaxParameters`/`MaxSqlLength`/`Returning<TResult>`/нативный `KeepIdentity`, и на нативные PG/SQL Server пути против контейнера (`DOCKER_HOST`-прогон аудитом не выполнялся). Тесты фичи не перезапускались.

**Итог.** 🔴 — **2** (134 — `Timeout` молча не работает на нативном пути; 135 — `SqlBulkCopy` не в транзакции контекста); 🟡 — **5** (136 — валидация источника returning-терминалов; 137 — `MaxBatchSize`/doc на натива; 138 — reflection/alloc на проекции строки; 139 — DRY батч-циклов; 140 — EOL) — маршрут `nextorm-design-engineer`, перф-часть 138 — `nextorm-db-perf-analyst` (публичную форму не менять). Публичная сторона — `API-NAMING-REVIEW.md`, BK1–BK6. Подавления, слоп, хэш-ключи и zero-cost-гейты — чисто.

**Проверка (23.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); подавления `src/` — **6+5=11** (0 неоправданных), новых в bulk-файлах — **0**; `Skip=` — 0, `Task.Delay` — 2 baseline (`InMemoryTests.cs:125,414`), `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `slopwatch` не установлен, скан паттернов вручную; `find -name 'PublicAPI*.txt'` — **0** (Шаг 5); `roslyn types BulkInsert` — **2** публичных типа (`BulkInsertBuilder<TEntity>`, `BulkInsertReturningBuilder<TEntity,TResult>`) + **3** internal (`BulkInsertCommand`, `PortableBulkInsertExecutor`, `IBulkInsertExecutor`) + `SyncToAsyncEnumerable<T>`; `roslyn members NextORM.Core.BulkInsertBuilder` — **14** public-методов + internal ctor; `roslyn members NextORM.Core.BulkInsertReturningBuilder` — **3** public-метода + internal ctor; `rg 'TimeoutSeconds' src/` — 7 упоминаний, нативные hooks timeout не принимают; `rg 'ExternalTransaction|SqlBulkCopyOptions' src/nextorm.sqlserver` — 0; `wc -l` — `BulkInsertBuilder.cs` 471, `BulkInsertReturningBuilder.cs` 207, `PortableBulkInsertExecutor.cs` 180, `BulkInsertCommand.cs` 129, `SyncToAsyncEnumerable.cs` 30, `PostgresDataContext.cs` 120, `SqlServerDataContext.cs` 186, `DataContext.cs` 788; EOL — 11/11 новых файлов LF-only, все изменённые — CRLF. Ссылка на MS-поведение `SqlBulkCopy`+transaction — `learn.microsoft.com/sql/connect/ado-net/sql/transaction-bulk-copy-operations`. Тесты фичи не запускались.

---

## 🔁 Ре-аудит 24.09.2026 — Bulk insert (options-API), uncommitted working tree; 🔴 — 0, 🟡 — 2 (новые 141–142), ℹ️ — 5

**Область и что изменилось.** Fluent-методы записи удалены из `BulkInsertBuilder<TEntity>` (**14 → 7** public) и вынесены в `BulkInsertOptions` (`src/nextorm.core/Builders/BulkInsertOptions.cs`, **217** строк) + `BulkInsertOptionsBuilder`; `BulkInsertBuilder<TEntity>` принимает `BulkInsertOptions` в `internal` ctor (`Builders/BulkInsertBuilder.cs:32`, **391**). `PortableBulkInsertExecutor.cs` **180**, `BulkInsertCommand.cs` **140**, `PostgresDataContext.cs` **154**, `SqlServerDataContext.cs` **226**. Прогресс — `Action<int, CancellationToken>`; `NotifyAfter` синхронный (native `SqlBulkCopy.SqlRowsCopied` не умеет await); SQL Server-натив передаёт `BulkCopyTimeout`/`BatchSize`/транзакцию и разворачивает `OperationAbortedException`. Тесты: core `BulkInsertBuilderTests` **9**, integration `CommonTestSuite.BulkInsert.cs` **9**, SQL-gen sqlite **6** / postgres **6** / sqlserver **4** / mysql **4**.

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors**. Подавления `src/` — **6 `SuppressMessage` + 5 `#pragma` = 11/11** (0 неоправданных), в bulk-файлах — **0**. Слоп: `Skip=` — 0; `Task.Delay` — 2 baseline (`tests/nextorm.core.tests/InMemoryTests.cs:125,414`); `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `Assert.SkipUnless` в integration — **3** (`CommonTestSuite.BulkInsert.cs:105,123,146`, рантайм-гейт по capability провайдера, не SW001). `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов вручную. `.editorconfig` — те же 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer` + `CA2254`). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5). EOL: все bulk-файлы CRLF (в т.ч. новый `BulkInsertOptions.cs` 217/217) — Находка 140 закрыта.

**Статус прежних находок (134–140).**

| # | Прежний ур. | Статус 24.09.2026 | Основание |
|---|-------------|-------------------|-----------|
| 134 | 🔴 P1 | **закрыто** | SQL Server выставляет `BulkCopyTimeout = commandTimeoutSeconds ?? 30` (`SqlServerDataContext.cs:202`); PostgreSQL `COPY` и portable-путь отклоняют timeout через `NotSupportedException` (`PostgresDataContext.cs:143-147`, `PortableBulkInsertExecutor.cs:110-115`); XML-doc `BulkInsertOptions.TimeoutSeconds` (`:38-43`) соответствует коду. |
| 135 | 🔴 P1 | **закрыто** | `CreateBulkCopy` передаёт текущую транзакцию: `transaction = (this as ITransactionManager)?.CurrentTransaction as SqlTransaction` (`SqlServerDataContext.cs:197`), `new SqlBulkCopy(connection, SqlBulkCopyOptions.Default, transaction)` `:199`. |
| 136 | 🟡 P2 | **закрыто** | `BulkInsertReturningBuilder.ToSql`/`ToList` вызывают `_bulk.ValidateSyncSource()` (`:48,:76`), `ToListAsync` — `_bulk.ValidateSource()` (`:114`); source-less returning-терминал теперь даёт `InvalidOperationException`, а не NRE. |
| 137 | 🟡 P2 | **закрыто (по контракту); остаток — 141** | `MaxBatchSize` доходит до натива (`DataContext.cs:371,386`), SQL Server выставляет `SqlBulkCopy.BatchSize` (`:203`) — «ignored»-дефекта больше нет. Остаток — устаревшая XML-doc-ссылка и буферизация `DataTable` целиком → Находка 141. |
| 138 | 🟡 P2 | **открыто** | `ToRow` по-прежнему `object?[]` на строку + `PropertyInfo.GetValue` (`BulkInsertBuilder.cs:201-208`); `BuildBatch` — `InsertValue[]` на колонку на батч (`PortableBulkInsertExecutor.cs:156-165`). Правок нет; маршрут `nextorm-design-engineer`/`nextorm-db-perf-analyst`. |
| 139 | 🟡 P2 | **открыто** | Батч-цикл по-прежнему продублирован: portable — 3 цикла (`PortableBulkInsertExecutor.cs:16-45,57-81,83-108`), returning — 3 (`BulkInsertReturningBuilder.cs:74,131,163`), нарезка первого батча — 2 (`BulkInsertBuilder.ToSql:120`, `BulkInsertReturningBuilder.ToSql:46`). Рефакторинга нет. |
| 140 | 🟡 P2 | **закрыто** | EOL-проверка: `cr = lines` у всех bulk-файлов, включая новый `BulkInsertOptions.cs`; изменённые (отслеживаемые) — CRLF. Смешанных EOL нет. |

**Новые находки.**

### 🟡 Находка 141 (P2, док/регрессия переименования) — XML-doc SQL Server-натива ссылается на удалённый `BulkInsertBuilder.MaxBatchSize` и вводит в заблуждение по памяти

**Место:** `src/nextorm.sqlserver/SqlServerDataContext.cs:111-114`.

**Что не так.** Док `BulkInsertRows` советует «bound the set with `BulkInsertBuilder.MaxBatchSize` for very large writes», но `BulkInsertBuilder.MaxBatchSize` удалён в этом же изменении (опция теперь `BulkInsertOptions.MaxBatchSize`/`BulkInsertOptionsBuilder.MaxBatchSize`). Плюс совет обманывает по памяти: набор целиком буферизуется в `DataTable` до `WriteToServer` (`:131-135`, `:171-175`), а `SqlBulkCopy.BatchSize` (`:203`) лишь дробит отправку на сервер и память не ограничивает.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** заменить ссылку на `BulkInsertOptions.MaxBatchSize` и явно сказать, что нативный путь буферизует набор целиком. Публичную форму не менять.

**Проверка:** `rg 'BulkInsertBuilder\.' docs src -g '!docs/specs/**'` — единственное совпадение `:114`; `roslyn members NextORM.Core.BulkInsertBuilder` — `MaxBatchSize` отсутствует. Публичная сторона — `API-NAMING-REVIEW.md`, BK7.

### 🟡 Находка 142 (P2, контракт/стоимость) — `Progress` с дефолтным `NotifyEvery = 1` даёт O(N) вызовов с дублирующимся накопленным счётом на portable-пути

**Место:** `src/nextorm.core/Builders/BulkInsertBuilder.cs:303-328` (`CreateProgressCallback`), `:229-245` (`BuildCommand`); `src/nextorm.core/Builders/BulkInsertOptions.cs:55-67` (`Progress`, `NotifyEvery = 1`); `src/nextorm.core/DataContext/PortableBulkInsertExecutor.cs:33,41,69,77,96,104` (`command.Progress?.Invoke(total)` раз на батч).

**Что не так.** Обёртка прогресса тикает `while (total >= next) { onProgress(total, token); next += every; }` (`:315-326`). На native-пути `total` приходит ровно кратно `NotifyEvery` (`SqlRowsCopied`/`NotifyAfter`), поэтому вызов ровно один. На portable-пути `command.Progress` вызывается **раз на батч** с накопленным `total`, который может быть много больше `NotifyEvery` (по умолчанию вообще `1`): тогда цикл выполняет `total / every` итераций, каждый раз передавая **один и тот же** `total`. Для `ctx.BulkInsertInto<E>(new BulkInsertOptions { Progress = (n, ct) => … }).Values(1_000_000).BulkInsert()` на portable-провайдере это **1 000 000** вызовов колбэка с `n = 1_000_000`. Интеграционный тест `BulkInsert_NotifyAfter_ShouldReportCumulativeProgress` маскирует дефект, задавая `MaxBatchSize(10)` рядом с `NotifyAfter(10, …)` (батч == интервал).

**Стало (рекомендация, маршрут `nextorm-design-engineer`/`nextorm-db-perf-analyst`):** на portable-пути репортить один раз на вызов колбэка (обновлять `next` за `total`), либо зафиксировать в доке/валидации, что `Progress` требует `NotifyEvery` не меньше размера батча, либо прореживать батчи по `NotifyEvery`. Публичную форму не менять.

**Проверка:** чтение `CreateProgressCallback`/`PortableBulkInsertExecutor`; `BulkInsert_NotifyAfter_ShouldReportCumulativeProgress` (`CommonTestSuite.BulkInsert.cs:48-66`) ставит `MaxBatchSize(10).NotifyAfter(10, …)`; теста на `Progress` без `NotifyAfter`/с дефолтным `NotifyEvery` нет.

ℹ️ **Наблюдения (ре-аудит, фикс не требуется).**

- **A. Синхронный `NotifyAfter` — осознанно.** Второй аргумент — `CancellationToken`; API намеренно не принимает `Func<..., ValueTask>`, потому что native `SqlBulkCopy.SqlRowsCopied` (и его `NotifyAfter`) синхронен и не может await (`BulkInsertOptions.cs:45-55`). Согласовано с доком; асинхронного варианта нет — не находка.
- **B. CTS в публичном record.** `BulkInsertOptions.ProgressCancellationTokenSource` — `IDisposable` в `sealed record`, задокументирован как caller-owned: nextorm не отменяет и не диспозит (`:56-64`); после `Dispose()` обращение к `.Token` бросит `ObjectDisposedException`. Дизайн-компромисс для cooperative `CancelAfter` (альтернатива — принимать `CancellationToken`), не дефект.
- **C. SQL Server unwrap корректен.** `catch (Microsoft.Data.OperationAbortedException ex) when (ex.InnerException is OperationCanceledException)` + `ExceptionDispatchInfo.Capture(ex.InnerException).Throw()` (`SqlServerDataContext.cs:145-148,185-188`) — не пустой `catch`, не `throw ex` (CA2200 чист); переводит отмену из `SqlRowsCopied` в единый `OperationCanceledException` на обоих путях. Это единственные `catch` в bulk-src.
- **D. Прочие категории чисты.** LINQ на горячем пути — нет (плоские `for`/`foreach`); новых хэш-ключей/`GetHashCode`/словарей — нет (bulk в план-кэш не пишется); новых disposable-полей/`using` — нет; god-классов >500 нет (`SqlServerDataContext` 226, `BulkInsertOptions` 217, `BulkInsertReturningBuilder` 207, `PortableBulkInsertExecutor` 180, `BulkInsertCommand` 140, `BulkInsertBuilder` 391); EOL — CRLF.
- **E. Пробелы покрытия.** Находки 135/136 закрыты кодом, но тестов на них нет; нет тестов на `MaxParameters`/`MaxSqlLength`/`Returning<TResult>`/нативные PG/SQL Server пути против контейнера; `DOCKER_HOST`-прогон аудитом не выполнялся. Публичная сторона — `API-NAMING-REVIEW.md`, BK1–BK8.

**Итог.** Закрыты 134/135/136/137/140; открыты 138/139; новые 141 (устаревшая док-ссылка) и 142 (O(N) вызовов `Progress`) — маршрут `nextorm-design-engineer` (142 — с перф-аналитиком). Новых P0/P1 нет: 🔴 — 0, 🟡 — 2, ℹ️ — 5. Подавления, слоп, хэш-ключи, EOL и zero-cost-гейты — чисто.

**Проверка (24.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0**; подавления `src/` — **6+5=11** (в bulk-файлах — 0); `Skip=` — 0, `Task.Delay` — 2 baseline, `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `slopwatch` не установлен, скан вручную; `find -name 'PublicAPI*.txt'` — **0**; `rg 'BulkInsertBuilder\.' docs src -g '!docs/specs/**'` — **1** (`SqlServerDataContext.cs:114`, Находка 141); EOL bulk-файлов — CRLF; тесты (Release, `--no-build`): core `BulkInsertBuilderTests` **9/9**; SQL-gen bulk — sqlite **6/6**, postgres **6/6**, sqlserver **4/4**, mysql **4/4**. Контейнерные тесты не запускались.

---

## 🔁 Ре-аудит 24.09.2026 — CTAS на SQL Server (`SELECT ... INTO`) и ClickHouse (persistent `CREATE TABLE ... ENGINE = MergeTree ... AS SELECT`), uncommitted working tree; 🔴 — 0, 🟡 — 2 (143–144; 143 исправлено 24.09.2026, 144 принято), ℹ️ — 9

**Область.** Новый незарелиженный инкремент над CTAS 23.09.2026 (находки 126–128 не переоткрываются). `ISqlDialect` (DIM): `SupportsTemporaryCreateTableAsSelect` (`DataContext/Dialect/ISqlDialect.cs:1477`), `SupportsCreateTableAsSelectIfNotExists` (`:1484`), `CreateTableAsSelectUsesSelectInto` (`:1490`), `MakeCreateTableAsSelectInto` (`:1522`); смысл `SupportsCreateTableAsSelect` уточнён до «постоянная материализация» (`:1469`); `SqlDialectBase`-двойники (`:891,894,897,913`). Override'ы: SQL Server (`src/nextorm.sqlserver/SqlServerDialect.cs:58,61,64,67,70`), ClickHouse (`src/nextorm.clickhouse/ClickHouseDialect.cs:79,82,85,88`). Рендер: `SqlMutationBuilder.ResolveCreateTableAsInto` (`DataContext/SqlMutationBuilder.cs:429`), `MakeCreateTableAsSelect` (`:458`), `ValidateCreateTableAs` (`:477`), `BuildCreateTableAsClause` (`:504`); диспетчер `DataContext.BuildCreateTableAsSql` (`DataContext/DataContext.cs:550-560`); инъекция в select-list `SqlBuilder.MakeSelect(QueryCommand, string? selectInto = null)` (`DataContext/SqlBuilder.cs:33,464-465`), проброс `QueryPlanner.RenderSource(..., string? selectInto = null)` (`DataContext/QueryPlanner.cs:70-73,77,109`). Тесты: `tests/nextorm.sqlserver.tests/CreateTableAsSqlGenerationTests.cs` (10: +`Table_WithKeywordCaseUpper_ShouldKeepKeywordCaseConsistent`, +`Table_WithQuotedIdentifiers_ShouldQuoteTargetAndColumns`), `tests/nextorm.clickhouse.tests/CreateTableAsSqlGenerationTests.cs` (6), интеграционный `CommonTestSuite.CreateTableAs.cs` (`CreateTableAs_Table_ShouldBeReadableOnTheSameContext`), `ITestProvider.SupportsTemporaryCreateTableAsSelect`. Публичная сторона — `API-NAMING-REVIEW.md`, CTAS5–CTAS7.

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено). Подавления `src/` — **6 `SuppressMessage` + 5 `#pragma` = 11/11** (0 неоправданных), новых в CTAS-файлах — **0**. Слоп: `Skip=` — **0**; `Task.Delay` — **2** baseline (`tests/nextorm.core.tests/InMemoryTests.cs:125,414`); `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — **0**; `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов вручную. `.editorconfig` — те же **7** inert `dotnet_diagnostic.*.severity = silent` (6 `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5). EOL — CRLF (12/12 CTAS-файлов). `-p:AnalysisLevel=latest-all` в этом проходе не перезапускался; дефолтный набор SDK чист (`0/0`).

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | ✅ Новых disposable-полей/локальных нет: правки — DIM/override'ы, `readonly struct SqlBuilder`, internal-статика `SqlMutationBuilder`; `CreateTableAsClause` — `readonly record struct` без ресурсов. `CA2000`/`CA2213`/`CA1816` не затронуты. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — **0**; соотношение проекта **11/11**. |
| 3. LINQ на горячем пути | ✅ В `ResolveCreateTableAsInto`/`MakeCreateTableAsSelect`/`BuildCreateTableAsClause` LINQ нет (плоские `for`, `new List<Parameter>`); `MakeSelect` — один `Append`. DDL-путь, не hot path. |
| 4. God-классы/методы | ⚠️ `SqlMutationBuilder.cs` **866 → 943** (>500 — **Находка 110**, не переоткрывается); `MakeCreateTableAsSelect` — **6** параметров + `KeywordCase` (см. ℹ️ C); методы <30 строк. |
| 5. Хэш-ключи / равенство | ✅ Новых `Dictionary`/`HashSet`/`GetHashCode` нет; `CreateTableAsClause` не ключ (мутации кэш не используют). Находка 128 (record-равенство о `Columns`) — без изменений, не переоткрывается. |
| 6. События / исключения | ✅ Новых подписок/`catch` нет; гейты — `NotSupportedException` с actionable-текстом (`SqlMutationBuilder.cs:480,485,488,491,494,497,500`). |
| 7. NRT / возвратный тип | ✅ `string? selectInto`/`string? withSql` аннотированы; сборка `0/0`; возвраты `(string, List<Parameter>)`/`string?`, `async void` нет. |

### ✅ Находка 143 (P2, консистентность/латентный дефект; **исправлено 24.09.2026**) — `INTO`-фрагмент и обёртка CTAS используют `KeywordCase`/`QuoteIdentifiers` контекста, а тело — источника: `WithKeywordCase(Upper)` даёт смешанный SQL

**Место (на момент находки):** `src/nextorm.core/DataContext/DataContext.cs:550-560` (`BuildCreateTableAsSql`: `ResolveCreateTableAsInto(Dialect, QuoteIdentifiers, command, KeywordCase)` и `MakeCreateTableAsSelect(..., KeywordCase)`); тело — `src/nextorm.core/DataContext/QueryPlanner.cs:95,109` (`KeywordCase = source.ResolvedKeywordCase`, `QuoteIdentifiers = source.ResolvedQuoteIdentifiers`); прецедент правильного разрешения — `DataContext.cs:509-511` (`MakeMergeSql`).

**Что не так.** `EntityBuilder<TEntity>.WithKeywordCase(Upper)` (`Builders/EntityBuilder.cs:1536`) кладёт `Upper` в `QueryCommand.KeywordCase` (`:211,269`), `QueryCommand.QueryPreparer.cs:29` разрешает в `ResolvedKeywordCase`; тело SELECT рендерится с `source.ResolvedKeywordCase`, а `into`-фрагмент (`MakeCreateTableAsSelectInto`) и `create table … as`/`engine = …`-обёртка — с `DataContext.KeywordCase` (по умолчанию `Lower`). Для `ctx.From<IOrder>().WithKeywordCase(Upper).Select(...).ToTable("t")` на SQL Server получается `SELECT id into t FROM …` (`into` в нижнем регистре), на PostgreSQL — `create table t as SELECT …`. Аналогично `WithQuoteIdentifiers(true)` на источнике котирует тело, но не CTAS-цель. MERGE в том же `BuildMutationSql` решает это в пользу источника (`:511`). Дефект латентный (смешанный регистр — валидный SQL), но публичный контракт «A single command can override the setting» (`DI/DataContextBuilder.cs:118-122`) для CTAS не выполняется; теста нет.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** в `BuildCreateTableAsSql` брать `var keywordCase = command.Source?.ResolvedKeywordCase ?? KeywordCase;` (и `quoteIdentifiers` от источника) и передавать их в `ResolveCreateTableAsInto`/`MakeCreateTableAsSelect`; SQL-gen тест `WithKeywordCase(Upper)` на SQL Server/PG. Публичную форму не менять.

**Проверка:** трассировка `EntityBuilder.cs:1536→211,269` → `QueryCommand.QueryPreparer.cs:29` → `QueryPlanner.cs:95,109`; `DataContext.cs:509-511,550-560`; `rg 'KeywordCase' tests/nextorm.*.tests/*CreateTableAs*` — на момент находки 0.

**Применено (24.09.2026, тот же worktree).** `BuildCreateTableAsSql` теперь берёт quoting/регистр из **nullable override'а команды-источника до её подготовки** — `command.Source.QuoteIdentifiers ?? QuoteIdentifiers`, `command.Source.KeywordCase ?? KeywordCase` (`DataContext.cs:556-557`) — и передаёт их и в `ResolveCreateTableAsInto`, и в `MakeCreateTableAsSelect`; тело получает то же значение через `RenderSource`. `Resolved*`-поля брать нельзя (до `RenderSource` команда не подготовлена), поэтому выбран именно override. Регрессионные SQL-gen тесты: `Table_WithKeywordCaseUpper_ShouldKeepKeywordCaseConsistent` (`SELECT id INTO archive_ids FROM simple_entity`) и `Table_WithQuotedIdentifiers_ShouldQuoteTargetAndColumns` (`select [id] into [archive ids] from [simple_entity]`).

### 🟡 Находка 144 (P2, дублирование/сопровождаемость) — `ValidateCreateTableAs` прогоняется дважды на каждый рендер CTAS

**Место:** `src/nextorm.core/DataContext/SqlMutationBuilder.cs:429-436` (`ResolveCreateTableAsInto` → `ValidateCreateTableAs`), `:458-466` (`MakeCreateTableAsSelect` → `ValidateCreateTableAs`); вызывающий — `DataContext.cs:554-556`.

**Что не так.** `BuildCreateTableAsSql` вызывает оба метода последовательно, и каждый независимо прогоняет полный набор гейтов (`:477-503`). Поведения это не меняет (детерминировано и дёшево), но это новый дубль, появившийся при разделении прежнего `MakeCreateTableAsSelect`; тот же класс, что 139 (DRY), малого масштаба.

**Стало:** валидировать один раз — в `ResolveCreateTableAsInto` (единственный безусловный вход) — и убрать повтор из `MakeCreateTableAsSelect`, либо вынести гейты в общий приватный вход. Публичную форму не менять.

**Проверка:** `DataContext.cs:554-560`; `SqlMutationBuilder.cs:431,460,477-503`.

**Принято (24.09.2026).** Двойной прогон `ValidateCreateTableAs` оставлен как есть: путь холодный (DDL, не выполняется на каждый запрос), проверки детерминированы и без аллокаций; рефакторинг признан нерентабельным. Публичную форму не менять.

### ℹ️ Наблюдения (фикс не требуется)

- **A. (a) Утечки `selectInto` во вложенные запросы нет.** `roslyn refs NextORM.Core.SqlBuilder.MakeSelect` — **10**: фрагмент передаёт только `QueryPlanner.cs:109` (через `RenderSource(source, selectInto)`, чей единственный 2-арг вызов — `DataContext.cs:554`); вложенные вызовы — UNION-ветка (`SqlBuilder.cs:300`), CTE-обход/рендер (`SqlSourceRenderer.cs:52,86`), тело data-modifying CTE (`:134`), производная таблица (`:430`), подзапросы (`MemberTranslator.cs:498`, `NormSqlTranslator.cs:191,242`) — все с одним аргументом. Два вызова `RenderSource` для INSERT/MERGE (`DataContext.cs:449,500`) — без `selectInto`. Утечки нет.
- **B. (d) `MakeSelect` — zero-cost.** Правка — `if (selectInto is not null) selectBuilder.Append(selectInto);` внутри `if (!_ctx.ParamMode)` (`SqlBuilder.cs:464-465`): для обычного SELECT один null-check, без LINQ/аллокаций; для CTAS — одна `Append` короткой строки. Регрессии нет.
- **C. (c) Рост аргументов `SqlMutationBuilder` — внутренний, в стиле файла.** `MakeCreateTableAsSelect` — **7** параметров (`:458`), `ResolveCreateTableAsInto` — **4** (`:429`), `BuildCreateTableAsClause` — **3** (`:504`); все `internal`, DDL-путь, рядом уже `RenderSourceInsert` на 12 параметров. Разделение «валидация+резолв» / «рендер» — переиспользуемость, не ухудшение; расширяет уже открытую Находку 110 (god-класс), не новую.
- **D. Гейты полны и согласованы.** Temporary (`:485`), Columns (`:488`), IfNotExists (`:491`), OnCommit (`:494,497`), WithNoData (`:500`); тексты обезличены до «a materialised table» (корректно для `SELECT ... INTO`). SQL Server отклоняет temp/IfNotExists/columns, ClickHouse — temp/columns; оба проходят persistent.
- **E. EOL CRLF.** 12/12 CTAS-файлов (core 6, sqlserver 1, clickhouse 1, tests 3, план 1) — `cr == lines`; рекуррентный LF-дефект (Находки 20/22/29/112/140) не воспроизвёлся.
- **F. Тесты зелёные (SQL-gen).** Release `--no-build`, фильтр `FullyQualifiedName~CreateTableAs`: core **4/4**, postgres **7/7**, sqlite **6/6**, mysql **6/6**, mariadb **5/5**, sqlserver **10/10**, clickhouse **6/6** — итого **44/44, 0 failed, 0 skipped** (перепрогон 24.09.2026 после фиксов 143/CTAS7). Контейнерная интеграция аудитом не перезапускалась (по отчёту автора — CTAS 14 passed / 6 capability-skipped / 0 failed, включая persistent `ToTable` на SQL Server и ClickHouse).
- **G. Пробелы покрытия.** (1) ~~`WithKeywordCase(Upper)`+CTAS — 0 тестов (триггер 143)~~ — **закрыто 24.09.2026** (2 SQL-gen теста, см. Находку 143); (2) `ToTable`/`ToTempTable` на готовом `QueryCommand<TResult>` — 0; (3) persistent round-trip только `CreateTableAs_Table_ShouldBeReadableOnTheSameContext` (без `IfNotExists`/columns/CTE); (4) T-SQL temp-через-имя `ToTable("#name")` — не тестируется.
- **H. Публичные доки — исправлено 24.09.2026** (`API-NAMING-REVIEW.md`, CTAS5): EN+RU `guide/22`, `providers/overview`, `limitations`, `api-reference` + internal `capability-matrix:79`, `sql-capabilities-gap-analysis` приведены к факту (`ToTable` на SQL Server/ClickHouse, `ToTempTable` — только на PG/SQLite/MySQL/MariaDB); `todo_create_table_as.md` удалён.
- **I. `.editorconfig`/анализаторы.** 7 `silent`-правил без изменений; `-p:AnalysisLevel=latest-all` в этом проходе не перепроверялся, дефолтная сборка `0/0`.

**Итог.** 🔴 — **0**; 143 — **исправлено** 24.09.2026 (2 регрессионных теста); 144 — **принято** (холодный DDL-путь). Публичная сторона — `API-NAMING-REVIEW.md`: CTAS5 и CTAS7 **исправлены** тем же проходом, CTAS6 отложен. Побег `selectInto`, аллокации `MakeSelect`, рост параметров и EOL — чисто.

**Проверка (24.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); подавления `src/` — **6+5=11** (0 неоправданных), новых в CTAS-файлах — **0**; `Skip=` — 0, `Task.Delay` — 2 baseline, `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `slopwatch` не установлен, скан паттернов вручную; `find -name 'PublicAPI*.txt'` — **0** (Шаг 5); `roslyn refs NextORM.Core.SqlBuilder.MakeSelect` — **10** (фрагмент несёт только `QueryPlanner.cs:109`); `roslyn refs NextORM.Core.QueryPlanner.RenderSource` — **2** (`DataContext.cs:449,500` — оба без `selectInto`); `wc -l` — `SqlMutationBuilder.cs` **943**, `SqlBuilder.cs` 850, `QueryPlanner.cs` 533, `DataContext.cs` 809, `ISqlDialect.cs` 1549, `SqlDialectBase.cs` 959, `SqlServerDialect.cs` 692, `ClickHouseDialect.cs` 829; EOL — CRLF; тесты (Release, `--no-build`, фильтр `~CreateTableAs`, перепрогон 24.09.2026): core 4, postgres 7, sqlite 6, mysql 6, mariadb 5, sqlserver 10, clickhouse 6 — **44/44, 0 failed, 0 skipped**. SQL Server `ORDER BY` в `SELECT ... INTO` подтверждён документацией (Microsoft Learn, INTO Clause: «Specifying an ORDER BY clause doesn't guarantee the rows are inserted in the specified order»; пример E его использует) — не дефект. Контейнерная интеграция не перезапускалась.

**Применено (24.09.2026, тот же worktree).**
- **143 (🟡, code) — исправлено.** `DataContext.BuildCreateTableAsSql` (`:556-557`) выводит quoting/регистр из nullable override'а команды-источника (`command.Source.QuoteIdentifiers ?? QuoteIdentifiers`, `command.Source.KeywordCase ?? KeywordCase`) и передаёт одно и то же значение и в `ResolveCreateTableAsInto`/`MakeCreateTableAsSelect`, и (через `RenderSource`) в тело. Регрессия: 2 SQL-gen теста (`Table_WithKeywordCaseUpper…`, `Table_WithQuotedIdentifiers…`); см. Находку 143.
- **CTAS7 (P2, API) — исправлено.** `ISqlDialect.cs:1484`/`SqlDialectBase.cs:894` теперь `false`; явные `true`-override'ы у PostgreSQL (`PostgresDialect.cs:409`), SQLite (`SqliteDialect.cs:253`), MySQL (`MySqlDialect.cs:418`), ClickHouse (`ClickHouseDialect.cs:82`); SQL Server — `false` (`SqlServerDialect.cs:64`). Поведение `ToTable`/`IfNotExists` не изменилось.
- **CTAS5 (P1, API) — исправлено.** EN+RU `guide/22`, `providers/overview`, `limitations`, `api-reference` + internal `capability-matrix:79` и `sql-capabilities-gap-analysis` (шапка 27, §5 п.21, §6 27) приведены к факту; `todo_create_table_as.md` удалён. `TempTableExtensions` XML-doc описан через три persistent-формы + temp-only-на-4-провайдерах.
- **CTAS6 (P2, API) — отложено** до заморозки публичного API (keep-as-is); **144 (🟡, code) — принято** (холодный DDL-путь).

---

### Аудит 24.09.2026 — режимы ожидания блокировки строк (`NOWAIT`/`SKIP LOCKED`), uncommitted working tree (🔴 — 0; 🟡 — 2: Находки 145/146; ℹ️ — наблюдения A–F)

**Область.** Фича `LockWaitMode { Wait, NoWait, SkipLocked }` (`src/nextorm.core/Query/LockWaitMode.cs:4`), `LockClause` расширен `Wait` (`src/nextorm.core/Query/LockClause.cs:8`), перегрузки `EntityBuilder.ForUpdate/ForShare(LockWaitMode)` (`src/nextorm.core/Builders/EntityBuilder.cs:779,797`), DIM `ILockRenderer.Render(LockMode, LockWaitMode, KeywordCase)` (`src/nextorm.core/DataContext/Dialect/DialectCapabilities.cs:267`), прокидывание режима в `SqlBuilder.cs:94,395`, override'ы в `PostgresLockRenderer` (`src/nextorm.postgres/PostgresDialect.cs:502-520`), `MySqlLockRenderer` (`src/nextorm.mysql/MySqlDialect.cs:450-482`), `MariaDbLockRenderer` (`src/nextorm.mariadb/MariaDbDialect.cs:52-71`, `MariaDbDialect.Lock` override `:49`) и `SqlServerLockRenderer` (`src/nextorm.sqlserver/SqlServerDialect.cs:666-683`). Транзакции/bulk-insert/sql-batch в worktree — вне прохода.

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors**. Подавления `src/` — **6 `SuppressMessage` + 5 `#pragma` = 11/11** (0 неоправданных), в lock-файлах — **0**. Слоп: `Skip=` — 0; пустых `catch`/`Thread.Sleep`/`NoWarn`/inline `Version` — 0; `Task.Delay` — **2** baseline (`tests/nextorm.core.tests/InMemoryTests.cs:132,421` — `Task.Delay(0)`); `Assert.SkipUnless` в новом integration-тесте — **1** (`tests/nextorm.integration.tests/CommonTestSuite.Locking.cs:10,13`, рантайм-гейт по capability провайдера, не SW001). `slopwatch` локально не установлен (`.config/dotnet-tools.json` — только coverage/reportgenerator/docfx), скан паттернов **вручную**. `.editorconfig` — те же 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее, не переоткрывается). `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`), `NoWarn` — **0** ⇒ CS1591 гейтится, build `0/0` = XML-doc новых публичных членов 100 %.

### ✅ Находка 145 (P2, хэш-ключи плана) — `QueryPlanEqualityComparer.GetHashCode` хэширует только `LockClause.Mode`, а `Equals` сравнивает весь record вместе с новым `Wait` — **исправлена 24.09.2026**

**Место:** `src/nextorm.core/Query/QueryPlanEqualityComparer.cs:412-415` (`GetHashCode`: `hash.Add(rowLock.Mode)`), `:84` (`Equals`: `if (x.RowLock != y.RowLock) return false;`), `src/nextorm.core/Query/LockClause.cs:8`.

**Что не так.** Фича добавила в `LockClause` второе поле `Wait` (`LockClause(LockMode Mode, LockWaitMode Wait = LockWaitMode.Wait)`); record-равенство и `!=` его учитывают, а хэш — нет. `ForUpdate(LockWaitMode.Wait)` и `ForUpdate(LockWaitMode.SkipLocked)` дают разные ключи по `Equals` (корректно) и одинаковый `GetHashCode` (лишняя коллизия в plan-кэше). Контракт `IEqualityComparer` не нарушен — коллизии допустимы, `Equals` доводит различие, — но это ровно та категория, что ведётся в реестре («Хэш-ключи»), и соседи по компаратору хэшируют **все** поля: `Temporal` Kind/From/To (`:405-410`), `TableSample` Method/Percent/Seed (`:398-403`). Реестровое описание плана-ключа (`code-smells-review.md:1870`, «`RowLock` — record-равенство + `Mode`») фиксировало состояние **до** фичи и стало неполным.

**Стало:** добавить `hash.Add(rowLock.Wait);` рядом с `hash.Add(rowLock.Mode);` в блоке `if (obj.RowLock is { } rowLock)` (`:412-415`); обновить описание плана-ключа в реестре на «record-равенство + `Mode` + `Wait`». Публичную форму не менять. ✅ **Применено 24.09.2026:** `QueryPlanEqualityComparer.cs:415` — `hash.Add(rowLock.Wait)`; хэш теперь покрывает оба поля record'а.

**Проверка:** `QueryPlanEqualityComparer.cs:84` (`x.RowLock != y.RowLock` — record-`!=` по обоим полям) и `:412-415` (только `Mode`); `LockClause.cs:8` — позиционный record с двумя членами; `rg 'rowLock\.' src/nextorm.core/Query/QueryPlanEqualityComparer.cs` — единственное вхождение `:414` (`Mode`), `Wait` не встречается; сверка с `Temporal`/`TableSample`-блоками (`:398-410`).

### ℹ️ Находка 146 (P2, DRY/сопровождаемость) — `LockWaitSuffix` продублирован в трёх рендерерах, SQL Server несёт четвёртую inline-копию — **принята 24.09.2026 (вариант (в))**

**Место:** `src/nextorm.postgres/PostgresDialect.cs:514-520`, `src/nextorm.mysql/MySqlDialect.cs:475-481`, `src/nextorm.mariadb/MariaDbDialect.cs:64-70` — три побайтово одинаковых `private static string LockWaitSuffix(LockWaitMode)`; `src/nextorm.sqlserver/SqlServerDialect.cs:676-682` — эквивалентный inline-switch (другие суффиксы: `, nowait`/`, readpast`).

**Что не так.** Одна таблица `Wait→""` / `NoWait→" nowait"` / `SkipLocked→" skip locked"` вместе с `ArgumentOutOfRangeException`-дефолтом скопирована трижды между тремя провайдерскими сборками, четвёртый рендерер держит ту же логику в теле `Render`. Изменение отображения `LockWaitMode` требует правки в четырёх местах — тот же класс, что Находки 139/144 (мелкий DRY).

**Стало (к решению).** Вынести общий суффикс в один `internal static`-хелпер в `nextorm.core`. Ограничение: `InternalsVisibleTo` в репозитории не используется (`rg InternalsVisibleTo src` — только пояснительный комментарий `Visitors/BaseExpressionVisitor.cs:76`), поэтому варианты: **(а)** добавить `InternalsVisibleTo` для 4 провайдерских сборок + `internal`-хелпер (предпочтительно, без роста публичной поверхности); **(б)** `MariaDbLockRenderer` переиспользует `internal`-член `MySqlLockRenderer` (nextorm.mariadb ссылается на nextorm.mysql), PG/SS оставить с копией; **(в)** принять как есть — путь холодный (один рендер на построение SQL, план кэшируется), объём 21 строка. Публичную форму не менять.

**Проверка:** `rg -n 'LockWaitSuffix' src` — 3 объявления (`PostgresDialect.cs:514`, `MySqlDialect.cs:475`, `MariaDbDialect.cs:64`); SQL Server — inline `:676-682`; `SqlKeywords.Of` получает уже собранную строку (`PostgresDialect.cs:512`, `MySqlDialect.cs:460`, `MariaDbDialect.cs:62`, `SqlServerDialect.cs:676`).

### ℹ️ Наблюдения (фикс не требуется)

- **A. `ArgumentOutOfRangeException`-дефолты — корректны, оставить.** `LockWaitMode` — публичный enum, значение вне диапазона достижимо через явный каст из публичного API (`ForUpdate((LockWaitMode)42)`), поэтому defensive-default в `LockWaitSuffix`/`SqlServerLockRenderer` (и throwing-тело DIM `DialectCapabilities.cs:270`) — не мёртвый код, а валидация входа.
- **B. Аллокаций/LINQ на горячем пути нет.** `Render` вызывается один раз на построение SQL (`SqlBuilder.cs:94,395`), результат план-кэшируется; LINQ/`.Count()`/итераторов нет, только конкатенация строк и `SqlKeywords.Of` — как у соседних capability-клозов. Регрессии нет.
- **C. Видимость корректна.** `LockWaitMode` — `public` (нужен в публичной сигнатуре), `LockClause` — `internal sealed record`, все 4 рендерера — `internal sealed`, публичный `MariaDbDialect.Lock` override (`MariaDbDialect.cs:49`) снабжён XML-`<summary>`.
- **D. God-класс не усугублён.** `EntityBuilder.cs` — **1993** строки, ведётся как пре-существующий god-класс (`:3475`, Находка 68); фича добавила ~40 строк плюмбинга (`ForUpdate`/`ForShare` перегрузки + `WithRowLock`), новой декомпозиции не требует и в Находке 68 не переоткрывается.
- **E. Plan-key `Equals` корректен.** `x.RowLock != y.RowLock` (`:84`) — record-равенство по `Mode` **и** `Wait`; неполнота только в хэше (Находка 145), `QueryCommand.Clone.cs:75` копирует record по ссылке (иммутабельно).
- **F. EOL.** Все 9 затронутых lock-файлов (core 4, PG/MySQL/MariaDB/SqlServer 4, integration-тест 1) — CRLF (`cr == lines`) после правок; рекуррентный LF-дефект (Находки 20/22/29/112/140) не воспроизвёлся.

**Итог.** 🔴 — **0**; 145 (🟡, хэш-ключ) — ✅ **исправлена 24.09.2026** (`hash.Add(rowLock.Wait)`); 146 (🟡, DRY) — ℹ️ **принята 24.09.2026 вариантом (в)** (холодный путь построения SQL, 4 короткие копии, `InternalsVisibleTo` вводить не стали); остальное — наблюдения/принято. Публичная сторона — `API-NAMING-REVIEW.md`: RLM1/RLM3/RLM4 ✅ закрыты, RLM2 — трекинг Шага 5 (все P2).

**Проверка (24.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); подавления `src/` — **6+5=11** (в lock-файлах — 0), `Skip=` — 0, `Task.Delay` — 2 baseline, `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `slopwatch` не установлен, скан вручную; `rg 'LockWaitSuffix' src` — 3 копии + 1 inline (146); `QueryPlanEqualityComparer.cs:414` — `hash.Add(rowLock.Mode)` без `Wait` (145); `find -name 'PublicAPI*.txt'` — **0** (Шаг 5). Кодовая сторона: PG `PostgresDialect.cs:502-520`, MySQL `MySqlDialect.cs:450-482`, MariaDB `MariaDbDialect.cs:52-71`, SQL Server `SqlServerDialect.cs:666-683`. SQL-gen/диалект-тесты проходят (Release, `--no-build`): postgres `ForUpdate*`/`ForShare*` + `KeywordCase_Upper` покрывают режимы (`SqlGenerationTests.cs:3327-3387`, `PostgresDialectTests.cs:55-58`), mysql (`:594-626`, `MySqlDialectTests.cs:26-31`), mariadb (`SqlGenerationTests.cs:32-51`, `MariaDbDialectTests.cs:32-37`), sqlserver (`SqlGenerationTests.cs:2191-2290`, `SqlServerDialectTests.cs:190-193`), sqlite/clickhouse — `ShouldThrow` (`nextorm.sqlite.tests/SqlGenerationTests.cs:2455-2471`, `nextorm.clickhouse.tests/SqlGenerationTests.cs:47-52`), in-memory — `NotSupportedException` (`nextorm.core.tests/InMemoryTests.cs:75-84`). Контейнерная интеграция (`CommonTestSuite.Locking.cs`) аудитом не перезапускалась.

---

## 🔎 Точечный аудит 24.09.2026 — CTE как источник multi-table `UPDATE`/`DELETE` (linq2db G18), uncommitted working tree; 🔴 — 1 (147), 🟡 — 2 (148–149), ℹ️ — 7 (A–G); **147/148/149 исправлены 24.09.2026 (ре-верификация в конце раздела)**, открыт pre-existing 🟡 150 (`RenderSource`/INSERT)

**Область.** Фикс G18 по плану `docs/specs/roadmap/todo_cte_in_update.md`: новый `internal static CteMerge.Merge` (`src/nextorm.core/Builders/CteMerge.cs:17`), слияние CTE обеих сторон join в `EntityBuilder<TEntity>.JoinCore` (`src/nextorm.core/Builders/EntityBuilder.cs:1179`) и в двух non-generic `EntityBuilder.JoinCore` (`:1915`, `:1990`); хойст `source.Ctes` в `QueryPlanner.RenderDeleteJoin` (`src/nextorm.core/DataContext/QueryPlanner.cs:295-300`) и `RenderUpdateJoin` (`:328-333`). Публичной поверхности фикс не меняет (`CteMerge` — `internal`). Сопутствующее (вне G18): `DialectCapabilities.cs:241` — `<see cref="Render"/>` → `<see cref="Render(LockMode, KeywordCase)"/>`.

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено; untracked `tests/nextorm.integration.tests/EfCoreSharedTransactionTests.cs` временно отложен при сборке и возвращён на место). Подавления `src/` — **6 `SuppressMessage` (все с `Justification`) + 5 `#pragma` (все с парным `restore`) = 11/11**, новых в G18-файлах — **0**. Слоп: `Skip=` — **0**; пустых `catch`/`NoWarn`/inline `Version` — **0**; `Task.Delay` — **2** baseline (`tests/nextorm.core.tests/InMemoryTests.cs`, `Task.Delay(0)`-yield); `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов **вручную**. `.editorconfig` — те же **7** инертных `dotnet_diagnostic.*.severity` (6 `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее, не переоткрывается). `find -name 'PublicAPI*.txt'` — **0** (Шаг 5). EOL `CteMerge.cs` — CRLF (54/54).

**Метод верификации.** Статика + динамика: собран отдельный probe в `/tmp` (без правок репозитория) поверх `nextorm.sqlite`/`nextorm.sqlserver`, генерирующий SQL для 2- и 3-табличных цепочек, QueryCommand-join и recursive CTE на SQL Server.

| # | Проверка | Итог |
|---|----------|------|
| 1. `IDisposable` | ✅ Ресурсов нет: `CteMerge` — статика без полей; правки `EntityBuilder`/`QueryPlanner` ресурсов не заводят (`CA2000`/`CA2213`/`CA1816` не затронуты). |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — **0**; соотношение проекта **11/11**. |
| 3. LINQ на горячем пути | ✅ `CteMerge.Merge`/`Contains` — плоские `for`, без LINQ; при пустой второй стороне `Merge` возвращает исходный список (0 аллокаций), копия `[.. left]` — только при реальном слиянии. Хойст — одна склейка строк на мутацию (мутации планом не кэшируются). |
| 4. God-классы/методы | ✅ `CteMerge.cs` — 54 строки; `EntityBuilder.cs` растёт на 3 строки (god-класс Находки 68 не переоткрывается), `QueryPlanner` +10 строк; методы <30 строк. |
| 5. Хэш-ключи/равенство | ✅ `source.Ctes` входит в план-ключ источника через `QueryCommand.CtesPlanHash` (`QueryPlanEqualityComparer.cs:455`, `QueryCommand.QueryPreparer.cs:160-197`); мутации план не кэшируют (каждая — свежие `DefaultParameterProvider`/список). Новых `GetHashCode`/`record` нет. |
| 6. События/исключения | ✅ Подписок нет; новый `throw InvalidOperationException` — actionable-текст (`CteMerge.cs:48`); см. ℹ️ D. |
| 7. NRT/возвратный тип | ✅ `IReadOnlyList<CteDefinition>?`/`string?` аннотированы; сборка `0/0`; `async void` нет. |

### ✅ Находка 147 (P1, некорректность/неполный фикс; **исправлено 24.09.2026**) — CTE, добавленный на 2-м и последующих join'ах, не сливался, а сам join падал `NullReferenceException`: фикс покрывал только arity-2

**Место (на момент находки):** `src/nextorm.core/Builders/Joins/JoinedEntityBuilder.cs:85,202,304,406,508,610` — все шесть `JoinCore<T(n+1)>` (арности 2→3 … 7→8) ставили `Ctes = Ctes` (только левая сторона) и `From = _dataProvider.GetFrom(typeof(T(n+1)), null)!`; базовый `EntityBuilder<TEntity>.JoinCore` для того же делает `CteMerge.Merge(Ctes, _.Ctes)` (`EntityBuilder.cs:1179`) и `GetJoinSource(_)` (`:1190`), чей XML-doc (`:1184-1192`) прямо описывает необходимость превратить raw/CTE-источник (`Table`) в `FromExpression`. **После фикса:** сигнатуры `JoinCore` — `:80,201,304,407,510,613`; резолв/merge — `:87-88,205-206,308-309,411-412,514-515,617-618`; новый общий резолвер — `src/nextorm.core/Builders/Joins/JoinSourceResolver.cs:13` (базовый `EntityBuilder.GetJoinSource`, `EntityBuilder.cs:1190-1191`, делегирует ему).

**Что не так.** Любой `.Join(...)` на уже соединённом билдере (`JoinedEntityBuilder<…>`) разрешается в `new`-метод (`JoinedEntityBuilder.cs:30` и аналоги) → собственный `JoinCore`, минующий базовый merge. Если правый источник — CTE (`ctx.With("c", …).From("c")` всегда даёт `EntityBuilder<TableAlias>` с `Table`, но без `SourceFrom`), он не резолвится в `FromExpression`, `join.From` остаётся `null` → `NullReferenceException` в `SqlSourceRenderer.MakeFrom` (`DataContext/SqlSourceRenderer.cs:344`) ещё до рендера; даже если источник починить, `WITH` такого CTE всё равно не был бы поднят — merge там не вызывается. **Воспроизведение (probe, Release, SQLite):**

- `e.Join(scope.From("c"), …).Join(other, …).UpdateJoin()` (CTE на **первом** join) → `with c as (select id from simple_entity where (id > 0)) update simple_entity as 't1' set id = $p0 from c as 't2', other_entity as 't3' …` — корректно (базовый merge сработал);
- `e.Join(other, …).Join(scope.From("c"), …).UpdateJoin()` (CTE на **втором** join) → `NullReferenceException` в `SqlSourceRenderer.MakeFrom:344`;
- три «физических» join без CTE рендерятся нормально (`update … from other_entity as 't2', third_entity as 't3' …`) — значит дефект именно на CTE/`TableAlias`-источнике, а не на arity-3 как таковой.

**Почему не поймано.** Все новые тесты двухтабличные: `UpdateJoin_FromCte_ShouldHoistWith` (sqlite/postgres/sqlserver/mysql/mariadb), `DeleteJoin_FromCte_ShouldHoistWith` (postgres) и обе интеграционные `…_FromCte_…` — 2 таблицы; 3+ таблицы с CTE — **0** тестов. Failure mode — громкий `NullReferenceException`, не тихая потеря данных.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** в `JoinedEntityBuilder<…>.JoinCore` слить `Ctes` (`CteMerge.Merge(Ctes, _.Ctes)`) и резолвить источник через общий `GetJoinSource`-эквивалент (или вынести единый join-строитель для базового/соединённого путей), затем SQL-gen тест `UpdateJoin_FromCteOnThirdTable_ShouldHoistWith` (3 таблицы, CTE на 2-м join). Публичную форму не менять.

**Проверка:** probe SQLite — три вывода выше; `JoinedEntityBuilder.cs:85` (`Ctes = Ctes`); базовый merge `EntityBuilder.cs:1179`; `GetJoinSource`-док `EntityBuilder.cs:1184-1192`; новые тесты `_FromCte_` — все двухтабличные (`grep`).

**Применено (24.09.2026).** Новый `internal static JoinSourceResolver.Resolve<T>(IDataContext, EntityBuilder<T>)` (`JoinSourceResolver.cs:13`) вынесен из `EntityBuilder.GetJoinSource`; все шесть `JoinedEntityBuilder<…>.JoinCore` теперь делают `From = JoinSourceResolver.Resolve(_dataProvider, _)` и `cb.Ctes = CteMerge.Merge(Ctes, _.Ctes)` (`:87-88,205-206,308-309,411-412,514-515,617-618`). Ре-верификация probe (Release, SQLite): `e.Join(other).Join(scope.From("c")).UpdateJoin()` → `with c as (select id from simple_entity where (id > 0)) update simple_entity as 't1' set id = $p0 from other_entity as 't2', c as 't3' where t1.id = t2.id and t1.id = t3.id` — WITH поднят, `NRE` исчез. Новый SQL-gen тест `UpdateJoin_CteOnSecondJoin_ShouldHoistWith` (sqlite, `UpdateJoinSqlGenerationTests.cs`) — **1/1 passed** (Release, `--no-build`).

### ✅ Находка 148 (P2, корректность/диагностика; **исправлено 24.09.2026 — (a); (b) принято как ограничение**) — `CteMerge.Contains` проверял имя раньше, чем находил ссылку: дубликат имени в одной области давал ложное «declared on both sides»; сравнение `Ordinal` пропускает `c`/`C`

**Место (на момент находки):** `src/nextorm.core/Builders/CteMerge.cs:39-53` (`Contains`: имя проверялось в том же проходе, что и `ReferenceEquals`, → ложный `throw`), вызывающий — `Merge` (`:17-37`); эмиссия имени — `SqlSourceRenderer.cs:70` (`QuoteIdentifiers ? QuoteQualifiedIdentifier(...) : cte.Name`). **После фикса:** `Contains` — `CteMerge.cs:39-58` (два раздельных прохода: сначала полный скан `ReferenceEquals`, затем скан имён).

**Что не так (a).** Цикл бросает по имени, как только встречает *любой* элемент с тем же именем, не досканировав список на ссылочное совпадение. Если одна `CteQuery` объявляет одно имя дважды (`ctx.With("c", q1).With("c", q2)` — `CteQuery.Append`, `Builders/CteQuery.cs:123-128`, коллизии не запрещает), то self-join этой области (left и right — один и тот же список) для второго `c2` спотыкается о первый `c1` и бросает «declared on both sides of the join», хотя обе декларации на одной стороне. **Воспроизведение (probe):** `scope = ctx.With("c", q1).With("c", q2); scope.From("c").Join(scope.From("c"), …)` → `InvalidOperationException: The common table expression 'c' is declared on both sides of the join…`. Такой запрос и так невалиден (`with c as (…), c as (…)`), поэтому это неверная диагностика, а не регрессия валидного запроса.

**Что не так (b).** Коллизия ищется `StringComparison.Ordinal` по `Name`, тогда как при `QuoteIdentifiers=false` (`SqlSourceRenderer.cs:70`) имя эмитится как есть: `c` и `C` пройдут проверку, но на провайдерах со складыванием неквотированных идентификаторов (PostgreSQL — в нижний регистр; SQL Server/MySQL/SQLite — обычно без учёта регистра) дадут `with c as (…), C as (…)` — ошибку или неверную привязку. Проверка недоохватывает (раньше правая декларация молча терялась — тоже плохо), но детекция должна учитывать quoting/складывание.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** в `Contains` сначала просканировать весь список на `ReferenceEquals`, и только затем — по имени (либо один раз дедуплицировать список левой стороны до цикла по правой); для (b) — сравнение с учётом `QuoteIdentifiers`/dialect-складывания либо явная фиксация ограничения `Ordinal`. Публичную форму не менять.

**Проверка:** probe «one-scope-dup-name-self-join» → ложное сообщение (прогон выше); `CteMerge.cs:44-49`; `CteQuery.cs:123-128`; `SqlSourceRenderer.cs:70`; probe «distinct-scopes-same-name» → то же сообщение (корректная ветка).

**Применено (24.09.2026).** `Contains` (`CteMerge.cs:39-58`) сначала полностью просканирует список на `ReferenceEquals`, и только затем бросает по имени. Ре-верификация probe (Release, SQLite): self-join области с дублем имени → `joined-no-throw` (ложного исключения нет); две разные области с именем `c` → по-прежнему `InvalidOperationException` (корректно). Часть **(b)** (`Ordinal` без складывания регистра) — **принятое решение автора**: `CteMerge` работает на этапе построения join'а, где ещё нет dialect/quote-настроек, а сценарий «две области объявляют `c` и `C`» признан контрафактным; фиксируется здесь как осознанное ограничение, не переоткрывается.

### ✅ Находка 149 (P2, латентный дефект; **исправлено 24.09.2026**) — хойст `WITH` отбрасывал `maxRecursionStmt` (`out _`): recursive CTE с `MaxRecursion` терял `option (maxrecursion n)` на SQL Server

**Место (на момент находки):** `src/nextorm.core/DataContext/QueryPlanner.cs:297` (`RenderDeleteJoin`) и `:330` (`RenderUpdateJoin`) — `SqlSourceRenderer.MakeWithClause(in ctx, ctes, out _)`; ср. `SqlBuilder.MakeSelect` (`DataContext/SqlBuilder.cs:64,498-509`), где `maxRecursionStmt` дописывается после оператора (или сворачивается в один `OPTION` через `RenderQueryHints`). **После фикса:** захват/дописывание — `QueryPlanner.cs:296-305` (`RenderDeleteJoin`) и `:334-343` (`RenderUpdateJoin`).

**Что не так.** `MakeWithClause` возвращает `out maxRecursionStmt` (`SqlSourceRenderer.cs:22,36-37`), который `MakeSelect` кладёт в конец оператора. Новый хойст мутаций отдаёт его в `out _`, поэтому `WITH nums AS (…) UPDATE … FROM nums …` на SQL Server рендерится **без** `option (maxrecursion n)` — заявленный лимит молча теряется (дефолт 100). **Воспроизведение (probe, SQL Server, Release, SQLite-подобный SQL там не нужен):** `WithRecursive("nums", body, 50)` как источник `UpdateJoin` → в выводе `option (maxrecursion 50)` отсутствует. Тот же `out _` уже есть в pre-existing `RenderSource` (`QueryPlanner.cs:102`) для `INSERT … SELECT` — дефект общий, не только G18; фикс расширяет его зону на `UPDATE`/`DELETE`.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** принять `maxRecursionStmt` из `MakeWithClause` и дописать его к итоговому SQL (`(withSql ?? "") + sql + (maxRecursionStmt is null ? "" : " " + maxRecursionStmt)`), синхронно с `MakeSelect`; заодно закрыть тот же пропуск в `RenderSource` (INSERT). Тест SQL-gen SQL Server `UpdateJoin`/`InsertFrom` с `WithRecursive(…, maxRecursion)` и проверкой trailing `option (maxrecursion n)`. Публичную форму не менять.

**Проверка:** probe SQL Server (вывод без `option`); `QueryPlanner.cs:295-300,328-333`; `SqlBuilder.cs:64,498-509`; `SqlServerDialect.cs:527-528,548-564`; `grep MaxRecursion tests` — на мутационных путях **0**.

**Применено (24.09.2026).** Оба рендерера захватывают `maxRecursionStmt` из `MakeWithClause` и дописывают `" " + maxRecursionStmt` к оператору (`QueryPlanner.cs:296-305,334-343`). Ре-верификация probe (Release, SQL Server): вывод теперь оканчивается на `… join nums as [t2] on t1.id = t2.n option (maxrecursion 50)`. Новый SQL-gen тест `UpdateJoin_RecursiveCte_ShouldAppendMaxRecursion` (sqlserver) — **1/1 passed** (Release, `--no-build`). **Pre-existing остаток:** тот же `out _` в `RenderSource` (`QueryPlanner.cs:102`, `INSERT … SELECT`) **не тронут** — вынесен в отдельную Находку 150 ниже.

### 🟡 Находка 150 (P2, pre-existing, вне G18) — `RenderSource` (`INSERT … SELECT`) также отбрасывает `maxRecursionStmt`

**Место:** `src/nextorm.core/DataContext/QueryPlanner.cs:102` — `withSql = SqlSourceRenderer.MakeWithClause(in ctx, ctes, out _);`; результат склеивается в `DataContext.BuildInsertSql` (`DataContext/DataContext.cs:449-453`, `withSql + insertSql`).

**Что не так.** Та же потеря `option (maxrecursion n)` (`SqlSourceRenderer.cs:22,36-37`), что и в закрытой 149, но на пути `INSERT … SELECT` (`ctx.WithRecursive(…).From(…).Select(…).InsertInto(…)`) на SQL Server: `WITH … INSERT INTO … SELECT …` рендерится без trailing `OPTION`. Гейта/теста нет. Находка **pre-existing** — фиксом G18 не затрагивалась; зарегистрирована, чтобы не потерять.

**Стало (рекомендация, маршрут `nextorm-design-engineer`):** принять `maxRecursionStmt` и дописать его в конец итогового `insertSql` (`withSql` стоит перед `insert into`, а `OPTION`/`maxrecursion` — в конце оператора); тест SQL-gen SQL Server `InsertFrom` с recursive CTE. Публичную форму не менять.

**Проверка:** `QueryPlanner.cs:102` (`out _`); `SqlSourceRenderer.cs:22,36-37`; `SqlBuilder.cs:64,498-509` (корректный образец); `DataContext.cs:449-453`.

### ℹ️ Наблюдения (фикс не требуется)

- **A. Хойст рендерит `WITH` ровно один раз, двойного рендера нет.** `SqlBuilder.MakeDeleteJoin`/`MakeUpdateJoin` (`DataContext/SqlBuilder.cs:543,637`) не читают `source.Ctes` вовсе и возвращают `_ctx.Params`; `RenderDeleteJoin`/`RenderUpdateJoin` — **единственные** читатели `source.Ctes` на мутационном пути (`grep .Ctes` — `SqlBuilder.cs:46` только в `MakeSelect`; `QueryPlanner.cs:100,297,335`). Поэтому `withSql + sql` не может задублировать `WITH`; в отличие от `RenderSource` (`:104-106`), клон/стрип `source.Ctes` тут не нужен.
- **B. Разделяемый провайдер/аккумулятор параметров корректен.** `MakeWithClause(in ctx, …)` и `MakeUpdateJoin`/`MakeDeleteJoin` используют один `ctx` (общий `@params` и `DefaultParameterProvider`, а не отдельный провайдер); CTE-тела рендерятся первыми, параметры тела продолжают ту же последовательность. Интеграционный тест с захваченным `key` (`x.Id == key`) исполняется и связывает WITH-параметр реально.
- **C. Намеренное невмешательство в `JoinCore(QueryCommand<T>)` — безопасно.** Derived-запрос рендерится производной таблицей через `SqlSourceRenderer.MakeFrom` → `MakeSelect(cmd)` (`:423-445`), и его собственный `WITH` инлайнится **внутри** скобок: probe дал `from (with c as (select id from simple_entity where (id > 0)) select id from c) as 't2'` — один рендер, ни потери, ни дубля. На мутационном пути `source.Ctes` из левой стороны заполниться не может (цель мутации обязана быть физической таблицей — иначе `MakeUpdateJoin`/`MakeDeleteJoin` бросают «can only target a physical table»), поэтому хойст derived-запрос не трогает. Замечание: `WITH` внутри производной таблицы — пре-существующая форма; грамматика SQL Server ограничивает `WITH` началом оператора — отдельный, не-G18 вопрос (проверять на реальном сервере).
- **D. Политика исключения — на момент join-стройки, и это осознанный выбор.** `CteMerge.Merge` вызывается в `JoinCore` (до `ToSql()`/исполнения) и в **общем** для SELECT join'ов generic-пути, т.е. коллизия имён бросается и на read-запросах — согласуется с open question #2 плана («ошибка, не переопределение») и с прочими builder-контрактными `InvalidOperationException` («Named windows must be declared after joins»). Фикса не требует.
- **E. `Merge` возвращает исходный список по ссылке — безопасно.** Списки CTE иммутабельны: `CteQuery.Append`/`MutationCteQuery.Append` всегда создают новый `List` (`CteQuery.cs:123-128`, `MutationCteQuery.cs:140-146`), `Ctes` — `IReadOnlyList` без in-place мутаций; поэтому алиасинг `left`/`right` не портится.
- **F. Сопутствующий cref — легитимен и API-нейтрален.** `DialectCapabilities.cs:241` (`<see cref="Render"/>` → `<see cref="Render(LockMode, KeywordCase)"/>`) необходим, потому что добавленный **отдельной** lock-фичей DIM `ILockRenderer.Render(LockMode, LockWaitMode, KeywordCase)` (`:267`, Находки 145/146 этого журнала) делает голый cref неоднозначным (CS0419 — ошибка при `TreatWarningsAsErrors=true`). `grep '<see cref="Render"' src` — других голых cref нет. К G18 не относится; существующая сигнатура `Render(LockMode, KeywordCase)` не менялась.
- **G. Пробелы покрытия/доков (после фиксов).** (1) SQL-gen-тесты используют литерал `x.Id > 0`, который **инлайнится**, поэтому в `WITH` нет параметра и `set … = @p0` проходит независимо от shared-provider — сценарий «анонимный `@pN` в CTE + анонимный `@pN` в теле» (риск коллизии нумерации) не покрыт; (2) ~~3+ таблицы с CTE — 0 тестов~~ — **закрыто 24.09.2026** (`UpdateJoin_CteOnSecondJoin_ShouldHoistWith`); (3) QueryCommand-join с CTE — 0 тестов (проверено только probe); (4) ~~recursive CTE + `MaxRecursion` на мутации — 0 тестов~~ — **закрыто 24.09.2026** (`UpdateJoin_RecursiveCte_ShouldAppendMaxRecursion`); (5) новая возможность не описана в `docs/guide/20-delete-statement.md`/`docs/guide/21-update-statement.md` (EN+RU не менялись; `-i "cte"` там даёт только «affe**cte**d») — маршрут `nextorm-design-engineer`, если фича должна быть публично задокументирована.

**Итог.** 147/148/149 **исправлены 24.09.2026** (ре-верификация ниже: build `0/0`, три новых теста passed, probe — все три сценария корректны; `QueryCommand`-путь остаётся безопасным). Осталось: 🟡 **Находка 150** (pre-existing `RenderSource`/`INSERT … SELECT`, вне G18) и ℹ️-пункты G(1)(3)(5). Публичную поверхность фиксы не меняют (`CteMerge`/`JoinSourceResolver` — `internal`), правок `API-NAMING-REVIEW.md` G18 не требует. Часть 148(b) (`Ordinal`) — принятое ограничение.

**Проверка (24.09.2026, перепрогон после фиксов).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено; `EfCoreSharedTransactionTests.cs` снова временно откладывался при сборке и возвращён). Подавления `src/` — **6+5=11**, новых — 0; `Skip=` — 0, `Task.Delay` — 2 baseline; `slopwatch` не установлен, скан вручную; `find -name 'PublicAPI*.txt'` — 0; EOL `CteMerge.cs` CRLF (54/54). Probe `/tmp` (SQLite + SQL Server, Release, **после фиксов**): 3-table CTE-second → `with c as (…) update … from other_entity as 't2', c as 't3'` (было `NullReferenceException`); self-join с дублем имени → без исключения; две области с `c` → `InvalidOperationException` (корректно); SQL Server recursive CTE → оканчивается `option (maxrecursion 50)`; QueryCommand-join → `from (with c as (…) select …) as 't2'` (без изменений); 2-table CTE-UPDATEJOIN и 3-table CTE-first — без регресса. Фокусные тесты (Release, `--no-build`): `UpdateJoin_CteOnSecondJoin_ShouldHoistWith` 1/1 + `UpdateJoin_FromCte_ShouldHoistWith` 1/1 (sqlite), `UpdateJoin_RecursiveCte_ShouldAppendMaxRecursion` 1/1 (sqlserver) — **3/3 passed**. По отчёту автора: core 273 / sqlite 371 / postgres 397 / sqlserver 328 / mysql 129 / mariadb 57 — 0 failed; контейнерная интеграция — 1265 passed / 60 skipped / **1 failed** (`SqlServerSpecificTests.XmlMethods_ShouldReturnValues`, `SqlException` таймаут; в изоляции проходит — environmental, 2 GiB Podman VM). Аудитом контейнерные прогоны не перезапускались.

---

## 🔎 Точечный аудит 24.09.2026 — перенос sampling с `EntityBuilder.TableSample/Sample` на объект опций `From` (`FromOptions`), uncommitted working tree; 🔴 — 0, 🟡 — 1 (151), ℹ️ — наблюдения A–G

**Область.** Новый публичный `NextORM.Core.FromOptions` (`src/nextorm.core/DataContext/FromOptions.cs:9`) с 3 fluent-методами (`TableSample` `:25`, `Sample` `:43`, `Sample(,,)` `:54`) и internal-состоянием (`:11-13`, private setters); 2 новых перегруза `DataContextExtensions.From` (`:747` typed, `:911` raw-table), `ResolveMetadata` `private`→`internal` (`:178`); **удалены** 3 fluent-метода `EntityBuilder<TEntity>` (`Sample`×2/`TableSample`). Internal-плумбинг сохранён (`EntityBuilder.cs:32,99,107,109,883-888`; `JoinedEntityBuilder.cs:85,203,306,409,512,615`). Публичная сторона — `API-NAMING-REVIEW.md`, раздел «Аудит 24.09.2026 — sampling…» (FO1–FO6, FM6 закрыто).

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors**. Подавления `src/` — **6 `SuppressMessage`** (все с `Justification`) **+ 5 парных `#pragma`** = **11/11**, новых в изменении — **0**. Слоп: `Skip=` — **0**; пустых `catch`/`NoWarn`/inline `Version`/`Thread.Sleep` — **0**; `Task.Delay` — **2** baseline (`tests/nextorm.core.tests/InMemoryTests.cs`, `Task.Delay(0)`-yield); `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов **вручную**. `.editorconfig` — те же **7** инерт-`silent`-правил (6 `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее, не переоткрывается). `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`), `NoWarn` — **0** ⇒ CS1591 гейтится, build `0/0` = XML-doc новых публичных членов 100 %.

| Категория навыка | Результат |
|---|---|
| 1. `IDisposable` | ✅ `FromOptions` ресурсов не держит и не `IDisposable`; новых disposable-полей/локалов нет (`CA2000`/`CA2213`/`CA1816` не затронуты). |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` — **0**; соотношение проекта **11/11**. |
| 3. LINQ на горячем пути | ✅ LINQ/`.Count()`/итераторов нет; `new FromOptions()` + вызов делегата — на старте построения запроса (холодный путь, до план-кэша), без аллокаций на кэш-хите. |
| 4. God-классы/методы | ✅ `EntityBuilder.cs` уменьшился на ~48 строк (pre-existing Находка 68 не переоткрывается); `FromOptions.cs` — 67 строк, методы <30 строк. |
| 5. Хэш-ключи/равенство | ✅ `FromOptions` — не часть план-ключа (в ключ входят те же `TableSample`/`SampleRatio`/`SampleOffset`, что и раньше; новых полей нет). `TableSampleClause` — positional record. |
| 6. События/исключения | ✅ Подписок нет; новый guard бросает `ArgumentOutOfRangeException` с `nameof`-параметром; `catch` нет. |
| 7. NRT/возвратный тип | ✅ `double? seed`/`double? SampleRatio` аннотированы; `Action<FromOptions>` non-nullable + `ArgumentNullException.ThrowIfNull`; `async void` нет; сборка `0/0`. |

### 🟡 Находка 151 (P2, doc-устаревание, вне публичной поверхности) — XML-doc `TableSampleClause` ссылается на удалённый `EntityBuilder.TableSample`

**Место:** `src/nextorm.core/Query/TableSampleClause.cs:4`.

**Что не так.** Summary внутреннего record'а гласит: «The `TABLESAMPLE` table modifier attached to a query through `EntityBuilder.TableSample`…». Метод `EntityBuilder<TEntity>.TableSample(...)` удалён этим же изменением; теперь модификатор задаётся через `FromOptions.TableSample` / `From<T>(o => o.TableSample(...))`. Ссылка стала устаревшей (doc-accuracy), поведение не затронуто.

**Стало:** переписать прозу на «configured through `FromOptions.TableSample` (the `From` options overload)» либо нейтрально («the `TABLESAMPLE` table modifier attached to the primary source»), без cref на удалённый метод. Маршрут `nextorm-design-engineer`; публичную форму не менять.

**Проверка:** `rg -n "EntityBuilder\\.TableSample|Builder\\.TableSample" src` — единственное вхождение `TableSampleClause.cs:4` (других stale-ссылок нет; `rg 'public EntityBuilder<TEntity> (Sample|TableSample)' src/nextorm.core/Builders/EntityBuilder.cs` — пусто).

### ℹ️ Наблюдения (фикс не требуется)

- **A. Утечки/алиасинга `FromOptions` нет.** Экземпляр создаётся на вызов (`DataContextExtensions.cs:752,917`), значения копируются в builder (`:757-763,920-926`); `TableSampleClause` — immutable positional record (`TableSampleClause.cs:7`), `SampleRatio`/`SampleOffset` — `double`/`double?` по значению; объект не удерживается. Мутация `FromOptions` после возврата builder'а на него не влияет. Публичного `IDisposable`-контракта не требуется.
- **B. Overload-resolution — CS0121 для `_ => {}` не воспроизводится (см. `API-NAMING-REVIEW.md`, FO4).** 1-параметровый metadata-overload выигрывает у 2-параметрового options-overload по tie-breaker'у C# (без подстановки optional-параметров); существующие call-sites member-touching — bind корректный. `null`-литерал амбивалентен, но это pre-existing (`Action<…>?` vs `EntityBuilder`/`QueryCommand`), не регрессия.
- **C. Два изменения поведения, не переименования (см. FO5/FO6).** (1) `TableSample` теперь отклоняет `double.NaN` (`FromOptions.cs:27`) — старая проверка `percent <= 0 || percent > 100` пропускала NaN и рендерила `tablesample NaN`. (2) `From<T>` теперь идёт через `ResolveMetadata`, который пересобирает метаданные и при пустом `TableName` в кэше (было `ContainsKey`) — выравнивание с `InsertInto`/`Update`.
- **D. Покрытие.** `tests/nextorm.core.tests/FromOptionsTests.cs` — **7** методов (`[Fact]`×2 + `[Theory]`×3 с 12 `InlineData`: NaN/±Infinity/границы) на chain + валидацию `TableSample`/`Sample`; SQL-gen/`InMemoryTests` переведены на `o => o.TableSample(...)`/`o => o.Sample(...)`. Пробел: нет SQL-gen-теста именно на `TableSample(double.NaN)` (unit-уровень покрыт `FromOptionsTests`), и нет теста на повторный `From<T>` (идемпотентность `ResolveMetadata`) — низкий приоритет.
- **E. `.editorconfig`/анализаторы — без изменений.** 7 `silent`-правил (пре-существующие инертные `S*` + `CA2254`), новых нет; дефолтная сборка `0/0`.
- **F. EOL.** Оба изменённых/новых core-файла (`FromOptions.cs`, `DataContextExtensions.cs`) после правок — CRLF; рекуррентный LF-дефект (Находки 20/22/29/112/140) не воспроизвёлся.
- **G. Шаг 5 открыт.** `find -name 'PublicAPI*.txt'` — **0**; новая публичная поверхность (`FromOptions` + 2 перегруза `From`) и удалённые методы — в трекинге FO1 (`API-NAMING-REVIEW.md`).

**Итог.** 🔴 — **0**; 🟡 **151** (P2, stale-ссылка на удалённый метод) — открыта, маршрут `nextorm-design-engineer`; остальное — наблюдения/принято. Ресурсов, подавлений, LINQ/аллокаций, новых хэш-ключей и усугубления god-класса не найдено. Публичная сторона — `API-NAMING-REVIEW.md`: **P0/P1 — нет**, FO1–FO6 (P2), FM6 закрыто.

**Проверка (24.09.2026).** `dotnet build nextorm.sln -c Release` — **0/0** (проверено); подавления `src/` — **6+5=11** (0 неоправданных), новых — **0**; `Skip=` — 0, `Task.Delay` — 2 baseline, `Thread.Sleep`/пустых `catch`/`NoWarn`/inline `Version` — 0; `slopwatch` не установлен, скан вручную; `rg 'public EntityBuilder<TEntity> (Sample|TableSample)' src/nextorm.core/Builders/EntityBuilder.cs` — пусто; `rg 'TableSampleClause|SampleRatio|SampleOffset' src/nextorm.core/Builders/{EntityBuilder,Joins/JoinedEntityBuilder}.cs` — плумбинг на месте; `find -name 'PublicAPI*.txt'` — 0. Тесты (Release, `--no-build`): core `FromOptionsTests` **13/13**, `InMemoryTests` **80/80**; SQL-gen — postgres **260/260**, sqlserver **210/210**, clickhouse **190/190**, mysql **57/57**, sqlite **211/211** (0 failed, 0 skipped). Probe `/tmp` — resolution overload'ов (см. наблюдение B). Контейнерная интеграция аудитом не перезапускалась.

---

## 🔎 Точечный аудит 24.09.2026 — `ForJson`/`ForXml` стали терминалами (`DocumentMode`, план-ключа), uncommitted working tree; 🔴 — 0, 🟡 — 2 (152, 153) — ✅ закрыты 24.09.2026, ℹ️ — наблюдения A–G

**Область.** `src/nextorm.core/Query/QueryCommand.TResult.cs`: `ForJson` (`:678`) / `ForXml` (`:700`) стали терминалами (`string?`), добавлены `ForJsonAsync` (`:687`) / `ForXmlAsync` (`:710`) → `Task<string?>`, `WithForJson` (`:640`) / `WithForXml` (`:659`) → `QueryCommand<TResult>` (старые тела), private `ExecuteDocument` (`:713`) / `ExecuteDocumentAsync` (`:719`) / `CreateDocumentCommand` (`:725-737`); `QueryCommand.cs` — `internal bool DocumentMode` (`:225`); `QueryCommand.Clone.cs:65` — копирование; `DataContext/QueryPlanner.cs:391` — маппер пропускается при `DocumentMode`. Тесты — `SqlServerForJsonTests.cs` (5), in-memory guard (`InMemoryTests.cs:58-77`), миграция SQL-gen (sqlserver/sqlite). Публичная сторона — `API-NAMING-REVIEW.md`, раздел «Аудит 24.09.2026 — `ForJson`/`ForXml` стали терминалами…» (FJ1–FJ3).

**База (этот проход).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors**; `dotnet docfx docs/docfx.json` — **0 warnings / 0 errors**. Подавления `src/`: **6 `SuppressMessage`** (все с `Justification`; `SelectExpression.cs:12`, `AggregateTerminalRewriter.cs:12`, `CorrelatedQueryExpressionVisitor.cs:9,10`, `DbPreparedQueryCommand.cs:85`, `QueryExecutor.cs:74`) + **5** `#pragma warning disable` (все с парным `restore`: `EntityBuilderExtensions.cs:123,140,156`, `InMemoryLinqSource.cs:82`, `ExpressionPlanEqualityComparer.cs:421`) = **11/11** оправданных, новых в изменении — **0** (в 4 изменённых core-файлах и `SqlServerForJsonTests.cs` — 0). Слоп: `Skip=` — **0**; пустых `catch` — **0**; `NoWarn` — **0**; `Thread.Sleep` — **0**; `Task.Delay` — в `src/` **0**, в тестах **2** (`Task.Delay(0)`-yield, `InMemoryTests.cs:153,442`), в `examples/` **2** (намеренные ожидания готовности БД, вне изменения); `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов **вручную**. `.editorconfig` — те же **7** `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer` + `CA2254`; пре-существующее, не переоткрывается). `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — **0** ⇒ CS1591 гейтится, XML-doc 6 новых/изменённых публичных методов — 100 %.

| Категория навыка | Результат |
|---|---|
| 1. `IDisposable` | ✅ Ресурсов нет: терминалы исполняют через существующий executor (reader/`DbCommand` — в `QueryExecutor`), `QueryCommand` не `IDisposable`; `CA2000`/`CA2213`/`CA1816` не затронуты. Guard бросает `NotSupportedException` до выполнения. |
| 2. Подавления | ✅ Новых `#pragma`/`SuppressMessage`/`NoWarn` в изменённых файлах — **0**; соотношение проекта **11/11**. |
| 3. LINQ на горячем пути | ✅ LINQ/`.Count()`/итераторов нет; `object[] boundParams = @params ?? []` (`:715,:721`) — 1 аллокация на терминальный вызов (план кэшируется, `ToList` и т.п. — вне изменения), холодный путь. |
| 4. God-классы/методы | ✅ `QueryCommand.TResult.cs` +87 строк (~730 → 817), `QueryCommand<TResult>` остаётся god-классом (pre-existing Находка 68 **не переоткрывается**); новые методы ≤30 строк, `CreateDocumentCommand` — 13. |
| 5. Хэш-ключи/равенство | ✅ **Находка 152 закрыта 24.09.2026:** `DocumentMode` теперь входит и в `Equals` (`QueryPlanEqualityComparer.cs:120`), и в `GetHashCode` (`:368`); null-мапперный план документа больше не делится ключом с `WithForJson`-командой. |
| 6. События/исключения | ✅ Подписок нет; новый `throw NotSupportedException("FOR JSON / FOR XML requires a relational database provider.")` — actionable (`:728`); пустых `catch` нет. |
| 7. NRT/возвратный тип | ✅ `string?`/`Task<string?>`/`object[]?` аннотированы; `ConfigureAwait(false)` в async-хелпере; `async void` нет; сборка `0/0`. |

### ✅ Находка 152 (P2, латентный дефект план-ключа; опровергает тезис «cannot collide»; **исправлено 24.09.2026**) — `DocumentMode` не входит в `QueryPlanEqualityComparer`: план документа (`MapDelegate=null`) может быть переиспользован материализующим терминалом → `NullReferenceException`

**Место.** `src/nextorm.core/Query/QueryPlanEqualityComparer.cs` (`Equals` `:43-167`; `GetHashCode` `:305-473`); чтение флага — `DataContext/QueryPlanner.cs:391`; установка — `QueryCommand.TResult.cs:734`; копия — `QueryCommand.Clone.cs:65`; падение null-маппера — `DataContext/QueryExecutor.cs:272` (`ToList`), `DataContext/ResultSetEnumerator.cs:153,163,182` (`_map!`).

**Что не так.** `DocumentMode` не сравнивается в `Equals` и не хешируется в `GetHashCode`. Ключ результата различает команды только по типу: `ResultPlanHash = typeof(TResult).GetHashCode()` (`QueryCommand.TResult.cs:37`), `ResultType = typeof(TResult)` (`:39`); comparer сравнивает `ResultType` (`:50`) и хеширует `ResultPlanHash` (`:324-325`). Когда проекция уже `string` (ровно рекомендованный для `FOR JSON` случай) терминальный `QueryCommand<string>` (`DocumentMode=true`, `MapDelegate=null`) и пользовательский `QueryCommand<string>` от `WithForJson()` (`DocumentMode=false`, маппер построен) совпадают по `ResultType`/`ResultPlanHash`/`ForJsonClause` (`Equals` `:112-114`; hash `:356-360`) и по `_sql` (у обоих `null` → `QueryPlan.Equals` `:79` считает планы равными). Значит, обе команды делят одну запись плана. Опасен порядок: если сначала выполнить `.ForJson()` (в кэш ложится план с `MapDelegate=null`), последующий `…WithForJson().ToList()`/`First()`/`ToEnumerable()`/`ToHashSet()` получает из кэша план **без** маппера → `compiledQuery.MapDelegate!` (`QueryExecutor.cs:272`) / `_map!` (`ResultSetEnumerator.cs:153,163,182`) → `NullReferenceException`. Обратный порядок безвреден: `ExecuteScalar` документа работает на сыром `DbCommand` мимо маппера (`QueryExecutor.cs:350-356`), поэтому терминал переживёт даже план с маппером.

**Воспроизведение (логика, план-ключ).** Для `ctx.From<E>().Select(x => x.String)` команды `WithForJson().ToList()` и `.ForJson()` дают равный план-ключ (различие только в `DocumentMode`). Последовательность `.ForJson()` → `.WithForJson().ToList()` на одном контексте/потоке роняет NRE; теста нет (новые покрывают терминалы и `WithForJson`-SQL-gen, но не материализацию `WithForJson`). Тезис «план-ключ фолдит тип результата, поэтому не столкнутся» **опровергнут**: для `TResult == string` хеши совпадают.

**Стало (рекомендация, маршрут `nextorm-design-engineer`).** Добавить дискриминатор в ключ: `if (x.DocumentMode != y.DocumentMode) return false;` в `Equals` и `hash.Add(obj.DocumentMode);` в `GetHashCode` (копирование уже есть — `Clone.cs:65`). Тест на смешанное использование либо явный запрет материализации `WithForJson`-команд (fail-fast вместо NRE). Публичную форму не менять.

**Проверка:** `rg 'DocumentMode' src/nextorm.core/Query/QueryPlanEqualityComparer.cs` — пусто (флага нет); `roslyn refs NextORM.Core.QueryCommand.DocumentMode` — только set (`TResult.cs:734`), copy (`Clone.cs:65`), read (`QueryPlanner.cs:391`); `ResultPlanHash`/`ResultType` — `QueryCommand.TResult.cs:37,39`; null-маппер падает — `QueryExecutor.cs:272`, `ResultSetEnumerator.cs:153,163,182`.

**Исправлено (24.09.2026).** `DocumentMode` добавлен в план-ключ: `if (x.DocumentMode != y.DocumentMode) return false;` (`QueryPlanEqualityComparer.cs:120`, сразу после `ForXmlClause`, с комментарием про null-маппер) и `hash.Add(obj.DocumentMode);` (`:368`, сразу после `ForXmlClause`-хеша). `CopyTo` уже копировал флаг (`QueryCommand.Clone.cs:65`), поэтому live-команда и cache-клон дают согласованный ключ (`QueryPlan.GetCacheVersion` Debug.Assert проходит). Коллизия теперь **невозможна**: любая разница `DocumentMode` даёт `Equals == false` и разный `GetHashCode`, так что null-мапперный план документа не может быть переиспользован `WithForJson().ToList()`/`First()` (и наоборот). Ре-верификация аудита: build `0/0`; core `TestForJson|TestForXml` 3/3, sqlserver `ForJson|ForXml` 6/6, sqlite 2/2.
...
- **A. `CopyTo(true)` + `Definition` — потерь нет (ответ на вопрос 2).** `CreateDocumentCommand` (`QueryCommand.TResult.cs:725-737`) строит `QueryCommand<string>(_dataContext, configured.Definition)` и клонирует `CopyTo(cmd, true)`: `_union`/`_ctes`/`_referencedQueries`/`_from`/`_customData` (`Clone.cs:104-111`), `_outerRefs`/`OuterRegistry` (`:54-55`), `_hints`/`TableHints`/`IndexHints` (`:44,:90-92`), quote/naming/keyword-настройки (`:45-50`), `ForJsonClause`/`ForXmlClause` (`:93-94`). `Definition` (`QueryCommand.cs:158-185`) несёт `Exp`/`Condition`/`Joins`/`Group`/`Having`/sorting/paging; `ResetPreparation()` (`:467-487`) чистит только производное подготовленное состояние (`_selectList`/`_from`/`Prepared*`/хеши), оригиналы не трогает. Union/CTE/referenced-запросы/hints/outer-refs/naming/quote/keyword не теряются; `` `_exp` `` доступен для повторной подготовки.
- **B. Guard корректен (ответ на вопрос 8).** `InMemoryDataContext : IDataContext` (`InMemoryDataContext.cs:13`) не наследует `DataContext` (`DataContext.cs:14`); roslyn implementations `IDataContext` — **8**, 7 реляционных — от `DataContext` (+`MariaDbDataContext : MySqlDataContext`). Иных не-реляционных контекстов нет. `_dataContext == null` даёт `NotSupportedException` (не NRE) — приемлемо для unbound-клона.
- **C. Nullability/анализ (ответ на вопрос 3).** `string?`/`Task<string?>` — честные NRT-аннотации, `<returns>` описывает `null`; `object[]?` аннотирован. Покрытие пустого пути — только `ForJson` (Находка 153). Analyzer-риск: `CA1068` (cancellation перед `params`) не сработал — в .NET 10 включён как **suggestion**, плюс прецедент `CreateAsyncEnumerator(CancellationToken, params object[]?)` (`QueryCommand.TResult.cs:64`); sync-`params` — осознанная единая форма.
- **D. Overload-resolution (ответ на вопрос 7).** 4-й параметр (`params`) новый: старые вызовы `ForJson(mode, root, includeNullValues)` / `ForXml(mode, elementName, root, elements)` биндятся без изменений (SQL-gen тесты зелёные); `ForJson(root: "x")` — ок; амбивалентности с `WithForJson`/`WithForXml` нет (разные имена). Единственное изменение — возврат терминала (source break, FJ1).
- **E. Дублирование.** `CreateDocumentCommand` повторно присваивает `ForJsonClause`/`ForXmlClause` (`:732-733`), хотя `CopyTo` уже их копирует (`Clone.cs:93-94`) — безвредно, можно убрать при следующем касании.
- **F. Аллокации/клоны — холодный путь.** `WithForJson` + `CreateDocumentCommand` = два клона + `ResetPreparation` на терминальный вызов, `@params ?? []` — одна аллокация; терминалы холодные, план документа кэшируется. На горячих путях (`ToList`/`Where`/материализация) изменений нет.
- **G. EOL/формат.** Изменённые файлы — CRLF (после правок файлов реестра нормализовано); `.editorconfig` не менялся; Шаг 5 открыт (`PublicAPI.*.txt` — 0).

**Итог.** 🔴 — **0**; 🟡 **152** и 🟡 **153** — ✅ **закрыты 24.09.2026** (152 — дискриминатор `DocumentMode` в `Equals`/`GetHashCode`; 153 — эмпирическая проверка NULL + тест `ForXml_NoRows_ShouldReturnNull`); остальное — наблюдения/принято. Публичная сторона — `API-NAMING-REVIEW.md`: **P0/P1 — нет**, FJ1–FJ2 (P2, открыты), FJ3 — ✅ закрыта (зеркало 153).

**Проверка (24.09.2026).** `dotnet build nextorm.sln -c Release` — **0 warnings / 0 errors** (проверено); `dotnet docfx docs/docfx.json` — **0 warnings / 0 errors**; подавления `src/` — **6+5=11** (0 неоправданных), новых — **0**; `Skip=` — 0, пустых `catch`/`NoWarn`/`Thread.Sleep` — 0; `Task.Delay` — src 0 / tests 2 (`Task.Delay(0)`) / examples 2 (вне изменения); `slopwatch` не установлен, скан вручную. Тесты (Debug): core `TestForJson|TestForXml` **3/3**, sqlserver `ForJson|ForXml` **6/6**, sqlite **2/2**; контейнерная интеграция SQL Server `SqlServerForJsonTests` — **6/6** (5 исходных + новый `ForXml_NoRows_ShouldReturnNull`). `roslyn refs NextORM.Core.QueryCommand.DocumentMode` — 4 ссылки (set `TResult.cs:734`, copy `Clone.cs:65`, read `QueryPlanner.cs:391`), сравнение/хеш — в comparer (`Equals:120`, `GetHashCode:368`); `find -name 'PublicAPI*.txt'` — 0.

---

## 🔎 Точечный аудит 24.09.2026 — C# string semantics (G12): `string.Format`/`ToString(fmt)`, per-expression `COLLATE`, ordinal `StringComparison`, uncommitted working tree; 🔴 — 2 (Находки 154–155), 🟡 — 5 (156–160), ℹ️ — наблюдения A–H

**Область (uncommitted).** Новое: `src/nextorm.core/Visitors/StringFormatTranslator.cs` (225 строк, `internal static`), `src/nextorm.core/Visitors/CompositeFormat.cs` (121 строка, `internal static` + `internal readonly record struct FormatPart`). Изменено: `src/nextorm.core/Visitors/StringFunctionTranslator.cs` (412 → 658 строк: `ToUpperInvariant`/`ToLowerInvariant` `:34,:37`, `ToUpper/ToLower(CultureInfo)` `:38-41`, отказы `Trim(char)` `:50-53`, `string.Equals` `:78,:92-93`, `Compare`/`CompareOrdinal` `:95-96`, `StringComparison` для `StartsWith`/`EndsWith`/`Contains`/`IndexOf`/`LastIndexOf` `:391-…, :448-…, :576-…`); `ScalarFunctionTranslator.cs` (маршрутизация `string.Format`/`ToString(fmt)` `:25-33`, `collate` `:107-130`); `Query/SqlFunctions.cs` (`collate` `:393`, `Sql` `:26` — был `default!`); `DataContext/Dialect/ISqlDialect.cs` (`StringFormats` `:746`, `SupportsCollation` `:753`, `MakeCollate` `:759`, `SupportsOrdinalComparison` `:765`, `MakeOrdinal` `:773`); `SqlDialectBase.cs` (`:488-502`); `DialectCapabilities.cs` (`IStringFormatFunctions` `:66-83`). Провайдеры: Postgres `:374-388` + `PostgresStringFormats` `:568`, SQL Server `:319-336` + `SqlServerStringFormats` `:733`, MySQL `:363-379` + `MySqlStringFormats` `:527` (MariaDB наследует), SQLite `:120-136` + `SqliteStringFormats` `:315`, ClickHouse `:482-492` + `ClickHouseStringFormats` `:848`. Тесты: 7 новых файлов, **41** тест (core 5 + sqlite 5 + postgres 14 + mysql 5 + mariadb 3 + sqlserver 5 + clickhouse 4). Публичная сторона — `API-NAMING-REVIEW.md`, SS1–SS4.

### 🔴 Находка 154 (P1, корректность/тихий неверный результат) — `SqlFunctions.Sql` стал реальным singleton'ом: in-memory исполняет **весь** `CommonFunctions`, а тела всех членов кроме `collate` возвращают `default!`

**Место.** `src/nextorm.core/Query/SqlFunctions.cs:26` (`public static CommonFunctions Sql { get; } = new CommonFunctions();`); тела-заглушки: `exists :360`, `like :368,:377`, `contains :400`, `freetext :407`, `current_user :414`, `session_user :421`, `current_schema :428`, `current_database :435`, `version :442`, `gen_random_uuid :450`, `uuidv7 :458`, `@in :467,:473,:479`, `any :487`, `all :495`, `nullif :498`, `iif :505`, `greatest`/`least :510+`; единственный нативный — `collate :393`. In-memory компилирует выражение без трансляции: `InMemoryConditionFactory.cs:44,77` (`condition.Compile()`), `InMemoryLinqSource.cs:128`, `InMemoryGrouping.cs:109,143,148`; спец-разбор `CommonFunctions` есть только для aggregates (`InMemoryAggregates.cs:36-44`) и `exists`/`@in` (`InMemoryCorrelatedSubqueryRewriter.cs:119-128`).

**Что не так.** До изменения `Sql` был `default!` (null), и `SqlFunctions.Sql.like(...)` в in-memory падал `NullReferenceException` — громко. Теперь `Sql` — рабочий экземпляр, но `default!`-тела отдают `false`/`null`/`default`, т.е. запрос **тихо** возвращает неверный результат. Док класса (`:302-305`) обещает «the in-memory provider may execute the small subset that has a native CLR equivalent», однако гейта нет — исполнима вся поверхность. `Postgres`/`SqlServer`/`ClickHouse` остались `default!` (`:34,:43,:63`) — поведение рассинхронизировано.

**Проверка (probe вне репозитория, `/tmp/opencode/probe`, Release).** `e.Where(x => SqlFunctions.Sql.like(x.Id.ToString(), "%1%"))` → **0 строк** (ожидалась 1); `e.Select(x => SqlFunctions.Sql.iif(x.Id > 0, "yes", "no"))` → `null`; `e.Select(x => SqlFunctions.Sql.collate(x.Id.ToString(), "C"))` → `1,2` (корректно). Тестов на «негативные» члены `Sql.*` в in-memory нет.

**Стало (маршрут `nextorm-design-engineer`).** Не-нативные члены `CommonFunctions` должны бросать `NotSupportedException` вместо `default!` (безопасно: SQL-транслятор читает `node.Method` и тело не вызывает — `ScalarFunctionTranslator`, `BuiltinFunctionTranslator`, `WindowSql`), либо `Sql` должен отдавать объект, у которого нативен только `collate`. Публичные подписи не менять; синхронизировать XML-doc `Sql`/`CommonFunctions` с фактическим поведением.

### 🔴 Находка 155 (P1, корректность/тихий неверный SQL) — `IndexOf(string, int startIndex, int count)`: `count` молча используется как `startIndex`

**Место.** `src/nextorm.core/Visitors/StringFunctionTranslator.cs:391-437`; цикл `:401-409` (`startIndex = i`), рендер `:435-436` (`MakeStringIndexOf(value, substring, start)` — у хукa нет `count`).

**Что не так.** Перегрузки `IndexOf(string,int)` и `IndexOf(string,int,int)` не различаются: цикл присваивает `startIndex` каждому `int`, поэтому при 3 аргументах в `start` уходит `count`. В `HEAD` (`args.Count is <1 or >2`) обе перегрузки отвергались; теперь строится неверный SQL. `LastIndexOf(string,int,int)` не затронут (`args.Count > 2` → отказ), `LastIndexOf(string,int)` корректно отвергается.

**Проверка (SQL-gen probe SQLite, `/tmp/opencode/probe2`, Release).** `IndexOf("a", 1)` → `instr(substring(Name, 1 + 1, length(Name) - (1)), 'a')`; `IndexOf("a", 1, 2)` → `substring(Name, 2 + 1, length(Name) - (2))` — стартом стало `2` (=`count`), т.е. сдвиг на `count` вместо `startIndex`. Покрытия нет (41 новый тест перегрузку не трогает).

**Стало (маршрут `nextorm-design-engineer`).** Различать по числу `int`-аргументов: `≥ 2` → `return false` или `NotSupportedException` (либо добавить `count` в dialect-хук). Тест на обе формы.

### 🟡 Находка 156 (P2, DRY/дублирование) — `TryMapDate` + `Match` продублированы 5 раз по провайдерам

**Место.** Postgres `:582`/`:604`, SQL Server `:751`/`:773`, MySQL `:551`/`:573`, SQLite `:337`/`:359`, ClickHouse `:871`/`:893`. Логика идентична (скан format, `Match` по токену, пропуск разделителей `- / . :` и пробела, `out native`, `false` на неизвестный символ); отличаются только таблица CLR-токен→native и размер `StringBuilder`. ~30 строк × 5 ≈ **150** строк дубля; `Match` — 5 копий.

**Стало.** Вынести в core `internal static bool TryMapDate(string format, ReadOnlySpan<(string Clr, string Native)> tokens, out string native)`, провайдер передаёт токен-таблицу. Холодный путь (SQL строится и кэшируется), поэтому, как и в Находке 146, допустимо принять осознанно — но 5 копий заметно больше 4 коротких строк, извлечение рекомендовано. Не публичная поверхность.

### 🟡 Находка 157 (P2, god class/длинный файл) — `StringFunctionTranslator.cs` перешёл 500 строк (412 → 658)

**Место.** `src/nextorm.core/Visitors/StringFunctionTranslator.cs` — **658** строк. Комментарий шапки (`:16`) гласит, что файл выделен из `ScalarFunctionTranslator` именно чтобы «оставаться ниже» порога; новые `TryTranslateEquals`/`TryTranslateCompare`/`ResolveOrdinal`/`RequireOrdinalComparison` (`:163-281`) и `StringComparison`-ветки (`:391-…,:576-…`) вывели его за порог god-класса (>500). `StringFormatTranslator.cs` (225) и `CompositeFormat.cs` (121) — **в норме** (не god-классы). `TryTranslate` (`:20-100`) остаётся длинным.

**Стало.** Вынести ordinal-сравнения (`Equals`/`Compare*`/`ResolveOrdinal`/`RequireOrdinalComparison`, ~120 строк) в отдельный `StringComparisonTranslator`; либо зафиксировать порог как осознанный для статических трансляторов. Не публичная поверхность.

### 🟡 Находка 158 (P2, EOL) — все 9 новых файлов записаны с LF-концевиками

**Место.** `src/nextorm.core/Visitors/StringFormatTranslator.cs`, `src/nextorm.core/Visitors/CompositeFormat.cs`; `tests/nextorm.{core,sqlite,postgres,mysql,mariadb,sqlserver,clickhouse}.tests/StringSemanticsSqlGenerationTests.cs` и `InMemoryStringSemanticsTests.cs` — **LF**; изменённые файлы — CRLF. Нарушение AGENTS.md («CRLF throughout»), повтор Находки 140.

**Стало.** `perl -pi -e 's/\r?\n/\r\n/g' <файл>` для 9 новых файлов (маршрут `nextorm-design-engineer`).

### 🟡 Находка 159 (P2, контракт/покрытие) — новые dialect-возможности (`StringFormats`/`SupportsCollation`/`SupportsOrdinalComparison`) не покрыты контрактным тестом

**Место.** `tests/nextorm.integration.tests/DialectCapabilityContractTests.cs:62-193` — перечисляет capability-объекты (`Iif`, `SessionInfoFunctions`, …, `Lock`), но **не** `StringFormats` (свойство, не объект) и не булевы `SupportsCollation`/`SupportsOrdinalComparison`. Провайдер, выставивший `SupportsOrdinalComparison = true` и забывший переопределить `MakeOrdinal` (база бросает, `SqlDialectBase.cs:501`), не будет поймана — тест зелёный. Аналог RLM3.

**Стало.** Добавить в цикл: `if (d.StringFormats is { } f) { …SupportsNumber/RenderNumber/SupportsDateFormat/RenderDate… }`; `if (d.SupportsCollation) d.MakeCollate("[x]", "C").Should().NotBeNullOrEmpty();`; `if (d.SupportsOrdinalComparison) foreach (var ic in new[] { false, true }) d.MakeOrdinal("[x]", ic).Should().NotBeNullOrEmpty();`. Маршрут `nextorm-design-engineer`.

### 🟡 Находка 160 (P2, контракт/аппроксимация) — `ToUpperInvariant`/`ToLowerInvariant` молча маппятся на locale-зависимые `upper()`/`lower()`

**Место.** `src/nextorm.core/Visitors/StringFunctionTranslator.cs:34,:37` (`case nameof(string.ToUpperInvariant)`/`ToLowerInvariant` → `MakeUpper`/`MakeLower`); комментарий `:30-32` признаёт отличие «only for non-ASCII». В `HEAD` эти методы **отвергались** (общий фолбэк `BaseExpressionVisitor.cs:191-195`: `String.ToUpperInvariant is not supported`) — т.е. это новый тихий переход от отказа к аппроксимации.

**Что не так.** `upper()`/`lower()` зависят от локали БД; `*Invariant` — от invariant-культуры. Для не-ASCII результат может отличаться (`ß`, турецкая `i`), а публичные доки об этом молчат (см. API SS2).

**Стало.** Либо ограничить ASCII/задокументировать в `limitations` (EN+RU) и XML-doc `MakeUpper`/`MakeLower`, либо не перехватывать `*Invariant` (вернуть громкий отказ). Публичную форму не менять. Мягкий P2.

### ℹ️ Наблюдения (фикс не требуется)

- **A. Гейт сборки — environment (вне G12).** `dotnet build nextorm.slnx -c Release` **падает**: uncommitted bump `NextOrmVersion` 1.0.4-alpha → 1.0.5-alpha (`Directory.Packages.props:9`) ещё не опубликован на nuget.org, и 3 example-проекта не восстанавливаются (`NU1102`, **6** ошибок). С `-p:NextOrmVersion=1.0.4-alpha` — **0 warnings / 0 errors**. Это отдельное (возможно, release-prep) изменение в дереве, не из файлов G12; тем не менее заявленный гейт `dotnet build nextorm.slnx -c Release` сейчас красный.
- **B. XML-doc/анализ.** Build `0/0` + `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`) ⇒ CS1591 не подавлен, покрытие новых публичных членов 100 %.
- **C. Подавления.** `src/` — **11** (6 `SuppressMessage` c `Justification` + 5 парных `#pragma`), в G12-файлах новых — **0**; `NoWarn` — 0; `TreatWarningsAsErrors=false` — 0; `Skip=`/`[Ignore]` — 0; пустых `catch` — 0. Соотношение **11/11**. `.editorconfig` — те же 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer`: `S125`,`S108`,`S3060`,`S1104`,`S3604`,`S2292`; + `CA2254`) — пре-существующее, не переоткрывается.
- **D. Аллокации — холодный путь.** `CompositeFormat.Parse` (`:22-64`) даёт `List<FormatPart>` + `StringBuilder` + по строке на литерал; `Emit` (`StringFormatTranslator.cs:64-109`) — `List<string>`. Всё на этапе построения SQL (план кэшируется), LINQ/`.Count()`/итераторов на горячем пути нет; `ToString(fmt)` в param-режиме (`:53-57`) — no-op. Претензий нет.
- **E. In-memory культура.** `RequireInvariantProvider` (`StringFormatTranslator.cs:186-197`) считает `string.Format(null, …)` invariant-вызовом, но CLR трактует `null`-провайдер как `CurrentCulture`; no-provider перегрузка — тоже current-culture. In-memory исполняет нативно, SQL — invariant ⇒ на не-invariant хосте возможно расхождение. Осознанное допущение плана («только Invariant или без него»), но в публичных доках не зафиксировано — см. API SS2.
- **F. `CompositeFormat` vs BCL.** `internal NextORM.Core.CompositeFormat` (`CompositeFormat.cs:20`) одноимён с `System.Text.CompositeFormat` (.NET 8+). Внутренний, публичного конфликта нет; ловушка — файл в namespace `NextORM.Core` с `using System.Text;` получит локальный тип (объявление в namespace важнее using), поэтому ссылку на BCL-тип придётся квалифицировать. ℹ️.
- **G. Расширяемость.** DIM `ISqlDialect.StringFormats`/`SupportsCollation`/`SupportsOrdinalComparison` source/binary-совместимы; `MakeCollate`/`MakeOrdinal` — **нет** (см. API SS3). База `SqlDialectBase` даёт ANSI/throwing-дефолты (`:488-502`).
- **H. Тесты.** 7 новых файлов, **41** тест: core 5, sqlite 5, postgres 14, mysql 5, mariadb 3, sqlserver 5, clickhouse 4; все **passed** (Release, `--no-build`). `IndexOf(string,int,int)` не покрыт (Находка 155); нет теста на throwing-дефолт базовых `MakeOrdinal`/`MakeCollate` (Находка 159).

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release -p:NextOrmVersion=1.0.4-alpha` — **0 warnings / 0 errors** (без override — `NU1102` ×6, см. A); тесты Release `--no-build`, фильтр `StringSemantics`: core **5/5**, sqlite **5/5**, postgres **14/14**, mysql **5/5**, mariadb **3/3**, sqlserver **5/5**, clickhouse **4/4** (все passed, 0 failed). Probe-подтверждения: `Sql.like` → 0 строк / `Sql.iif` → null / `Sql.collate` → `1,2` (`/tmp/opencode/probe`); `IndexOf("a",1,2)` → `substring(...,2+1,...)` (`/tmp/opencode/probe2`). Подавления `src/` — 11/11, новых 0; EOL: 9 новых файлов LF, остальные CRLF; `find -name 'PublicAPI*.txt'` — 0. `slopwatch` не установлен, скан вручную.

### ✅ Статус после применения (24.09.2026, тот же рабочий tree)

- 🔴 **154 — исправлено.** `SqlFunctions.Sql` возвращён к `=> default!` (маркер, не singleton), поэтому тихих `default!`-путей у `CommonFunctions` нет. `collate` в in-memory поддержан через новый `InMemoryStringFunctionRewriter` (`src/nextorm.core/DataContext/InMemoryStringFunctionRewriter.cs`), который **до компиляции** заменяет `SqlFunctions.Sql.collate(v, c)` на `v` — в условии (`InMemoryConditionFactory`), в колонке-проекции (`InMemoryDataContext.MapColumn`) и в single-column проекции (`InMemoryRowMaterializer`). Тест `InMemoryStringSemanticsTests` — **5/5**.
- 🔴 **155 — исправлено.** `StringFunctionTranslator.TryTranslateIndexOf` при второй `int`-аргументации (перегрузка `IndexOf(string,int,int)`) возвращает `false` → громкий общий отказ; `count` больше не принимается за `startIndex`. Тест `StringSemanticsSqlGenerationTests.IndexOfWithCount_ShouldThrow` (postgres) — **passed**.
- 🟡 **156 — принято (не исправлено).** Токен-маппер `TryMapDate`/`Match` остаётся продублированным по 5 провайдерам: вынос в core требует нового **публичного** helper-типа (провайдеры — отдельные сборки, `InternalsVisibleTo` в репозитории нет), а поверхность и так растёт. Зафиксировано как осознанный компромисс, не переоткрывать без решения по `InternalsVisibleTo`/API.
- 🟡 **157 — принято (не исправлено).** `StringFunctionTranslator.cs` > 500 строк после G12; вынос ordinal-семейства в `StringComparisonTranslator` — отдельный рефакторинг, отложен, чтобы G12 остался одним изменением.
- 🟡 **158 — исправлено.** Все новые/изменённые файлы нормализованы в CRLF (`perl -pi -e 's/\r?\n/\r\n/g'`).
- 🟡 **159 — исправлено.** `DialectCapabilityContractTests` расширен: цикл по `StringFormats` (`SupportsNumber`/`RenderNumber`, `SupportsDateFormat`/`RenderDate`), `SupportsCollation`/`MakeCollate`, `SupportsOrdinalComparison`/`MakeOrdinal` + `AtLeastOneDialect` для трёх новых возможностей.
- 🟡 **160 — принято с документированием.** `ToUpperInvariant`/`ToLowerInvariant` маппятся на `upper`/`lower` (осознанное подмножество); ограничение и оговорка про не-ASCII зафиксированы в `docs/guide/11-scalar-functions.md` (+RU), `docs/advanced/limitations.md` (+RU) и XML-doc. Поведение не менялось.
- ℹ️ **E (культура)** — оговорка про `string.Format`/invariant зафиксирована в публичных доках (EN+RU).

---

## 🔎 Точечный аудит 24.09.2026 — колонки-длительности (`TimeSpan`/`DurationAttribute`/`DurationUnit`), uncommitted working tree; 🔴 — 2 (Находки 161–162), 🟡 — 4 (163–166), ℹ️ — наблюдения A–H

**Область (uncommitted working tree + коммит 993ff5c).** Новое: `src/nextorm.core/DurationUnit.cs` (публичный enum, 7 членов), `DurationAttribute.cs` (публичный `sealed`-атрибут), `DurationStorage.cs` (internal). Изменено ядро: `Meta/IPropertyMetadata.cs` (2 DIM), `Meta/Implementation/PropertyMetadata.cs`, `Meta/EntityMetadataBuilder.cs`, `Meta/EntityPropertyBuilder.cs` (fluent `Duration`); `Expressions/SelectExpression.cs` (2 публичных свойства + `TimeSpan`/`DateTimeOffset` в `GetDataRecordMethod`), `Expressions/SelectExpressionPlanEqualityComparer.cs`; `Query/QueryCommand.QueryPreparer.cs` (`ResolveDuration`); `DataContext/RowMapperFactory.cs` (non-native чтение + ключ кэша); `DataContext/SqlMutationBuilder.cs`, `DataContext/QueryPlanner.cs`, `DataContext/SqlSourceRenderer.cs` (`DurationStorage.ToParameterValue`); `Builders/ReturningProjection.cs`, `Builders/InsertBuilder.Values.cs` (свёртка `x => DateTime.Now` в параметр); `Visitors/MemberTranslator.cs`, `Visitors/BaseExpressionVisitor.cs`, `Visitors/PredicateTranslator.cs` (`DurationUnitContext`), `Visitors/TypeFacts.cs`. Протокол диалекта (`ISqlDialect.SupportsNativeDuration`/`MakeDurationType`, `SqlDialectBase` + Postgres/MySQL/ClickHouse) введён коммитом `993ff5c` и в реестре до этого прохода не отражён (`grep -n -i "duration\|timespan"` — **0** совпадений). Тесты: SQLite `DurationColumnTests` (6), изменения `InsertMetadataTests`, провайдерные hook-тесты, `CommonTestSuite.Duration.cs` (1 на 5 провайдеров).

### 🔴 Находка 161 (P1, корректность/путь записи) — native bulk-путь обходит `DurationStorage.ToParameterValue`: `TimeSpan` уходит в bigint-колонку

**Место.** `src/nextorm.sqlserver/SqlServerDataContext.cs:222` (`BuildTable`: тип `DataColumn` = `columns[i].PropertyInfo.PropertyType`, т.е. `System.TimeSpan`); `src/nextorm.core/Builders/BulkInsertBuilder.cs:205` (`ToRow`: сырое `PropertyInfo.GetValue`). Нормализация есть только в переносимом `INSERT ... VALUES` — `src/nextorm.core/DataContext/SqlMutationBuilder.cs:878` (`AppendValuesRows`) и `PortableBulkInsertExecutor.BuildBatch:162`.

**Что не так.** `SupportsBulkCopy => true` (`src/nextorm.sqlserver/SqlServerDialect.cs:43`), поэтому `InsertInto<T>().Values(...).BulkInsert()` идёт по native-пути. Для не-native провайдера колонка-длительность — `bigint` (`MakeTypeName(TimeSpan)` → `"bigint"`, `SqlDialectBase.cs:373`), а `SqlBulkCopy` по CLR-типу `DataColumn` (`System.TimeSpan`) мапит в SQL `time`; значение при этом не приводится к объявленной единице. Два пути записи (параметрический vs bulk) расходятся; bulk-теста на `TimeSpan` нет.

**Стало (маршрут `nextorm-design-engineer`).** Нормализовать значение колонки через `DurationStorage.ToParameterValue(value, column, Dialect)` и создавать `DataColumn` целочисленного типа (`typeof(long)`) для non-native duration (в `SqlServerDataContext.BuildTable`/`BulkInsertBuilder.ToRow`); тест bulk+duration на SQL Server.

### 🔴 Находка 162 (P1, корректность/остаточный контекст) — `DurationUnitContext` выставляется и не сбрасывается: единица «протекает» на последующие `TimeSpan`-параметры

**Место.** `src/nextorm.core/Visitors/BaseExpressionVisitor.cs:75` (свойство), `:82` (`NormalizeDurationValue`), `:464` (`TryEmitValue`); установка — `src/nextorm.core/Visitors/MemberTranslator.cs:256` (рендер колонки-длительности) и `src/nextorm.core/Visitors/PredicateTranslator.cs:348-354` (перед сравнением). `Clone()` (`BaseExpressionVisitor.cs:522`) создаёт новый визитор, поэтому утечка — внутри одного визитора.

**Что не так.** Контекст — ambient-mutable и нигде не восстанавливается. После рендера колонки `x.DurSeconds` (unit=`Seconds`) он остаётся выставленным; любой `TimeSpan`-параметр/литерал, свёрнутый дальше в том же визиторе и **не** привязанный к колонке-длительности, нормализуется по устаревшей единице. Например, проекция `Select(x => new { x.DurSeconds, L = localSpan })` (порядок биндингов — declaration order: колонка, затем локальная) запишет `L` в секундах, а не в тиках. В `PredicateTranslator` сравнение без duration-колонки (`durationUnit is null`) контекст **не** сбрасывает — старое значение сохраняется. Тихо неверное значение параметра; только на не-native диалектах (SQL Server/SQLite/ClickHouse).

**Проверка.** Статический разбор пути (writer `MemberTranslator:256`/`PredicateTranslator:353` → reader `NormalizeDurationValue:82`/`TryEmitValue:464`); пробу не запускал — в этой сессии запись вне двух реестров запрещена политикой. По коду сценарий достижим.

**Стало (маршрут `nextorm-design-engineer`).** Сохранять/восстанавливать предыдущее значение вокруг рендера колонки и вокруг сравнения (`var saved = visitor.DurationUnitContext; try { ... } finally { visitor.DurationUnitContext = saved; }`) либо явно сбрасывать в `null`. Тест: проекция `new { x.DurSeconds, L = local }` на SQLite/ClickHouse проверяет значение `L`.

### 🟡 Находка 163 (P2, потеря единицы) — `ResolveDuration` понимает только голый `MemberExpression`: вычисленные/агрегатные/navigation-проекции длительности читаются как тики

**Место.** `src/nextorm.core/Query/QueryCommand.QueryPreparer.cs:439-448` (`ResolveDuration`); fallback — `src/nextorm.core/DataContext/RowMapperFactory.cs:43` (`column.DurationUnit ?? DurationUnit.Ticks`).

**Что не так.** `SelectExpression.DurationUnit` заполняется, только когда проекция — прямое `x.Prop` и `srcType` владеет `pi`. `Select(g => g.Min(x.DurSeconds))`, `Select(x => x.DurSeconds - x.Offset)`, `Select(x => x.Nav.DurSeconds)` получают `DurationUnit = null` и на не-native диалекте читаются `FromStorage(value, Ticks)` — секунды интерпретируются как тики. Молча неверное значение; тестов на вычисленную duration-проекцию нет.

**Стало.** Либо распространять единицу через вычисляемое выражение, либо задокументировать ограничение (проецировать duration можно только как прямой member) и вернуть громкий отказ; тест на `Min`/join-проекцию.

### 🟡 Находка 164 (P2, мёртвая публичная поверхность/DDL) — `MakeDurationType` не вызывается из продакшена, `DurationAttribute.Precision` не имеет эффекта

**Место.** `ISqlDialect.cs:324` (override — `PostgresDialect.cs:132`, `MySqlDialect.cs:324`; база — `SqlDialectBase.cs:107`); `DurationAttribute.Precision` (`DurationAttribute.cs:44`) → `PropertyMetadata.DurationPrecision` → `SelectExpression.DurationPrecision` (`SelectExpression.cs:40`) → `ReturningProjection.cs:92`, `QueryCommand.QueryPreparer.cs:275,305,347,401`.

**Что не так.** `roslyn callers NextORM.Core.ISqlDialect.MakeDurationType` — **0** продакшен-вызовов (10 call sites, все в тестах и в `CommonTestSuite.Duration.cs:48`, который сам строит DDL). Фактический тип колонки/каста даёт `MakeTypeName(Type)` (`SqlDialectBase.cs:373`, `PostgresDialect.cs:124`, `MySqlDialect.cs:316`, `ClickHouseDialect.cs:640`), игнорируя `unit`/`precision`. `DurationAttribute.Precision`, `EntityPropertyBuilder.Duration(unit, precision)` и `SelectExpression.DurationPrecision` только записываются и нигде не читаются продакшеном. Публичный `Precision` с доком про `TIME(3)`/`interval(3)` — no-op; hook-тесты проверяют `MakeDurationType` напрямую и потому зелёные.

**Стало.** Либо подключить `MakeDurationType` в рендер типа колонки (передавая `DurationUnit`/`Precision` из метаданных) и добавить DDL-тест, либо до заморозки поверхности убрать/перевести в internal неиспользуемый hook и `Precision` (или пометить `[Obsolete]`). Публичную форму не менять без решения.

### 🟡 Находка 165 (P2, аллокация на горячем пути чтения) — non-native duration читается через `IDataRecord.GetValue` + `Convert.ToInt64(object)`: бокс на каждую колонку×строку

**Место.** `src/nextorm.core/DataContext/RowMapperFactory.cs:18-20` (`GetValueMI`/`ToInt64MI`), `:39-46` (не-native ветка). Класс-док (`:22-25`) декларирует «typed getters only (no `GetValue`/boxing)».

**Что не так.** `GetValue` пакует `long` в `object`, `Convert.ToInt64(object)` распаковывает — бокс-аллокация на каждую duration-ячейку каждой строки. Тип хранения не-native duration гарантированно `bigint`/`Int64` (`MakeDurationType`/`MakeTypeName`), а `GetInt64` работает и на `int`/`smallint`. Прецедент-наблюдение о массивах/`Tuple` (`:4089`) бокса не имеет — там ссылочные типы; здесь value-type.

**Стало.** Использовать типизированный `GetInt64` (`SelectExpression.GetDataRecordMethod()`/`GetInt64MI`) вместо `GetValue`+`Convert`; измерение — `nextorm-db-perf-analyst`.

### 🟡 Находка 166 (P2, EOL) — 5 новых файлов записаны с LF

**Место.** `src/nextorm.core/DurationUnit.cs`, `DurationAttribute.cs`, `DurationStorage.cs`; `tests/nextorm.integration.tests/CommonTestSuite.Duration.cs`; `tests/nextorm.sqlite.tests/DurationColumnTests.cs` — **LF**; изменённые файлы CRLF. Нарушение AGENTS.md («CRLF throughout»), повтор Находок 140/158.

**Стало.** `perl -pi -e 's/\r?\n/\r\n/g' <файл>` (маршрут `nextorm-design-engineer`).

### ℹ️ Наблюдения (фикс не требуется)

- **A. Кэш-ключи — корректны.** `RowMapperFactory.BuildSignature` включает `DurationUnit` (`RowMapperFactory.cs:215`), `SelectExpressionPlanEqualityComparer` — в `Equals` (`:54`) и хэше (`:82`); `MapperCacheKey` включает `ProviderType` (`MapperCache.cs:15`). Одинаковый SQL на двух провайдерах записи не делит. `DurationPrecision` в ключи не входит — безопасно, пока Находка 164 стоит.
- **B. Per-provider решение НЕ вшито в глобальный `SelectExpression`.** `SelectExpression.DurationUnit` — provider-независимые метаданные; `SupportsNativeDuration` применяется в момент сборки маппера (`DataContext.MapColumnExpression` → `RowMapperFactory.MapColumn`) и записи (`DurationStorage.ToParameterValue(..., dialect)`), а глобальный `DataContextCache.SelectListCache` хранит provider-независимые `SelectExpression`. Соответствующей находки нет (закрывает пункт 3 брифинга).
- **C. Подавления.** `src/` — **11** (6 `SuppressMessage` c `Justification` + 5 парных `#pragma`), новых — **0**; `NoWarn`/`Skip=`/пустых `catch`/`Task.Delay` в src — **0**. Соотношение **11/11**. `.editorconfig` — те же 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` без `SonarAnalyzer` + `CA2254`) — пре-существующее, не переоткрывается.
- **D. XML-doc/анализ.** Build Release `0/0` + `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true` ⇒ CS1591 гейтится; новые публичные члены документированы 100 %.
- **E. Шаг 5 открыт.** `PublicAPI.*.txt` — **0**; ApiCompat/API-approval не подключены. Новая публичная поверхность (`DurationUnit`, `DurationAttribute`, `EntityPropertyBuilder<T>.Duration`, DIM `IPropertyMetadata.DurationUnit/DurationPrecision`, DIM `ISqlDialect.SupportsNativeDuration/MakeDurationType`, `SelectExpression.DurationUnit/DurationPrecision`) трекается через `API-NAMING-REVIEW.md` (DU1).
- **F. In-memory.** Хранилища нет: `DurationUnit`/`DurationAttribute` не влияют, значение остаётся `TimeSpan`; сравнение/чтение — нативные. Осознанное расхождение с реляционными non-native провайдерами; в `CommonTestSuite.Duration` in-memory не участвует.
- **G. Косметика.** `MemberTranslator.cs:107` — открывающая `{` на строке сигнатуры с лишними пробелами; отступы `:253-256` сдвинуты на один уровень. Не влияет.
- **H. Тесты/покрытие.** SQLite `DurationColumnTests` — 6/6 (round-trip, raw-хранилище, null, сравнение с константой/захваченной переменной, отрицательное значение, scalar-проекция); hook-тесты по 5 провайдерам; интеграционный `CommonTestSuite.Duration` — 1 на провайдер. **Не покрыто:** bulk+duration (161), вычисленная/агрегатная duration-проекция (163), утечка контекста (162), фактический DDL с `Precision` (164).

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release -p:NextOrmVersion=1.0.4-alpha` — **0 warnings / 0 errors** (без override — `NU1102` ×6 из-за uncommitted bump 1.0.5-alpha, Наблюдение A предыдущего раздела, не переоткрывается). `roslyn callers NextORM.Core.ISqlDialect.MakeDurationType` — 10 вызовов, **все** в тестах; `roslyn refs SelectExpression.DurationPrecision` — 5 ссылок, все — записи; `roslyn refs NextORM.Core.DurationStorage.ToParameterValue` — 5 (`QueryPlanner.cs:251`, `SqlMutationBuilder.cs:283,367,878`, `SqlSourceRenderer.cs:795`). Подавления `src/` — 11/11, новых 0; EOL: 5 новых файлов LF; `find -name 'PublicAPI*.txt'` — 0; публичные доки EN+RU — 0 упоминаний (`grep -rniE "durationattribute|durationunit" docs -g '!docs/specs/**'`). `slopwatch` не установлен, скан вручную.

---

## 🔎 Точечный аудит 24.09.2026 — ширина `date_diff` (§8.3) и sub-day promotion (§8.2): `date_diff_big`/`MakeDateDiffBig`/`MakeDateDiffCore`, `PromoteDateOperand`, uncommitted working tree; 🔴 — 0, 🟡 — 1 (Находка 167), ℹ️ — наблюдения A–G

**Область.** `Query/SqlFunctions.cs` (`date_diff_big`), `Visitors/BuiltinFunctionTranslator.cs` (promotion в `EmitDateAdd` `:311`, dispatch `MakeDateDiffBig` `:338`), `Visitors/DateTimeFunctionTranslator.cs` (`:47`), `DataContext/Dialect/ISqlDialect.cs` (2 DIM), `DataContext/Dialect/SqlDialectBase.cs` (`MakeDateDiffCore`/`IsSubDayField`/`PromoteDateOperand`), `SqlServerDialect.cs`, `PostgresDialect.cs`, `ClickHouseDialect.cs`. Тесты: SQL-gen/диалектные SQL Server/ClickHouse/Postgres, интеграционный `CommonTestSuite.Functions.DateDiffBig` + ClickHouse. Публичная сторона — `API-NAMING-REVIEW.md`, DB1–DB2; изменение полностью аддитивное.

### 🟡 Находка 167 (P2, DRY/сопровождаемость) — `SqlServerDialect.MakeDateDiff` и `MakeDateDiffBig` дублируют один и тот же маппинг plural→singular T-SQL частей

**Место.** `src/nextorm.sqlserver/SqlServerDialect.cs:420-425` и `:433-438` — идентичный `switch` (`"microseconds" => "microsecond"`, `"milliseconds" => "millisecond"`, `_ => field`).

**Что не так.** Новый `MakeDateDiffBig` скопировал маппинг из `MakeDateDiff` целиком; при добавлении/изменении части правку нужно вносить в двух местах (ClickHouse для сравнения держит один `MakeDateDiff`). Путь холодный (сборка SQL), но дубль реальный.

**Стало (маршрут `nextorm-design-engineer`).** Вынести `private static string TSqlDatePart(string field)` и вызывать из обоих методов: `$"datediff({TSqlDatePart(field)}, {start}, {end})"` / `datediff_big`. Поведение не меняется (покрыто `DateDiffBig_ShouldEmitDatediffBigFunction` и `SqlServerDialectTests`). **✅ Устранено 24.09.2026:** извлечён `SqlServerDialect.TSqlDatePart`, оба метода делегируют; SQL-server unit-тесты зелёные (Date*: 32/32).

### ℹ️ Наблюдения (фикс не требуется)

- **A. Promotion только в render-режиме — корректно, не запах.** `PromoteDateOperand` вызывается после раннего `if (visitor.IsParamMode) { visitor.Visit(...); return; }` (`BuiltinFunctionTranslator.cs:300-311`, `DateTimeFunctionTranslator.cs:35-47`). Параметрический проход не эмитит SQL и не формирует строку-операнд; хук оборачивает уже отрендеренный текст (`cast(...)`/`toDateTime(...)`), а binding параметра остаётся прежним CLR `DateTime` — пропуск в param-mode не влияет ни на `@pN`, ни на значение. Так ведут себя все `Make*`-хуки; принудительный каст в param-проходе был бы ошибкой.
- **B. `date_diff` (`int?`) на ClickHouse/MySQL остаётся узким — принято.** `date_diff_big` на ClickHouse рендерит тот же `dateDiff(...)` (DIM → `MakeDateDiff`), отличие только в read-типе (`SelectExpression.cs:95-101`: `long` → `GetInt64`, `int` → `GetInt32`). Широкий диапазон обязан идти через `date_diff_big`; ограничение задокументировано (`todo_timespan_columns.md` §8.3), не переоткрывается.
- **C. `MakeDateDiffCore(..., bool big)` — flag-параметр.** Новый `protected` хелпер с поясняющим `<summary>`; enum ширины/два метода не окупаются (одна ветка). Не запах.
- **D. Подавления/слоп.** `src/` — **5** `#pragma warning disable` + **6** `SuppressMessage` = **11** (0 неоправданных), новых в файлах изменения — **0**; `NoWarn` — 0; `Skip=` — 0; `Task.Delay` — src **0** / tests **2** (baseline `Task.Delay(0)`); пустых `catch` — 0. `slopwatch` не установлен → скан паттернов вручную.
- **E. XML-doc/анализ.** Build `0/0` + `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true` ⇒ CS1591 гейтится; все новые public/protected члены документированы. `.editorconfig` — те же 7 инертных `S*`/`CA2254` `silent` (пре-существующее, не переоткрывается).
- **F. EOL/Шаг 5.** Все **15/15** затронутых файлов — CRLF; `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт).
- **G. Покрытие.** SQL-gen unit для promotion/`date_diff_big` есть у SQL Server/ClickHouse/Postgres; у SQLite/MySQL/MariaDB хук делегирующий — их `date_diff_big` покрыт только общим интеграционным `DateDiffBig_ShouldReturn64BitSpan`. Отдельный SQL-gen для них смысла не имеет (SQL совпадает с `date_diff`), но `DateTime.Add*` promotion на SQLite/MySQL/MariaDB unit-тестами не зафиксирован.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**. `roslyn refs NextORM.Core.ISqlDialect.MakeDateDiffBig` — **4** (`BuiltinFunctionTranslator.cs:338` + 3 теста); `roslyn refs NextORM.Core.ISqlDialect.PromoteDateOperand` — **9** (прод: `BuiltinFunctionTranslator.cs:311`, `DateTimeFunctionTranslator.cs:47`; 7 тестовых). Сфокусированные unit-тесты (Release, `--no-build`, фильтр `FullyQualifiedName~Date`): sqlserver **32/32**, clickhouse **23/23**, postgres **41/41** — 0 failed, 0 skipped. Контейнерная интеграция (`DOCKER_HOST=unix:///mnt/wsl/podman-sockets/podman-machine-default/podman-user.sock`, фильтр `~DateDiffBig`) — **5/5, 0 failed, 0 skipped**; полный прогон автора — 1551 total / 0 failed / 67 skipped (целиком аудитом не перезапускался). `wc -l` — `ISqlDialect.cs` 1648, `SqlDialectBase.cs` 1010, `SqlFunctions.cs` 820, `BuiltinFunctionTranslator.cs` 499, `DateTimeFunctionTranslator.cs` 50, `SqlServerDialect.cs` 796, `ClickHouseDialect.cs` 911, `PostgresDialect.cs` 625. Подавления 11/11; EOL 15/15 CRLF; `PublicAPI*.txt` — 0; публичные доки EN+RU содержат `date_diff_big`.

**Итог.** 🔴 — **0**; 🟡 **Находка 167** (DRY, P2, маршрут `nextorm-design-engineer`); остальное — наблюдения/принято. Публичная сторона — `API-NAMING-REVIEW.md`, DB1–DB2.

---

## 🔎 Точечный аудит 24.09.2026 — трансляция `Regex.IsMatch`/`Regex.Replace` (G7/#68), uncommitted working tree; 🔴 — 0, 🟡 — 2 (Находки 168–169), ℹ️ — наблюдения A–H

**Область (uncommitted working tree).** Новое: `src/nextorm.core/Visitors/RegexSqlTranslator.cs` (**214** строк, `internal static`; маршрут `ScalarFunctionTranslator.cs:47-48`), `TypeFacts.cs:41-42` (`IsPredicateCall` узнаёт `Regex.IsMatch`). Изменено ядро: `ISqlDialect.cs` — 3 члена (`SupportsRegex` DIM `:766`, `MakeRegexMatch` DIM-throwing `:774`, `MakeRegexReplace` DIM-throwing `:782`); `SqlDialectBase.cs:451-459` — виртуальные дефолты. Провайдеры: Postgres `PostgresDialect.cs:410-420`, MySQL `MySqlDialect.cs:392-403`, MariaDB `MariaDbDialect.cs:17-28`, ClickHouse `ClickHouseDialect.cs:495-506`, SQLite `SqliteDialect.cs:142-152` + `SQLiteFunctions.cs:39-46` (`regexp`/`regexp_replace`; вызов — `SqliteDataContext.OnConnectionCreated:48`). Тесты: `StringSemanticsSqlGenerationTests.cs` 6 проектов (postgres 6, mysql 4, mariadb 4, clickhouse 4, sqlite 3, sqlserver 1 throw), `InMemoryStringSemanticsTests.cs` (+3), `CommonTestSuite.Functions.cs:299-336` (3), `ITestProvider.cs:116-121`. Публичная сторона — `API-NAMING-REVIEW.md`, RX1–RX3. План — `docs/specs/roadmap/todo_regex.md`.

### 🟡 Находка 168 (P2, DRY/дублирование) — `QuoteRegexLiteral` скопирован 5 раз; core-хелпер `SqlLiteral.ToSqlStringLiteral` недостижим для провайдеров, но `SqlDialectBase` — общая база

**Место.** Quote-only вариант: `PostgresDialect.cs:420`, `SqliteDialect.cs:152` — идентично `"'" + value.Replace("'", "''") + "'"` (это же тело у `internal SqlLiteral.ToSqlStringLiteral`, `Visitors/SqlLiteral.cs:12`). Backslash+quote вариант: `MySqlDialect.cs:402-403`, `MariaDbDialect.cs:27-28`, `ClickHouseDialect.cs:505-506` — `"'" + value.Replace("\\", "\\\\").Replace("'", "''") + "'"` (MySQL/ClickHouse трактуют `\` как escape в строковом литерале). Итого **5** приватных копий одного хелпера в 2 вариантах.

**Что не так.** `SqlLiteral` — `internal` в `nextorm.core`, `InternalsVisibleTo` в репозитории нет, поэтому провайдеры не могут вызвать его напрямую (прецедент — Находка 156: `TryMapDate`/`Match` оставлены продублированными). Но носитель здесь другой: все провайдеры наследуют **публичный** `SqlDialectBase`, т.е. ограничение 156 («нужен новый публичный тип») не действует.

**Стало (маршрут `nextorm-design-engineer`).** Вынести на `SqlDialectBase` `protected static string QuoteStringLiteral(string value)` (quote-only) и backslash-вариант (перегрузка `QuoteStringLiteral(string value, bool escapeBackslash)` либо отдельный хелпер для MySQL-семейства + ClickHouse); провайдеры удаляют локальные `QuoteRegexLiteral`. Поведение не меняется (SQL-gen-тесты провайдеров зелёные).

**Резолюция (24.09.2026).** Вынесено на `SqlDialectBase.QuoteStringLiteral(string value, bool escapeBackslash = false)`; все пять провайдеров используют его, локальные `QuoteRegexLiteral` удалены. SQL-gen-тесты провайдеров зелёные.

### 🟡 Находка 169 (P2, контракт/покрытие) — новая возможность `SupportsRegex` не входит в `DialectCapabilityContractTests`: пара «флаг ↔ рендерер» не проверяется

**Место.** `tests/nextorm.integration.tests/DialectCapabilityContractTests.cs:188-192` (проверки `SupportsCollation`/`SupportsOrdinalComparison`, добавленные при закрытии Находки 159) и `:219-220` (`AtLeastOneDialect`) — `SupportsRegex` отсутствует. Дефолты, которые провайдер обязан переопределить, — `ISqlDialect.cs:774,782` и `SqlDialectBase.cs:455,458` (бросают).

**Что не так.** Провайдер, выставивший `SupportsRegex = true` и забывший `MakeRegexMatch`/`MakeRegexReplace`, не будет пойман: интеграционные тесты гейтятся тем же флагом (`ITestProvider.cs:121`, `CommonTestSuite.Functions.cs:302`), а контрактный тест молчит. Аналог Находки 159/RLM3.

**Стало.** В цикле (после `:192`) добавить `if (dialect.SupportsRegex) { dialect.MakeRegexMatch("[x]", "^a", false).Should().NotBeNullOrEmpty(); dialect.MakeRegexReplace("[x]", "^a", "#", false).Should().NotBeNullOrEmpty(); }`; в `AtLeastOneDialect_ShouldExposeEachClassACapability` — `dialects.Should().Contain(d => d.SupportsRegex);`. Маршрут `nextorm-design-engineer`.

**Резолюция (24.09.2026).** Пара «флаг ↔ рендерер» и `dialects.Should().Contain(d => d.SupportsRegex)` добавлены в `DialectCapabilityContractTests`; оба теста зелёные.

### ℹ️ Наблюдения (фикс не требуется)

- **A. Портируемость ядер regex не гейтится и не документирована (смежно с RX2/RX3).** Транслятор принимает любой константный паттерн/replacement и передаёт его ядру провайдера, а ядра разные: ClickHouse — RE2 (нет backreferences/lookaround), PostgreSQL — POSIX/advanced, MySQL 8 — ICU, SQLite — CLR `Regex` (ближе всех к семантике C#). Синтаксис групп в replacement тоже различается: .NET — `$1`, PostgreSQL `regexp_replace`/ClickHouse `replaceRegexpAll` — `\1`. Паттерн/replacement, валидный на SQLite/MySQL, может дать ошибку в рантайме на ClickHouse или тихо подставить литерал вместо группы на PG/ClickHouse. Открытый вопрос §9.1 плана (`todo_regex.md`) не решён; отдельного кодового фикса не предлагаю, но ограничения обязаны попасть в публичную прозу (см. RX2/RX3).
- **B. `Expression.Compile` на пути трансляции — по прецеденту, холодный путь.** `TryGetRegex` (`RegexSqlTranslator.cs:190-191`) для захваченного параметр-фри инстанса `Regex` (в т.ч. локальной переменной в `regex.IsMatch(...)`) выполняет `Expression.Lambda<Func<object?>>(Expression.Convert(receiver, typeof(object))).Compile()()` — тот же приём, что в `InValuesEvaluator.cs:30,38`, `InsertBuilder.Values.cs:79`, `UpdateBuilder.cs:257`. SQL строится один раз и кэшируется планом; на горячем пути исполнения компиляции нет. ℹ️.
- **C. Смешанный контракт отказа vs. константная свёртка (статический разбор, без пробы).** `Reject` (`:207-213`) намеренно оставляет вызов **без** query-параметра на константную свёртку вызывающего (`EmitFoldedParameter`, `BaseExpressionVisitor.cs:203-206`), но `ResolveOptions` (`:137-145`) и проверка `SupportsRegex` (`:107-108`) бросают **безусловно** — до проверки параметров. Поэтому parameter-free `Regex.IsMatch("abc", "^a", RegexOptions.Multiline)` или любой constant-only `Regex`-вызов на SQL Server бросает вместо клиентской свёртки, хотя `Reject` для того же класса входа вернул бы `false`. Влияние узкое (вырожденные выражения без колонок); если контракт `Reject` считать нормативным — перенести обе проверки под `if (node.Has<ParameterExpression>())` либо явно задокументировать, что валидный `Regex` транслируется всегда.
- **D. God-class — чисто.** `RegexSqlTranslator` — **214** строк, `ScalarFunctionTranslator` — 192 (диспетчер), `StringFunctionTranslator` — 679 (пре-существующий, Находка 157 принята). `ISqlDialect.cs` вырос 1648 → **1673** (3 члена, ~25 строк) — крупный интерфейс-накопитель; отдельной находкой не переоткрывается.
- **E. Подавления/слоп.** `src/` — **5** `#pragma warning disable` + **6** `SuppressMessage` = **11** (0 неоправданных), в файлах фичи новых — **0**; `NoWarn` — 0 (только комментарий в `Directory.Build.props:37`); `TreatWarningsAsErrors=false` — 0; `Skip=`/`[Ignore]` — 0; пустых `catch` — 0; `throw ex;`/`async void` — 0. `Task.Delay` — src **0** / tests **2** (baseline `Task.Delay(0)`, `InMemoryTests.cs:153,442`). `slopwatch` не установлен (`dotnet-tools.json` — только coverage/reportgenerator/docfx) → скан паттернов вручную. Соотношение **11/11**.
- **F. XML-doc/анализ.** Build Release **0/0** + `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`), `NoWarn` — 0 ⇒ CS1591 гейтится; новые public/protected члены документированы (`ISqlDialect.cs:761-783`, `SqlDialectBase.cs:451-459`, override'ы — `/// <inheritdoc/>`). `.editorconfig` — те же 7 инертных `S*`/`CA2254` `silent` (SonarAnalyzer не подключён; `Directory.Packages.props:25` — только `Microsoft.CodeAnalysis.Analyzers` 5.9.0) — пре-существующее, не переоткрывается.
- **G. SQL Server — осознанный gate-off.** `SqlServerDialect` не переопределяет `SupportsRegex` (база `SqlDialectBase.cs:451` → `false`), regex-движка нет; unit-тест `RegexIsMatch_ShouldThrowBecauseNoRegexEngine` фиксирует `NotSupportedException`. Соответствует §4/§7.4 плана.
- **H. Тесты/покрытие.** Unit (Release, `--no-build`, фильтр `FullyQualifiedName~Regex`): core **3/3**, postgres **10/10** (6 новых + 4 пре-существующих `regexp_*`/TVF), mysql **4/4**, mariadb **4/4**, clickhouse **4/4**, sqlite **3/3**, sqlserver **1/1** (throw) — 0 failed. Контейнерная интеграция (`DOCKER_HOST=…`, тот же фильтр): **15 total / 12 succeeded / 3 skipped** — 3 SQL Server пропущены из-за сбоя контейнера (`container state improper`), не из-за кода; PG/MySQL/MariaDB/ClickHouse/SQLite зелёные. **Не покрыто:** живой round-trip `Regex.Replace` с группами/`$n`; SQLite-регистрация `regexp` покрыта только интеграционно; MariaDB `(?-i)`-путь — SQL-gen, без интеграции.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; unit-фильтр `~Regex` — 3+10+4+4+4+3+1 = **29 passed / 0 failed**; интеграция `~Regex` — 12 passed / 3 skipped (SQL Server container). Подавления `src/` — **11/11**, новых 0; `find -name 'PublicAPI*.txt'` — **0**; `rg -ni 'IsMatch|Regex\.Replace|System\.Text\.RegularExpressions' docs -g '!docs/specs/**' -g '!docs/_site/**' -g '!docs/api/**'` — **0 совпадений**; `roslyn callers NextORM.Sqlite.SQLiteFunctions.Register` — 1 (`SqliteDataContext.OnConnectionCreated:48`). `slopwatch` не установлен, скан вручную.

**Итог.** 🔴 — **0**; 🟡 **Находки 168–169** (P2, маршрут `nextorm-design-engineer`); остальное — наблюдения/принято. Публичная сторона — `API-NAMING-REVIEW.md`, RX1–RX3.

---

## 🔎 Точечный аудит 24.09.2026 — колонковая коллация (`#28`): `CollationAttribute`, DIM `IPropertyMetadata.Collation`, `EntityPropertyBuilder<T>.Collation`, `ResolveCollation`/`AppendColumn`/`SuppressColumnCollation`, uncommitted working tree; 🔴 — 0, 🟡 — 1 (Находка 170, исправлено), ℹ️ — наблюдения A–H

**Область (uncommitted working tree).** Новое: `src/nextorm.core/CollationAttribute.cs` (публичный `sealed`-атрибут; в публичную поверхность входит — `API-NAMING-REVIEW.md`, CN1–CN2). Изменено ядро: `DataContext/Meta/IPropertyMetadata.cs` (DIM `Collation => null` `:85`), `…/Implementation/PropertyMetadata.cs` (реализация `Collation` `:15`), `DataContext/Meta/EntityPropertyBuilder.cs` (fluent `Collation` `:97` + запись в `Build` `:113`), `DataContext/Meta/EntityMetadataBuilder.cs` (чтение `[Collation]`, в т.ч. с интерфейсного свойства, `:65,:78`); `Visitors/MemberTranslator.cs` — `ResolveCollation` (`:110-124`), `AppendColumn` (`:130-148`), вызовы в entity-ветке (`:206`) и join-ветке (`:439-442`); `Visitors/BaseExpressionVisitor.cs` — `SuppressColumnCollation` (`:77`) и `VisitToStringSuppressingColumnCollation` (`:391-397`); `Visitors/StringFunctionTranslator.cs` (ordinal-ветки `:203-204`, `:252-253`, `:440-441`, `:476-477`, `:606-608`); `Visitors/ScalarFunctionTranslator.cs` (`collate` `:133`). Хуки `ISqlDialect.SupportsCollation`/`MakeCollate` — пре-существующие (G12/SS, здесь не переоткрываются). Тесты: 6 SQL-gen-файлов (PG 7 + SQL Server/MySQL/MariaDB/SQLite по 2 + ClickHouse 1 = **16**), in-memory **1**, интеграционные `ColumnCollation_ShouldApplyDeclaredBinaryCollation` **4** (PG/SQL Server/MySQL/SQLite). План — `docs/specs/roadmap/todo_column_collation.md`.

### 🟡 Находка 170 (P2, корректность/состояние визитора) — `SuppressColumnCollation` не переносится `Clone()`: во вложенном рендере подавление теряется (двойной `COLLATE`, ложный отказ на ClickHouse)

**Место.** `src/nextorm.core/Visitors/BaseExpressionVisitor.cs:391-397` (`VisitToStringSuppressingColumnCollation`), `:542-547` (`Clone()` → `new BaseExpressionVisitor(_options)`, флаг не копируется), `src/nextorm.core/Visitors/ArraySqlTranslator.cs:439-442` (`HigherOrderLambdaVisitor.Clone()` — тоже новый визитор без копирования). Флаг читает `AppendColumn` — `src/nextorm.core/Visitors/MemberTranslator.cs:132`.

**Что не так.** `VisitToStringSuppressingColumnCollation` создаёт клон и выставляет `SuppressColumnCollation = true` (`:393-394`). Но любой вложенный `VisitToString` — а его вызывают все string/date-трансляторы, например `EmitStringFunction` (`StringFunctionTranslator.cs:138`) — снова клонирует визитор, а `Clone()` создаёт визитор без состояния и флаг теряет. Поэтому подавление действует только на members, отрендеренные **прямо** на клоне, и не действует на members внутри вложенных вызовов. Сценарий: `string.Equals(x.Name!.ToUpper(), "A")` → `TryTranslateEquals` (`StringFunctionTranslator.cs:203`) вызывает `VisitToStringSuppressingColumnCollation(x.Name.ToUpper())`; `ToUpper` рендерится через `EmitStringFunction` → `visitor.VisitToString(x.Name)` → колонка рендерится **с** коллацией. Итог на PG: `upper(name collate "C") collate "C"` вместо `upper(name) collate "C"`; при объявленной не-бинарной коллации внутренняя функция считается под ней, а сравнение — бинарное. На ClickHouse (`SupportsCollation => false`, `ClickHouseDialect.cs:486`) прямой `string.Equals(x.Name, "A")` **не** бросает (подавление срабатывает, `AppendColumn:132`), а `string.Equals(x.Name.ToUpper(), "A")` бросает `NotSupportedException` из вложенного `VisitToString` — fail-fast срабатывает непоследовательно. Существующие тесты закрывают только прямой member (`CollatedColumn_OrdinalEquals_ShouldRenderBinaryCollation`), вложенный случай не покрыт.

**Стало (маршрут `nextorm-design-engineer`).** Переносить флаг в `BaseExpressionVisitor.Clone()` (`var c = new BaseExpressionVisitor(_options) { SuppressColumnCollation = SuppressColumnCollation }; return c;`) и в override `HigherOrderLambdaVisitor.Clone()` (или сбросить флаг в исходном визиторе сразу после обёртки). Тест: `string.Equals(x.Name!.ToUpper(), "a")` на PG даёт `upper(name) collate "C"` (внутреннего `collate` нет), на ClickHouse — не бросает.

**✅ Исправлено (24.09.2026).** Флаг теперь переносится обоими `Clone()`: `BaseExpressionVisitor.cs:546` (`new BaseExpressionVisitor(_options) { SuppressColumnCollation = SuppressColumnCollation }`) и `ArraySqlTranslator.cs:442` (`HigherOrderLambdaVisitor`). Регрессионный тест `CollatedColumn_NestedInOrdinalFunction_ShouldNotCollateTwice` (`tests/nextorm.postgres.tests/ColumnCollationSqlGenerationTests.cs`) фиксирует `upper(name) collate "C" = 'A' collate "C"`. PG SQL-gen **8/8**, build **0/0**.

### ℹ️ Наблюдения (фикс не требуется)

- **A. `ResolveCollation` на горячем пути — чист.** `MemberTranslator.cs:110-124`: цикл `for` по `metadata.Properties` (индексный доступ к `IList`), ни LINQ, ни лямбд/замыканий, ни аллокаций; вызывается на каждый mapped-member рендер (`:206`, `:441`). SQL строится один раз и кэшируется планом, так что это и не горячий путь исполнения. Пункт 1 брифинга закрыт — замечаний нет. (Соседний `ResolveDurationUnit:97-105` использует `.FirstOrDefault(p => …)` — это duration-scope, вне данного аудита.)
- **B. `AppendColumn` — одна строковая аллокация на колонку с коллацией.** `MemberTranslator.cs:141-146`: идентификатор рендерится в общий builder, затем `builder.ToString(start, len)` (`:144`) копируется в строку, builder усекается (`Length = start`) и оборачивается `MakeCollate`. Аллокация только для колонок с **объявленной** коллацией (без неё путь — просто `AppendIdentifier`, `:132-135`); путь холодный (сборка SQL). ℹ️, не блокер.
- **C. Fail-fast на `SupportsCollation == false` — корректен и покрыт.** `AppendColumn.cs:138-139` бросает `NotSupportedException` до подстановки идентификатора; `ClickHouseDialect.SupportsCollation => false` (`:486`); тест `CollatedColumn_ShouldThrowBecauseNoCollateClause` (`tests/nextorm.clickhouse.tests/ColumnCollationSqlGenerationTests.cs`) фиксирует отказ и текст `*COLLATE*`. Срабатывает на **любую** ссылку на колонку, в т.ч. на чистую проекцию `Select(x => x.Name)` — т.е. сущность с `[Collation]` на ClickHouse непригодна в любом запросе. Это соответствует §5/§9.2 плана (`fail-fast в момент рендера`, как у `SqlFunctions.Sql.collate`) — принято, не переоткрывается.
- **D. Внешние ссылки коррелированных подзапросов — принято как фаза 2.** `MemberTranslator.cs:297` (New-ветка `OuterRefMarker`) и `:565` (`TryTranslateProjectionOuterReference`) рендерят `alias.col` через `AppendIdentifier`, минуя `AppendColumn`, поэтому коллация внешней ссылки не применяется. План (`todo_column_collation.md` §9.1) осознанно откладывает это; listed deviation, не находка.
- **E. Состояние флага не «протекает» между запросами.** `BaseExpressionVisitor.SuppressColumnCollation` выставляется только на свежем клоне (`BaseExpressionVisitor.cs:393-394`), оригинальный визитор не мутируется, клон возвращается в пул по `using`. Отличается от `DurationUnitContext` (Находка 162): здесь нет restore-less записи в живой визитор. Единственный дефект — Находка 170 (неперенос флага).
- **F. Подавления/слоп.** `src/` — **5** `#pragma warning disable` + **6** `SuppressMessage` = **11/11** (0 неоправданных), в коллационных файлах новых — **0**; `NoWarn` — **0** (только комментарий `Directory.Build.props:37`); `Skip=`/`[Ignore]` — **0**; пустых `catch` — **0**; `throw ex;`/`async void` — 0; `Task.Delay` — src **0** / tests **2** (baseline). `slopwatch` не установлен (`dotnet-tools.json` — coverage/reportgenerator/docfx) → скан паттернов вручную. Соотношение **11/11** — не systematic (0 новых).
- **G. EOL/анализ/Шаг 5.** Все **8** новых коллационных файлов (1 `src` + 7 `tests`) — **CRLF** (Находка 166 не повторяется); `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт, `API-NAMING-REVIEW.md` CN1). Build Release **0/0** + `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true` ⇒ CS1591 гейтится; новые public-члены (`CollationAttribute`, DIM `IPropertyMetadata.Collation`, `EntityPropertyBuilder<T>.Collation`) документированы. `.editorconfig` — те же 6 инертных `S*` + `CA2254` (`SonarAnalyzer` не подключён) — пре-существующее, не переоткрывается.
- **H. Тесты/покрытие.** SQL-gen: PG **7/7** (Where/OrderBy/Projection/Like/Join/Fluent/Ordinal), SQL Server/MySQL/MariaDB/SQLite — по **2** (Where/OrderBy), ClickHouse — **1** (throw); in-memory — **1**; интеграция — **4** (`ColumnCollation_ShouldApplyDeclaredBinaryCollation`, по одной на PG/SQL Server/MySQL/SQLite). **Не покрыто:** вложенный ordinal/`collate` (Находка 170); внешние ссылки `OuterRefMarker` (принято, §9.1); контрактный тест `CollationAttribute` ↔ `SupportsCollation` — уже покрыт общим `DialectCapabilityContractTests` (Находка 159 закрыта).

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**. Статический разбор пути: `VisitToStringSuppressingColumnCollation` (`BaseExpressionVisitor.cs:391-397`) → `Clone()` (`:542-547`, без копирования `SuppressColumnCollation`) → `EmitStringFunction` (`StringFunctionTranslator.cs:138`, `VisitToString`) → `AppendColumn` (`MemberTranslator.cs:132`); пробу не запускал — запись вне двух реестров запрещена политикой, сценарий достижим по коду (аналогично Находке 162). Подавления `src/` — **11/11**, новых 0; EOL — 8/8 CRLF; `find -name 'PublicAPI*.txt'` — **0**; публичные доки EN+RU — **0** упоминаний `CollationAttribute`/`IPropertyMetadata.Collation`/`.Collation(` (`rg -n 'CollationAttribute|IPropertyMetadata\.Collation|\.Collation\(' docs -g '!docs/specs/**' -g '!docs/_site/**' -g '!docs/api/**'`) — сторона API-NAMING, CN2. `slopwatch` не установлен, скан вручную.

**Итог.** 🔴 — **0**; 🟡 **Находка 170** (P2, корректность, маршрут `nextorm-design-engineer`); остальное — наблюдения/принято. Публичная сторона — `API-NAMING-REVIEW.md`, CN1–CN2.

---

## 🔎 Точечный аудит 24.09.2026 — интерцепторы Фазы 1 (`IQueryInterceptor`/`IConnectionInterceptor`, `AddInterceptor`, `InterceptorHooks`), uncommitted working tree; 🔴 — 0, 🟡 — 3 (Находки 171–173), ℹ️ — наблюдения A–H

**Область (uncommitted).** Новый публичный `NextORM.Core.IQueryInterceptor` (`src/nextorm.core/Interceptors/IQueryInterceptor.cs:25`) + `CommandEventData` (`:11`); `NextORM.Core.IConnectionInterceptor` (`Interceptors/IConnectionInterceptor.cs:22`) + `ConnectionEventData` (`:11`); регистрация `DataContextBuilder.AddInterceptor(IQueryInterceptor)` (`DI/DataContextBuilder.cs:151`) / `(IConnectionInterceptor)` (`:166`); per-instance `DataContext.AddInterceptor` (`DataContext/DataContext.cs:170,182`). Internal-инфра: `InterceptorHooks` (`DataContext/DataContextDependencies.cs:44`, COW-массивы + `Empty` `:57`), `QueryExecutor` (`QueryExecutor.cs:52-208` хелперы/обёртки, `:274` mutation-init), `QueryPlanner.RaiseCommandInitialized` (`QueryPlanner.cs:555`, вызов `:428`), `DbConnectionManager` (`DbConnectionManager.cs:52,70`), `ResultSetEnumerator.InitEnumerator`/`ExecuteReader*` (`ResultSetEnumerator.cs:191,205,235`). Тесты: `tests/nextorm.core.tests/InterceptorBuilderTests.cs`, `tests/nextorm.sqlite.tests/InterceptorTests.cs`.

| Пункт | Состояние |
|---|---|
| 1. `IDisposable` | ✅ Новых владельцев ресурсов нет; `QueryExecutor` по-прежнему `using var cmd` для мутаций (`:288,300,311,324,341,361`), reader/соединение — существующая семантика `ResultSetEnumerator`. `CA2000/CA2213/CA1816` не затронуты. |
| 2. Подавления | ✅ В фиче — **0** новых `SuppressMessage`/`#pragma`/`NoWarn`. База `src/` — **6** `SuppressMessage` + **5** `#pragma` = **11/11** (0 неоправданных). `Skip=` — 0, пустых `catch` — 0, новых `Task.Delay` — 0. |
| 3. LINQ на горячем пути | ✅ В raise-циклах — только `for` по массиву; LINQ/лямбд/замыканий нет. |
| 4. События/исключения | ⚠️ Поднятие событий продублировано 11 раз (Находка 171); семантика исключений интерцептора не оговорена (ℹ️ B). |
| 5. Хэш-ключи/равенство | ✅ `CommandEventData`/`ConnectionEventData` — `readonly record struct`, в план-ключ не входят; `InterceptorHooks` — не ключ. |
| 6. God-классы | ⚠️ `QueryExecutor.cs` **533 → 745** строк (>500 — пре-существующий размер, +212 строк фичи — в основном дублирующие обёртки; закрывается Находкой 171, Находка 68 не переоткрывается). |

### 🟡 Находка 171 (P2, DRY/сопровождаемость) — цикл поднятия событий продублирован 11 раз в 3 классах

**Место.** `QueryExecutor.cs:58-60` (`CommandInitialized`), `:66-68` (`CommandExecuting`), `:76-79` (`CommandExecuted`), `:85-87` (`CommandFailed`); `ResultSetEnumerator.cs:211-213/222-224/228-230` (sync `ExecuteReader`) и `:241-243/252-254/258-260` (async `ExecuteReaderAsync`) — 6 inline-циклов; `QueryPlanner.cs:561-563` (`CommandInitialized` — копия хелпера executor'а). Плюс 7 конструкций `new CommandEventData(...)` (`QueryExecutor` ×4, `ResultSetEnumerator` ×2, `QueryPlanner` ×1).

**Что не так.** Один и тот же паттерн «взять массив, early-return на `Length == 0`, создать `CommandEventData`, `for`-цикл» скопирован 11 раз. `ResultSetEnumerator.ExecuteReader`/`ExecuteReaderAsync` — почти дословная копия `QueryExecutor.RunReader`/`RunReaderAsync` (различия: `_compiledQuery.Behavior` и `_context!` vs `_context`); `QueryPlanner.RaiseCommandInitialized` — копия `QueryExecutor.RaiseCommandInitialized`. Новое событие или смена сигнатуры `CommandEventData` теперь требует правки 11 мест; `BeginCommand`/`EndCommand`/`FailCommand` уже вынесены в executor, но не переиспользованы enumerator'ом. Это ровно тот DRY между `QueryExecutor` и `ResultSetEnumerator`, который просил проверить брифинг.

**Стало (маршрут `nextorm-design-engineer`).** Централизовать поднятие в `InterceptorHooks` (владелец массивов) или отдельный `internal static class InterceptorDispatcher`: методы `RaiseInitialized(IDataContext, DbCommand)`, `RaiseExecuting(...)`, `RaiseExecuted(..., long started)`, `RaiseFailed(..., Exception)` сами делают early-return и цикл. `QueryExecutor`, `QueryPlanner` и `ResultSetEnumerator` вызывают их; у enumerator'а уходит `_context`-дублирование события, у planner'а — приватная копия. Ожидаемый эффект: **11 → 1** цикл на каждый тип события, минус ~90 строк дублей в `QueryExecutor.cs`.

### 🟡 Находка 172 (P2, аллокации/горячий путь) — `async`-обёртки `Run*Async`/`ExecuteReaderAsync` добавляют state machine на пустом наборе: критерий «пустой путь не аллоцирует» не выполнен на async-терминалах

**Место.** `QueryExecutor.cs:110` (`RunNonQueryAsync`), `:150` (`RunScalarAsync`), `:190` (`RunReaderAsync`), `ResultSetEnumerator.cs:235` (`ExecuteReaderAsync`). Контрольная точка приёмки — `docs/specs/roadmap/todo_interceptors.md:12` («при отсутствии зарегистрированных интерцепторов путь исполнения не аллоцирует»).

**Что не так.** Синхронный путь чист: `RunNonQuery`/`RunScalar`/`RunReader` (`:90,130,170`) после `if (interceptors.Length == 0)` возвращают результат провайдера напрямую — ни `CommandEventData`, ни `Stopwatch`, ни делегатов/замыканий. Но `Run*Async` объявлены `async Task<...>` и в пустой ветке делают `return await command.Execute*Async(...)`; метод компилируется в отдельный async state machine и аллоцирует собственную `Task`, даже когда интерцепторов нет. До фичи терминалы (`ExecuteNonQueryAsync`/`ToListAsync`/…) вызывали `await cmd.Execute*Async(...)` напрямую — один async-frame; теперь их два (+1 `Task` на вызов на каждом async-терминале). Комментарий `QueryExecutor.cs:48-50` («no delegates, closures or timestamps are allocated») узко верен — перечисленного действительно нет, — но заявленный в RFC инвариант «не аллоцирует» на async-терминалах не держится. Это не «нулевая» регрессия по величине, но горячий путь чтения/записи.

**Стало (маршрут `nextorm-design-engineer`).** Внешний метод сделать не-`async` и возвращать `Task`:

```csharp
private Task<int> RunNonQueryAsync(DbCommand command, CancellationToken ct)
{
    var interceptors = _interceptors.QueryInterceptors;
    if (interceptors.Length == 0)
        return command.ExecuteNonQueryAsync(ct);
    return RunNonQueryInstrumentedAsync(command, interceptors, ct);
}
```

(инструментированная ветка с `try/catch` — отдельный приватный `async`-метод). Так пустой путь не создаёт промежуточный state machine/Task, а `ConfigureAwait(false)` теряется только в обёртке без продолжения (у вызывающего терминала остаётся). Замер — за `nextorm-db-perf-analyst` (SQLite `Insert`/`Where` хот-путь), здесь — только структурный вывод.

### 🟡 Находка 173 (P2, потокобезопасность/мёртвый код) — `InterceptorHooks.Empty` — общий статический экземпляр с 0 ссылок: мёртвый код с латентной мутацией shared-state

**Место.** `DataContextDependencies.cs:57` (`internal static readonly InterceptorHooks Empty = new([], []);`), `Add` — `:63` и `:74`.

**Что не так.** `roslyn refs NextORM.Core.InterceptorHooks.Empty` — **0 ссылок** (мёртвый мембер). При этом `Empty` — единственный **общий** экземпляр: `Add` мутирует `_query`/`_connection` через `Volatile.Write` на самом `this`. `DataContext` сейчас всегда создаёт `new InterceptorHooks(...)` (`DataContext.cs:61`), поэтому дефект не проявляется. Но если `Empty` когда-нибудь станет дефолтом (как предполагал RFC-скетч `todo_interceptors.md:123`), то `ctx.AddInterceptor(...)` одной контекста заменит массив во **всех** контекстах, использующих `Empty`, — протечка интерцепторов между контекстами, ровно тот класс ошибки, от которого защищает COW.

**Стало (маршрут `nextorm-design-engineer`).** Удалить `Empty` (0 ссылок) — при необходимости дефолта возвращать новый экземпляр либо сделать `Add` не мутирующим (возвращать новый `InterceptorHooks`). Сам COW-механизм корректен (см. ℹ️ A), правка — только про мёртвый shared-статик.

### ℹ️ Наблюдения (фикс не требуется)

- **A. Copy-on-write — корректен.** `Volatile.Read(ref _query)` на чтении (`:59,61`), запись под `lock (_sync)` с публикацией `Volatile.Write` (`:63-72,74-83`): читатель видит либо старый, либо новый массив, порванного состояния нет; `Add` — не горячий путь (редкое расширение снимка). Явных гонок нет.
- **B. Исключения — без проглатывания.** Везде `throw;` (не `throw ex;`) — стек сохраняется (`QueryExecutor.cs:106,126,146,166,186,206`; `ResultSetEnumerator.cs:225,255`). `catch (Exception)` используется только чтобы уведомить и перебросить; `CA1031` не включён (build `0/0`), пустых/логирующих `catch` нет. Не оговорено контрактом (наблюдение, не дефект по текущему RFC): (1) бросок интерцептора в `CommandExecuting` (`QueryExecutor.cs:68`) прерывает вызов до execute и `CommandFailed` **не** поднимается; (2) бросок в `EndCommand`/`CommandExecuted` у executor'а (внутри `try`) приводит к `CommandFailed` + перебросу, а у enumerator'а цикл `CommandExecuted` стоит **вне** `try` (`:228-230,258-260`) — `CommandFailed` не будет; (3) если бросает сам `FailCommand` (`CommandFailed`), исключение провайдера маскируется (строка `throw;` недостижима). Имеет смысл либо задокументировать, либо изолировать броски интерцепторов.
- **C. `CommandInitialized`: план — раз на miss, мутация — каждый раз.** `QueryPlanner.cs:428,555` поднимает событие только в ветке cache-miss (`planCache is null`), т.е. для плановой команды — один раз; `QueryExecutor.CreateMutationCommand` (`:260-278`, вызов `:274`) поднимает `CommandInitialized` на **каждое** исполнение мутации (команда не переиспользуется). Публичный док говорит «Called once … for a query plan» (`IQueryInterceptor.cs:27-32`) — расхождение, сторона API-NAMING, INT2. Следствие для per-instance: интерцептор, добавленный в контекст, переиспользующий чужой кэшированный план, увидит `CommandExecuting`/`Executed`, но не `CommandInitialized` (план создан другим контекстом) — это корректно по смыслу «команда создана», но не описано.
- **D. `IDisposable` — без изменений.** Новых полей-`IDisposable` нет; `ResultSetEnumerator.Dispose(bool)` с пустым `if (disposing) { }` — пре-существующий (Находка 2/68), не переоткрывается.
- **E. God-класс не оправдывает отдельной находки.** `QueryExecutor.cs` уже был >500 строк (533); +212 строк фичи — почти целиком копипаста обёрток, снимаемая Находкой 171. Находка 68 (god-классы) не переоткрывается.
- **F. Тесты — 3/3 core + 9/9 sqlite.** Покрыто: порядок `initialized → executing → executed`, видимость привязанных параметров (`Parameters[0].Value == 43`), порядок регистрации 2 интерцепторов, per-instance `AddInterceptor`, `executing → failed` при ошибке провайдера (`SqliteException`), streaming async, `opening → opened` sync+async один раз, исполнение при 0 интерцепторов. **Не покрыто:** cache-hit не поднимает `CommandInitialized` повторно; `DataContext.AddInterceptor(IConnectionInterceptor)` (per-instance протестирован только query-overload); семантика броска самого интерцептора (ℹ️ B); отсутствие аллокаций на пустом пути (нет ассерта/бенчмарка — Находка 172).
- **G. Доки/Шаг 5.** Публичной прозы EN+RU по интерцепторам нет (`rg -l "Interceptor|интерцеп" docs -g '!docs/specs/**'` — **0**; `docs/guide/19-insert-statement.md` — посторонний), `find -name 'PublicAPI*.txt'` — **0**. Сторона API-NAMING, INT3/INT4.
- **H. EOL/анализ.** Все **11** файлов фичи (2 новых `src`, 7 правленых `src`, 2 теста) — **CRLF**. `.editorconfig` не менялся: те же 7 `dotnet_diagnostic.*.severity = silent` — 6 инертных `S*` без `SonarAnalyzer` + `CA2254` (пре-существующее, не переоткрывается). `QueryExecutor.cs:242` (`SuppressMessage` S2583) — пре-существующий, не в диффе фичи.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; тесты: `nextorm.core.tests -class nextorm.core.tests.InterceptorBuilderTests` — **3/3**, `nextorm.sqlite.tests -class nextorm.sqlite.tests.InterceptorTests` — **9/9** (0 failed/skipped); `roslyn refs NextORM.Core.InterceptorHooks.Empty` — **0** (Находка 173); подавления `src/` — **11/11** (в фиче 0); `Skip=`/`[Ignore]`/пустых `catch` — 0, новых `Task.Delay` — 0; EOL 11/11 CRLF; `find -name 'PublicAPI*.txt'` — **0**; `slopwatch` локально не установлен (`.config/dotnet-tools.json` — coverage/reportgenerator/docfx), скан паттернов вручную.

**Итог.** 🔴 — **0**; 🟡 **Находки 171–173** (все P2: DRY-дублирование, аллокация async-обёрток, мёртвый shared-статик; маршрут `nextorm-design-engineer`; замер Находки 172 — `nextorm-db-perf-analyst`); остальное — наблюдения. Публичная сторона — `API-NAMING-REVIEW.md`, INT1–INT4.

---

## 🔎 Точечный аудит 24.09.2026 — follow-up колонок-длительностей: DIM `ISqlDialect.MakeNullableDurationType` (коммит `18ed028`) и фикс `make_interval` (`EmitMakeInterval`, uncommitted working tree); 🔴 — 0, 🟡 — 1 (Находка 174, ✅ исправлено этим изменением), ℹ️ — наблюдения A–F

**Область.** (1) `MakeNullableDurationType` — уже в HEAD `18ed028` («#68 regex implementation», смешанный): DIM `ISqlDialect.cs:338` (`=> MakeDurationType(unit, precision)`), `public virtual` `SqlDialectBase.cs:110`, override `ClickHouseDialect.cs:671` (`Nullable(...)`). (2) Uncommitted: `Visitors/ExtendedScalarFunctionTranslator.cs` — `make_interval` убран из `DirectFunctions` (`:38`) и разобран новым `EmitMakeInterval` (`:102-107`, `:170-201`). (3) Тесты: `CommonTestSuite.Duration.cs` (untracked) — nullable-колонка DDL через `MakeNullableDurationType` (`:54`, `:58`); dialect-ассерты ×6; PG SQL-gen `MakeIntervalAndJustify_ShouldEmit` (`SqlGenerationTests.cs:2867`); PG-интеграция `IntervalFunctions_ShouldMaterialiseTimeSpan` (`PostgresFunctionsTests.cs:462`). Публичная сторона — `API-NAMING-REVIEW.md`, ND1/ND2/MI1/MI2.

### ✅ Находка 174 (P2, поведенческая корректность; **исправлено этим изменением**) — `make_interval` рендерился позиционно без PG-параметра `weeks`: аргументы после `months` сдвинуты, секунды терялись

**Было.** `DirectFunctions` → `SqlOperandTranslator.EmitFunction` → `make_interval(a0..a5)`. PG читает позиционно 7 параметров `(years, months, weeks, days, hours, mins, secs)`, а C#-сигнатура `PostgresFunctions.make_interval` — 6 без `weeks`. Для `make_interval(0, 0, 1, 2, 3, 4.0)` (замысел «1 день 2:03:04») PG получал `weeks=1, days=2, hours=3, mins=4` → `7 days 02:03:00`; тихий неверный результат на всей PG-поверхности `make_interval`.

**Стало.** `EmitMakeInterval` вставляет литерал `0` в позицию `weeks` (`:191-192`): `make_interval(y, mo, 0, d, h, mi, s)`; C#-сигнатура не менялась (non-breaking). Param-mode — как у соседнего `EmitPgTypeOf` (`Visit` по аргументам, без SQL). **Проверка.** `SqlGenerationTests.MakeIntervalAndJustify_ShouldEmit` → `make_interval(0, 1, 0, 2, 3, 4, 5)` — **1/1** (локальный прогон); PG-интеграция `IntervalFunctions_ShouldMaterialiseTimeSpan` (набор изменён, прогон `--no-build` — по evidence рабочего tree).

### ℹ️ Наблюдения (фикс не требуется)

- **A. `MakeNullableDurationType` — аддитивен, но test-only.** Новый член не ломает ни внешних реализаторов `ISqlDialect`, ни наследников `SqlDialectBase` (DIM + `virtual`, дефолт не бросает). Но продакшен-вызовов — **0**: `roslyn refs ISqlDialect.MakeNullableDurationType` — **7** ссылок, все в тестах (`ClickHouseDialectTests:20`, `CommonTestSuite.Duration.cs:54`, MariaDb/MySql/Postgres/Sqlite/SqlServer DialectTests); тип колонки по-прежнему даёт `MakeTypeName(Type)`. Это **та же** мёртвая поверхность, что Находка 164 (не переоткрывается): ClickHouse-фикс `Nullable(Int64)` доказан только вручную собранным DDL теста (`CommonTestSuite.Duration.cs:58`), а не продакшен-рендером. Публичная сторона — ND2.
- **B. `EmitMakeInterval` — корректен, но позиция `weeks` захардкожена.** `if (i == 2)` (`:191`) — магический индекс внешнего (PG) параметра, гварда `args.Count == 6` нет. Пока C#-сигнатура 6-параметровая, всё верно; добавление `weeks` в C#-поверхность даст 8 позиций без падения. Харднинг (не блокер): `const int WeeksPosition = 2;` + `Debug.Assert(args.Count == 6)`.
- **C. Подавления — 11/11, новых 0.** `src/` — те же 6 `SuppressMessage` (с `Justification`) + 5 парных `#pragma`; в файлах изменения `SuppressMessage`/`#pragma`/`NoWarn`/`Skip=`/пустых `catch`/`Task.Delay` — **0**. `.editorconfig` не менялся: 7 `dotnet_diagnostic.*.severity = silent` (6 инертных `S*` + `CA2254`) — пре-существующее.
- **D. IDisposable/LINQ/god — чисто.** Новых `IDisposable`/полей/`using` нет; `EmitMakeInterval` — обычный `for`, LINQ на пути нет; `ExtendedScalarFunctionTranslator.cs` — 285 строк (не god-класс).
- **E. Кэш-ключи не затронуты.** `make_interval` меняет только текст SQL (поведение), состав ключа плана не менялся; `MakeNullableDurationType` в ключи не входит. `DurationPrecision` в ключи не входит — безопасно, пока Находка 164 стоит.
- **F. EOL — чисто.** Все 7 проверенных файлов изменения (4 `src`, 3 теста) — **CRLF** (100 %); Находка 166 (LF у новых файлов) к этому изменению не относится.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `dotnet test tests/nextorm.postgres.tests -c Release --no-build --filter "FullyQualifiedName~MakeIntervalAndJustify"` — **1/1** (800 ms); `roslyn refs ISqlDialect.MakeNullableDurationType` — 7, все в тестах (наблюдение A); `grep -rn "SuppressMessage|#pragma warning disable" src/` — **11**; slop-паттерны в файлах изменения — 0; EOL 7/7 CRLF; `find -name 'PublicAPI*.txt'` — **0**. `slopwatch` локально не установлен — скан паттернов вручную. Источник `MakeNullableDurationType` — коммит `18ed028`; фикс `make_interval` — uncommitted.

**Итог.** 🔴 — **0**; ✅ **Находка 174** (P2, `make_interval`-сдвиг; исправлено, подтверждено тестом); новых открытых 🟡 нет; мёртвая публичная поверхность — пре-существующие 164/DU2, не переоткрывается. Публичная сторона — `API-NAMING-REVIEW.md`, ND1/ND2/MI1/MI2.
