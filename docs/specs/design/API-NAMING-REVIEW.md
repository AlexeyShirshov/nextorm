...
**Предрелизный аудит v1.0.6-alpha (25.09.2026, HEAD `9e82392` + release-prep uncommitted: bump `Version` 1.0.5→1.0.6-alpha, `docs/index.md`, xref-правки в 6 файлах доков, stale LastIndexOf-ожидания в 4 тест-проектах).** **Открытых P0/P1 по публичному API нет; alpha-релиз не блокируется.** P0 по именам — нет; P1 по именам — нет; P1-доковые находки прошлых срезов (**CHJ1**, **CHCB1**, **CHARR2**, **HOAF2**, **TR1**, **TR5**, **CTAS5**, **BK1**) закрыты. Поверхность v1.0.6 (issues #28 коллация, #30 интерцепторы, #68 regex, #71 string semantics, #72 Duration, #75 SQL batch, #77 `SqlFunctions.Sql`, #78–#83 провайдерные пробелы, плюс батч/temp-table-билдеры и гайд 29 «Optimistic concurrency») прошла по-фично; все выявленные несогласованности — **P2** (трекинг Шага 5 и форма/доки). BCL-конфликтов имён (CA1716/CA1724/CA1002/CA1051) на новых публичных типах нет. XML-doc — 100 % (build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — 0). Uncommitted xref-правки в 6 файлах доков (`IScalarFunctions.Supports(System.String)`, `MakeDurationType(System.Nullable{NextORM.Core.DurationUnit},System.Int32)`) корректны и лечат разрешение cref; новые страницы EN+RU (`guide/26`–`guide/29`) присутствуют в обеих ветках. Шаг 5 — **P2 открыт**: `PublicAPI.Shipped/Unshipped.txt` — **0**, ApiCompat/API-approval-теста нет; релиз (alpha) не блокирует.

**Предрелизный аудит v1.0.7-beta (25.09.2026, HEAD `da5caad` + release-prep uncommitted).** **Открытых P0/P1 по публичному API нет; beta-релиз не блокируется.** P0 по именам — нет; P1 по именам — нет; P1-доковые находки прошлых срезов закрыты. XML-doc — **100 %** (build Release `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — 0) ⇒ новых публичных членов без документации нет. Релизная дельта (`v1.0.6-alpha..da5caad`; issues #15/#31/#63/#64/#66/#85–#90) добавила **14** публичных типов (DML-output `OutputIntoBuilder`; range `Range<T>`/`RangeColumnsAttribute`/`RangeColumnsMetadata`; converters/JSON `IPropertyValueConverter`/`ValueConverter<TModel,TProvider>`/`ValueConverterAttribute`/`EnumToStringConverter<TEnum>`/`JsonColumnAttribute`/`JsonColumnOptions`/`JsonColumnConverter<TModel,TProvider>`/`JsonColumnStorage`; `TableFunctionSchema`; `TempTableSource<TResult>`) и новые члены `PostgresFunctions`/`CommonFunctions`/`ISqlDialect`/`EntityPropertyBuilder<T>`/`QueryCommand<TResult>`; BCL-конфликтов/аббревиатур на новых именах нет (`Range<T>` — generic, с `System.Range` не сталкивается; `IPropertyValueConverter`/`JsonColumnConverter<,>` разведены с `System.Windows.Data.IValueConverter`/`System.Text.Json.Serialization.JsonConverter<T>`; SQL-токены в lower_snake — принятая конвенция P1-10). Новых P0/P1/P2 по именам этим проходом не выявлено; открытые P2 — только ранее зарегистрированные (актуальность — по разделам ниже): **Шаг 5** (`PublicAPI.*.txt` — **0**), **RM1**, **VC1/VC3/VC4**, **RPC1/RPC2**, **PSW2**, **TT4–TT7**, **CTAS2/CTAS8/CTAS9**, **RX-SS1**, **DRS1**, **PG81-T1**, **PA1** и производные; `PublicApiAnalyzers`/`EnablePackageValidation`/ApiCompat/API-approval-теста нет. Release-prep xref-правка `docs/providers/postgres.md` + RU (`NextORM.Core.SqlFunctions.PostgresFunctions` → `NextORM.Core.PostgresFunctions`) **корректна**: `roslyn types PostgresFunctions` → `NextORM.Core.PostgresFunctions` (top-level, `Query/SqlFunctions.Postgres.cs:12`); прочие правки — `docs/index.md` (главы 30/31, Status/Roadmap/Releases) и `readme.md` (`Alpha`→`Beta`). ℹ️ — план релиза `docs/specs/release-1.0.7-b.md` LF-only (EOL-гигиена; кодовая сторона — `code-smells-review.md`, наблюдение A); `CreateTableOptions? options = null` на 16 членах — открытая P2 Находка 188/TT5 (кодовая сторона — `code-smells-review.md`, наблюдение B).
...
**Обновление 22.09.2026 (SQLite-поверхность `SqlFunctions.Sqlite`, issue #82).** Добавлены публичный тип `SqliteFunctions : CommonFunctions` (`src/nextorm.core/Query/SqlFunctions.Sqlite.cs`), маркер `SqlFunctions.Sqlite`, вложенные формы строк `SqlFunctions.IJsonEachRow`/`SqlFunctions.IJsonTreeRow`, capability-интерфейс `ISqliteFunctions` (+ `SqlDialectBase.SqliteFunctions`/`ISqlDialect.SqliteFunctions`), транслятор `SqliteFunctionTranslator` и SQLite-рендерер `SqliteFunctionRenderer` (`src/nextorm.sqlite/SqliteFunctionRenderer.cs`). Новый публичный тип задокументирован XML-`<summary>` у каждого члена (CS1591); `dotnet build nextorm.slnx -c Release` — 0/0. P0/P1 по **именам** нет: имена зеркалят SQL-токены (согласованный стиль P1-10), ключевые слова объявлены как `@typeof`/`@if`/`@char`. Открыт P2 **SQLF1** (трекинг `PublicAPI.Unshipped.txt`, Шаг 5). Доки EN+RU: `guide/provider-specific/sqlite.md` (+toc), `providers/sqlite.md`.
...
**Обновление 25.09.2026 (uncommitted worktree — command timeout per-context/per-query, issue #93).** Добавлены публичные члены: `DataContextBuilder.UseCommandTimeout(int) -> DataContextBuilder` и `DataContextBuilder.CommandTimeout.get -> int?`; DIM `IContextEnvironment.CommandTimeout.get -> int?` (default `null`); `DataContext.CommandTimeout.get -> int?`; `EntityBuilder.WithCommandTimeout(int) -> EntityBuilder`, `EntityBuilder<TEntity>.WithCommandTimeout(int) -> EntityBuilder<TEntity>`, `QueryCommand<TResult>.WithCommandTimeout(int) -> QueryCommand<TResult>`; `QueryCommand.CommandTimeout.get -> int?` (setter `internal`); `internal QueryCommand.ResolvedCommandTimeout`. Новых публичных **типов** нет → Приложение A без изменений. P0/P1 по **именам** нет (`UseCommandTimeout`/`WithCommandTimeout` зеркалят `UseQuotedIdentifiers`/`WithQuotedIdentifiers` и linq2db; значение — `int` секунд, как `BulkInsertOptions.TimeoutSeconds`). DIM-дефолт `null` не ломает внешних реализаторов `IContextEnvironment`. Открыт P2 **CTO1** (трекинг `PublicAPI.*.txt`, Шаг 5). XML-doc 100 %, build Release `0/0`. Доки EN+RU: `guide/16-connections-and-logging.md` §«Command timeout», `advanced/api-reference.md`, `advanced/limitations.md`; gap-analysis §4 п.52 → shipped; план `todo_command_timeout.md` — раздел «Статус реализации».

**Обновление 26.09.2026 (uncommitted worktree — динамические колонки, issue #94, read side).** Добавлены новый публичный тип `DynamicColumnsAttribute` (`src/nextorm.core/DynamicColumnsAttribute.cs`), публичный метод `EntityPropertyBuilder<T>.DynamicColumnsStore()` (`src/nextorm.core/DataContext/Meta/EntityPropertyBuilder.cs`), два default-члена интерфейсов `IPropertyMetadata.IsDynamicColumnsStore`/`IEntityMetadata.DynamicColumnsStore` (`src/nextorm.core/DataContext/Meta/{IPropertyMetadata,IEntityMetadata}.cs`), внутренние слоты `PropertyMetadata.IsDynamicColumnsStore`/`EntityMetadata.DynamicColumnsStore`, маркер `SelectExpression.IsDynamicColumnsStore`/`PhysicalColumnName` и `internal`-хелпер `DynamicColumns`. Сущности без хранилища не затронуты: select-list/план/материализация для них байт-в-байт прежние; при хранилище `ColumnsPlanHash` учитывает маркер через `SelectExpressionPlanEqualityComparer`, а SQL рендерит `select <mapped>, *`. P0/P1 по **именам** нет (`DynamicColumnsAttribute` зеркалит `ColumnAttribute`; `DynamicColumnsStore()` — fluent-двойник `JsonColumn`; `IsDynamicColumnsStore` — булев флаг в ряду `IsKey`/`IsIdentity`/`IsComputed`). Открыты P2: **DC1** (трекинг `PublicAPI.*.txt`, Шаг 5) и **DC2** (write-side отложен, `docs/advanced/limitations.md`). XML-doc 100 %, build Release `0/0`. Доки EN+RU: `guide/32-dynamic-columns.md` (+`guide/toc.yml`/`ru/toc.yml`), `advanced/api-reference.md` (+RU), `advanced/limitations.md` (+RU); `specs/comparison/linq2db-backlog-gap-analysis.md` §4 → Partial. Тесты: core 402, sqlite 516, postgres 522, sqlserver 389, mysql 169, mariadb 94, clickhouse 339 — все зелёные.

**Обновление 26.09.2026 (uncommitted worktree — варианты хинтов: join/subquery/tables-in-scope, issue #96).** Добавлены **3** публичных метода `EntityBuilder<TEntity>` (`src/nextorm.core/Builders/EntityBuilder.cs`): `WithJoinHint(string)`, `WithSubQueryHint(string)`, `WithTablesInScopeHint(params string[])`. Новых публичных **типов** нет; добавлены члены `ISqlDialect`/`SqlDialectBase`: `SupportsJoinHints`, `SupportsSubQueryHints`, `SupportsTablesInScopeHints`, `SupportsInlineHints`, перегрузка `MakeJoinKeyword(JoinType, JoinStrictness, bool, string?, KeywordCase)` и `MakeTablesInScopeHints(IReadOnlyList<string>, KeywordCase)`; слоты `JoinExpression.JoinHint`, `FromExpression.SubQueryHint` (внутренний), `QueryCommand.TablesInScopeHints`. Имена следуют конвенции `With*` (`WithTableHint`/`WithIndex`) и аналогам linq2db; P0/P1 по **именам** нет. SQL Server рендерит join-хинт внутри `JOIN` (`inner loop join`) и tables-in-scope как `WITH (...)`; PostgreSQL/MySQL/MariaDB сворачивают все три в `/*+ ... */`; SQLite/ClickHouse/in-memory отклоняют `NotSupportedException`. Открыт P2 **HV1** (трекинг `PublicAPI.*.txt`, Шаг 5). XML-doc 100 %, build Release `0/0`. Доки EN+RU: `guide/17-query-hints.md`, `providers/overview.md`, `advanced/limitations.md`, `advanced/api-reference.md`; план — `specs/roadmap/todo_hint_variants.md` §«Статус реализации».
...
| `docs/scalar-functions/index.md:675`, `docs/ru/scalar-functions/index.md:691` | матрица: SQLite — `NotSupportedException` | `max(a, b)` / `min(a, b)`, одноаргументная форма — `(a)` |
| `docs/scalar-functions/index.md:476`, `docs/ru/scalar-functions/index.md:485` | список провайдеров без SQLite | добавить SQLite |
| — | `[ ]` | `[x]` |
...
только в `scalar-functions/index.md`.
...
| Файл:строка | Сейчас | Должно быть |
|---|---|---|
| `docs/scalar-functions/index.md:202-204`, `docs/ru/scalar-functions/index.md:207-209` | Перечислены `.Year`/`.Month`/`.Day`/`.Hour`/`.Minute`/`.Second`, `.DayOfYear` отсутствует | Добавить `.DayOfYear` и `doy`-рендер (SQL Server `datepart(dayofyear, …)` после фикса) |
...
| J4 | P2 | `docs/scalar-functions/index.md:28`, `docs/ru/scalar-functions/index.md:30` | Обзор ClickHouse-поверхности: «`arg_min`/`arg_max` и `-If`» — нет `JSONExtract*`/`JSONHas` | Добавить строковый JSON (оба зеркала) |
...
по `docs/providers/clickhouse.md`, `docs/scalar-functions/index.md`, `docs/advanced/api-reference.md`
...
| `docs/scalar-functions/index.md:24-29`, `docs/ru/scalar-functions/index.md:24-30` | ClickHouse = «`arg_min`/`arg_max` и `-If`» | Добавить строковый JSON; при желании — секция и строка матрицы (`:692`/`:708`) |
...
| V2 | P2 | `docs/scalar-functions/index.md:28-29`, `docs/ru/scalar-functions/index.md:31` | Обзор ClickHouse-поверхности: «…and the string-JSON `JSONExtract*` family» — быстрый разбор плоского JSON не упомянут (файл затронут J4-дополнением) | Добавить `visitParamExtract*` (либо сослаться на `docs/guide/18-json.md:22-25`) в оба зеркала |
...
| Файл:строка | Сейчас | Должно быть |
|---|---|---|
| `docs/scalar-functions/index.md:28-29`, `docs/ru/scalar-functions/index.md:30-31` | Обзор ClickHouse-поверхности: «`arg_min`/`arg_max`, the `-If` combinator, the string-JSON … fast path» — функции словарей отсутствуют | Добавить `dict_get`/`dict_get_or_default`/`dict_has` в оба зеркала |
...
| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| UG1 | P2 | `SqlFunctions.cs:225,233`; `docs/scalar-functions/index.md:277-284` (EN+RU) | Асимметрия имён: v4 сохранил PostgreSQL-написание `gen_random_uuid`, v7 — версионное `uuidv7`. Теперь семейство кросс-провайдерное, и PG-имя утекает в вызовы SQL Server/MariaDB/ClickHouse (`SqlFunctions.Sql.gen_random_uuid()` → `newid()`/`uuid_v4()`/`generateUUIDv4()`). Имя унаследовано от `PostgresFunctions` (не регресс), а snake_case-зеркало уже принято в §3 | При заморозке Шага 5 рассмотреть симметричную пару `uuidv4`/`uuidv7` (alpha-политика — замена на месте: код, тесты, XML-доки, `docs/**` + `docs/ru/**`). Не блокирует |
...
(`docs/scalar-functions/index.md:260-272` и RU, `docs/providers/*.md`,
...
| I5 | P2 | `docs/scalar-functions/index.md:496`, `docs/ru/scalar-functions/index.md:505` | Раздел «Conditional helpers»/«Условные функции» (только `nullif`/`greatest`/`least`/`num_*`) не упоминает новое `iif`/`choose`; обновлены лишь `providers/sqlserver.md` и EN api-reference | Добавить `iif`/`choose` + `SupportsIifChoose` в оба зеркала |
...
`docs/scalar-functions/index.md`, `docs/guide/10-window-functions.md`,
...
| I5 | ✅ закрыта | `docs/scalar-functions/index.md:556,561-563`; RU `:564,570-572` |
...
| RD4 | P2 | `docs/advanced/api-reference.md:59` + `docs/ru/advanced/api-reference.md:59` | Строка `CommonFunctions` перечисляет `date_trunc`/`date_add`/`date_diff`/`date_from_parts`/`end_of_month`, но не новые `extract`/`date_part` (EN и RU синхронно). Гайд `docs/scalar-functions/index.md` (+RU) и `docs/providers/postgres.md` (+RU) обновлены — расхождения класса QM5 нет | Дополнить перечисление `extract`/`date_part` в обеих ветках |
...
**7/7**; `rg --files -g 'PublicAPI*.txt'` — пусто (подтверждает X1); EN+RU `docs/scalar-functions/index.md`,
...
**P2-3 — пользовательские доки EN+RU не обновлены.** `docs/scalar-functions/index.md`,
`docs/guide/provider-specific/sqlserver.md`, `docs/providers/sqlserver.md`, `docs/advanced/api-reference.md`,
...
- **Generic `TRow` вместо именованного row-интерфейса — обосновано.** У `url`/`s3`/`file` схема задаётся
  ...
  приём. `format`/`merge`/`input` оставлены на динамическую схему ([Dynamic result schema](../../guide/13-table-valued-functions.md#dynamic-result-schema)).
...
| HOAF2 ✅ закрыта 22.09.2026 | **P1 (док)** | `docs/guide/provider-specific/clickhouse.md:197-201` (+`docs/ru/guide/provider-specific/clickhouse.md:199-204`) | EN/RU прямо противоречили реализации («higher-order array functions … are out of scope today») | Снято: пункт убран из «Not yet supported»/«Пока не поддерживается»; флаг + 9 методов описаны в `scalar-functions/index.md` (EN+RU) |
| HOAF3 ✅ закрыта 22.09.2026 | P2 | `docs/scalar-functions/index.md` (+RU); `docs/advanced/api-reference.md` (+RU); `docs/specs/roadmap/sql-capabilities-gap-analysis.md` §4 п.5 | Новые 9 методов и флаг не были добавлены в доки EN+RU; §4 п.5 гласил «has not been started» | Таблица guide/11, список api-reference и §4 п.5 (EN+RU) дополнены; comparison-спеки обновлены |
...
**Область (uncommitted worktree).** Поверхность расширена; актуализация 25.09.2026: добавлены configure-перегрузки и `CreateTableOptionsBuilder`, `CreateTableOptions` получил `DropExisting`, тип `CreateTableAsOptions` переименован в `CreateTableOptions` (файл `Builders/CreateTableOptions.cs`). Новое (к заморозке Шага 5):

- **`NextORM.Core.TempTableExtensions`** (`src/nextorm.core/Builders/TempTableExtensions.cs`, `public static class`) — **29** публичных extension-методов: 15 базовых (6 имён × 2 приёмника + `AsTempTable`×2 + `ToBatchSql`) и **14** configure-перегрузок `Func<CreateTableOptionsBuilder, CreateTableOptionsBuilder>` (25.09.2026). Актуализированные номера строк:
  - `ToTempTable<TResult>(this EntityBuilder<TResult>, string, CreateTableOptions? = null) -> void` `:30`; configure `:43`; на `QueryCommand<TResult>` — opts `:55` / configure `:65`;
  - `ToTempTableAsync<TResult>(this EntityBuilder<TResult>, string, CreateTableOptions? = null, CancellationToken = default) -> Task` `:76`; configure `:91`; на `QueryCommand<TResult>` — opts `:105` / configure `:117`;
  - `ToTempTableSql<TResult>(this EntityBuilder<TResult>, string, CreateTableOptions? = null) -> string` `:127`; configure `:141`; на `QueryCommand<TResult>` — opts `:154` / configure `:165`;
  - `ToTable<TResult>(this EntityBuilder<TResult>, string, CreateTableOptions? = null) -> void` `:174`; configure `:187`; на `QueryCommand<TResult>` — opts `:199` / configure `:209`;
  - `ToTableAsync<TResult>(this EntityBuilder<TResult>, string, CreateTableOptions? = null, CancellationToken = default) -> Task` `:220`; configure `:235`; на `QueryCommand<TResult>` — opts `:249` / configure `:261`;
  - `ToTableSql<TResult>(this EntityBuilder<TResult>, string, CreateTableOptions? = null) -> string` `:271`; configure `:285`; на `QueryCommand<TResult>` — opts `:298` / configure `:309`;
  - `AsTempTable<TResult>(this QueryCommand<TResult>, CreateTableOptions? = null) -> TempTableSource<TResult>` `:322`; configure `:338`; на `EntityBuilder<TResult>` — opts `:355` / configure `:371`; `ToBatchSql<TResult>(this QueryCommand<TResult>) -> string` `:389` (ленивый источник — аудит 25.09.2026, TT4–TT7).
  - **8 name-less** `ToTempTable()`/`ToTempTableAsync()` (аудит 24.09.2026, TT1) в текущей поверхности **отсутствуют** — заменены configure-перегрузками (TT1 в этой части **superseded**).
- **`NextORM.Core.CreateTableOptions`** (`src/nextorm.core/Builders/CreateTableOptions.cs:22`, `public sealed record`; переименован из `CreateTableAsOptions`, файл `CreateTableAsOptions.cs` удалён) — **5** `init`-свойств: `IfNotExists` `:25`, **`DropExisting`** (новое) `:33`, `WithData` (default `true`) `:39`, `OnCommit` (тип `TempTableOnCommit`, default `PreserveRows`) `:46`, `Columns` (`IReadOnlyList<string>?`) `:52`; плюс сгенерированные record-члены (public ctor, `Equals`/`GetHashCode`/`ToString`) и internal `Validate(bool temporary)` `:58` / `ValidateCombination()` `:69`.
- **`NextORM.Core.CreateTableOptionsBuilder`** (`:82`, `public sealed class`, новое 25.09.2026) — **6** публичных методов: `IfNotExists(bool = true)` `:93`, `DropExisting(bool = true)` `:106`, `WithData(bool = true)` `:115`, `OnCommit(TempTableOnCommit)` `:124`, `Columns(params string[])` `:135`, `Build() -> CreateTableOptions` `:156`; internal static `Build(Func<CreateTableOptionsBuilder, CreateTableOptionsBuilder>)` `:175`. По образцу существующего `BulkInsertOptionsBuilder`.
...
| CTAS3 | P2 (трекинг) | `PublicAPI.*.txt` отсутствуют; `TempTableExtensions.cs:30-389`; `CreateTableOptions.cs:22-69`; `CreateTableOptionsBuilder.cs:82-175` (тот же файл); `CreateTableAsClause.cs:15`; `TempTableOnCommit` (`CreateTableOptions.cs:7`); `ISqlDialect.cs:1477-1540`; `SqlDialectBase.cs:891-913`; `PostgresDialect.cs:397-441`; `SqliteDialect.cs:253`; `MySqlDialect.cs:418`; `ClickHouseDialect.cs:82` | Вся CTAS-поверхность (**5** типов: `TempTableExtensions`, `CreateTableOptions`, `CreateTableOptionsBuilder`, `TempTableOnCommit`, `CreateTableAsClause`; **29** extension-методов, **6** fluent-методов builder'а, **5** свойств options + 2 internal `Validate`; **4** DIM + `MakeCreateTableAsSelect`, **8** провайдерных override; record-сгенерированные члены) не трекается: `PublicApiAnalyzers`/ApiCompat/API-approval не подключены, `PublicAPI.*.txt` — **0** (Шаг 5 открыт). **Продолжение MRG2/MRG7/MRG10/UPD2/IDML2/DEL3**, не новая проблема | При заморозке внести подписи (точный текст — из анализатора): 5 типов + `TempTableExtensions` × 29 (15 базовых + 14 configure) + `CreateTableOptionsBuilder` (6) + `CreateTableOptions` (ctor/5 props/record-члены) + `CreateTableAsClause` (ctor + 6 props) + `TempTableOnCommit` (3 значения); DIM `SupportsCreateTableAsSelect*`/`MakeCreateTableAsSelect` + override'ы. Расширяет перечень MRG7, отдельной записи surface-lock не требуется |
| CTAS4 | P2 (именование) | `CreateTableOptions.cs:7` (тип) ↔ `:46` (свойство `OnCommit`) | **`TempTableOnCommit` дублирует имя свойства**: `options.OnCommit` имеет тип `TempTableOnCommit`, т. е. «OnCommit типа TempTableOnCommit»; члены `PreserveRows`/`DeleteRows`/`Drop` без контекста читаются как действия над таблицей, а не как значения `ON COMMIT` | До заморозки рассмотреть `OnCommitAction`/`TempTableOnCommitAction` (или свойство `OnCommitAction`). Alpha допускает rename; EN+RU доки + guide 22 в том же изменении |

ℹ️ **Наблюдения.** (1) **XML-doc 100 %:** build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` по `.csproj` — **0** (CS1591 не подавлен) ⇒ покрытие новых публичных членов полное. (2) **Публичные доки обновлены в этом же изменении — P1 нет:** новый `docs/guide/22-create-table-as.md` EN+RU (ToTempTable/ToTable, options-таблица, провайдеры, read-back через `From("name")`), `docs/guide/toc.yml` EN (`:44`) / `docs/ru/toc.yml` RU (`:60`), `docs/advanced/api-reference.md:49` EN+RU (строка `TempTableExtensions`), `docs/providers/overview.md:56` EN+RU (строка «Materialize a query»), `docs/advanced/limitations.md:15` EN+RU (CTAS снят из «not implemented», провайдеры перечислены). `rg 'ToTempTable|ToTable|CreateTableOptions|TempTableOnCommit' docs -g '!specs/**'` — совпадения EN+RU. (3) **12 перегрузок — принято:** dual-приёмник (builder + готовый `QueryCommand<TResult>`) повторяет ось других терминалов (`Delete`/`DeleteAsync`/`ToSql` на билдере и joined-билдере); сокращение поверхности не рекомендуется. (4) **`Temporary` вне `CreateTableOptions` — принято:** две терминальные ветки дают явную интенцию и не оставляют молчаливого флага; цена — имя `CreateTableOptions` (CTAS1). (5) **`CreateTableAsClause` публичен — необходимо и согласовано:** параметр публичного члена публичного `ISqlDialect` обязан быть публичным; `internal`-вариант унёс бы хук с интерфейса (ломает паттерн всех `Make*`); публичные `*Clause` в сигнатурах диалекта уже есть (`TemporalClause`, `ForJsonClause`, `ForXmlClause`). (6) **`OnCommit` только с `ToTempTable`, проверка на этапе сборки SQL — принято:** согласовано с контрактом «опция не поддержана ⇒ `NotSupportedException` при рендере» (флаги колонок/`WITH NO DATA`); ошибка ловится до БД (`ToTableSql` тоже), текст actionable; `PreserveRows` — дефолт и молчаливая no-op-опция. (7) **`record`-равенство о reference-член `Columns`** — кодовая сторона (`code-smells-review.md`, Находка 128). (8) **CTE-источник** рендерится невалидно — кодовая сторона (🔴 Находка 126), не API-находка. (9) 25.09.2026: `CreateTableAsOptions` → `CreateTableOptions` и `BatchBuilder.CreateTableAs`/`CreateTempTableAs` → `CreateTable`/`CreateTempTable`; поверхность uncommitted; `docs/specs/**` — internal-план, не публичная поверхность.
...
### Аудит 24.09.2026 — SQL-батч: `BatchExtensions.Batch`, `BatchBuilder`, `BatchQuery<TResult>`, `ToTempTableThen`/`ToTableThen`, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — 4: BAT1–BAT4)

**Область (uncommitted).** Новые **3** публичных типа: `NextORM.Core.BatchExtensions` (`src/nextorm.core/Builders/BatchBuilder.cs:27`; extension `Batch(IDataContext) -> BatchBuilder` `:33`), `NextORM.Core.BatchBuilder` (`:50`, `sealed`, ctor `internal`; **4** public-метода — `Add(QueryCommand) -> BatchBuilder` `:60`, `CreateTableAs(string, QueryCommand, CreateTableOptions?) -> BatchBuilder` `:75`, `CreateTempTableAs(...) -> BatchBuilder` `:87`, `Query<TResult>(QueryCommand<TResult>) -> BatchQuery<TResult>` `:96`), `NextORM.Core.BatchQuery<TResult>` (`:120`, `sealed`, ctor `internal`; **4** члена — `ToSql() -> string` `:134`, `ToList() -> List<TResult>` `:139`, `ToListAsync(CancellationToken = default) -> Task<List<TResult>>` `:149`, `AsAsyncEnumerable(CancellationToken = default) -> IAsyncEnumerable<TResult>` `:162`). Новые **4** публичных extension-метода на существующем `TempTableExtensions` (`Builders/TempTableExtensions.cs`): `ToTempTableThen<TSource,TResult>` `:175`, `ToTempTableThen<TResult>` `:192`, `ToTableThen<TSource,TResult>` `:210`, `ToTableThen<TResult>` `:227`. Новые **2** DIM `ISqlDialect` (`DataContext/Dialect/ISqlDialect.cs:1279,1289`): `SupportsBatch { get; } => false`, `BatchUsesJoinedCommand { get; } => false` + `SqlDialectBase`-двойники (`:791,793`) + override'ы PostgreSQL (`PostgresDialect.cs:34`), SQL Server (`SqlServerDialect.cs:46,49`), MySQL/MariaDB (`MySqlDialect.cs:48`), SQLite (`SqliteDialect.cs:292`); ClickHouse дефолт `false`. `internal` (в поверхность не входят): `IBatchExecutor`/`BatchStepSpec`/`BatchPlan`/`BatchStatement` (`DataContext/Roles/IBatchExecutor.cs`). Публичные доки EN+RU **не обновлены** — BAT3. Кодовая сторона — `code-smells-review.md`, Находки 175–178.

**P0 — нет.** Имена — PascalCase, без аббревиатур; BCL-конфликтов нет (`CA1716`/`CA1724`): `BatchExtensions`/`BatchBuilder`/`BatchQuery`/`Batch` в BCL отсутствуют. `Async`-суффикс только там, где есть синхронный близнец (`ToListAsync` ↔ `ToList`); `AsAsyncEnumerable` — см. BAT2. `CancellationToken` — последним/с дефолтом; `BatchQuery<TResult>.ToList() -> List<TResult>` **согласован** с существующей конвенцией (`QueryCommand<TResult>.ToList()` `Query/QueryCommand.TResult.cs:130`, `EntityBuilderExtensions.ToList` `:181`).

**P1 по именам — нет.** `SupportsBatch` следует `SupportsBulkCopy`/`SupportsInsertIgnore`; `BatchUsesJoinedCommand` — по образцу `CreateTableAsSelectUsesSelectInto` (факт-свойство, не `Supports*`). Оба DIM аддитивны/source-binary-совместимы для внешних реализаторов `ISqlDialect`/наследников `SqlDialectBase`.

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| BAT1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; `BatchBuilder.cs:27-168`; `TempTableExtensions.cs:175-231`; `ISqlDialect.cs:1279,1289`; `SqlDialectBase.cs:791,793`; 4 provider-override | Вся batch-поверхность (**3** типа, **8** членов, **4** extension, **2** DIM + **5** override) не трекается: `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). Продолжение **CTAS3/INT4/ND1/DU1/SS1/…**, не новая проблема | При заморозке внести подписи (точный текст — из анализатора); DIM — с дефолтами, `BatchBuilder`/`BatchQuery<TResult>` — с `internal`-ctor (в файле появится `internal`-конструктор, публичная поверхность — только методы) |
| BAT2 | P2 (нэйминг/консистентность) | `BatchQuery<TResult>.AsAsyncEnumerable` (`Builders/BatchBuilder.cs:162`) ↔ `EntityBuilderExtensions.ToAsyncEnumerable` (`:22,32`), `QueryCommand<TResult>.ToAsyncEnumerable` (`Query/QueryCommand.TResult.cs:115,121`) | Единственный потоковый терминал назван `AsAsyncEnumerable`, тогда как остальная публичная поверхность nextorm использует `ToAsyncEnumerable`. Имя заимствовано у EF Core/`System.Linq.Async`, но внутри проекта это расхождение (аналог CTAS2) | До заморозки решить: `ToAsyncEnumerable` (консистентность с nextorm) либо зафиксировать `AsAsyncEnumerable` как исключение в `<remarks>` `BatchQuery<TResult>`. Любое переименование — EN+RU доки |
| BAT3 | P2 (доки/охват) | `docs/guide/22-create-table-as.md` (+RU), `docs/guide/toc.yml` (+RU), `docs/advanced/api-reference.md` (+RU), `docs/providers/overview.md` (+RU), `docs/advanced/limitations.md` (+RU); `rg -l 'BatchBuilder\|ToTempTableThen\|ToTableThen\|\.Batch\(\)' docs -g '!specs/**'` — **0** | Публичная проза EN+RU не добавлена: ни `BatchExtensions.Batch`/`BatchBuilder`/`BatchQuery<TResult>`, ни `ToTempTableThen`/`ToTableThen`, ни оговорка про интерцепцию/логирование батча не упомянуты ни в одном публичном документе. RFC §9 требовал обновить guide 22, limitations и providers/overview EN+RU. `rg 'batch\|батч' docs/advanced/limitations.md` — 0 | Добавить EN+RU: `guide/22` §«Batch» (один round-trip/один backend под pgbouncer, `DbBatch` vs `;`-fallback, temp-таблица, ограничения интерцепции), строки в `toc`/`api-reference`/`providers/overview`, оговорку в `limitations`. Маршрут `nextorm-design-engineer`; ссылки на `docs/specs/**` из публичных доков запрещены (AGENTS.md) |
| BAT4 | P2 (контракт/тихий no-op; кодовая сторона — Находка 176) | `BatchBuilder.cs:96-101` | `Query<TResult>()` фиксирует `_steps.ToArray()`, XML-doc объявляет «it must be the last statement» (`:91`), но `Add`/`CreateTableAs`/`CreateTempTableAs` после него продолжают менять `_steps` и молча не влияют на уже возвращённый терминал | Либо бросать `InvalidOperationException`, либо явно задокументировать контракт; публичную форму можно не менять |

ℹ️ **Наблюдения.** (1) **P0/P1 по именам нет.** (2) **Extend-only:** 3 новых типа + 8 членов + 4 extension + 2 DIM — аддитивны; DIM не ломают внешних реализаторов; удалений/переименований/смены подписей нет; поверхность uncommitted/alpha. (3) **XML-doc 100 %:** build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` по `.csproj` — **0** (CS1591 гейтится) ⇒ у всех новых публичных типов/членов есть `<summary>`/`<param>`/`<typeparam>`/`<returns>`/`<exception>`, у `BatchExtensions` — `<example>`. (4) **`List<TResult>`-возврат — конвенция, не находка** (см. выше). (5) **`BatchUsesJoinedCommand`** — реализационный флаг драйвера на публичном `ISqlDialect`; принят по образцу `CreateTableAsSelectUsesSelectInto`. (6) **Дублирование приёмника** `ToTempTableThen`/`ToTableThen` (builder + `QueryCommand<TResult>`) повторяет ось `ToTempTable`/`ToTable` (CTAS-наблюдение 3) — принято. (7) Переименований нет; `docs/specs/**` — internal. (8) Кодовая сторона — `code-smells-review.md`, Находки 175–178.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.BatchBuilder` — 4 public (`Add`/`CreateTableAs`/`CreateTempTableAs`/`Query`), `BatchQuery`1` — 4 public, `TempTableExtensions` — 16 public (12 CTAS + 4 новых); `rg 'SupportsBatch|BatchUsesJoinedCommand' src` — `ISqlDialect.cs:1279,1289`, `SqlDialectBase.cs:791,793`, PG `:34`, SQL Server `:46,49`, MySQL `:48`, SQLite `:292`, ClickHouse — дефолт `false` (подтверждён тестом `Dialect_ShouldNotReportBatchSupport`); `find -name 'PublicAPI*.txt'` — **0** (BAT1/Шаг 5); `rg -l 'BatchBuilder|ToTempTableThen|ToTableThen|\.Batch\(\)' docs -g '!specs/**'` — **0** (BAT3); подавления `src/` — **11/11** (в фиче 0), новых `Skip=`/`Task.Delay`/пустых `catch` — 0; EOL новых файлов — CRLF 100 %. Тесты (Release, `--no-build`): core `BatchTests` **3/3**, PG `BatchSqlGenerationTests` **6/6**, SQL Server **3/3**, ClickHouse **3/3**. Контейнерная интеграция аудитом не перезапускалась. Кодовая сторона — `code-smells-review.md`, Находки 175–178.

**Итог.** 🔴 — 0 (по API); P0/P1 по именам — нет; 🟡 P2 — BAT1–BAT4 (трекинг Шага 5, консистентность `AsAsyncEnumerable`, доки EN+RU, контракт `Query`-последний). Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта.

**Fix (24.09.2026).**
- **BAT2 — закрыто:** потоковый терминал переименован `BatchQuery<TResult>.AsAsyncEnumerable` → **`ToAsyncEnumerable`** (консистентность с `EntityBuilderExtensions.ToAsyncEnumerable`/`QueryCommand<TResult>.ToAsyncEnumerable`); интеграционный тест переименован соответственно.
- **BAT4 — закрыто (кодовая сторона Находки 176):** `BatchBuilder` бросает `InvalidOperationException` при `Query` после `Query` и при материализации после `Query`; тесты `Batch_MaterialisationAfterQuery_ShouldThrow` / `Batch_SecondQuery_ShouldThrow`.
- **Находка 175 — закрыто:** публичный `BatchBuilder.Add(QueryCommand)` удалён (a `QueryCommand` is always a reading SELECT, so a preceding row-returning statement was indistinguishable from the result under reader navigation). `BatchBuilder` теперь: `CreateTempTableAs`/`CreateTableAs` (любое число) + ровно один `Query<TResult>` последним; навигация по result-set корректна.
- **Находка 177 — закрыто:** логирование/выполнение вынесено в `BatchRunner` и применяется одинаково на `DbBatch` и `;`-путях; интерцепция батча не поддерживается (события несут `DbCommand`) и зафиксирована в `docs/advanced/limitations.md` (+RU).
- **Находка 178 — закрыто:** батч-примитив вынесен в `internal sealed class BatchRunner` (`DataContext/BatchRunner.cs`); `QueryExecutor` вернулся под порог.
- **BAT3 — закрыто:** новая страница EN+RU `guide/28-sql-batch.md` («Executing statements in one batch» / «Выполнение утверждений одним батчем») + ссылки из `guide/22-create-table-as.md` (+RU) и `advanced/api-reference.md` (+RU) + `toc.yml`/`ru/toc.yml`; `advanced/limitations.md` (+RU), `providers/overview.md` (+RU); спецификации `sql-capabilities-gap-analysis.md` §4 п. 24 → done.
- **BAT1 — открыто** (трекинг Шага 5 / issue #53, не новая проблема): поверхность по-прежнему не в `PublicAPI.*.txt`.

---

### Аудит 24.09.2026 — cross-provider скалярные функции: `IScalarFunctions`, DIM `ISqlDialect.ScalarFunctions`, 14 членов `CommonFunctions`, промоушен из `PostgresFunctions`, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — 5: SC1–SC5)

**Область (uncommitted).** Новый публичный `NextORM.Core.IScalarFunctions` (`src/nextorm.core/DataContext/Dialect/DialectCapabilities.cs:340`; **2** члена — `bool Supports(string)` `:343`, `string Render(string, IReadOnlyList<string>)` `:350`). Новый **DIM** `ISqlDialect.ScalarFunctions { get; } => null` (`DataContext/Dialect/ISqlDialect.cs:424`) + `public virtual` `SqlDialectBase.cs:139`; **5** публичных override'ов (Postgres `:336`, SqlServer `:587`, MySql `:192`, ClickHouse `:317`, Sqlite `:78`). **14** новых публичных членов `CommonFunctions` (`Query/SqlFunctions.cs`): `left(string?,int) -> string?` `:530`, `right` `:539`, `lpad(string?,int,string?=" ")` `:547`, `rpad` `:555`, `repeat(string?,int)` `:562`, `reverse(string?)` `:569`, `space(int)` `:576`, `concat_ws(string?, params object?[])` `:585`, `translate(string?,string?,string?)` `:595`, `ascii(string?) -> int?` `:602`, `@char(int) -> string?` `:610`, `mod<T>(T?,T?) where T : System.Numerics.INumber<T> -> T?` `:616`, `log10(double?) -> double?` `:622`, `power(double?,double?) -> double?` `:629`. Промоушен: из `PostgresFunctions` (`Query/SqlFunctions.Postgres.cs`) **удалены 9** одноимённых членов — `left`/`right`/`lpad`/`rpad`/`repeat`/`reverse`/`translate`/`concat_ws`/`mod` (теперь наследуются от `CommonFunctions`, т.к. `PostgresFunctions : CommonFunctions`); мёртвые записи убраны из `ExtendedScalarFunctionTranslator`. `internal` и в публичную поверхность не входят: `InMemoryScalarFunctions`, `InMemoryScalarFunctionRewriter` и 5 `internal sealed *ScalarFunctions`. Публичные доки EN+RU **не обновлены** — SC3. Кодовая сторона — `code-smells-review.md`, Находки 179–185.

**P0 — нет.** BCL-конфликтов нет (`CA1716`/`CA1724` чисто): `IScalarFunctions` не затеняет BCL; `left`/`right`/`mod`/`power` затеняют лишь CLR-члены `Math.*`/байтовые helper'ы, как и существующая SQL-поверхность (P1-10, «Отмечено, но менять не рекомендуется»); `@char` вынужден `@`-эскейпом C#-ключевого слова — как `@in`. Имена snake_case/SQL-зеркала (`concat_ws`, `log10`) — принятое DSL-исключение.

**P1 по именам — нет.** `IScalarFunctions` — префикс `I` + plural `Functions`, ровно как `ISessionInfoFunctions`/`IUuidGenerators`/`ITableSampleMethods`; `Supports`/`Render` повторяют их контракт; `ScalarFunctions` — по образцу `SessionInfoFunctions`/`UuidGenerators`. Параметры `(value, n)`/`(value, length, pad)`/`(separator, values)` — цель→обязательные→optional; `CancellationToken` не применим (синхронные рендеры). XML-doc полный: `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`), `NoWarn` — **0** ⇒ CS1591 гейтится, build `0/0` = покрытие «1 интерфейс + 2 метода + DIM + 5 override + 14 членов `CommonFunctions`» **100 %**.

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| SC1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; поверхность выше | Новая публичная поверхность не трекается: `IScalarFunctions` (тип + 2 члена), DIM `ISqlDialect.ScalarFunctions` + `SqlDialectBase`-двойник + 5 override'ов, 14 членов `CommonFunctions`. Плюс **удаление 9 членов `PostgresFunctions`** даст пару RS0017+RS0016 (промоушен производный→базовый). `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). Продолжение BAT1/CTAS3/INT4/ND1/DU1/SS1/XP7/S1/UG2/I8 | При заморозке внести подписи (точный текст — из анализатора); DIM — с дефолтом `null`; 5 override'ов — как обычные члены. Отдельной записи поверх BAT1/XP7 не требуется |
| SC2 | P2 (API-контракт/BC) | `Query/SqlFunctions.Postgres.cs` (было 9 членов); `SqlFunctions.cs:547,555,616` | Промоушен 9 членов `PostgresFunctions`→`CommonFunctions` + **смена сигнатур**: `PostgresFunctions.lpad/rpad(value,length,fill)` (fill *обязателен*) → `CommonFunctions.lpad/rpad(value,length,pad=" ")` (default + другое имя параметра); `mod<T>(T?,T?)` без ограничения → `mod<T>(T?,T?) where T : INumber<T>`. Source-совместимо для обычных вызовов (наследование), но `named-arg fill:` и `T` без `INumber` ломаются; для предкомпилированного потребителя токен метода `PostgresFunctions.left/…` исчез (binary break, `MissingMethodException`). Поверхность alpha/не заморожена ⇒ не P0 | Зафиксировать в release notes как Minor: «обёртки переехали на `SqlFunctions.Sql`; `SqlFunctions.Postgres.*` наследуются; `mod` ограничен `INumber<T>`; `lpad/rpad` получили default pad». Канонический путь — `SqlFunctions.Sql.*`; форму не менять |
| SC3 | P2 (доки/охват) | `docs/scalar-functions/index.md:278-283,352` (+RU), `docs/advanced/api-reference.md:75` (+RU); `rg -ni 'concat_ws\|lpad\|IScalarFunctions' docs -g '!specs/**'` — **0** | **Публичная проза EN+RU устарела/не дополнена.** `guide/11` по-прежнему называет эти функции `SqlFunctions.Postgres.left/right/repeat/reverse/translate/mod` (PG-only таблица), хотя они стали cross-provider; `concat_ws`/`lpad`/`rpad`/`space`/`ascii`/`char`/`log10`/`power` и `IScalarFunctions`/`ScalarFunctions` нигде в публичных доках не упомянуты. План (`todo_cross_provider_scalar_functions.md` §10) прямо требовал `guide/11`, `providers/overview`, `limitations`, `api-reference` (EN+RU) | Добавить EN+RU: cross-provider раздел/строки «Scalar functions» (`SqlFunctions.Sql.left/lpad/concat_ws/…`, `IScalarFunctions`/`ScalarFunctions`, per-provider гейты: SQLite без `lpad/rpad/repeat/reverse/space/translate`, MySQL/MariaDB без `translate`, ClickHouse NULL-`concat_ws` и байтовые `left/right` — см. Находки 180/184), строки в `advanced/api-reference` и `limitations`. Маршрут `nextorm-design-engineer`; ссылки на `docs/specs/**` запрещены (AGENTS.md) |
| SC4 | P2 (контракт/stringly-typed) | `DialectCapabilities.cs:340-351`; `Visitors/BuiltinFunctionTranslator.cs:31-38`; 5× `Supports` | `IScalarFunctions` stringly-typed (`Supports(string)`/`Render(string,args)`), а список имён продублирован в `BuiltinFunctionTranslator.ScalarFunctionNames` + 5 провайдерных `Supports`; нет enum/const и нет контракт-теста «Supports ⇒ Render» (кодовая Находка 183). Форма **принята** (зеркалит `ISessionInfoFunctions`/`IUuidGenerators`/`IXmlFunctions`; переименование/энум до заморозки не требуется) | Не менять форму; вынести общий набор имён и добавить контракт-тест (см. Находку 183), чтобы «одна строка» не разъезжалась по трём местам |
| SC5 | P2 (XML-doc/семантика) | `SqlFunctions.cs:523-539,587-610`; `ClickHouseDialect.cs:950-953,959-960` | Публичный XML-doc расходится с фактической семантикой ClickHouse. `CommonFunctions.left/right` (`:523-539`) утверждают «PostgreSQL treats a negative n as „all but the last/first |n| characters“; the other providers do not» — но ClickHouse `left`/`right` при отрицательном offset дают ту же семантику, а in-memory реализует именно её (кодовая Находка 179/180); `ascii`/`@char` обещают «code point» (`:597-610`), тогда как CH `ascii` не определён вне ASCII/Latin-1, а `char` строит байтовую строку | Скорректировать `<summary>` `left/right` (negative-offset поведение перечислить по провайдерам) и `ascii`/`@char` (оговорить ASCII/byte-ограничения CH); связать с code-smells 180/184. Публичную форму не менять |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 по именам нет.** (2) **Extend-only в границах:** `IScalarFunctions` — новый тип, `ISqlDialect.ScalarFunctions` — DIM (`=> null`), 14 членов `CommonFunctions` аддитивны, 5 override'ов расширяют `virtual`; внешний реализатор `ISqlDialect` компилируется без доработки (CS0535 нет; дефолт не бросает). Единственная не-аддитивная часть — промоушен 9 членов `PostgresFunctions` (SC2), для alpha принято по установленному образцу XP7/S1/UG2/I8. (3) **XML-doc 100 %** (build `0/0`, CS1591 гейтится). (4) **BCL/имена:** `IScalarFunctions`/`ScalarFunctions`/`Supports`/`Render` — не затеняют; `lpad`/`rpad`/`concat_ws`/`log10`/`@char` — SQL-зеркало (принято); НЕТ конфликта `left`/`right` с `string.Left` (таких членов в BCL нет). (5) **API-форма `mod<T>` улучшена** добавлением `INumber<T>` (исключает бессмысленные `T`), но это source-break относительно прежнего неограниченного `PostgresFunctions.mod<T>` (SC2). (6) **Доки EN+RU не тронуты** (SC3); `docs/specs/**` — internal. (7) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0** (SC1). (8) Кодовая сторона (SQLite `right`-баг, ClickHouse байты, `mod`-cast, DRY, EOL) — `code-smells-review.md`, Находки 179–185.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.IScalarFunctions` — `Supports`/`Render` (`DialectCapabilities.cs:343,350`); `roslyn refs ISqlDialect.ScalarFunctions` — 5 override'ов + `SqlDialectBase` + `BuiltinFunctionTranslator`; `grep -n 'public override IScalarFunctions ScalarFunctions' src/nextorm.*/*.cs` — 5; `find -name 'PublicAPI*.txt'` — **0** (SC1); `rg -ni 'concat_ws|lpad|IScalarFunctions' docs -g '!specs/**'` — **0** (SC3); `rg -n 'SqlFunctions.Postgres.left|SqlFunctions.Postgres.mod' docs/scalar-functions/index.md` — EN+RU строки на месте (устаревшая привязка, SC3); подавления `src/` — **11/11**, новых 0; XML-doc гейтится build `0/0`. Тесты (Release, `--no-build`): core `InMemoryScalarFunctions` **3/3**; SQL-gen `~CrossProviderScalarSqlGeneration` — sqlite **6/6**, postgres **1/1**, mysql **2/2**, mariadb **2/2**, sqlserver **1/1**, clickhouse **1/1**. Контейнерная интеграция не перезапускалась (нет `DOCKER_HOST`).

**Итог.** 🔴 — 0 (по API); P0/P1 по именам — нет; 🟡 P2 — SC1–SC5 (трекинг Шага 5 + промоушен `PostgresFunctions`, доки EN+RU, stringly-typed контракт, XML-doc семантика). Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта. Кодовые 🔴 Находки 179–180 маршрутизированы `nextorm-design-engineer`.

---

### Аудит 24.09.2026 — DML-шаги в SQL-батче: `BatchBuilder.Execute<TEntity>` ×4 (`InsertBuilder`/`UpdateBuilder`/`DeleteBuilder`/`TruncateBuilder`), uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — 2: BAT5–BAT6; BAT1 — продолжение)

**Область (uncommitted).** Четыре новых **публичных** метода на существующем `NextORM.Core.BatchBuilder` (`src/nextorm.core/Builders/BatchBuilder.cs`): `Execute<TEntity>(InsertBuilder<TEntity>) -> BatchBuilder` `:99`, `Execute<TEntity>(UpdateBuilder<TEntity>)` `:115`, `Execute<TEntity>(DeleteBuilder<TEntity>)` `:131`, `Execute<TEntity>(TruncateBuilder<TEntity>)` `:147`; private `AddMutation` `:186`. Итого `BatchBuilder` — **7** public-методов (`CreateTableAs`/`CreateTempTableAs`/`Execute`×4/`Query`) против 4 в прошлом аудите (публичный `Add(QueryCommand)` удалён закрытием кодовой Находки 175). Новых публичных типов нет. `internal` (в поверхность не входят): `BuildBatchCommand()` (`InsertBuilder.cs:395`, `UpdateBuilder.cs:216`, `DeleteBuilder.cs:174`, `TruncateBuilder.cs:66`), `TruncateBuilder.DataContext` (`:24`), `BatchStepSpec.Mutation`/`ForMutation` (`IBatchExecutor.cs:77,92`), `BuildBatchMutationSql` (`DataContext.cs:697`). Публичные доки EN+RU под DML **не обновлены** — BAT6. Кодовая сторона — `code-smells-review.md`, Находки 186–187.

**P0 — нет.** BCL-конфликтов нет (`CA1716`/`CA1724`): `Execute` — не затеняет; новых типов нет. Все четыре overload'а аддитивны; перегрузка разрешается по типу builder'а (все `sealed`, между собой не наследуются). `*ReturningBuilder<TEntity,TResult>` — **отдельные** `sealed`-классы (`roslyn types ReturningBuilder` — 5), поэтому RETURNING/OUTPUT-терминалы не принимаются по типу (заявленное исключение подтверждено); `UpdateJoinBuilder`/`DeleteJoinBuilder`/`MergeBuilder` — тоже отдельные классы. Удалений/переименований/смены подписей нет.

**P1 по именам — нет.**

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| BAT5 | P2 (нэйминг/консистентность) | `BatchBuilder.cs:99,115,131,147` ↔ `BatchBuilder.cs:73,86` (`CreateTableAs`/`CreateTempTableAs`), `InsertBuilder.Insert()`, `DeleteBuilder.Delete()`, `TruncateBuilder.Execute()` (`:37`), `IMutationExecutor.Execute` | **`Execute` читается как «выполнить», а метод только ставит шаг в батч.** Рядом `BatchBuilder` ставит шаги SQL-глаголом (`CreateTableAs`), а терминал `TruncateBuilder` — `Execute()` — **немедленно** выполняет (`docs/guide/20-delete-statement.md:83`), как и `IMutationExecutor.Execute(command)`. Форма продолжает ту же ось, что открытая **DEL6** (терминал TRUNCATE выбивается из SQL-глагольного ряда). Не баг, но поверхность замораживается Шагом 5 | До заморозки решить: (а) `Insert`/`Update`/`Delete`/`Truncate` (зеркало SQL-глаголов, как `CreateTableAs`; согласуется с DEL6), либо (б) `AddInsert`/`AddUpdate`/…, либо (в) явно зафиксировать `Execute` в `<remarks>` `BatchBuilder` как «stage, not run». Любое переименование — одновременно `docs/**` и `docs/ru/**` (AGENTS.md) |
| BAT6 | P2 (доки/охват) | `docs/guide/28-sql-batch.md:31-48,64` (+RU `:31-48,64`), `docs/advanced/api-reference.md:51` (+RU), `docs/advanced/limitations.md:15` (+RU), `docs/providers/overview.md:58` (+RU); `grep -nE 'Execute\(\|INSERT\|UPDATE\|DELETE\|TRUNCATE\|DML' docs/guide/28-sql-batch.md docs/ru/guide/28-sql-batch.md` — **0** | **Публичная проза EN+RU не знает о DML-шагах батча и теперь неточна.** `guide/28` §«Building a batch directly» (`:31-48`) перечисляет только `CreateTempTableAs`/`CreateTableAs` + `Query`; §Limitations (`:64`) прямо утверждает «The statements before it are materialisations, which return no columns», хотя после DML-расширения перед запросом может стоять `INSERT`/`UPDATE`/`DELETE`/`TRUNCATE`. `api-reference.md:51` — «`CreateTempTableAs`/`CreateTableAs` … plus exactly one `Query<TResult>`» (4 `Execute` отсутствуют); `limitations.md:15`/`providers/overview.md:58` описывают батч только как материализацию+чтение; `Execute(builder)` не упомянут ни в одном публичном документе (единственные `Execute(` — терминал `TruncateBuilder` в `guide/20`). План `todo_batch_dml.md` требовал обновить guide 28, api-reference, limitations EN+RU. Замыкает охват BAT3 (та закрыта для CTAS-поверхности) | Добавить EN+RU в `guide/28`: `Execute(InsertBuilder/UpdateBuilder/DeleteBuilder/TruncateBuilder)` (не-результатный шаг перед `Query`, одна сессия; SQLite — `TRUNCATE`→`NotSupportedException`), поправить формулировку §Limitations («materialisation **or DML**»), строку в `api-reference.md`, оговорку в `limitations.md` (RETURNING/OUTPUT, UPDATE-JOIN/DELETE-JOIN/MERGE вне батча) и `providers/overview.md`; ссылки на `docs/specs/**` запрещены (AGENTS.md). Маршрут `nextorm-design-engineer` |
| BAT1 | P2 (трекинг Шага 5 — продолжение) | `PublicAPI.*.txt` — **0**; `BatchBuilder.cs:99-151` | 4 новых публичных `Execute`-перегрузки добавляются к уже нетрекаемой batch-поверхности (BAT1). `PublicApiAnalyzers`/ApiCompat/API-approval не подключены | При заморозке внести и эти 4 подписи (точный текст — из анализатора); BAT1 переоткрывать не нужно |

ℹ️ **Наблюдения.** (1) **P0/P1 по именам нет; новых публичных типов нет.** (2) **Extend-only:** 4 overload'а аддитивны; BCL/`CA1716`/`CA1724` чисто. (3) **XML-doc 100 %** (build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — 0): у всех 4 `Execute` есть `<summary>`/`<typeparam>`/`<param>`/`<returns>`/`<exception>`; неточность самих `<exception>`-списков — кодовая ℹ️, не охват. (4) **`Execute` — не перегрузка exec-терминала:** `TruncateBuilder.Execute()` (без параметров) и `BatchBuilder.Execute(builder)` не конфликтуют; см. BAT5. (5) `TruncateBuilder.DataContext` — `internal`-двойник уже существующих `Insert/Update/DeleteBuilder.DataContext`, консистентно. (6) **Доки EN+RU «не тронуты»** (BAT6); `docs/specs/**` — internal. (7) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0** (BAT1). (8) Кодовая сторона — `code-smells-review.md`, Находки 186–187.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.BatchBuilder` — 7 public (`CreateTableAs`, `CreateTempTableAs`, `Execute` `:99/115/131/147`, `Query`), `AddMutation` private `:186`; `roslyn members NextORM.Core.BatchQuery`1` — 4 public (`ToSql`/`ToList`/`ToListAsync`/`ToAsyncEnumerable`); `roslyn types ReturningBuilder` — 5 отдельных `sealed`-классов (RETURNING не принимается по типу); `find -name 'PublicAPI*.txt'` — **0** (BAT1); `grep -nE 'Execute\(\|INSERT\|UPDATE\|DELETE\|TRUNCATE\|DML' docs/guide/28-sql-batch.md docs/ru/guide/28-sql-batch.md` — **0** (BAT6); `grep -rn 'Execute(' docs -g '!specs/**'` — только `TruncateBuilder.Execute()` в `guide/20` EN+RU и `api-reference.md:49` (BAT6); подавления `src/` — **11/11**, новых 0; EOL DML-файлов — CRLF 100 %. Тесты (Release, `--no-build`): core `BatchTests` **3/3**, PG `BatchSqlGenerationTests` **16/16**, SQL Server **4/4**, SQLite **2/2**, ClickHouse **3/3**. Контейнерная интеграция не перезапускалась. Кодовая сторона — `code-smells-review.md`, Находки 186–187.

**Итог.** 🔴 — 0 (по API); P0/P1 по именам — нет; 🟡 P2 — BAT5–BAT6 + BAT1-продолжение (трекинг Шага 5, доки EN+RU под DML-расширение). Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта.

**Закрытие (24.09.2026).**
- **BAT5 — закрыто:** четыре overload'а переименованы `Execute` → **`Insert`/`Update`/`Delete`/`Truncate`** (вариант «а»; зеркало SQL-глаголов рядом с `CreateTableAs`, согласуется с осью DEL6). Публичная поверхность `BatchBuilder` — `CreateTableAs`/`CreateTempTableAs`/`Insert`/`Update`/`Delete`/`Truncate`/`Query`.
- **BAT6 — закрыто:** EN+RU `guide/28-sql-batch.md` получил пример и список DML-методов + рендер `update … ; select …`, §Limitations поправлен («side-effecting — materialisations and DML»), обновлены `advanced/api-reference.md` (+RU), `advanced/limitations.md` (+RU); `providers/overview.md` механизм не менялся.
- **BAT1 — открыто** (трекинг Шага 5, issue #53): теперь 6 новых публичных подписей (`Insert`/`Update`/`Delete`/`Truncate` + ранее не трекавшаяся batch-поверхность).

---

### Аудит 24.09.2026 — многострочный батч-`ToSql` (`MultilineBatchSql`) и name-less-ремедиация CTAS (`TempTableExtensions`), uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — 4: MBS1, TT1–TT3; MBS2 — ℹ️)

**Область (uncommitted).**
- **Multiline batch SQL.** `DataContextBuilder.MultilineBatchSql.get -> bool` (`src/nextorm.core/DI/DataContextBuilder.cs:55`, `{ get; private set; }`) + `DataContextBuilder.UseMultilineBatchSql(bool value = true) -> DataContextBuilder` (`:159`); DIM `IContextEnvironment.MultilineBatchSql.get -> bool => false` (`DataContext/Roles/IContextEnvironment.cs:56`); `internal ContextEnvironment(…, bool multilineBatchSql = false)` + `ContextEnvironment.MultilineBatchSql` (`DataContext/ContextEnvironment.cs:15,53`); `DataContext.MultilineBatchSql.get -> bool` (`DataContext/DataContext.cs:147`), пробрасывается в `IBatchExecutor.RenderBatch` → `new BatchPlan(…, multiline: MultilineBatchSql)` (`DataContext.cs:663`; `internal BatchPlan(…, bool multiline = false)` `DataContext/Roles/IBatchExecutor.cs:109`).
- **TempTableExtensions, name-less-ремедиация.** 8 новых публичных членов (`src/nextorm.core/Builders/TempTableExtensions.cs`): `ToTempTable<TResult>(EntityBuilder<TResult>) -> string` `:54`, `ToTempTable<TResult>(EntityBuilder<TResult>, CreateTableOptions) -> string` `:70`, `ToTempTable<TResult>(QueryCommand<TResult>) -> string` `:86`, `ToTempTable<TResult>(QueryCommand<TResult>, CreateTableOptions) -> string` `:99`, `ToTempTableAsync<TResult>(EntityBuilder<TResult>, CancellationToken = default) -> Task<string>` `:139`, `ToTempTableAsync<TResult>(EntityBuilder<TResult>, CreateTableOptions, CancellationToken = default) -> Task<string>` `:156`, `ToTempTableAsync<TResult>(QueryCommand<TResult>, CancellationToken = default) -> Task<string>` `:172`, `ToTempTableAsync<TResult>(QueryCommand<TResult>, CreateTableOptions, CancellationToken = default) -> Task<string>` `:186`. Итого `TempTableExtensions` — **20** публичных членов (12 name-ful + 8 name-less).
- **Удалены в этом же uncommitted-цикле:** `ToTempTableThen<TSource,TResult>` ×2, `ToTableThen<TSource,TResult>` ×2 и `ToBatchThen` — **5** публичных extension-методов (фигурировали в аудите 24.09.2026 BAT; их member-list этой записью **superseded**). Продолжение `BatchExtensions.Batch`/`BatchBuilder`/`BatchQuery<TResult>` (BAT-раздел) остаётся на месте.
- **Кодовая сторона:** `code-smells-review.md`, Находка 188 + решение 24.09.2026 (тот же рабочий tree).

**P0 — нет.** BCL-конфликтов нет (`CA1716`/`CA1724`): `MultilineBatchSql`/`UseMultilineBatchSql` не затеняют; `ToTempTable`/`ToTempTableAsync` — существующая ось, 8 перегрузок аддитивны. `Async`-суффикс — есть sync-пара; CA1068 (`CancellationToken` последним/с дефолтом) соблюдён. Удаление continuation-сахара — pre-release alpha (поверхность никогда не shipped), SemVer-импакт нулевой; отдельных BC-действий до release не требуется (TT2).

**P1 по именам — нет.** `MultilineBatchSql` — факт-свойство (+ `Use*`-сеттер) ровно по образцу `QuoteIdentifiers`/`KeywordCase`/`NamingConvention`; DIM `=> false` аддитивен и source/binary-совместим для внешних реализаторов `IContextEnvironment`. `TempTableExtensions` name-less/required-options overload'ы следуют уже принятой форме (пара «без параметра + обязательный options») — имя и `Async`-суффикс не меняются.

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| MBS1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; `DataContextBuilder.cs:55,159`; `IContextEnvironment.cs:56`; `DataContext.cs:147` | Новая `MultilineBatchSql`-поверхность (**4** публичных члена: property + `Use*` + DIM + `DataContext` property) не трекается. Продолжение BAT1/SC1/CTAS3, не новая проблема | При заморозке внести подписи (точный текст — из анализатора); DIM — с дефолтом `false` |
| TT1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; `TempTableExtensions.cs:54,70,86,99,139,156,172,186` | **8** новых name-less `ToTempTable`/`ToTempTableAsync` не трекаются; `TempTableExtensions` 12 → **20** публичных членов. Продолжение CTAS3/BAT1 | При заморозке внести 8 подписей (и обновить прежние 12). DIM-членов в поверхность не добавляется |
| TT2 | P2 (BC/removal) | было `Builders/TempTableExtensions.cs:175,192,210,227,233` — удалены `ToTempTableThen`×2, `ToTableThen`×2, `ToBatchThen` | **5** публичных extension-методов удалены в этом же uncommitted-цикле; аудит BAT (24.09.2026) регистрировал их как публичные — его member-list этим **superseded**. Поверхность никогда не shipped (alpha) ⇒ SemVer-импакт нулевой | Зафиксировать в release notes как «continuation-сахар заменён name-less `ToTempTable` + `BatchBuilder`»; отдельного PublicAPI-действия нет |
| TT3 | P2 (форма/консистентность) | `TempTableExtensions.cs:30,42,113,127,199,212,221,233,244,258,268,281`; `BatchBuilder.cs:75,88` | **14** name-ful CTAS-членов по-прежнему `CreateTableOptions? options = null`, тогда как name-less-двойники в этом же цикле отремедированы в пару «без параметра + обязательный `CreateTableOptions`» (Находка 188). `ToTable("t", null)`/`CreateTableAs("t", q, null)` компилируются и молча означают default-опции | Довести ремедиацию до name-ful: `(…, name)` + `(…, name, CreateTableOptions)` с `ArgumentNullException.ThrowIfNull`; **+14** публичных членов учесть при заморозке. Маршрут `nextorm-design-engineer` |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 по именам нет.** (2) **Ремедиация подтверждена компилятором:** probe file-based app (`#:project …nextorm.core.csproj`) `builder.ToTempTable(null)` даёт **CS8625** с указанием на обязательную перегрузку `TempTableExtensions.cs:72`; пара «без параметра / обязательный options» уже даёт искомый gate — TT3 просит распространить её на name-ful. (3) **MBS2 — `bool value = true` под критерий не попадает:** `UseMultilineBatchSql(bool value = true)` (`:159`) — non-null дефолт, как и `UseQuotedIdentifiers` `:108`/`UseUppercaseKeywords` `:148`/`UseKeywordCase(KeywordCase = Lower)` `:136`; `T? x = null`-smell (решение 24.09.2026) не применим, фикс не требуется. (4) **Extend-only:** MBS — аддитивно (2 public-члена + 1 DIM + 1 `DataContext` property); DIM-дефолт `false` не ломает внешних реализаторов `IContextEnvironment`; TT — аддитивно с одной осознанной заменой continuation-сахара (TT2). (5) **XML-doc 100 %:** build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — **0** (CS1591 гейтится) ⇒ у всех 4 MBS-членов и 8 TT-оверлоадов есть `<summary>`/`<param>`/`<returns>`/`<exception>`. (6) **Публичные доки EN+RU обновлены:** `UseMultilineBatchSql` — `docs/advanced/api-reference.md:131` (+RU:130) и `docs/guide/28-sql-batch.md:88` (+RU:88); name-less `ToTempTable()`/`ToTempTableAsync()` — `docs/advanced/api-reference.md:50` (+RU:50) и `docs/guide/22-create-table-as.md:23,29` (+RU:27,33); удалённые `ToTempTableThen`/`ToTableThen` в публичных доках не фигурировали — **0** ссылок (остались только в `docs/specs/**`, internal). (7) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0**. (8) Кодовая сторона — `code-smells-review.md`, Находка 188.

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.DataContextBuilder` — `MultilineBatchSql.get` `:55`, `UseMultilineBatchSql(bool)` `:159`; `rg -n 'MultilineBatchSql' src/` — `DataContextBuilder.cs:55,159`, `IContextEnvironment.cs:56`, `ContextEnvironment.cs:15,53`, `DataContext.cs:147,663`, `IBatchExecutor.cs:109`; `roslyn members NextORM.Core.TempTableExtensions` — **20** public (12 name-ful + 8 name-less), `ToTempTableThen`/`ToTableThen`/`ToBatchThen` — **0**; probe `ToTempTable(null)` — **CS8625**; `find -name 'PublicAPI*.txt'` — **0**; `rg -n 'MultilineBatchSql' docs -g '!specs/**'` — EN+RU api-reference и `guide/28`; `rg -n 'ToTempTableThen|ToTableThen' docs -g '!specs/**'` — **0**; `rg -n 'NoWarn' src/**/*.csproj` — **0**; подавления `src/` — **11/11** (в этом проходе 0 новых). Тесты аудитом не перезапускались (audit формы публичного API).

**Итог.** 🔴 — 0 (по API); P0/P1 по именам — нет; 🟡 P2 — MBS1, TT1–TT3 (трекинг Шага 5; supersede member-list BAT; доведение `= null`-ремедиации на name-ful). ℹ️ MBS2 — `bool value = true` под критерий не попадает. Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта.


---

### Аудит 24.09.2026 — MySQL/MariaDB-поверхность: `MySqlFunctions` (28 членов), `SqlFunctions.MySql`, `IMySqlFunctions`, DIM `ISqlDialect.MySqlFunctions`, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — MY1–MY2)

**Область (uncommitted).** Новый публичный `NextORM.Core.MySqlFunctions : CommonFunctions` (`src/nextorm.core/Query/SqlFunctions.MySql.cs`; **28** членов, instance-методы — тип намеренно нестатический и наследуемый для #79): `find_in_set(string?,string?) -> int?`, `field<T>(T?, params T?[]) -> int?`, `elt<T>(int, params T?[]) -> T?`, `substring_index(string?,string?,int) -> string?`, `format(decimal?,int) -> string?`, `str_to_date(string?,string?) -> DateTime?`, `date_format(DateTime?,string?) -> string?`, `from_unixtime(long?) -> DateTime?`, `unix_timestamp(DateTime?) -> long?`, `md5`/`sha1`/`sha2(string?,int)`, `inet_aton(string?) -> long?`, `inet_ntoa(long?) -> string?`, `json_set`/`json_insert`/`json_replace(string?,string?,string?)`, `json_remove(string?,string?)`, `json_merge_patch`/`json_merge_preserve(string?,string?)`, `json_array_append`/`json_array_insert(string?,string?,string?)`, `json_depth(string?) -> int?`, `json_keys(string?) -> string?`, `json_length(string?) -> int?`, `json_type(string?) -> string?`, `uuid_to_bin(string?) -> byte[]?`, `bin_to_uuid(byte[]?) -> string?`. Новый публичный маркер `SqlFunctions.MySql { get; }` (`Query/SqlFunctions.cs`). Новый публичный интерфейс `NextORM.Core.IMySqlFunctions` (**2** члена: `bool Supports(string)` / `string Render(string, IReadOnlyList<string>)`), DIM `ISqlDialect.MySqlFunctions => null` + `public virtual` в `SqlDialectBase`, **1** переопределение в `MySqlDialect` и **1** в `MariaDbDialect` (декоратор `internal sealed MariaDbNativeFunctions`). Новых публичных типов провайдеров нет. Публичная проза EN+RU добавлена — новая страница `docs/guide/provider-specific/mysql.md` (+RU, +`toc.yml`).

**P0 — нет.** BCL-конфликтов нет (`CA1716`/`CA1724`): `MySqlFunctions` не затеняет (`PostgresFunctions`/`SqlServerFunctions`/`ClickHouseFunctions` — ряд); `IMySqlFunctions` — префикс `I` + plural `Functions`, как `IScalarFunctions`/`ISessionInfoFunctions`; `Supports`/`Render` повторяют их контракт; `MySqlFunctions`-свойство — по образцу `ScalarFunctions`. `format`/`md5`/`sha1` не конфликтуют в CLR (одноимённые PG-члены наследуются через другой тип; транслятор распознаёт по `DeclaringType`).

**P1 по именам — нет.** Имена snake_case/SQL-зеркала (`find_in_set`, `str_to_date`, `json_merge_patch`, `uuid_to_bin`) — принятое DSL-исключение (P1-10). Параметры `(value, …)`. XML-doc полный: build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — 0 ⇒ CS1591 гейтится.

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| MY1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; поверхность выше | Новая поверхность не трекается: `MySqlFunctions` (тип + 28 членов), маркер `SqlFunctions.MySql`, `IMySqlFunctions` (тип + 2), DIM + `SqlDialectBase` + 2 override. `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). Продолжение SC1/BAT1 | При заморозке внести подписи (точный текст — из анализатора); DIM — с дефолтом `null`. Отдельной записи поверх BAT1/SC1 не требуется |
| MY2 | P2 (контракт/stringly-typed) | `DialectCapabilities.cs` (`IMySqlFunctions`); `NormSqlTranslator.cs`; `MySqlFunctionTranslator.cs`; `MySqlDialect.cs`/`MariaDbDialect.cs` | `IMySqlFunctions` stringly-typed; набор имён продублирован в `MySqlNativeFunctions.Supports` и трансляторе. Форма **принята** (зеркалит `IScalarFunctions`/`ISessionInfoFunctions`; SC4). Контракт-тест `DialectCapabilityContractTests` новую поверхность пока не покрывает | Не менять форму; при желании добавить `MySqlFunctions`-ветку в контракт-тест (Supports ⇒ Render без исключения) |

ℹ️ **Наблюдения.** (1) **Extend-only:** `MySqlFunctions` — новый тип (наследует `CommonFunctions`), `IMySqlFunctions` — новый интерфейс, DIM `=> null`, override'ы расширяют `virtual`; внешний реализатор `ISqlDialect` компилируется без доработки. (2) **`uuid_to_bin`/`bin_to_uuid` гейтятся на MariaDB per-name** (`MariaDbNativeFunctions`), т.к. MariaDB их не реализует (проверено на `mariadb:11.4`; преобразование через `CAST(... AS BINARY(16))`/`CAST(... AS UUID)`) — не family-флаг. (3) Имена типов/членов не переименовывались. (4) `docs/specs/**` internal; публичные ссылки на specs отсутствуют. (5) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0** (MY1).

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.MySqlFunctions` — 28; `roslyn members NextORM.Core.IMySqlFunctions` — 2; `grep -n 'public override IMySqlFunctions MySqlFunctions' src/nextorm.*/*.cs` — 2 (mysql, mariadb); SQL-gen `~MySqlFunctions` — mysql **7/7**, mariadb **6/6**; негатив `~MySqlFunctions` — postgres/sqlite/sqlserver/clickhouse **2/2** каждый; core `InMemoryMySqlFunctions` **1/1**; контейнерно — MySQL `~MySqlFunctions` **7/7**, MariaDB `~MariaDbFunctions` **7/7**. `find -name 'PublicAPI*.txt'` — **0** (MY1).

**Итог.** 🔴 — 0 (по API); P0/P1 по именам — нет; 🟡 P2 — MY1–MY2 (трекинг Шага 5, stringly-typed контракт). Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта.

---

### Аудит 24.09.2026 — SQL Server-only T-SQL скаляры: `ISqlServerFunctions`, DIM `ISqlDialect.SqlServerFunctions`, 27 членов `SqlServerFunctions`, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — SS1–SS2)

**Область (uncommitted).** Новый публичный `NextORM.Core.ISqlServerFunctions` (`src/nextorm.core/DataContext/Dialect/DialectCapabilities.cs`; **2** члена — `bool Supports(string)`, `string Render(string, IReadOnlyList<string>)`). Новый **DIM** `ISqlDialect.SqlServerFunctions { get; } => null` (`DataContext/Dialect/ISqlDialect.cs`) + `public virtual` `SqlDialectBase.cs`; **1** публичный override (`SqlServerDialect.cs`). **27** новых публичных членов `SqlServerFunctions` (`Query/SqlFunctions.SqlServer.cs`): `patindex`/`quotename(value)`/`quotename(value,quote)`/`soundex`/`difference`/`string_escape`/`unicode`/`nchar`/`format(value,fmt)`/`format(value,fmt,culture)`, `acos`/`asin`/`atan`/`atn2`/`cot`/`degrees`/`radians`/`pi`/`square`, `datename`/`date_bucket`(×2), `hashbytes`/`newsequentialid`, `json_array`/`json_object`/`json_arrayagg`/`json_objectagg`/`json_contains`/`json_path_exists`. `internal` (в поверхность не входят): `SqlServerSpecificFunctions`, `SqlServerScalarFunctionTranslator`. `ASCII`/`CHAR`/`TRANSLATE` не дублируются (уже в `CommonFunctions`), `LOG10` не заводится (`Math.Log10`).

**P0 — нет.** BCL-конфликтов нет: `Supports`/`Render` не затеняют, `ISqlServerFunctions` — новый тип; `SqlServerFunctions` уже существовал, члены аддитивны. DIM (`=> null`) не ломает внешних реализаторов `ISqlDialect`. Удалений/переименований/смены подписей нет.

**P1 по именам — нет.** Имена зеркалят SQL-токены (согласованный стиль P1-10: `patindex`, `quotename`, `atn2`, `json_arrayagg`, …).

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| SS1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; поверхность выше | Новая публичная поверхность не трекается: `ISqlServerFunctions` (тип + 2 члена), DIM `ISqlDialect.SqlServerFunctions` + `SqlDialectBase`-двойник + 1 override, 27 членов `SqlServerFunctions`. `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). Продолжение BAT1/SC1 | При заморозке внести подписи (точный текст — из анализатора) |
| SS2 | P2 (контракт/stringly-typed) | `DialectCapabilities.cs`; `SqlServerScalarFunctionTranslator.cs`; `SqlServerDialect.cs` | `ISqlServerFunctions` stringly-typed (`Supports(string)`/`Render(string,args)`), набор имён продублирован в трансляторе и в `Supports`; нет контракт-теста «Supports ⇒ Render» в `DialectCapabilityContractTests.ScalarNames`. Форма **принята** (зеркалит `IScalarFunctions`/`ISessionInfoFunctions`/`IXmlFunctions`) | Не менять форму; при желании расширить контракт-тест на новые имена (сейчас покрыты диалект-тестом `SqlServerDialectTests`) |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 по именам нет.** (2) **Extend-only:** новый интерфейс + DIM + `virtual` + аддитивные члены `SqlServerFunctions`. (3) **XML-doc 100 %** (build `0/0`). (4) `json_contains`/`json_path_exists` возвращают `bool` (как `isjson`) и внесены в `TypeFacts.IsPredicateCall`; `json_object`/`json_objectagg` рендерят `key : value`. (5) Доки EN+RU обновлены: `guide/provider-specific/sqlserver.md` (+RU), `advanced/api-reference.md` (+RU); спеки — `sql-function-coverage-gap.md`, ~~`todo_sqlserver_function_gaps.md`~~ (файл удалён 24.09.2026, shipped). (6) Шаг 5 открыт (SS1).

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.ISqlServerFunctions` — `Supports`/`Render`; `roslyn implementations NextORM.Core.ISqlServerFunctions` — `SqlServerSpecificFunctions`; SQL-gen `SqlServerFunctionsSqlGenerationTests` **9/9** (SQL Server) + `SqlServerFunctionsSqlGenerationTests` **5/5** (SQLite, негативные); core `InMemoryScalarFunctionsTests` **3/3**; контейнерная интеграция SQL Server (`SqlServerSpecificTests.SqlServer*`) **5/5**; полный `~SqlServer` — **358/358** (+22 задокументированных skip).

**Итог.** 🔴 — 0; P0/P1 по именам — нет; 🟡 P2 — SS1–SS2 (трекинг Шага 5, stringly-typed контракт). Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта.

---

### Аудит 24.09.2026 — MariaDB-имена на общей поверхности: `MySqlFunctions` (+19 членов = 47), `MariaDbNativeFunctions`, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — MD1)

**Область (uncommitted).** На существующий публичный `NextORM.Core.MySqlFunctions` добавлены **19** MariaDB-only членов (issue #79): `regexp_instr(string?,string?) -> int?`, `regexp_substr(string?,string?) -> string?`, `regexp_replace(string?,string?,string?) -> string?`, `nvl<T>(T?,T?) -> T?`, `nvl2<T>(T?,T?,T?) -> T?`, `add_months(DateTime?,int) -> DateTime?`, `months_between(DateTime?,DateTime?) -> double?`, `to_char(DateTime?,string?) -> string?`, `to_date(string?,string?) -> DateTime?`, `to_number(string?,string?) -> double?`, `kdf(string?,string?,string?,string?) -> byte[]?`, `xxh3(string?) -> ulong?`, `xxh32(string?) -> long?`, `json_detailed(string?) -> string?`, `json_compact(string?) -> string?`, `next_value_for(string?) -> long?`, `nextval(string?) -> long?`, `setval(string?,long) -> long?`, `lastval(string?) -> long?`. Реализация — `internal sealed MariaDbNativeFunctions : IMySqlFunctions` (без нового типа/интерфейса/DIM): `Supports`/`Render` добавляют эти имена, делегируют наследуемые MySQL-имена и отклоняют `uuid_to_bin`/`bin_to_uuid`. `MySqlNativeFunctions` (MySQL) имена не сообщает ⇒ MySQL их отклоняет. Публичная проза EN+RU обновлена (`docs/guide/provider-specific/mysql.md#mariadb` +RU, `docs/providers/mariadb.md` +RU).

**P0 — нет.** Новых публичных типов нет; `MySqlFunctions` не затеняет, `nvl`/`nvl2`/`to_char`/`xxh3` не конфликтуют в CLR (`CA1716`/`CA1724` чисто).

**P1 по именам — нет.** snake_case/SQL-зеркала (`regexp_instr`, `add_months`, `json_detailed`, `next_value_for`) — принятое DSL-исключение (P1-10). XML-doc полный (build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — 0 ⇒ CS1591 гейтится).

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| MD1 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; поверхность выше | 19 новых публичных членов `MySqlFunctions` не трекаются. `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). Продолжение MY1 | При заморозке внести подписи (точный текст — из анализатора); отдельной записи поверх MY1 не требуется |

ℹ️ **Наблюдения.** (1) **Extend-only:** только новые члены существующего типа; `IMySqlFunctions`/DIM/override не менялись, внешний реализатор `ISqlDialect` компилируется без доработки. (2) Имена последовательностей рендерятся идентификатором (`next value for s`, `nextval(s)`) — строковый аргумент снимает кавычки, документировано. (3) `xxh32` типизирован `long?` (в движке нет reader-пути для `uint`), `xxh3` — `ulong?`; `kdf` — `byte[]?`. (4) `docs/specs/**` internal; публичные ссылки на specs отсутствуют. (5) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0** (MD1).

**Проверка (24.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; SQL-gen MariaDB `~MySqlFunctions` — **13/13** (в т.ч. 4 новых), MySQL-негатив `~MariaDbOnlyFunctions` — **1/1**; core `InMemoryMySqlFunctions` — **2/2**; контейнерно MariaDB `~MariaDbFunctions` — **13/13** на `mariadb:11.4` (regexp/`nvl`/`add_months`/`to_char`/`kdf`/`json_*`/последовательности; `months_between` 12.2+, `to_date` 12.3+, `to_number` 12.2+, `xxh3`/`xxh32` 13.1+ — SQL-gen-only). `roslyn members NextORM.Core.MySqlFunctions` — 47. `find -name 'PublicAPI*.txt'` — **0** (MD1).

**Итог.** 🔴 — 0 (по API); P0/P1 по именам — нет; 🟡 P2 — MD1 (трекинг Шага 5). Поверхность uncommitted/alpha, заморозка (Шаг 5, issue #53) открыта.

---

### Аудит 25.09.2026 — ленивый временный источник (`AsTempTable`/`TempTableSource<TResult>`/`From(TempTableSource)`/`ToBatchSql`), uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — TT4–TT7)

**Область (uncommitted).** Новый публичный тип `NextORM.Core.TempTableSource<TResult>` (`src/nextorm.core/Builders/TempTableSource.cs:16`, `public sealed class`, ctor `internal` `:18`; публичные `Name.get` `:26`, `Options.get` `:29`; явная реализация internal `ITempTableSource.Source` `:33`; сам интерфейс — internal `:41`). Новые публичные члены существующего `TempTableExtensions` (`src/nextorm.core/Builders/TempTableExtensions.cs`): `AsTempTable<TResult>(QueryCommand<TResult>, CreateTableOptions?) -> TempTableSource<TResult>` `:322`, `AsTempTable<TResult>(EntityBuilder<TResult>, CreateTableOptions?)` `:355`, `ToBatchSql<TResult>(QueryCommand<TResult>) -> string` `:389` (актуализировано 25.09.2026; итого `TempTableExtensions` — **29** публичных членов: 15 базовых + 14 configure-перегрузок, добавленных вторым инкрементом 25.09.2026 — см. ниже). Новый overload `DataContextExtensions.From<TResult>(IDataContext, TempTableSource<TResult>) -> EntityBuilder<TableAlias>` (`src/nextorm.core/DataContext/DataContextExtensions.cs:981`). Internal-механика (не поверхность): `ITempTableSource` (`TempTableSource.cs:41`), `FromExpression(ITempTableSource)` (`Expressions/FromExpression.cs:43`, `Table = tempTable.Name` `:46`; маркер `:91`), `QueryCommand.HasTemporaryTableSource`/`CollectTempTableSources` (`Query/QueryCommand.TempTables.cs:14,61`), `IBatchExecutor.RenderTemporaryTableBatch`/`BuildTemporaryTableBatch` (`DataContext/DataContext.cs:296,305`), `NextTempTableName` (`TempTableExtensions.cs:463`).

**P0 — нет.** BCL-конфликтов нет (`CA1716`/`CA1724` чисто): `TempTableSource`/`AsTempTable`/`ToBatchSql` в BCL отсутствуют. `As*`-префикс корректен для ленивой обёртки (`AsEnumerable`/`AsQueryable` против `To*`-терминалов); тип-хэндл назван по образцу `CteDefinition`/`BatchQuery<TResult>`; overload `From(IDataContext, …)` зеркалит существующие `From(QueryCommand<TResult>)`/`From(EntityBuilder<TResult>)`; `Async`-близнец не нужен (`AsTempTable` ничего не исполняет, async-чтение дают `ToListAsync`/`ToAsyncEnumerable`); CA1068/порядок параметров, непустые возвраты и `sealed` соблюдены.

**P1 по именам — нет.**

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| TT4 | P2 (нэйминг/консистентность) | `TempTableExtensions.cs:329` ↔ `Builders/BatchBuilder.cs:226` (`BatchQuery<TResult>.ToSql`) | Два члена «отрендерить батч без исполнения» названы по-разному: `BatchQuery<TResult>.ToSql()` и новый `QueryCommand<TResult>.ToBatchSql()`. `ToBatchSql` согласован с `ToTempTableSql`/`ToTableSql` (CTAS2) и с тем, что `ToSql()` на `QueryCommand<TResult>` отсутствует, но расходится с `ToSql()`-конвенцией `BatchQuery` и остального диагностического surface. Форма принята, но замораживается Шагом 5 | До заморозки либо согласовать (`ToSql()`-перегрузка на `QueryCommand<TResult>`), либо зафиксировать `ToBatchSql` исключением в `<remarks>` и в api-reference; любое переименование — EN+RU доки |
| TT5 | P2 (форма/compile-time) | `TempTableExtensions.cs:322,355` | `AsTempTable` ×2 сохраняют `CreateTableOptions? options = null`; `AsTempTable(null)` компилируется и молча означает default-опции (`options ?? new CreateTableOptions()` `:326`; builder-перегрузка делегирует в query-перегрузку `:359`). Это ровно smell решения 24.09.2026. 25.09.2026 добавлена non-null configure-перегрузка (`Func<CreateTableOptionsBuilder, CreateTableOptionsBuilder>`), но `options = null`-перегрузка не снята; name-less `ToTempTable`/`ToTempTableAsync` (TT1/TT3) в текущей поверхности отсутствуют | Дать пару «без параметра» + обязательный `CreateTableOptions` (`ArgumentNullException.ThrowIfNull`); **+2** публичных члена учесть при заморозке. Либо принять с оговоркой. Кодовая сторона — `code-smells-review.md`, Находка 188 (расширение 14 → **16**) |
| TT6 | P2 (доки/охват) | `docs/advanced/api-reference.md:50` (+RU:50) | Строка `TempTableExtensions` в курируемом api-reference перечисляет `ToTable`/`ToTempTable`/`ToTableSql`/`ToTempTableSql`, но не ленивую форму `AsTempTable`/`TempTableSource<TResult>`/`ToBatchSql`/`From(source)`. `docs/guide/22-create-table-as.md` EN+RU (§«Lazy temporary tables») документирует её полностью ⇒ **не P1**, но индекс неполон | Дополнить строку EN+RU ссылкой на `guide/22` §Lazy |
| TT7 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; `TempTableSource.cs:16,26,29`; `TempTableExtensions.cs:322,355,389`; `DataContextExtensions.cs:981`; плюс 14 configure-перегрузок и `CreateTableOptionsBuilder` (CTAS-раздел, актуализация 25.09.2026) | Новый публичный тип `TempTableSource<TResult>` (+2 свойства) + 2 `AsTempTable` + `ToBatchSql` + overload `From` не трекаются: `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). 25.09.2026: `TempTableExtensions` 23 → **29**, новый `CreateTableOptionsBuilder`, `CreateTableOptions` 4 → **5** свойств. Продолжение TT1/CTAS3/BAT1 | При заморозке внести подписи (точный текст — из анализатора); не переоткрывать TT1/CTAS3 отдельной записью |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 по именам нет.** (2) **XML-doc 100 %:** `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`), `TreatWarningsAsErrors=true`, `NoWarn` — **0** (CS1591 гейтится); у `TempTableSource<TResult>`/`AsTempTable`/`ToBatchSql`/`From` есть `<summary>`/`<typeparam>`/`<param>`/`<returns>`/`<exception>`. (3) **Crefs резолвятся:** сгенерированный `nextorm.core.xml` содержит нормализованные doc-ID — `TempTableSource.cs` `<see cref="M:NextORM.Core.DataContextExtensions.From``1(NextORM.Core.IDataContext,NextORM.Core.TempTableSource{``0})"/>` (xml:4710) и `...TempTableExtensions.AsTempTable``1(...)` (xml:6733); `CS1574` отсутствует (build `0/0`). (4) **Минимальность/закрытость:** `TempTableSource<TResult>` — `sealed` с `internal` ctor и явной реализацией internal-интерфейса ⇒ снаружи конструируется только через `AsTempTable`; публично видны лишь `Name`/`Options`, а non-generic `Source` остаётся движковой деталью. Двойной приёмник `AsTempTable` (builder + готовый `QueryCommand<TResult>`) повторяет принятую ось CTAS (наблюдение 3 прошлого аудита); `ToBatchSql` — только на построенной команде (рендер требует чтения) — корректно. (5) **TT2 уточнён:** удалённый continuation-сахар (`ToTempTableThen`/`ToTableThen`/`ToBatchThen`) заменён именно `AsTempTable` + `From(source)` + `ToBatchSql`, а не «name-less `ToTempTable` + `BatchBuilder`» — формулировку TT2 считать дополненной этой записью. (6) **Plan-cache/hash:** `FromExpression` кладёт `Table = tempTable.Name` (`Expressions/FromExpression.cs:46`), поэтому план-компаратор видит имя через `Table` (уникальный Guid); вдобавок `GetPreparedTemporaryTableCommand` ставит `Cache = false` (`DataContext.cs:287`) и `storeInCache: false` — закэшированной чужой SQL быть не может. Маркер `TempTable` в `FromExpressionPlanEqualityComparer` не сравнивается — безопасно только за счёт уникальности имени и `Cache=false` (ℹ️, не находка). (7) **Тесты:** SQL-gen `TempTableSourceSqlGenerationTests` (sqlite **4/4**; отдельные кейсы в mariadb/sqlserver/clickhouse) + интеграционные `CommonTestSuite.CreateTableAs.cs` (`TempTableSource_*`, **6** кейсов; контейнерно аудитом не перезапускались). (8) **Подавления/слоп:** `src/` — **11/11** (6 `SuppressMessage` + 5 парных `#pragma`), новых в файлах фичи — **0**; `Skip=`/пустых `catch`/`NoWarn`/новых `Task.Delay` — 0; `.editorconfig` — те же 7 инертных `silent`. (9) EOL: `TempTableSource.cs`/`TempTableExtensions.cs` — CRLF.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `dotnet build src/nextorm.core/nextorm.core.csproj -c Release -p:AnalysisLevel=latest-all` — на новых файлах **0** design-rule срабатываний (единственные ошибки — пре-существующие системные `CA1051`/`CA1002`/`CA1040`/`CA1819`/`CA1019`/`CA1805` вне диффа); `roslyn members NextORM.Core.TempTableSource` — ctor `internal`, `Name.get`/`Options.get` публичные, `Source` — explicit internal; `roslyn members NextORM.Core.TempTableExtensions` — **29** public (15 базовых + 14 configure; актуализировано 25.09.2026); `roslyn members NextORM.Core.DataContextExtensions` — новый `From<TResult>(IDataContext, TempTableSource<TResult>)` `:981` (рядом с `From<TResult>(…QueryCommand<TResult>)` `:959`); `find -name 'PublicAPI*.txt'` — **0**; `rg 'SuppressMessage|#pragma warning disable' src/` — **11/11**; `rg 'NoWarn' src/ Directory.Build.props` — **1** (комментарий), эффективно 0; тесты (Release, `--no-build`): sqlite `TempTableSourceSqlGenerationTests` **4/4**, mariadb/sqlserver `TempTableSource*` **1/1** каждый. Внутренние specs — `code-smells-review.md`, Точечный аудит 25.09.2026 (расширение Находки 188).

---

### Аудит 25.09.2026 (второй инкремент) — опции материализации: `CreateTableOptions` (+`DropExisting`) и `CreateTableOptionsBuilder` + 16 configure-перегрузок, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — CTAS8–CTAS9)

**Область (uncommitted).** Второй инкремент поверх ленивого временного источника (аудит 25.09.2026 TT4–TT7 **не переоткрывается**). Переименование и новые публичные члены (номера строк — из `roslyn members`):

- `NextORM.Core.CreateTableAsOptions` → **`NextORM.Core.CreateTableOptions`** (`src/nextorm.core/Builders/CreateTableOptions.cs:22`; старый файл `Builders/CreateTableAsOptions.cs` удалён). **5** `init`-свойств: `IfNotExists` `:25`, **`DropExisting`** (новое) `:33`, `WithData` `:39`, `OnCommit` `:46`, `Columns` `:52`; record-члены; internal `Validate(bool temporary)` `:58` (при `DropExisting && temporary` → `NotSupportedException`), internal `ValidateCombination()` `:69` (`DropExisting && IfNotExists` → `ArgumentException`).
- Новый **`NextORM.Core.CreateTableOptionsBuilder`** (`:82`, `public sealed class`) — **6** публичных методов: `IfNotExists(bool value = true)` `:93`, `DropExisting(bool value = true)` `:106`, `WithData(bool value = true)` `:115`, `OnCommit(TempTableOnCommit value)` `:124`, `Columns(params string[] names)` `:135`, `Build() -> CreateTableOptions` `:156`; internal static `Build(Func<CreateTableOptionsBuilder, CreateTableOptionsBuilder>)` `:175`. Все fluent-методы возвращают `this`; `Build()` уже отвергает `DropExisting`+`IfNotExists`.
- **16** новых публичных configure-перегрузок `Func<CreateTableOptionsBuilder, CreateTableOptionsBuilder>` (**14** в `TempTableExtensions` + **2** в `BatchBuilder`) — полный перечень в обновлённом CTAS-инвентаре выше:
  - `TempTableExtensions` (`Builders/TempTableExtensions.cs`): `ToTempTable` `:43`/`:65`, `ToTempTableAsync` `:91`/`:117`, `ToTempTableSql` `:141`/`:165`, `ToTable` `:187`/`:209`, `ToTableAsync` `:235`/`:261`, `ToTableSql` `:285`/`:309`, `AsTempTable` `:338`/`:371`;
  - `BatchBuilder` (`Builders/BatchBuilder.cs`): `CreateTable` `:89`, `CreateTempTable` `:116` (методы переименованы из `CreateTableAs` `:75` / `CreateTempTableAs` `:88` в `HEAD`).
- **Удалены** 8 name-less `ToTempTable`/`ToTempTableAsync` (аудит 24.09.2026 TT1): в текущей поверхности их нет, функциональность auto-name перешла к `AsTempTable` + `From(source)` + `ToBatchSql`. TT1 в этой части **superseded** (поверхность alpha/uncommitted, SemVer-импакт нулевой).
- Счётчики: `TempTableExtensions` **29** public (15 базовых + **14** configure); `BatchBuilder` **9** public (**4** CTAS + `Insert`/`Update`/`Delete`/`Truncate`/`Query`); `CreateTableOptionsBuilder` **6** public + 1 internal static; `CreateTableOptions` **5** public init-свойств.

**Именование — P0 нет.** Образец — существующий `BulkInsertOptionsBuilder` (fluent-метод ↔ init-свойство): `CreateTableOptionsBuilder.IfNotExists`/`DropExisting`/`WithData`/`OnCommit`/`Columns` зеркалят 5 свойств `CreateTableOptions`, `Build()` — терминал. `DropExisting` — PascalCase, глагол+суффикс, без аббревиатур; BCL-конфликтов нет (`CA1716`/`CA1724`). `configure` — стандартное имя делегата (ср. `BulkInsertOptionsBuilder`). Порядок параметров `(receivers, name, configure/options, CancellationToken)` — CA1068 соблюдён.

**P1 по именам — нет.**

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| CTAS8 | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; `CreateTableOptions.cs:22-69,82-175`; `TempTableExtensions.cs:43,65,91,117,141,165,187,209,235,261,285,309,338,371`; `BatchBuilder.cs:89,116` | Новый публичный тип `CreateTableOptionsBuilder` (6 членов), **16** configure-перегрузок и свойство `DropExisting` не трекаются: `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). Продолжение CTAS3/TT1/TT7 | При заморозке внести подписи (точный текст — из анализатора); отдельной записи поверх CTAS3/TT7 не требуется |
| CTAS9 | P2 (контракт/форма) | `CreateTableOptions.cs:58,69`; `BatchBuilder.cs:75,102` | Взаимоисключение `DropExisting`+`IfNotExists` проверяется только на `Build()`/`Validate` (`ArgumentException`), а не типом; `DropExisting` на temp-цели → `NotSupportedException` при построении SQL. Форма **принята** (согласована с контрактом «опция не поддержана ⇒ `NotSupportedException` при рендере», прецеденты `OnCommit`/`Columns`/`WithData`); ошибка ловится до БД (в т. ч. в `ToTempTableSql`/`ToTableSql` и в `BatchBuilder`) | Не менять форму; при желании дополнить XML-doc `<exception>` указанием на детерминированный отказ `DropExisting`+`IfNotExists`. Публичную форму не трогать |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 по именам нет.** (2) **Extend-only:** `CreateTableOptionsBuilder` — новый `sealed`-тип; 16 configure-перегрузок аддитивны; `DropExisting` — новое init-свойство record'а (source/binary-совместимо для alpha). Удаление 8 name-less — замена pre-release API, SemVer-импакт нулевой. (3) **XML-doc 100 %:** build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — **0** (CS1591 гейтится); у новых членов есть `<summary>`/`<param>`/`<returns>`/`<exception>`. (4) **`Validate`/`ValidateCombination` — internal:** в публичную поверхность не входят, но задают контракт (`ArgumentException`/`NotSupportedException`); вызываются из рендера CTAS. (5) **`Build()` (public) vs `Build(Func<…>)` (internal static)** — разные по видимости перегрузки, не `CS0111`. (6) **Публичные доки EN+RU:** `guide/22`/`api-reference` перечисляют options-API; после переименования `CreateTableAsOptions` → `CreateTableOptions` нужно убедиться, что EN+RU не содержат старого имени (AGENTS.md: `docs/**` **и** `docs/ru/**`). (7) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0** (CTAS8). (8) Кодовая сторона — `code-smells-review.md`, точечный аудит 25.09.2026 (второй инкремент); Находка 188 не расширяется (16 без изменений).
...
## Аудит 25.09.2026 — трансляция захваченной коллекции по индексу (`dict[column]`) (uncommitted worktree; публичной поверхности нет; P0/P1/P2 — нет)

**Область.** `src/nextorm.core/Query/DictionaryLookup.cs` (`internal readonly struct LookupEntry`, `internal static class DictionaryLookup`), `src/nextorm.core/Visitors/DictionaryLookupTranslator.cs` (`internal static`), `Query/InValues.cs` (`ComputeShapeHash` + `out bool hasMatch`, `ShapeVisitor`), `Query/QueryCommand.cs` (+`internal LookupPartitions`/`ShapeScanned`), `Query/QueryCommand.Clone.cs`/`QueryCommand.QueryPreparer.cs`, `Visitors/BaseExpressionVisitor.cs` (`EmitFoldedParameter` `private`→`internal`). Тесты: SQL-gen в 6 проектах (sqlite **7**, остальные по **2**), `CommonTestSuite.DictionaryLookup.cs` (**4**), `InMemoryTests.TestDictionaryLookup` (**1**).

**P0/P1/P2 по публичному API — нет.** Новые типы и члены — `internal`: `LookupEntry`, `DictionaryLookup`, `DictionaryLookupTranslator`, `QueryCommand.LookupPartitions`/`ShapeScanned`; `BaseExpressionVisitor.EmitFoldedParameter` был `private`, стал `internal` (не `public`/`protected`) — публичная/protected-поверхность не менялась. Именование согласовано с соседями: `DictionaryLookup` ↔ `InValues`, `DictionaryLookupTranslator` ↔ `InValuesTranslator`, `LookupPartitions` ↔ `InValuesPartitions`, `TryTranslate`/`TryGetLookup`/`Evaluate` — конвенции реестра; BCL-конфликтов нет. XML-doc новых internal-типов полный (build Release `0/0`, CS1591 гейтится, `NoWarn` — 0); устаревание доков соседних полей (`HasTopLevelInValues` и др.) — внутренняя сторона, см. `code-smells-review.md` Находку 193. `Async`-суффикс/CA1068/порядок параметров не затронуты.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; новых `public`/`protected` членов в фиче нет (все объявления новых типов — `internal`, полей — `internal readonly`); `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт). Дефекты поведения — в `code-smells-review.md` (Находки 191–194).

## Аудит 25.09.2026 — конвертеры значений и JSON-колонки (`IPropertyValueConverter`/`ValueConverter<,>`/`ValueConverterAttribute`/`JsonColumnAttribute`/`JsonColumnStorage`/`JsonColumnOptions`/`JsonColumnConverter<,>`), uncommitted worktree (P0 — нет; P1 по именам — нет; P2 — VC1–VC3)

**Область.** `src/nextorm.core/DataContext/Meta/ValueConverter.cs` (новый: `public interface IPropertyValueConverter`, `public abstract class ValueConverter<TModel,TProvider>`, `public sealed class ValueConverterAttribute`), `src/nextorm.core/DataContext/Meta/JsonColumnConverter.cs` (новый: `public enum JsonColumnStorage`, `public sealed class JsonColumnAttribute`, `public sealed class JsonColumnOptions`, `public sealed class JsonColumnConverter<TModel,TProvider>`; `internal IJsonColumnConverter`/`JsonColumnConverterFactory`/`JsonColumnAutoConverter<T>`), `IPropertyMetadata.Converter` (DIM `=> null`), `PropertyMetadata.Converter`, `EntityMetadataBuilder`/`EntityPropertyBuilder` (атрибут + `.HasConversion`×3 + `.JsonColumn`), `SelectExpression.ProviderType`/`Converter`, `RowMapperFactory`, `DurationStorage.ToParameterValue`, `SelectExpressionPlanEqualityComparer`. Тесты: core **19**, SQL-gen в 5 проектах по **3**, интеграция `CommonTestSuite.ValueConverters.cs`/`JsonColumn.cs` (**4** SQLite-кейса).

**P0 — нет.** BCL-конфликтов нет (`CA1716`/`CA1724` чисто): интерфейс назван `IPropertyValueConverter`, а не `IValueConverter` (не пересекается с `System.Windows.Data.IValueConverter`); JSON-тип назван `JsonColumnConverter<,>`, а не `JsonConverter<,>` (не пересекается с `System.Text.Json.Serialization.JsonConverter<T>`); `ValueConverter<,>`/`JsonColumnStorage`/`JsonColumnOptions` в BCL отсутствуют. Атрибут `ValueConverterAttribute` разрешается в `[ValueConverter(...)]` рядом с generic-типом `ValueConverter<,>` без неоднозначности (разные арности).

**P1 по именам — нет.** `ValueConverter<TModel,TProvider>`/`JsonColumnConverter<TModel,TProvider>` — тип-параметры `TModel`/`TProvider` (по образцу маппинга), `JsonColumnStorage.{Auto,Native,Text}` — PascalCase, `HasConversion`/`JsonColumn` — глагольные fluent-методы; параметр `converter`/`configure` единообразен; `Async`-суффикс/CA1068 не затрагиваются.

**P2 — VC1–VC3.** (VC1) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0**. (VC2) Константы в предикатах/скалярных проекциях (`Where(x => x.Status == Status.Active)`, `Select(x => x.Status)`) **не конвертируются** — фаза 2 issue [#31]; задокументировано в `docs/guide/30-value-converters.md` (+RU). (VC3) `JsonColumnConverter<,>` публично связывает `System.Text.Json.JsonElement` для нативного хранения (25.09.2026: нативное представление переведено с disposable `JsonDocument` на non-disposable `JsonElement` — `code-smells-review.md`, Находки 195/196); для AOT/source-gen (`JsonTypeInfo<T>`) — отдельная фаза.

**Extend-only.** Все типы новые; `IPropertyMetadata.Converter` — DIM `=> null` (внешние реализаторы компилируются); `EntityPropertyBuilder` — аддитивные методы; `SelectExpression.ProviderType`/`Converter` — новые `public`-свойства (тип `sealed`); `ValueConverterAttribute` не несёт `ConvertsNulls` (единственный источник — экземпляр конвертера). Новые `internal`-типы (`IJsonColumnConverter`, `JsonColumnAutoConverter<T>`, `JsonColumnConverterFactory`, `ValueConverterReflection`, `ExpressionValueConverter<,>`) в публичную поверхность не входят.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; XML-doc у всех новых `public`-членов (CS1591 гейтится, `NoWarn` — 0); доки EN+RU: `docs/guide/30-value-converters.md` (+RU, +`toc.yml`), `docs/advanced/api-reference.md` (+RU); `find -name 'PublicAPI*.txt'` — **0**.

**Независимый аудит (25.09.2026, `nextorm-code-auditor`).** Подтверждено: P0/P1 по неймингу/BCL-конфликтам — нет; **7** новых `public`-типов (`IPropertyValueConverter`, `ValueConverter<TModel,TProvider>`, `ValueConverterAttribute`, `JsonColumnStorage`, `JsonColumnAttribute`, `JsonColumnOptions`, `JsonColumnConverter<TModel,TProvider>`) и новые члены `EntityPropertyBuilder`/`SelectExpression`/`IPropertyMetadata` полностью покрыты XML-doc — CS1591 реально гейтится (`src/Directory.Build.props:11` `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`), build `0/0`; EN+RU доки обновлены; расширение extend-only (DIM `=> null`, аддитивные методы/свойства). Поверхность не заморожена (`PublicAPI*.txt` — **0**, Шаг 5 / issue #53 открыт). Дефекты жизненного цикла/контракта — `code-smells-review.md`, Находки **195–197** (JsonDocument read/write disposal; fluent `HasConversion` без валидации модели). P2 VC1–VC3 — без изменений. **Доработка 25.09.2026 (`nextorm-design-engineer`):** Находки 195/196 закрыты переводом нативного JSON на non-disposable `JsonElement` (write через `SerializeToElement`, read через `GetFieldValue<JsonElement>`), Находка 197 — валидацией модели конвертера в `EntityPropertyBuilder<T>.Build()`; build Release `0/0`, тесты core/postgres/sqlite/sqlserver зелёные, нативный `jsonb`-round-trip подтверждён на реальном PostgreSQL (4/4). Смена нативного представления `JsonDocument` → `JsonElement` — pre-release (Шаг 5 не заморожен), учтена для заморозки.

## Аудит 25.09.2026 — PostgreSQL range-типы и `overlaps` (G11, uncommitted worktree)

**Область.** Новый публичный value-тип `NextORM.Core.Range<T>` (`Query/Range.cs`); публичные члены
`NextORM.Core.PostgresFunctions` (`Query/SqlFunctions.Postgres.cs`, класс теперь `sealed`); DIM
`ISqlDialect.SupportsRanges => false` (+ `SqlDialectBase` override) — PG-only рендер без новых
`MakeRange*`; PostgreSQL-only транслятор `Visitors/PostgresRangeSqlTranslator.cs`; read/write-хуки и
конвертация в `src/nextorm.postgres/{PostgresDataContext,PostgresDialect,PostgresRange}.cs`; in-memory
`InMemoryScalarFunctionRewriter`/`InMemoryScalarFunctions`.

**Публичные подписи.** `Range<T> where T : struct, IComparable<T>`: конструкторы
`(T,T)`, `(T,T,bool,bool)`, `(T,T,bool,bool,bool,bool)`, `(T?,T?,bool,bool,bool,bool,bool)`; свойства
`Lower`/`Upper`/`LowerInclusive`/`UpperInclusive`/`LowerInfinite`/`UpperInfinite`/`IsEmpty`; `static Empty`;
`IEquatable<Range<T>>` + `==`/`!=`/`GetHashCode`/`ToString`. `PostgresFunctions` (все
`where T : struct, IComparable<T>`): `overlaps`, `range_contains(Range<T>,T)`,
`range_contains(Range<T>,Range<T>)`, `range_contained_by`, `range_union`, `range_intersection`,
`range_difference`, `range_adjacent`, `range_strictly_left_of`, `range_strictly_right_of`,
`range_not_extend_right_of`, `range_not_extend_left_of`, `isempty`, `lower`, `upper`, `lower_inc`,
`upper_inc`, `lower_inf`, `upper_inf`, `empty_range<T>()`, `int4range`/`int8range`/`numrange`/
`tsrange`/`tstzrange`/`daterange` (по 2 перегрузки: без bounds и с `string bounds`).
`ISqlDialect.SupportsRanges` — default interface method `=> false`.

**Именование — P0/P1 нет.** Только generic `Range<T>` (не-generic `Range` дал бы `CS0104` с
`System.Range` и боксинг). Операторы/функции названы SQL-токенами в lower_snake (`overlaps`,
`range_contains`, `range_contained_by`), как `array_contains`/`array_overlaps` в том же файле; голый
`Contains` не используется (коллизия с полнотекстовым `CommonFunctions.contains`). Форма API
зафиксирована как **static** (instance-форма `x.During.Overlaps(...)` отклонена: `Range<T>` не знает
диалекта и не может отрендерить SQL). `lower`/`upper`/`isempty`/`lower_inc`/`upper_inc`/`lower_inf`/
`upper_inf` повторяют имена PostgreSQL-функций. `sealed` у `PostgresFunctions` — лист без наследников
(проверено `roslyn refs`); расширение наследника — поведенческое (компиляция внешнего
`class X : PostgresFunctions` станет ошибкой) и фиксируется в реестре как осознанное.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**.
Fast-тесты: `tests/nextorm.postgres.tests` — **488** passed (вкл. `RangeOverlaps`/`RangeContains`/
`RangeConstructors`/`RangeFunctions`/`RangeOperators`, gate-тесты `PostgresDialectTests`),
`tests/nextorm.core.tests` — **337** passed (вкл. `RangeTests`, in-memory range), `tests/nextorm.sqlite.tests`
— **465** passed (вкл. `PostgresRangeRejectionTests`). Интеграция PG — **2/2**
(`Range_RoundTripAndOverlaps_ShouldPreserveBounds`) в контейнере (по прогону оператора). EOL — CRLF.

**Верификация аудита (`nextorm-code-auditor`, 25.09.2026).** Независимо воспроизведено: `dotnet build nextorm.slnx -c Release` — **0/0**; `--filter "FullyQualifiedName~Range"` (Release, `--no-build`): core **10/10**, postgres **10/10**, sqlite `PostgresRangeRejection` **2/2** (0 failed / 0 skipped); интеграционный PG-кейс — **2/2** по прогону оператора (аудит контейнеры не перезапускал). **P0/P1 по именам — нет**, подтверждено:

- `Range<T>` — единственный публичный тип; generic-имя не конфликтует с `System.Range` (разная арность, bare `Range` не добавлен); BCL-конфликтов/аббревиатур нет; `IEquatable<Range<T>>` + `==`/`!=`/`GetHashCode` согласованы, `default` == `Empty`.
- SQL-токены в lower_snake (`overlaps`, `range_contains`, `range_contained_by`, `range_union`, …) — та же конвенция, что `array_contains`/`array_overlaps` (`SqlFunctions.Postgres.cs`); голый `Contains` не введён (коллизия с полнотекстовым `CommonFunctions.contains`); `int4range`/`int8range`/… и `lower`/`upper`/`isempty`/`lower_inc`/`lower_inf` зеркалят PostgreSQL-имена; `empty_range<T>()` — обёртка над литералом `'empty'::<range type>`. Форма API — **static** на `PostgresFunctions` (instance-форма `x.During.Overlaps(...)` невозможна: `Range<T>` не знает диалекта); `where T : struct, IComparable<T>` на всех членах.
- `ISqlDialect.SupportsRanges => false` — DIM (обратная совместимость внешних реализаций), `SqlDialectBase`/`PostgresDialect` override; XML-doc полный.
- `PostgresFunctions` (был `public class`) → **`sealed`**: source-break для внешних наследников, наследников в репозитории **0** (`roslyn implementations`), pre-1.0 alpha — **принято осознанно**.
- Неймспейс/`Async`-суффикс/CA1068/порядок параметров — не затронуты; XML-doc **100 %** (CS1591 гейтится сборкой, build `0/0`).

**Шаг 5 (заморозка `PublicAPI.*.txt`, issue #53) — P2 открыт; новых публичных членов ≈52** (`Range<T>` ≈19 doc-ID-строк: тип, 4 конструктора, 7 свойств, `Empty`, `Equals`×2, `GetHashCode`, `==`/`!=`, `ToString`; `PostgresFunctions` **32** метода; `ISqlDialect.SupportsRanges` **1**). В `PublicAPI.Unshipped.txt` при заморозке должны попасть все они; `EnablePackageValidation`/ApiCompat не подключены. Кодовые дефекты фичи — `code-smells-review.md`, Находки **195** (`Range<T>.Empty` vs `lower_inf('empty')`) и **196** (дублирующие type-map).

**Правки дизайн-инженера (`nextorm-design-engineer`, 25.09.2026, worktree `b1-pg-ranges`).** Находки `code-smells-review.md` 195 (семантика `Range<T>.Empty` vs `lower_inf`/`upper_inf`) и 196 (единая карта «bound type → PG range name + `NpgsqlDbType`») закрыты **без изменений публичных подписей**: `Range<T>` сохранил набор свойств (изменилось только поведение `LowerInfinite`/`UpperInfinite` на пустом диапазоне — приведено к PostgreSQL), `PostgresRangeTypes` остался `internal`, `PostgresRange.DbTypeFor` удалён (internal, единственный вызов — внутри провайдера). Новых публичных членов/переименований нет — учёт Шага 5 (≈52 doc-ID-строки) без изменений. Детали и прогоны — `code-smells-review.md`, блок «Правки дизайн-инженера» в разделе аудита PostgreSQL range-типов.

## Аудит 25.09.2026 — динамическая схема результата TVF (ClickHouse `values`, PostgreSQL `jsonb_to_record(set)`, issue #63; uncommitted worktree)

**Область.** `SqlTableFunctionAttribute` (+новый публичный enum `TableFunctionSchema` и init-свойство `ResultSchema`), `TableFunctionExpression` (+`ResultType`/`ResultSchema`), `FromExpressionPlanEqualityComparer` (схема в ключе плана), `ISqlDialect` (+DIM `SupportsTypedResultSchema`/`MakeResultColumnType`/`MakeTableFunctionAlias`), `SqlDialectBase` (virtual-двойники), `ClickHouseDialect`/`PostgresDialect` (override + гейты `values`/`jsonb_to_record`/`jsonb_to_recordset`), `SqlSourceRenderer.MakeTableFunction`/`RenderResultSchema`, `SqlFunctions.ClickHouse.values<TRow>`/`SqlFunctions.Postgres.jsonb_to_record<TRow>`/`jsonb_to_recordset<TRow>`.

**P0/P1 по именам — нет.** `TableFunctionSchema`/`ResultSchema` согласованы с `WithClause`/`CallClause`/`VerbatimArguments`; `SupportsTypedResultSchema`/`MakeResultColumnType`/`MakeTableFunctionAlias` следуют конвенции `Supports*`/`Make*`; имена `values`/`jsonb_to_record`/`jsonb_to_recordset` — SQL-токены (P1-10). Новые DIM source/binary-совместимы. Открыт P2 **DRS1** — трекинг `PublicAPI.*.txt` (Шаг 5).

**Проверка.** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; SQL-gen: clickhouse 328, postgres 483, core 332; интеграция (без `DOCKER_HOST`) — PG/CH классы skipped (оркестратор запустит контейнеры).

### Дополнение (code-auditor, 25.09.2026)

**P2 DRS2 — коллизия имён `TableFunctionSchema` и `SqlTableFunctionAttribute.Schema`.** `src/nextorm.core/SqlTableFunctionAttribute.cs:9` (enum), `:66` (свойство `Schema`), `:107` (свойство `ResultSchema`). Свойство `Schema` атрибута — это SQL-владелец (`schema.name`), а новый публичный enum `TableFunctionSchema` описывает **место** рендера результата, не SQL-схему; читатель `ResultSchema` типа `TableFunctionSchema` может принять тип за имя схемы. Имена членов (`None`/`LeadingArgument`/`AliasColumnList`) однозначны — неоднозначен только тип. Предложение: `TableFunctionResultSchema` либо `ResultSchemaPlacement`; свойство `ResultSchema` менять не обязательно. Замена на месте (alpha, Шаг 5 открыт); при переименовании — EN+RU-доки.

**ℹ️ DRS3 — паритет доков EN/RU.** Новые члены отражены в `docs/advanced/api-reference.md` (+RU), `docs/advanced/limitations.md` (+RU), `docs/guide/13-table-valued-functions.md` (+RU), `docs/guide/provider-specific/postgresql.md` (+RU), `docs/providers/clickhouse.md` и `docs/providers/postgres.md`. Зеркала `docs/ru/providers/clickhouse.md`/`postgres.md` в diff **отсутствуют** и `values`/`jsonb_to_record`/`SupportsTypedResultSchema` в них не упомянуты (проверено текстовым поиском; RU-руководство обновлено). Дополнить RU-провайдерные страницы. **✅ Закрыто (25.09.2026, design-engineer):** `docs/ru/providers/clickhouse.md` дополнен `values` (буллет + строка таблицы), `docs/ru/providers/postgres.md` — `jsonb_to_record`/`jsonb_to_recordset` (текст + строка таблицы); устаревшие xref-и `SupportsTypedResultSchema` заменены на `SupportsResultSchema(TableFunctionSchema)` во всех EN+RU-страницах (`api-reference`, `guide/13`, `providers`).

**XML-doc coverage.** `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` + `NoWarn` — **0**, поэтому CS1591 гейтится сборкой: Release `0/0` доказывает, что у всех новых `public`/`protected` членов (enum `TableFunctionSchema` и его члены, `SqlTableFunctionAttribute.ResultSchema`, `TableFunctionExpression.ResultType`/`ResultSchema`, 3 DIM `ISqlDialect`, 3 `virtual` `SqlDialectBase`, override'ы ClickHouse/PostgreSQL, `values<TRow>`/`jsonb_to_record<TRow>`/`jsonb_to_recordset<TRow>`) есть `<summary>`. Новых публичных типов без доков нет → Приложение A без изменений. `PublicAPI.*.txt` — **0** (Шаг 5 открыт, **DRS1**).

**Проверка (code-auditor).** `dotnet build nextorm.slnx -c Release` — **0/0**; unit Release `--no-build`: core `DynamicResultSchemaTests` **5/5**, clickhouse `TableFunction_Values*` **2/2**, postgres `TableFunction_JsonbToRecord*` **2/2** (0 failed / 0 skipped). Контейнерная интеграция (CH `Values_WithDeclaredSchema`, PG `JsonbToRecord`/`JsonbToRecordset`, **3/3**) — по данным запуска исполнителя, аудитом не перезапускалась. Дефекты — в `code-smells-review.md` (Находки 195–197).

### Дополнение (design-engineer, 25.09.2026)

**Per-placement capability вместо единого bool (в рамках фикса Находки 196).** `SupportsTypedResultSchema` заменён на `bool SupportsResultSchema(TableFunctionSchema placement) => false` (`ISqlDialect.cs:1050`, DIM; `SqlDialectBase.cs:701-702`); ClickHouse отвечает `true` только для `LeadingArgument` (`ClickHouseDialect.cs:385`), PostgreSQL — только для `AliasColumnList` (`PostgresDialect.cs:236`). Один bool не различал два ортогональных размещения, из-за чего пользовательская TVF с непарной формой молча рендерилась без списка колонок; теперь `RenderResultSchema` гейтит конкретную форму (`SqlSourceRenderer.cs:639-641`) и бросает `NotSupportedException`. Имя следует конвенции `Supports*` (`SupportsTableFunction(string)`); параметр типизирован `TableFunctionSchema`. Source/binary-разрыв — только внутри незакоммиченной фичи (CSV-поверхность новой фичи, `PublicAPI.*.txt` ещё не заведён, **DRS1** открыт). **DRS2** (имя enum) остаётся открытым.

**✅ Проверка (design-engineer, 25.09.2026).** Build Release — **0/0**; unit Debug: core **332/332**, clickhouse **329/329**, postgres **484/484** — 0 failed/0 skipped (негативные тесты формы — `code-smells-review.md`, Находка 196); интеграция без `DOCKER_HOST` (`-method "*DeclaredSchema*"`) — 3 skipped (CH/PG). XML-doc нового DIM полон (`<summary>`/`<param>`/`<returns>`), cref-и в `SqlFunctions.ClickHouse.cs`/`SqlFunctions.Postgres.cs` обновлены, build `0/0` подтверждает отсутствие CS1591/CS1574.
...
## Аудит 25.09.2026 — конвертеры значений, фаза 2 (`EnumToStringConverter<TEnum>`, закрытие VC2); uncommitted worktree (P0 — нет; P1 по именам — нет; P2 — VC1/VC3/VC4)

**Область.** Новый публичный тип `src/nextorm.core/DataContext/Meta/EnumToStringConverter.cs` — `public sealed class EnumToStringConverter<TEnum> : ValueConverter<TEnum,string>` (`where TEnum : struct, Enum`), два override `ConvertToProvider`/`ConvertFromProvider`. Изменения фазы 2 в `Visitors/*` и `Query/QueryCommand.QueryPreparer.cs` новых `public`/`protected` членов не добавляют (только `internal`/`private`) — проверено diff'ом. Доки EN+RU: `docs/guide/30-value-converters.md` (+RU), `docs/advanced/api-reference.md` (+RU), ~~`docs/specs/roadmap/todo_value_converters.md`~~ (файл удалён 25.09.2026).

**P0 — нет.** BCL-конфликтов нет: `EnumToStringConverter` в BCL отсутствует; CA1716/CA1724 чисты. Тип-параметр `TEnum` с ограничением `struct, Enum` — корректная конвенция. XML-doc полный (CS1591 гейтится, `NoWarn` — 0, build `0/0`).

**P1 по именам — нет.** Суффикс `Converter` согласован с `ValueConverter<,>`/`IPropertyValueConverter`/`JsonColumnConverter<,>`; `ConvertToProvider`/`ConvertFromProvider` — имена базовых абстракций; `Async`-суффикс/CA1068/порядок параметров не затронуты.

**P2 — VC4 (новый) и статус VC1/VC2/VC3.**
- **VC4 (P2, новый публичный тип вне заявленного scope).** Фаза 2 заявлена как «нового публичного API нет», но `EnumToStringConverter<TEnum>` — новый публичный тип (`src/nextorm.core/DataContext/Meta/EnumToStringConverter.cs:11`). Он покрыт XML-doc и доками EN+RU, но **должен попасть в инвентарь заморозки** (VC1): при Шаге 5 `PublicAPI.Unshipped.txt` получит 4 doc-ID-строки (`NextORM.Core.EnumToStringConverter<TEnum>`, `.ctor`, `.ConvertToProvider(TEnum) -> string?`, `.ConvertFromProvider(string?) -> TEnum`). Имя совпадает с одноимённым конвертером EF Core `Microsoft.EntityFrameworkCore.Storage.ValueConversion.EnumToStringConverter<TEnum>` (другой namespace): при импорте обоих namespace'ов возникает неоднозначность. nextorm уже уводил `IValueConverter`/`JsonConverter` от коллизий; предложение — принять (EF Core — не BCL, конфликт только при явном `using`) с записью в доки, либо назвать `EnumNameConverter<TEnum>`. Замена на месте (alpha, Шаг 5 не заморожен); при переименовании — EN+RU-доки.
- **VC2 — ✅ закрыт.** Фаза 1 отмечала: «Константы в предикатах/скалярных проекциях (`Where(x => x.Status == Status.Active)`, `Select(x => x.Status)`) не конвертируются». Фаза 2 конвертирует константу/захваченное значение в сравнении (`BaseExpressionVisitor.VisitConstant`/`TryAddCapturedParameter` через `NormalizeParameterValue`), элементы `IN`/`Contains` (`InValuesTranslator`) и несёт `ProviderType`/`Converter` в скалярную/анонимную/member-init проекцию (`QueryPreparer.ResolveConverter`). Дефекты реализации — `code-smells-review.md`, Находки 199–201.
- **VC1 — P2 открыт.** Шаг 5 (issue #53) не заморожен: `find -name 'PublicAPI*.txt'` — **0**; ApiCompat/`EnablePackageValidation`/API-approval-теста нет. Инвентарь пополнен VC4 (≈4 doc-ID-строки).
- **VC3 — без изменений** (нативный JSON — non-disposable `JsonElement`).

**Extend-only.** `EnumToStringConverter<TEnum>` — новый тип (расширение), существующие сигнатуры не менялись; `internal`-сторона фазы 2 (`ConverterContext`, `ResolvedConverter`, `NormalizeParameterValue`, `ConvertToProviderValue`) в публичную поверхность не выходит.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; единственный новый публичный тип — `EnumToStringConverter<TEnum>`, XML-doc полный; `NoWarn` — **0**; `find -name 'PublicAPI*.txt'` — **0**. Release-тесты: core `EnumToStringConverterTests`+`ValueConverterTests` **14/14**, postgres `ValueConverterSqlGenerationTests` **9/9**. Публичные доки EN+RU обновлены.

## Аудит 25.09.2026 — PostgreSQL multirange, фаза 2 range-поверхности (uncommitted worktree) — P0 — нет; P1 по именам — нет; P2 — RM1 (Шаг 5), ℹ️ — RM2–RM4

**Область.** Продолжение range-поверхности (фаза 1 — раздел «Аудит 25.09.2026 — PostgreSQL range-типы и `overlaps`» выше): **30** новых публичных членов `NextORM.Core.PostgresFunctions` (`src/nextorm.core/Query/SqlFunctions.Postgres.cs:960-1157`), публичного типа представления многодиапазона `Range<T>[]` и внутренних multirange-хуков в `InMemoryScalarFunctions`/`InMemoryAggregates`/`InMemoryScalarFunctionRewriter`, `src/nextorm.postgres/{PostgresRange,PostgresDialect,PostgresDataContext}.cs`. Новых публичных **типов** нет; `PostgresRangeTypes.MultirangeNameFor`/`MultirangeDbTypeFor` и `PostgresRange.ToDriverMultirange`/`ToRanges` — `internal`. Доки EN+RU обновлены.

**Публичные подписи (30 членов, все `where T : struct, IComparable<T>`).** Агрегаты: `range_agg<T>(Range<T>) -> Range<T>[]` (`:983`), `range_intersect_agg<T>(Range<T>) -> Range<T>[]` (`:989`). Конструктор/слияние: `multirange<T>(Range<T>) -> Range<T>[]` (`:964`), `range_merge<T>(Range<T>, Range<T>) -> Range<T>` (`:971`), `range_merge<T>(Range<T>[]) -> Range<T>` (`:977`). Предикаты (multirange-перегрузки): `overlaps` ×3 (`:996`, `:1003`, `:1010`), `range_contains` ×4 (`:1017`, `:1024`, `:1031`, `:1038`), `range_contained_by` ×3 (`:1045`, `:1052`, `:1059`), `range_union` (`:1066`), `range_intersection` (`:1073`), `range_difference` (`:1080`), `range_adjacent` (`:1115`), `range_strictly_left_of` (`:1087`), `range_strictly_right_of` (`:1094`), `range_not_extend_right_of` (`:1101`), `range_not_extend_left_of` (`:1108`). Inspection: `isempty` (`:1121`), `lower` (`:1127`), `upper` (`:1133`), `lower_inc` (`:1139`), `upper_inc` (`:1145`), `lower_inf` (`:1151`), `upper_inf` (`:1157`). Представление: PostgreSQL multirange ↔ `Range<T>[]`.

**P0 — нет.** BCL-конфликтов нет: `multirange`, `range_agg`, `range_intersect_agg`, `range_merge` отсутствуют в BCL; `Range<T>` (арность 1) не конфликтует с `System.Range` (фаза 1, не переоткрывается); CA1716/CA1724 чисты. XML-doc у всех **30** членов полный (`<summary>`/`<typeparam>`/`<param>`/`<returns>`, CREf-ов нет) — `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` + `NoWarn` — **0** ⇒ CS1591 гейтится сборкой, Release `0/0` это доказывает.

**P1 по именам — нет.** SQL-токены в lower_snake (`multirange`, `range_merge`, `range_agg`, `range_intersect_agg`) — та же конвенция, что `array_agg`/`json_agg`/`bit_and` в этом файле; перегрузки `overlaps`/`range_contains`/`range_contained_by` сохраняют имена фазы 1 (голый `Contains` не введён). Порядок параметров согласован (`inner`/`outer` — по роли, как в phase-1 `range_contained_by(inner, outer)`); `where T : struct, IComparable<T>` единообразно; `Async`-суффикс/CA1068 не затронуты. Форма — add-only (extend-only): ни одна подпись фазы 1 не изменена, добавлены только перегрузки; `range_agg`/`range_intersect_agg` — instance-методы на `PostgresFunctions`, как `array_agg`/`json_agg`. Возврат массива (`Range<T>[]`) использует существующий прецедент файла (`array_agg<T> -> T[]?`, `regexp_split_to_array -> string[]`), а не `IReadOnlyList` — консистентно с array-поверхностью (`ISqlDialect.SupportsArrays`).

**P2 — RM1 (открыт).** Шаг 5 (issue #53) не заморожен: `find -name 'PublicAPI*.txt'` — **0**, ApiCompat/`EnablePackageValidation`/API-approval-теста нет. Фаза 2 добавляет **30** doc-ID-строк в `PublicAPI.Unshipped.txt` (по одной на метод) к ≈52 фазы 1 ⇒ инвентарь заморозки **≈82** doc-ID-строки. Отдельно заморозить представление публичного многодиапазона как `Range<T>[]` (не только имена): при возможном переходе на `IReadOnlyList<Range<T>>` это source/binary-разрыв — зафиксировать выбор (см. `code-smells-review.md`, Находка 205). Новых публичных типов нет → Приложение A без изменений.

**ℹ️ RM2.** `range_agg`/`range_intersect_agg` не имеют filtered-перегрузки `(…, Expression<Func<bool>> filter)`, которая есть у `array_agg` (`SqlFunctions.Postgres.cs:279`). In-memory всё равно отклоняет filtered-агрегаты (`InMemoryGroupBy.cs:30-31`), поэтому это feature-пробел, не дефект нейминга; решение — `implementing-todo-features`, не переименование.

**ℹ️ RM3.** Доки EN+RU обновлены параллельно: `guide/provider-specific/postgresql.md` §Multiranges (+RU), `providers/postgres.md` (+RU), `advanced/limitations.md` (+RU). Анкоры корректны (`#multiranges` в EN, `#multirange` в RU — совпадают с заголовками); таблица «range types» намеренно оставлена range-only, multirange вынесен в отдельный раздел. Ссылок на `docs/specs/**` из публичных доков нет (проверено).

**ℹ️ RM4 (для заморозки).** Нуллабилити выбора: `range_agg`/`range_intersect_agg` возвращают **не**-nullable `Range<T>[]` (PG возвращает пустой multirange при отсутствии не-null входов), тогда как `array_agg` возвращает `T[]?` (PG возвращает NULL). Различие осознанное и корректное; зафиксировать в doc-ID-строке (`Range<T>[]` без `?`).

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; XML-doc **30/30** новых членов (CS1591 гейтится); `find -name 'PublicAPI*.txt'` — **0**; `NoWarn` — **0**. Release-тесты `--no-build`: postgres `~Multirange` — **2/2**; core `~InMemoryScalarFunctions` — **14/14**; контейнерная интеграция PG (`DOCKER_HOST` = Podman-сокет) — **Total 3, Failed 0, Skipped 0** (`Multirange_RoundTripAndOperators`, `RangeAggregates_ShouldAggregateRanges`, `Range_AllTypes_RoundTrip`). Доки EN+RU обновлены; дефекты кода — `code-smells-review.md`, Находки 204–206; Находки 1–203 (в т.ч. VC4/DRS2) не переоткрываются.

## Аудит 25.09.2026 — `[RangeColumns]`: `Range<T>` парой скалярных колонок (`RangeColumnsAttribute`/`RangeColumnsMetadata`/`EntityPropertyBuilder<T>.RangeColumns`, DIM `IPropertyMetadata.RangeColumns`, DIM `ISqlDialect.SupportsRangeColumns`), uncommitted worktree — P0 — нет; P1 по именам — нет; P2 — RPC1 (Шаг 5), RPC2 (доки EN+RU); ℹ️ — RPC3–RPC5

**Область.** Фича хранит `Range<T>` как две скалярные колонки на провайдерах без нативного range-типа (SQL Server/MySQL/MariaDB/SQLite/ClickHouse) и транслирует предикаты/инспекцию PG над парой (`src/nextorm.core/RangeColumnsAttribute.cs`, `DataContext/Meta/RangeColumnsMetadata.cs`, `DataContext/Meta/RangeColumnPairs.cs` — `internal`, `Expressions/RangeColumnRole.cs` — `internal`, `Visitors/RangeColumnsSqlTranslator.cs` — `internal static`). Правки публичной поверхности: DIM `IPropertyMetadata.RangeColumns` (`DataContext/Meta/IPropertyMetadata.cs:22`), DIM `ISqlDialect.SupportsRangeColumns` (`DataContext/Dialect/ISqlDialect.cs:199`) + `SqlDialectBase` override и 5 провайдерных override, публичный fluent-метод `EntityPropertyBuilder<T>.RangeColumns` (`DataContext/Meta/EntityPropertyBuilder.cs:164-172`). Кодовые находки — `code-smells-review.md`, Находки 211–219. Доки фичи — [Range columns](../../guide/31-range-columns.md).

**Публичные подписи (2 новых типа, 3 новых члена существующих типов).** `NextORM.Core.RangeColumnsAttribute` (`sealed`, `AttributeUsage(Property, AllowMultiple=false, Inherited=true)`): ctor `(string lowerColumn, string upperColumn)` + `LowerColumn`/`UpperColumn` (get) + `LowerInclusive`/`UpperInclusive` (get/init, defaults `true`/`false`); `ToMetadata()` — `internal`. `NextORM.Core.RangeColumnsMetadata` (`sealed`): ctor `(string, string, bool lowerInclusive = true, bool upperInclusive = false)` + 4 get-only свойства. `NextORM.Core.IPropertyMetadata.RangeColumns.get -> RangeColumnsMetadata?` (DIM `=> null`). `NextORM.Core.EntityPropertyBuilder<T>.RangeColumns(string lowerColumn, string upperColumn, bool lowerInclusive = true, bool upperInclusive = false) -> EntityPropertyBuilder<T>`. `NextORM.Core.ISqlDialect.SupportsRangeColumns.get -> bool` (DIM `=> false`), `SqlDialectBase.SupportsRangeColumns` (`virtual`), override `=> true` в PG/SQLite/SQL Server/MySQL/ClickHouse.

**P0 — нет.** BCL-конфликтов/аббревиатур нет: `RangeColumnsAttribute` (суффикс `Attribute` ✓), `RangeColumnsMetadata` (суффикс `Metadata` согласован с `IPropertyMetadata`/`IEntityMetadata`), BCL типов `RangeColumns*` нет; `CA1716`/`CA1724`/`CA1002`/`CA1051` чисты. XML-doc у всех новых публичных членов полный (`<summary>`/`<param>`/`<returns>`, `<remarks>` у атрибута) — `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`) + `NoWarn` — **0** ⇒ `CS1591` гейтится сборкой, Release `0/0` это доказывает.

**P1 по именам — нет.** `LowerColumn`/`UpperColumn`/`LowerInclusive`/`UpperInclusive` согласованы с `Range<T>` (`Lower`/`Upper`/`LowerInclusive`/`UpperInclusive`); `RangeColumns` на builder'е — глагольная форма (как `HasConversion`/`JsonColumn`/`Duration`), `SupportsRangeColumns` — по образцу `SupportsRanges`; параметры упорядочены (обязательные → опциональные), `CancellationToken` не затронут (CA1068). Форма — add-only/extend-only: оба новых члена существующих интерфейсов объявлены **default interface methods** (бинарно/источниково-совместимо для внешних реализаторов `IPropertyMetadata`/`ISqlDialect`), ни одна существующая подпись не изменена; `RangeColumnsMetadata` — конструируемый публичный дескриптор (нужен внешним реализаторам DIM). Новые типы `internal` (`RangeColumnPairs`/`RangeColumnRole`/`RangeColumnsSqlTranslator`) в поверхность не выходят; `EntityMetadataBuilder<T>.IsRangeType` — `internal`.

**P2 — RPC1 (открыт).** Шаг 5 (issue #53) не заморожен: `find -name 'PublicAPI*.txt'` — **0**, ApiCompat/`EnablePackageValidation`/API-approval-теста нет. Фича добавляет **≈17** doc-ID-строк к инвентарю заморозки (было ≈82 ⇒ **≈99**): `RangeColumnsAttribute` — 8 (тип, ctor, 4 get + 2 `set`/`init`), `RangeColumnsMetadata` — 6 (тип, ctor, 4 get), `IPropertyMetadata.RangeColumns.get` — 1, `EntityPropertyBuilder<T>.RangeColumns(...)` — 1, `ISqlDialect.SupportsRangeColumns.get` — 1. Точные строки сгенерировать при подключении `PublicApiAnalyzers` (init-аксессоры финализировать по формату анализатора).

**P2 — RPC2 (открыт, доки EN+RU).** Публичные доки не обновлены под фичу, а один пункт им **противоречит**. `docs/advanced/limitations.md` (+RU) в текущем дереве утверждает: «Emulating a range as a `from`/`to` pair of columns — and translating the operators over them — is not implemented» (EN `:59`, RU `:58`), тогда как `[RangeColumns]` именно это и реализует (для SQLite/SQL Server/MySQL/MariaDB/ClickHouse; PG — тоже, `SupportsRangeColumns => true`). Нет ни guide-страницы с `[RangeColumns]`/`RangeColumns(...)` (например `guide/provider-specific/*` или общий guide по range), ни строки в `advanced/api-reference.md` (+RU). План фичи (~~`todo_range_pair_columns.md`~~ (план удалён 25.09.2026) §5) прямо перечисляет эти доки. Нужно: переписать limitation-пункт (оставить непреобразуемыми только range-**возвращающие** операторы/агрегаты над парой), добавить `[RangeColumns]`-раздел в guide + api-reference EN+RU; ссылок на `docs/specs/**` из публичных доков не добавлять (AGENTS.md).

**ℹ️ RPC3.** `RangeColumnsMetadata` — публичный `sealed`-класс с get-only свойствами и без equality, тогда как соседние метаданные — интерфейсы (`IPropertyMetadata`/`IEntityMetadata`). Форма приемлема: DIM `IPropertyMetadata.RangeColumns` возвращает конструируемый извне дескриптор, а интерфейс без реализации не дал бы внешним реализаторам создать значение. Кандидат на equality/деконструкцию — только если попадёт в ключи/сравнения (сейчас не попадает).

**ℹ️ RPC4.** Нуллабилити выбор: `IPropertyMetadata.RangeColumns` возвращает `RangeColumnsMetadata?` (null = одиночная колонка) — согласовано с DIM-политикой «default → безопасное значение»; `SupportsRangeColumns` — `bool` без nullable. `RangeColumnsAttribute` и `RangeColumnsMetadata` Nullable-aware по `Range<T>?` (поддержка заявлена в ~~`todo_range_pair_columns.md`~~ (план удалён 25.09.2026) §3). Зафиксировать в doc-ID при заморозке.

**ℹ️ RPC5 (для заморозки/доков).** План §3 упоминал опциональный `NullIsUnbounded` (default true). В реализации отдельного члена нет — `NULL`-граница **всегда** unbounded (задокументировано в `<remarks>` атрибута), а `Range<T>?` = null на записи даёт `(null, null)` и читается как полностью unbounded (не null). Решение осознанное, но его следует зафиксировать в guide, а не только в XML-doc; кодовая сторона (пустой/null диапазон) — `code-smells-review.md`, Находка 219 и наблюдение H.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; XML-doc **100 %** новых публичных членов (CS1591 гейтится); `NoWarn` — **0**; `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт). Release `--no-build`: `tests/nextorm.sqlite.tests` `~RangeColumnsTests` — **11 passed / 0 failed / 0 skipped**. Рантайм/PG-репро не выполнялся (роль — read-only реестры). Новых публичных типов — **2**, новых публичных членов существующих типов — **3** (2 DIM + 1 fluent-метод); Приложение A не затрагивается. Находки 1–203 (в т.ч. RM1/VC4) не переоткрываются.

## Аудит 25.09.2026 — портативные скалярные обёртки (`SqlFunctions.Sql.bit_length`/`octet_length`/`cot`/`degrees`/`radians`/`pi`, `Math.Acos`/`Asin`/`Atan`/`Atan2`); uncommitted worktree, ветка `1.0-b.1` — P0 — нет; P1 по именам — нет; P2 — PSW1 (**✅ закрыт 25.09.2026**)/PSW2; ℹ️ — PSW3–PSW5 (**PSW5 ✅ проверен**)

**Область.** **6** новых публичных членов `NextORM.Core.CommonFunctions` (`src/nextorm.core/Query/SqlFunctions.cs:625` `bit_length(string?) -> int?`, `:633` `octet_length(string?) -> int?`, `:641` `cot(double?) -> double?`, `:648` `degrees(double?) -> double?`, `:655` `radians(double?) -> double?`, `:662` `pi() -> double?`). Трансляция `Math.Acos`/`Asin`/`Atan`/`Atan2` (`src/nextorm.core/Visitors/MathFunctionTranslator.cs:24-27`) и `SqlServerDialect.MakeMathFunction("atan2", 2) => atn2` (`src/nextorm.sqlserver/SqlServerDialect.cs:478`) публичной поверхности не добавляет. `internal`: рендеры `IScalarFunctions` 5 диалектов, `InMemoryScalarFunctions.BitLength`/`Cot`, записи `InMemoryScalarFunctionRewriter`, +6 имён `CrossProviderScalarTranslator`. Кодовая сторона — `code-smells-review.md`, Находка 222.

**P0 — нет.** BCL-конфликтов нет: `bit_length`/`octet_length`/`cot`/`degrees`/`radians`/`pi` отсутствуют в BCL (`Math`-методы — иной namespace/имя); `CA1716`/`CA1724` чисты. XML-doc у всех **6** членов полный (`<summary>` + `<paramref>`; `<param>` не требуется — параметры у членов с одним `paramref` не вызывают CS1573), `GenerateDocumentationFile=true` (`src/Directory.Build.props:11`) + `TreatWarningsAsErrors=true` (`Directory.Build.props:40`) + `NoWarn` — **0** ⇒ CS1591 гейтится, Release `0/0`. Нуллабилити (`string?`/`double?`/`int?`) согласована с соседними обёртками `CommonFunctions`.

**P1 по именам — нет.** lower_snake SQL-токены (`bit_length`, `octet_length`, `cot`, `degrees`, `radians`, `pi`) — та же конвенция, что `left`/`concat_ws`/`@char` в этом типе; `@`-эскейп не нужен (ключевых слов нет); `pi()` — корректная zero-arg форма. `Async`-суффикс/CA1068/порядок параметров не затронуты. Форма add-only (extend-only): только новые члены/перегрузки, существующие подписи не менялись; `Math.*`-трансляция новых публичных имён не вводит.

**P2 — PSW1 (✅ закрыт 25.09.2026, был `new`-hiding).** 10 существующих публичных членов теперь `new`-скрыты новыми базовыми: `PostgresFunctions.degrees`/`radians`/`pi` (`SqlFunctions.Postgres.cs:315,318,321`), `SqliteFunctions.degrees`/`pi`/`radians` (`SqlFunctions.Sqlite.cs:163,175,178`), `SqlServerFunctions.cot`/`degrees`/`radians`/`pi` (`SqlFunctions.SqlServer.cs:242,248,254,260`). `new` заглушает CS0108, а не устраняет дубль одинаковой сигнатуры на одной цепочке (`*Functions : CommonFunctions`): две doc-ID-строки на одну операцию, вызов через провайдерный тип идёт в `ExtendedScalarFunctionTranslator` (`CrossProviderScalarTranslator.cs:28` гейтит `DeclaringType == typeof(CommonFunctions)`), через `SqlFunctions.Sql` — в переносимый путь; рендер сейчас идентичен. Рекомендация — удалить 10 переобъявлений (source-совместимо) либо зафиксировать обоснование; подробно — `code-smells-review.md`, Находка 222. **Применён вариант «удалить»:** переобъявления сняты, наследуемые переносимые члены сохраняют источниковую совместимость; заодно убран мёртвый провайдерный диспатч и имена приведены к инварианту «ровно провайдерно-специфичные» в `ISqlServerFunctions`/`ISqliteFunctions`-рендерах. Детали и evidence — `code-smells-review.md`, Находка 222 («Исправлено»). Отдельно: `SqliteFunctions.octet_length(object?)` — не скрытие (другая сигнатура), но перекрывает наследуемый `octet_length(string?)`; зафиксировано blob-назначение.

**P2 — PSW2 (открыт, Шаг 5).** Шаг 5 (issue #53) не заморожен: `find -name 'PublicAPI*.txt'` — **0**, ApiCompat/`EnablePackageValidation`/API-approval-теста нет. Фича добавляет **6** doc-ID-строк к инвентарю заморозки (было ≈99 ⇒ **≈105**): `NextORM.Core.CommonFunctions.bit_length(string?) -> int?`, `.octet_length(string?) -> int?`, `.cot(double?) -> double?`, `.degrees(double?) -> double?`, `.radians(double?) -> double?`, `.pi() -> double?` (nullability — по `#nullable enable`-формату анализатора). 10 `new`-членов doc-ID не добавляют (существовали и прежде) и **сняты** (PSW1 ✅). Новых публичных **типов** нет → Приложение A без изменений.

**ℹ️ PSW3 (байтовая семантика length-обёрток).** `bit_length`/`octet_length` — переносимые имена над разными байтовыми семантиками: SQL Server `DATALENGTH` даёт UTF-16-байты (`nvarchar` = 2/символ), PG/MySQL/in-memory — кодировку сервера/UTF-8, ClickHouse `length(String)` — байты, SQLite — `octet_length` 3.43+. Для non-ASCII значения расходятся; задокументировано в `01-string-functions.md` (+RU). При заморозке не обещать побайтовую идентичность между провайдерами.

**ℹ️ PSW4 (`pi()` nullable).** `pi()` объявлен `double?`, хотя SQL `pi()`/`Math.PI` не бывают NULL — согласовано с обёрточной конвенцией `T?` этого типа (SQL-вызов в теории может вернуть NULL). Не дефект; для doc-ID фиксируется как `double?`.

**ℹ️ PSW5 (✅ проверено и закрыто 25.09.2026).** Дока `IScalarFunctions` (`DataContext/Dialect/DialectCapabilities.cs:326-335`) теперь перечисляет `left`/`right`, `lpad`/`rpad`, `repeat`/`reverse`/`space`, `concat_ws`, `translate`, `ascii`/`char`, `bit_length`/`octet_length`, `cot`/`degrees`/`radians`/`pi` — устаревшие `mod`/`log10`/`power` из списка убраны (реальные CLR-пути `%`/`Math.Log10`/`Math.Pow` остаются вне `CommonFunctions`, см. `sql-function-coverage-gap.md`). Правка докиди-онли, публичную поверхность не меняет.

**Extend-only.** Только новые члены `CommonFunctions`; ни одна существующая подпись не изменена. 10 `new`-переобъявлений удалены (PSW1): это удаление публичных членов, но источниковая совместимость сохранена наследованием от `CommonFunctions`, а бинарная заморозка (Шаг 5, PSW2) ещё не выполнена. `internal`-сторона (рендеры, in-memory хелперы, translator-имена) в поверхность не выходит; после удаления переносимые имена убраны из провайдерных `ISqlServerFunctions`/`ISqliteFunctions`-рендеров (они больше не «провайдерно-специфичные»).

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; XML-doc **6/6** новых членов (CS1591 гейтится); `NoWarn` — **0**; `find -name 'PublicAPI*.txt'` — **0** (Шаг 5 открыт). Release `--no-build`: core `~InMemoryScalarFunctionsTests` — **17/17**; `~CrossProviderScalarSqlGenerationTests` — postgres **2/2**, sqlite **8/8**, sqlserver **2/2**, mysql **3/3**, mariadb **3/3**, clickhouse **3/3** (0 failed, 0 skipped). Порядок `ATN2(y,x)` сверен с MS Learn. Доки EN+RU (`scalar-functions/01`, `02`, `sql-function-coverage-gap.md`) обновлены. Новых публичных типов — **0**, новых публичных членов — **6**; Находки 1–221 не переоткрываются.

**Закрытие PSW1 (25.09.2026, `nextorm-design-engineer`).** Удалены **10** публичных `new`-переобъявлений (`PostgresFunctions.degrees`/`radians`/`pi`, `SqliteFunctions.degrees`/`pi`/`radians`, `SqlServerFunctions.cot`/`degrees`/`radians`/`pi`) с сохранением источниковой совместимости через наследование. Провайдерные поверхности и их рендеры приведены к инварианту «ровно провайдерно-специфичные имена»; переносимые имена остаются на `IScalarFunctions`. Доки EN+RU переведены на `SqlFunctions.Sql.*` для `cot`/`degrees`/`radians`/`pi`; `rg` по `docs/**` (без `specs`) — **0** ссылок на удалённые провайдерные написания. `dotnet build nextorm.slnx -c Release` — **0/0**; Debug: core **397**, postgres **521**, sqlite **513**, sqlserver **384**, mysql **168**, mariadb **93**, clickhouse **338** — **0 failed / 0 skipped**. Контейнерная интеграция не перезапускалась этим проходом (SQL-стек и рантайм-пути не изменились: наследуемый член идёт тем же `IScalarFunctions`-рендером).

---

### Аудит 25.09.2026 — снятие gate-off regex на SQL Server 2025+ (`SqlServerDialect.SupportsRegex => true`, `MakeRegexMatch`/`MakeRegexReplace`), uncommitted working tree (P0 — нет; P1 — нет; P2 — RX-SS1; уточняет RX1)

**Область.** `src/nextorm.sqlserver/SqlServerDialect.cs:363` — `public override bool SupportsRegex => true`; `:366-367` `MakeRegexMatch` → `regexp_like(value, pattern, 'c'|'i')`; `:370-371` `MakeRegexReplace` → `regexp_replace(value, pattern, replacement, 1, 0, 'c'|'i')` (сигнатуры и `occurrence = 0` = «all occurrences» сверены с MS Learn). Публичный XML-doc `ISqlDialect` дополнен SQL Server 2025+: `SupportsRegex` (`:826-841`), `MakeRegexMatch` (`:849`), `MakeRegexReplace` (`:861`). **Supersedes** наблюдение (4) аудита #68 (24.09.2026) «SQL Server — осознанный gate-off» и уточняет счёт override'ов в RX1; запись 24.09.2026 не переписывается. Тестовая сторона: `SqlServerTestProvider.SupportsRegex => true` (реальный SQL Server 2025 в общем прогоне), контейнер `mcr.microsoft.com/mssql/server:2022-latest` → `:2025-latest`.

**Именование — P0/P1 нет.** Новых публичных **имён** не добавлено: `SupportsRegex`/`MakeRegexMatch`/`MakeRegexReplace` — уже существующие члены `ISqlDialect` (DIM) и `SqlDialectBase`; SQL Server лишь переопределяет их (`public override`) — source/binary-совместимо, BCL-конфликтов нет (CA1716/CA1724 чисто). XML-doc: `<inheritdoc/>` на методах + `<summary>` про compat level 170 на `SupportsRegex`; build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true` ⇒ CS1591 гейтится.

**Контрактный тест — покрыт, нового действия не требует.** Пара «флаг ↔ рендерер» уже проверяется `tests/nextorm.integration.tests/DialectCapabilityContractTests.cs:216-220` (`if (dialect.SupportsRegex) { MakeRegexMatch...; MakeRegexReplace...; }`), а `AtLeastOneDialect_ShouldExposeEachClassACapability` содержит `dialects.Should().Contain(d => d.SupportsRegex);` (`:250`). SQL Server теперь попадает в обе проверки автоматически. Находка 169 (`code-smells-review.md`) закрыта 24.09.2026 — переоткрытия нет.

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| RX-SS1 | P2 (контракт/версия) | `SqlServerDialect.cs:363`; `ISqlDialect.cs:841` | Безусловный `SupportsRegex => true` на SQL Server 2019/2022 (или 2025 с `compatibility_level < 170`) заставит транслятор рендерить несуществующие `REGEXP_*` — отказ в рантайме вместо прежнего детерминированного `NotSupportedException`; диалект не знает версию сервера. Прецедент — намеренно выключенный `ANY_VALUE` (XP1) | См. `code-smells-review.md`, Находка 223: opt-in/версия сервера либо явная фиксация требования «SQL Server 2025+ и `compatibility_level >= 170`» в EN+RU доках и `limitations`, с гарантией compat 170 в тест-контейнере. Форму (`SupportsRegex`/`MakeRegex*`) не менять |
| RX1 (уточнение) | P2 (трекинг Шага 5) | `PublicAPI.*.txt` — **0**; `ISqlDialect.cs:841,849,861`; `SqlDialectBase.cs:474,478,481`; `SqlServerDialect.cs:363,366,370` | Остаётся открытым. Счётчик override'ов в записи 24.09.2026 «15 override'ов (5×3)» **устарел**: теперь **18 (6×3)** — SQL Server добавляет 3 (`SupportsRegex`, `MakeRegexMatch`, `MakeRegexReplace`) | При заморозке внести подписи (точный текст — из анализатора); DIM — с дефолтными реализациями |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 нет.** (2) **Extend-only:** переопределение существующего `public virtual`/DIM — аддитивно; удалений/переименований/смены подписей нет. (3) XML-doc 100 % (build `0/0`). (4) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0**; `docs/specs/**` — internal. (5) Портируемость ядер — без изменений (наблюдение A аудита #68); SQL Server 2025 использует RE2, подтверждено MS Learn. (6) `RegexSqlTranslator` — `internal`, в поверхность не входит.

**Проверка (25.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn refs NextORM.Core.ISqlDialect.SupportsRegex` — **6** override'ов (`MySqlDialect.cs:404`, `MariaDbDialect.cs:17`, `ClickHouseDialect.cs:511`, `PostgresDialect.cs:448`, `SqliteDialect.cs:154`, `SqlServerDialect.cs:363`); `MakeRegexMatch` (`SqlDialectBase.cs:478`) / `MakeRegexReplace` (`:481`) — по **6** override'ов; `find -name 'PublicAPI*.txt'` — **0** (RX1); Release `--no-build`, `tests/nextorm.sqlserver.tests` фильтр `FullyQualifiedName~Regex` — **5/5** (0 failed/0 skipped); контейнерная интеграция (реальный SQL Server 2025) аудитом не перезапускалась — нет `DOCKER_HOST`.

**Итог.** P0 — **нет**; P1 по **именам** — **нет**; P2 — **RX-SS1** (контракт/версия, продолжение `code-smells-review.md`, Находка 223) и **RX1** (трекинг Шага 5; счёт override'ов 15 → **18**). Наблюдение (4) аудита #68 — **⛔ superseded**. Находки/записи 24.09.2026 не переоткрываются.

---

## Верификация аудитором 26.09.2026 — `[RangeColumns]`: P2-5 (симметричный fail-fast `HasColumnName`/`RangeColumns` vs `[Column]`/`[RangeColumns]`); новых публичных имён/членов нет (P0 — нет, P1 по именам — нет, P2 — RPC6 ✅ закрыт контрактно; Шаг 5/RPC1 без изменений)

**Область.** Проверка исправления дизайн-ревью P2-5 (кодовая сторона — `code-smells-review.md`, Находка 229) в `src/nextorm.core/DataContext/Meta/EntityPropertyBuilder.cs` и `DataContext/Meta/EntityMetadataBuilder.cs`. Вопрос аудита: не порождает ли фикс нового публично-API-нейминг-дефекта. Продолжение раздела «Аудит 25.09.2026 — `[RangeColumns]`…» (RPC1–RPC5); запись 25.09.2026 не переписывается.

**P0 — нет.** Новых публичных **типов** и **имён** не добавлено; BCL-конфликтов/аббревиатур нет (`CA1716`/`CA1724`/`CA1002`/`CA1051` — чисто, Release `0/0`, `NoWarn` — 0).

**P1 по именам — нет.** Добавленное поведение — `InvalidOperationException` внутри существующих публичных методов, а не новые члены. `EntityPropertyBuilder<T>.EnsureNoRangeColumns` — `private` (`:210`), в поверхность не выходит; `RangeColumns`, `HasColumnName`, `HasConversion` ×3, `JsonColumn`, `Build` сохраняют прежние подписи. `EntityMetadataBuilder<T>` — изменение только тела `Build` (сигнатуры/члены не менялись).

**RPC6 (P2, публичный контракт/тихий no-op) — ✅ закрыт 26.09.2026.** Ранее fluent `HasColumnName` и `RangeColumns` (а также атрибутная пара `[Column]`+`[RangeColumns]`) молча выигрывали друг у друга. Теперь — симметричный fail-fast `InvalidOperationException` (`EntityPropertyBuilder.cs:38-42,178-185,210-215,231-233`; `EntityMetadataBuilder.cs:75-85`) с обновлённым XML-doc `<exception>`. Публичная форма/сигнатуры не изменились. Продолжение RPC1–RPC5, не переоткрытие; запись 25.09.2026 не переписывается.

**Публичные подписи (без изменений).** `roslyn members NextORM.Core.EntityPropertyBuilder'1`: публичный ctor + `Build()`, `Collation(string)`, `Computed()`, `Duration(DurationUnit,int)`, `HasColumnName(string)`, `HasConversion(IPropertyValueConverter)`, `HasConversion<TModel,TProvider>(ValueConverter<TModel,TProvider>)`, `HasConversion<TModel,TProvider>(Expression<Func<TModel,TProvider>>, Expression<Func<TProvider,TModel>>)`, `Identity()`, `JsonColumn(Action<JsonColumnOptions>?)`, `Key()`, `RangeColumns(string,string,bool,bool)` — те же **12** публичных методов + ctor, что и до фикса (`RangeColumns` — из RPC-записи 25.09, не новое этим проходом); `EnsureNoRangeColumns`/`ValidateConverterModel` — `private`.

**Шаг 5 (RPC1) — без изменений.** `find -name 'PublicAPI*.txt'` — **0**; `PublicApiAnalyzers`/`EnablePackageValidation`/ApiCompat/API-approval-теста нет. XML-`<exception>` не меняет doc-ID (только текст доки), поэтому инвентарь заморозки (≈99 + 6 из PSW2) этим проходом **не растёт**.

**Проверка (26.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.EntityPropertyBuilder'1` — публичный набор без новых членов (`EnsureNoRangeColumns` — `private`); `find -name 'PublicAPI*.txt'` — **0** (RPC1/Шаг 5 открыт); `NoWarn` — **0**; XML-doc/CS1591 гейтится build. Тесты `tests/nextorm.core.tests/InsertMetadataTests.cs` `ColumnAndRangeColumnsAttributes_ShouldThrow`/`FluentColumnNameThenRangeColumns_ShouldThrow`/`FluentRangeColumnsThenColumnName_ShouldThrow` — зелёные (верификация сессии: core **401/0 failed**).

**Итог.** P0 — **нет**; P1 по **именам** — **нет**; P2 — **RPC6** ✅ (P2-5 закрыт контрактно; новых публичных имён/членов/doc-ID нет), **RPC1** (Шаг 5) — открыт без изменений. RPC2–RPC5 и запись 25.09.2026 не переоткрываются.

**Актуализация устаревших ссылок (26.09.2026).** Удалены планы `docs/specs/roadmap/todo_sqlserver_function_gaps.md`, `todo_value_converters.md`, ~~`todo_range_pair_columns.md`~~ (план удалён 25.09.2026): соответствующие ссылки в реестре помечены как устаревшие/удалённые.

---

### Аудит 26.09.2026 — паритет опций bulk copy: `CheckConstraints`/`TableLock`/`KeepNulls`/`FireTriggers`, публичный `BulkCopyFlags`, расширение protected-хуков `DataContext`, uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — BCO1/BCO2)

**Область.** Новый публичный `NextORM.Core.BulkCopyFlags` (`src/nextorm.core/Builders/BulkCopyFlags.cs:13`, `public readonly record struct`, 4 позиционных булевых параметра — `CheckConstraints`/`TableLock`/`KeepNulls`/`FireTriggers`; статическое `None` `:16`; `IsAny` `:19`; `ThrowIfRequested(string)` `:27`). **4** новых `bool?` init-свойства `BulkInsertOptions` (`src/nextorm.core/Builders/BulkInsertOptions.cs:47,55,63,72`) и **4** новых fluent-метода `BulkInsertOptionsBuilder` (`:224,233,242,251`). Сигнатуры **2 protected-хуков** `DataContext` (`DataContext.cs:529,545`) расширены параметром `BulkCopyFlags bulkCopy`; internal-механика: `BulkInsertCommand` (+`BulkCopy` `:132` + ctor-параметр `:69`), `BulkInsertBuilder.BuildCommand` (`:239`), `PortableBulkInsertExecutor.EnsureTimeoutSupported` → `EnsurePortableOptionsSupported` (`:110`), `SqlServerDataContext.MapBulkCopyOptions(bool,bool,bool,bool)` (`:225`, `internal static`, для теста — добавлен `InternalsVisibleTo` в `nextorm.sqlserver.csproj`), PostgreSQL `bulkCopy.ThrowIfRequested("the PostgreSQL COPY path")` (`PostgresDataContext.cs:200,238`).

**Именование — P0 нет; новых P1 нет.** BCL-конфликтов (CA1716/CA1724) нет: `BulkCopyFlags` не сталкивается с `System.Data.SqlClient.SqlBulkCopyOptions`/`Microsoft.Data.SqlClient.SqlBulkCopyOptions` (имена разных типов), а `CheckConstraints`/`TableLock`/`KeepNulls`/`FireTriggers` дословно зеркалят члены `SqlBulkCopyOptions` и linq2db `BulkCopyOptions` (паритет — цель пункта). `None`/`IsAny` повторяют прецедент `BulkBatchOptions.IsUnbounded`; `ThrowIfRequested(path)` — короткий глагол-императив, как `ArgumentException.ThrowIfNullOrEmpty`. `bool?` (а не `bool`) согласован с остальными опциями `BulkInsertOptions` и документирован так, что `null` и `false` эквивалентны (флаг не выставляется) — zero-cost дефолт. XML-doc полный: build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — 0 (CS1591 не подавлен).

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| BCO1 | P2 (трекинг) | `PublicAPI.*.txt` — **0**; `Builders/BulkCopyFlags.cs:13`; `BulkInsertOptions.cs:47-72`; `DataContext.cs:529,545` | Поверхность выросла на **1** публичный тип (`BulkCopyFlags` + 3 члена), **4** init-свойства + **4** метода `BulkInsertOptionsBuilder`, изменение **2** protected-сигнатур. `PublicApiAnalyzers`/ApiCompat/API-approval не подключены (Шаг 5 открыт). **Продолжение MRG7/TR4/CTAS3/UPD2/IDML18/BK4**, не новая проблема | При заморозке внести подписи (точный текст — из анализатора); расширяет перечень BK4 |
| BCO2 | P2 (расширяемость/совместимость) | `DataContext.cs:529,545`; `Builders/BulkCopyFlags.cs:13` | (1) Публичный `BulkCopyFlags` **вынужден** быть public: параметр `protected`-хука не может быть менее доступным, чем сам метод (иначе CS0051). Это расширяет публичную поверхность насквозь через `protected`-контракт, хотя тип задумывался internal. (2) Добавление параметра к `protected virtual BulkInsertRows`/`BulkInsertRowsAsync` (не overload) — source-breaking для внешних наследников `DataContext`, переопределяющих хук. Прецедент уже принят в этом же незарелиженном bulk-инкременте (BK1: сигнатуры уже расширялись `int? commandTimeoutSeconds, int? maxBatchSize`) | Принять: bulk-поверхность uncommitted/alpha; параметр добавлен осознанно (план пункта). Альтернатива на будущее — объект-носитель `BulkInsertCommand`/`BulkCopyOptions` вместо роста списка параметров. Публичную форму bulk больше не трогать |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 нет.** (2) **Extend-only по отношению к опубликованному API:** переименований/удалений в `main` нет (весь bulk-инкремент uncommitted); новые члены `ISqlDialect` не трогались. (3) Отказ реализован **явно** на каждом невыразимом пути (PostgreSQL `COPY` — `PostgresDataContext`; portable, включая SQL Server `Returning*`/`KeepIdentity` — `PortableBulkInsertExecutor.EnsurePortableOptionsSupported`), InMemory падает раньше общим read-only-отказом — тихого игнорирования нет, как и требует criterion 3. (4) ClickHouse `MaxDegreeOfParallelism`/`WithoutSession` добавлены **не** были: драйвер `ClickHouse.Driver` 1.4.0 держит `MaxDegreeOfParallelism` только на `[Obsolete] ClickHouseBulkCopy`, `WithoutSession` отсутствует (сессия — `UseSession` в строке подключения), нативный путь не подключён — публичная опция, которая всегда бросала бы, противоречит API-дизайну; зафиксировано в `limitations.md` (EN+RU). (5) `UseInternalTransaction` не выставляется (nextorm не открывает неявную транзакцию). (6) Тестовый шов `MapBulkCopyOptions` + `InternalsVisibleTo("nextorm.sqlserver.tests")` — единственный способ покрыть маппинг флагов без контейнера.

**Проверка (26.09.2026).** `dotnet build nextorm.slnx -c Release -m:2` — **0 warnings / 0 errors**; `roslyn types BulkCopyFlags` — 1 публичный тип; тесты без контейнеров: `tests/nextorm.core.tests` **404/0 failed**, `tests/nextorm.sqlserver.tests` **393/0 failed** (включая `MapBulkCopyOptions_ShouldSetEveryEnabledFlag`/`_None_ShouldBeDefault`, `BulkInsert_Returning_WithBulkCopyFlag_ShouldThrow`, `BulkInsert_KeepIdentity_WithBulkCopyFlag_ShouldThrow`), `tests/nextorm.postgres.tests` **522/0 failed** (включая `BulkInsert_WithSqlServerOnlyFlag_ShouldThrow`), `tests/nextorm.clickhouse.tests` **338/0 failed**; `find -name 'PublicAPI*.txt'` — **0** (BCO1/Шаг 5); CRLF сохранён, `git diff --check` — чисто. Контейнерная интеграция (реальный `SqlBulkCopy` с флагами) аудитом не перезапускалась — нет `DOCKER_HOST`.

## Запись 26.09.2026 — Query tag (`WithTag`): P0 — нет, P1 по именам — нет, P2 — нет

**Область (uncommitted).** Новый публичный fluent-метод `EntityBuilder<TEntity>.WithTag(string?)` (+ перегрузка `EntityBuilder.WithTag(string?)`) и `QueryCommand<TResult>.WithTag(string?)`; новое публичное свойство-геттер `QueryCommand.Tag` (сеттер — `internal`); `internal string? Tag` у обоих builder-ов. Внутренние детали: рендер `/* tag */` в `SqlBuilder.MakeTagComment`, участие `Tag` в `QueryPlanEqualityComparer`.

**P0 — нет.** Новых публичных **типов** нет; коллизий с BCL/аббревиатур нет. XML-`<summary>` есть у всех новых публичных членов; `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**.

**P1 по именам — нет.** `WithTag` следует конвенции репозитория `With*` (`WithQuotedIdentifiers`/`WithKeywordCase`/`WithForJson`); backlog-токен linq2db `TagQuery` намеренно не берётся. Свойство `Tag` — PascalCase, тип `string?`, сеттер `internal` (как у `QuoteIdentifiers`/`KeywordCase`): мутация только через fluent-метод, план-кэш безопасен.

**P2 — нет.** Метка — комментарий, а не хинт; это отдельное состояние, `Hint` не затрагивается. Экранирование `*/`, `/*` и CR/LF задокументировано и покрыто тестами. План-ключ: непустой `Tag` добавляется в `XxHash32` напрямую (как `Settings`), в `Equals` — ordinal-сравнение, поэтому контракт `Equals => equal hash` сохранён без отдельного `TagPlanHash`.

**Шаг 5 — без изменений.** `find -name 'PublicAPI*.txt'` — **0**; при заморозке внести 5 подписей: `EntityBuilder<TEntity>.WithTag(string?)`, `EntityBuilder.WithTag(string?)`, `QueryCommand<TResult>.WithTag(string?)`, `QueryCommand.Tag.get`, `QueryCommand.Tag.set` (`internal`).

**Проверка (26.09.2026).** Build Release — **0/0**; SQL-gen `QueryTag` на 6 провайдерах — по **2/2**; `PlanKeyUniquenessTests` — **7/7**; core `InMemoryTests` — **88/88**; полные прогоны: core 403, sqlite 516, postgres 523, sqlserver 390, mysql 170, mariadb 95, clickhouse 340 — **0 failed**. Доки EN+RU: `guide/17-query-hints.md`, `advanced/api-reference.md`, `providers/overview.md`; спеки: `sql-capabilities-gap-analysis.md` §5 п.53, `linq2db-backlog-gap-analysis.md`.

## Аудит 26.09.2026 — управление кэшем (`DataContextCache.Clear`/`CacheSlidingExpiration`, `DataContextBuilder.UseQueryCache`/`UseCacheSlidingExpiration`, `QueryCacheEnabled`); uncommitted worktree (P0 — нет; P1 по именам — нет; P2 — QCC1 (Шаг 5); ℹ️ — QCC2–QCC4)

**Область.** Закрытие backlog-пункта `docs/specs/roadmap/todo_query_cache_controls.md` (G10, tracking issue #97): `src/nextorm.core/DataContext/DataContextCache.cs` (новые `Clear()`, `CacheSlidingExpiration`), `src/nextorm.core/TimedDictionary.cs` (новый `internal`), `src/nextorm.core/DI/DataContextBuilder.cs` (`QueryCacheEnabled`, `UseQueryCache`, `UseCacheSlidingExpiration`), `src/nextorm.core/DataContext/DataContext.cs`/`InMemoryDataContext.cs` (`QueryCacheEnabled` + локальный гейт `storeInCache`), `src/nextorm.core/DataContext/Cache/QueryPlanStore.cs` (generation-clear).

**Именование — P0/P1 нет.** Имена зеркалят linq2db (`ClearCache`/`UseDisableQueryCache`/`UseCacheSlidingExpiration`), но приведены к локальному словарю: `Clear` (как `PurgeQueryCache`), `CacheSlidingExpiration` (существительное свойства), `UseQueryCache(bool)` (согласовано с `QueryCacheEnabled`). BCL-конфликтов/аббревиатур нет (`CA1716`/`CA1724`/`CA1710` — чисто; `TimedDictionary` — `internal`, суффикс `Dictionary` корректен для коллекции). `QueryCacheEnabled` не конфликтует с существующим `InMemoryDataContext.CacheExpressions` (другая ось).

**Контракт — same-name property на двух контекстах.** `DataContext.QueryCacheEnabled` (get/set) и `InMemoryDataContext.QueryCacheEnabled` (get/set) намеренно одноимённы: обе — политика кэша контекста. `DataContextBuilder.QueryCacheEnabled` — get-only (настраивается через `UseQueryCache`).

| # | Ур. | Место | Проблема | Рекомендация |
|---|-----|-------|----------|--------------|
| QCC1 | P2 (трекинг Шага 5) | `DataContextCache.cs` (`Clear`, `CacheSlidingExpiration`); `DataContextBuilder.cs` (`QueryCacheEnabled`, `UseQueryCache`, `UseCacheSlidingExpiration`); `DataContext.cs` (`QueryCacheEnabled`); `InMemoryDataContext.cs` (`QueryCacheEnabled`); `PublicAPI.*.txt` — **0** | Шаг 5 (issue #53) не заморожен: ApiCompat/API-approval-теста нет. Фича добавляет **7** doc-ID-строк (3 метода + 4 свойства) к инвентарю заморозки (≈105 + 7 ⇒ **≈112**). Новых публичных **типов** нет → Приложение A без изменений | При заморозке внести подписи (точный текст — из анализатора); DIM не вводится, типы существуют |

ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 нет.** (2) **Extend-only:** все члены аддитивны, существующие подписи не менялись; `QueryPlanStore.Clear()` сохранил сигнатуру, изменив реализацию (per-thread → generation-clear), — внутренний тип. (3) XML-doc 100 % новых публичных членов (build `0/0` при `GenerateDocumentationFile=true` + `TreatWarningsAsErrors=true`, `NoWarn` — **0**). (4) Шаг 5 открыт: `find -name 'PublicAPI*.txt'` — **0**; `docs/specs/**` — internal и в публичные доки не линкуется. (5) Sticky-флаг `QueryCommand.Cache` не мутируется: отключение выражается локальным `storeInCache: false` (AGENTS.md «Shared query commands & plan cache»); регресс-тест `QueryCacheControlsTests.Disabled_QueryCache_Should_Rebuild_Without_Mutating_Command_Cache_Flag`/`StoreInCache_False_Should_Not_Mutate_Command_Cache_Flag`.

**Проверка (26.09.2026).** `dotnet build nextorm.slnx -c Release` — **0 warnings / 0 errors**; `roslyn members NextORM.Core.DataContextCache`/`NextORM.Core.DataContextBuilder` — новые члены на месте; XML-doc у всех 7 (CS1591 гейтится); `find -name 'PublicAPI*.txt'` — **0** (QCC1). Debug native runner: `tests/nextorm.core.tests` — **410/410** (в т.ч. 9 новых `QueryCacheControlsTests`), `tests/nextorm.sqlite.tests` — **515/515** (в т.ч. новый `DataContextCacheClearTests`), 0 failed / 0 skipped. Доки EN+RU (`guide/15-query-reuse.md`, `advanced/api-reference.md`) обновлены; ссылок на `docs/specs/**` из публичных доков нет.

**Итог.** P0 — **нет**; P1 по **именам** — **нет**; P2 — **QCC1** (трекинг Шага 5; инвентарь +7 doc-ID). Новых публичных типов — **0**, новых публичных членов — **7**. Находки/записи 25–26.09.2026 не переоткрываются.

## Самоаудит 26.09.2026 — per-query переопределение источника (`WithTableName`/`WithSchema`/`WithDatabase`/`WithServer`/`WithTableExpression`), uncommitted working tree (P0 — нет; P1 по именам — нет; P2 — SRC1 (Шаг 5); субагенты `nextorm-code-auditor`/`nextorm-design-engineer` не запускались — нет инструмента `task`)

**Область.** Закрытие Gap-пункта `todo_source_override.md` (linq2db `TableName`/`SchemaName`/`DatabaseName`/`ServerName`/`WithTableExpression`). Новые **публичные** члены [`EntityBuilder<TEntity>`](xref:NextORM.Core.EntityBuilder`1): `WithTableName(string)`, `WithSchema(string)`, `WithDatabase(string)`, `WithServer(string)`, `WithTableExpression(string)` (все возвращают `EntityBuilder<TEntity>`; XML-doc `<summary>` + `<param>` + `<returns>`). Новые члены контракта диалекта: DIM `ISqlDialect.SupportsCrossDatabase => false`, `SupportsLinkedServer => false`, `MakeQualifiedTableName(string?,string?,string?,string)` с ANSI-дефолтом; те же три как `public virtual` в `SqlDialectBase`; переопределения `SupportsCrossDatabase`/`SupportsLinkedServer`/`MakeQualifiedTableName` в `SqlServerDialect` (4-частное `server.database.schema.table`), `MySqlDialect` (одночастное `database.table`; MariaDB наследует), `ClickHouseDialect` и `SqliteDialect` (одночастное; SQLite — attached-база). Внутренние слоты `FromExpression` (`TableNameOverride`/`SchemaOverride`/`DatabaseOverride`/`ServerOverride`/`TableExpressionOverride`) — `internal readonly`, в поверхность не входят; `WithOverrides`/`HasQualificationOverride` — `internal`; `EntityBuilder<TEntity>.ResolveSource` — `internal`.

**P0 — нет.** Новых публичных типов нет; BCL-конфликтов/аббревиатур нет (Release `0/0`). Пять имён повторяют linq2db-словарь (`WithTableName`/`WithSchema`/`WithDatabase`/`WithServer`/`WithTableExpression`) и не конфликтуют с существующими членами; `MakeQualifiedTableName` — по образцу уже принятого `MakeFunction(name, schema)`.

**P1 по именам — нет.** Extend-only: новых `new`/переименований/смены подписей нет; DIM добавлены с дефолтами, поэтому внешние реализации `ISqlDialect` продолжают компилироваться. `WithTableExpression` намеренно повторяет `WithSql`-семейство и документирует «сырой SQL — ответственность вызывающего».

**SRC1 (P2, трекинг Шага 5) — открыт без изменений.** `find -name 'PublicAPI*.txt'` — **0**; `PublicApiAnalyzers`/ApiCompat/API-approval-теста нет. При заморозке внести 5 подписей `EntityBuilder<TEntity>` и 3 члена `ISqlDialect`/`SqlDialectBase`.

**Проверка (26.09.2026).** `dotnet build nextorm.slnx -c Release -m:2` — **0 warnings / 0 errors**; SQL-gen тесты `tests/nextorm.{sqlserver,postgres,mysql,mariadb,clickhouse,sqlite}.tests/SourceOverrideSqlGenerationTests.cs` + `tests/nextorm.core.tests/SourceOverrideTests.cs` — зелёные; полные прогоны core (406) / sqlite (521) / sqlserver (397) / postgres (526) / mysql (173) / mariadb (96) / clickhouse (341) — 0 failed. Контейнерная интеграция не запускалась (задача без контейнеров).

**Итог.** P0 — **нет**; P1 по **именам** — **нет**; P2 — **SRC1** (Шаг 5, трекинг заморозки). Реестр именования выровнен.
