...
- **TempTableExtensions, name-less-ремедиация.** 8 новых публичных членов (`src/nextorm.core/Builders/TempTableExtensions.cs`): `ToTempTable<TResult>(EntityBuilder<TResult>) -> string` `:54`, `ToTempTable<TResult>(EntityBuilder<TResult>, CreateTableAsOptions) -> string` `:70`, `ToTempTable<TResult>(QueryCommand<TResult>) -> string` `:86`, `ToTempTable<TResult>(QueryCommand<TResult>, CreateTableAsOptions) -> string` `:99`, `ToTempTableAsync<TResult>(EntityBuilder<TResult>, CancellationToken = default) -> Task<string>` `:139`, `ToTempTableAsync<TResult>(EntityBuilder<TResult>, CreateTableAsOptions, CancellationToken = default) -> Task<string>` `:156`, `ToTempTableAsync<TResult>(QueryCommand<TResult>, CancellationToken = default) -> Task<string>` `:172`, `ToTempTableAsync<TResult>(QueryCommand<TResult>, CreateTableAsOptions, CancellationToken = default) -> Task<string>` `:186`. Итого `TempTableExtensions` — **20** публичных членов (12 name-ful + 8 name-less).
...
| TT3 | P2 (форма/консистентность) | `TempTableExtensions.cs:30,42,113,127,199,212,221,233,244,258,268,281`; `BatchBuilder.cs:75,88` | **14** name-ful CTAS-членов по-прежнему `CreateTableAsOptions? options = null`, тогда как name-less-двойники в этом же цикле отремедированы в пару «без параметра + обязательный `CreateTableAsOptions`» (Находка 188). `ToTable("t", null)`/`CreateTableAs("t", q, null)` компилируются и молча означают default-опции | Довести ремедиацию до name-ful: `(…, name)` + `(…, name, CreateTableAsOptions)` с `ArgumentNullException.ThrowIfNull`; **+14** публичных членов учесть при заморозке. Маршрут `nextorm-design-engineer` |
