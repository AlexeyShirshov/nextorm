...
ℹ️ **Наблюдения.** (1) **P0 нет; новых P1 по именам нет.** (2) **Extend-only:** новый интерфейс + DIM + `virtual` + аддитивные члены `SqlServerFunctions`. (3) **XML-doc 100 %** (build `0/0`). (4) `json_contains`/`json_path_exists` возвращают `bool` (как `isjson`) и внесены в `TypeFacts.IsPredicateCall`; `json_object`/`json_objectagg` рендерят `key : value`. (5) Доки EN+RU обновлены: `guide/provider-specific/sqlserver.md` (+RU), `advanced/api-reference.md` (+RU); спеки — `sql-function-coverage-gap.md`, `todo_sqlserver_function_gaps.md`. (6) Шаг 5 открыт (SS1).
