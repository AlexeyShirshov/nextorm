...
**Обновление 26.09.2026 (uncommitted worktree — eager loading графа, issue #95, решение без кода).** Публичный API **не менялся**: `LoadWith`/`Include` не добавлены. Зафиксировано решение «не планируется до появления навигационных свойств» (Вариант B): nextorm не моделирует ни навигации, ни метаданные связей (workstream 13), поэтому `LoadWith(x => x.Children)`/`Include` не на что навести, а селекторный level-1 был бы другим API и новой двухзапросной подсистемой (вложенная коллекция, keyset родителя + дочерний `IN`, ститчер), а не аддитивным членом. Новых публичных типов/членов нет → Приложение A без изменений. P0/P1 по именам нет. Доки EN+RU: `advanced/limitations.md` (+RU, строка «Eager loading»), `specs/roadmap/todo_eager_loading.md` (провайдерная матрица + «Статус реализации»), `specs/roadmap/sql-capabilities-gap-analysis.md` (§4 п.49 → ledger §5 п.49, workstream 13).
