# WIP: Транзакции (enlistment, общая транзакция с EF Core / Dapper)
...
> Дополняет [WIP_interceptors.md](WIP_interceptors.md) (перехват соединения) и
> [linq2db-comparison.md](linq2db-comparison.md:56) (у linq2db интеграция с EF Core есть — у nextorm нет).
...
3. **Пробел в матрице.** В `todo_mssql.md` и `todo_clickhouse.md` транзакции перечислены как
...
(DIP), а роль соединения остаётся про жизненный цикл (`WIP_interceptors.md:158-159`).
...
- **Фаза 2:** `TransactionScope`/ambient, savepoints и вложенные транзакции, `BeginTransactionAsync`
  с таймаутом, интеграция с интерцепторами (`IConnectionInterceptor`, `WIP_interceptors.md`).
...
- Документация: `docs/guide/16-connections-and-logging.md` (+ `docs/ru/guide/`),
  ...
  `docs/specs/roadmap/linq2db-comparison.md` (+RU), `docs/specs/roadmap/todo_*.md`.
