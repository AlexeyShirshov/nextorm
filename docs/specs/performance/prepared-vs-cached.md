...
`DataContext._queryPlanCache` is a **`[ThreadStatic]` static** dictionary
(`src/nextorm.core/DataContext/DataContext.cs:40-41`). The key is a `(Type ContextType, QueryPlan Plan)`
...
`PrepareCommand(!storeInCache, ct)` (`DataContext.cs:303`). The public terminals always pass
...
* Runtime parameters are named `norm_p{index}` (`SqlFunctions.Parameter<T>(idx)` in the query). A command
  "needs a refresh" when any parameter matches `IsRuntimeParam` (`DataContext.cs:174`).
* On a hit, values are only refreshed when `NeedsParamRefresh` is set; otherwise the values captured at
  build time are reused (`DataContext.cs:426-446`).
...
* Entries live until `PurgeQueryCache()` is called (`DataContext.cs:1202`; the in-memory provider clears
...
* `Dispose()` iterates *all* cached entries on the thread and calls `ResetConnection` when the context owns
  the connection (`DataContext.cs:697-716`), which nulls `DbCommand.Connection` so the command rebinds on the
...
* It **is** bound to the provider *representation*: `DataContext.AsDbCommand` rejects anything that is not a
  `DbPreparedQueryCommand<TResult>` with an `ArgumentException`, so a command prepared by
  `SqliteDataContext` cannot be executed by a `PostgresDataContext`.
...
1. **Crash: buffered then streaming on the same query shape** (fixed 2026-09-16).
   ...
   (`if (createEnumerator && compiledQuery.Enumerator is null)` in `DataContext.cs`). Covered by
...
2. **`Prepare()` default + streaming failed with a bare `NullReferenceException`** (fixed 2026-09-16).
   ...
   fails through `DataContext.RequireEnumerator` with an `InvalidOperationException` that says to use
...
3. **`GetAnyCommand` / `ReplaceCommand`** mutates the shared cached `AnyCommand`, so concurrent use of one
   `DataContext` can let different predicates observe the wrong plan (correctness, tracked separately).
...
dotnet run --project tests/nextorm.sqlite.tests -c Release
```

See also `docs/specs/performance/performance-findings.md` (finding M12) for the full decomposition and the history of this
investigation.
