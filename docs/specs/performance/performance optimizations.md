...
- `Cached_PlanOnly_Param` — без БД, с `SqlFunctions.Parameter`: построение запроса + plan lookup + `ExtractParams`. Дельта к NoParam изолирует `ExtractParams`.
...
- `_sbPool` через `ObjectPool<StringBuilder>` — `src/nextorm.core/DataContext/SqlBuilder.cs:12`, `src/nextorm.core/DataContext/DataContext.cs:20`.
...
`src/nextorm.core/DataContext/DataContext.cs:385` вызывает `ExtractParams` → `MakeSelect(paramMode: true)`
(`DataContext.cs:141-152`) на каждом исполнении. Внутри заново аллоцируются `List<Param>`,
`DefaultColumnsProvider`, `DefaultAliasProvider`, `DefaultParameterProvider` и прогоняются все visitor'ы —
только чтобы достать значения. Замерено: ≈1.1 KB/вып.

**Сделано.** Реальные значения `SqlFunctions.Parameter`-плейсхолдеров всё равно подставляет
...
параметров есть не-`SqlFunctions.Parameter` (проверка `DataContext.IsRuntimeParam`). На cache-hit `ExtractParams`
...
- `src/nextorm.core/DataContext/DataContext.cs` — вычисление флага, условие на cache-hit, `IsRuntimeParam`.
...
- Создаётся локально (`DataContext.cs:155`, `SqlBuilder.cs:137`, `BaseExpressionVisitor.cs:222/268/469/591/789`),
...
Разложил per-execution оверхед кэшированного пути (`SqlFunctions.Parameter`, без БД) новыми arm'ами
...
- Legacy-сравнители перенесены из production-сборки в `tests/nextorm.core.tests` (**2344 строки**):
