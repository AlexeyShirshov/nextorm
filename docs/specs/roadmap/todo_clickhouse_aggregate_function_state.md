...
- **[TYPE]/[KISS] 🔴** Публичный API не определён (`:45-49` — 3 bullet-черновика, ни одной сигнатуры): нет имён/сигнатур (`uniq_state`, `uniq_merge`, …) и поверхности; нет имени/формы capability-объекта + свойства `ISqlDialect`; нет типа материализации `AggregateFunction` и его маппинга в row reader; не решено, как колонка состояния типизируется в `[Column]`/проекции (Q §53-54); нет тест-плана. Fix: довести §«Дизайн» до уровня планов функций (см. [ClickHouse-specific SQL](../../guide/provider-specific/clickhouse.md)).
