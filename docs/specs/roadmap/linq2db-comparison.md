...
* **Correlation**: nextorm supports one level of correlated scalar subqueries and correlated
  `EXISTS`/`IN`/`ANY`/`ALL` on the SQL providers; it does **not** support nested (depth > 1)
  correlation, and correlated subqueries on the in-memory provider. The applied `APPLY`/`LATERAL`
  source still cannot reference the outer row (see
  [`sql-capabilities-gap-analysis.md`](sql-capabilities-gap-analysis.md) §4).
