...
* **Корреляция**: nextorm поддерживает один уровень коррелированных скалярных подзапросов и
  коррелированные `EXISTS`/`IN`/`ANY`/`ALL` у SQL-провайдеров; он **не** поддерживает вложенную
  (глубина > 1) корреляцию и коррелированные подзапросы в провайдере in-memory. Применяемый
  источник `APPLY`/`LATERAL` по-прежнему не может ссылаться на внешнюю строку (см.
